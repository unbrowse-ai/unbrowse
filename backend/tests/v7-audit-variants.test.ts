 ...
 *     a category error — , each variant is a distinct
 ...
 * substrate).
 *
 *  — every fill is witnessed. The receipt is the witness; the
 * variant is the witness-shape.
 */
 ...
 *
 *  — the variant IS the witness-shape; the factory enforces it
...
describe("POST /v1/audit/header-inject — valid Ed25519 body", () => {
  test("returns 200 + receiptId; stores headerNameHash, not raw header name", async () => {
    ...
    //  — the hash is the witness; the raw name was never
