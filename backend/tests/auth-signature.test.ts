...
describe("signature-auth (web3-PK root) — golden + adversarial edges", () => {
  ...
  it("EDGE — unbound: a valid signature for a pubkey no key has bound → its OWN wallet principal, never another agent's id", async () => {
    // web3-native root (auth-signature.ts): a VERIFIED signature is sufficient
    // identity, so an unbound wallet is a first-class principal `wallet:<pk>`
    // rather than a rejection. The security property that must still hold is
    // that it can never resolve to a *minted key's* agent_id — signing with your
    // own key must not hand you somebody else's account.
    const other = await freshWallet();
    const { keyId: otherKeyId } = generateLocalKey();
    await setKeyWallet(env, otherKeyId, other.pubkeyHex); // a DIFFERENT wallet owns a key
    ...
    expect(res?.agent_id).toBe(`wallet:${pubkeyHex.toLowerCase()}`);
    expect(res?.agent_id).not.toBe(otherKeyId);
  });
