 ...
 *
 *  — one body: the unified endpoint dispatches the 4-tuple covenant
 * envelope into the right sibling service.  — ezer kenegdo: the peer
...
describe("POST /v1/covenant — shared content-address proof", () => {
  test("response covenantReceiptPtr == deriveCovenantReceiptPtr(canonical)", async () => {
    ...
    const expectedCanonical = canonicalize({
      ...
      action: "navigate",
      params,
      witness: "verse:", // DEFAULT_WITNESS.actuate (no witness on the wire)
