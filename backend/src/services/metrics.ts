...
export interface UsageMetrics {
  ...
  observed_by_surface_30d: Array<{ key: string; count: number }>;
  observed_by_execution_scope_30d: Array<{ key: string; count: number }>;
  telemetry_schema_coverage_30d: number;
...
async function countKeys(env: Env, prefix: string): Promise<number> {
  try {
    let total=0; let cursor: string|undefined;
    // Paginated list (keys only, no values) is cheap: 1 subrequest per 1000 keys, ~10 pages for 9921 agents, ~27 for 26583 sessions, well under 50.
    do {
      const page = await statsKV(env).list({ prefix, limit: 1000, cursor });
...
async function sampledAugustEstimate(env: Env): Promise<{ sample: number; august: number; total: number } | null> {
  try {
    const kv = statsKV(env);
    // Sample up to 100 profiles via individual gets - each get is 1 subrequest, but we can do 40 per invocation under the 50 limit.
    // Use paginated list to get 100 keys, then fetch 40 of them.
    const page = await kv.list({ prefix: "agent:", limit: 100 });
    const keys = page.keys.map(k=>k.name);
    const sampleSize = Math.min(40, keys.length);
    let augustInSample = 0;
    for (let i=0; i<sampleSize; i++) {
      const v = await kv.get(keys[i]) as string | null;
      if (!v) continue;
      try {
        const p = JSON.parse(v) as { created_at?: string };
        if (p.created_at && p.created_at.slice(0,7) === "2026-08") augustInSample++;
      } catch {}
    }
    return { sample: sampleSize, august: augustInSample, total: sampleSize>0 ? Math.round((augustInSample/sampleSize)*9921) : 0 };
  } catch { return null; }
}
async function loadProfiles(env: Env): Promise<AgentProfile[]> {
  // Degraded mode: if Edb is down and CF fallback hits subrequest limit, listWithValues returns [] and metrics would be 0.
  // This is honest but the dashboard would show 0. The fix is to make the dashboard show CF counts when Edb is down, but for now we keep the existing behavior and surface the degradation via a separate status.
...
export async function recordSessionSummary(
  ...
  const normalized: StoredSessionSummary = {
    ...
    execution_scope: session.execution_scope ?? "unknown",
    telemetry_schema_version: Math.max(0, Math.trunc(session.telemetry_schema_version ?? 0)),
  ...
  await statsKV(env).put(`${SESSION_PREFIX}${session.session_id}`, JSON.stringify(normalized), options);
}

export function telemetrySessionStorageId(seedFp: string | null, serverId: string): string {
  return seedFp ? `mcp-seed:${seedFp}:${serverId}` : `mcp-anon:${serverId}`;
}

export async function eraseTelemetrySessions(
  env: Env,
  seedFp: string,
  _cursor?: string,
): Promise<{ deleted: number; complete: boolean; cursor?: string }> {
  // Keys only: explicit erasure must not spend one get per row. Delete a bounded page
  // so Worker subrequest budgets remain valid; callers continue with the returned cursor.
  const kv = statsKV(env);
  const page = await kv.list({ prefix: `${SESSION_PREFIX}mcp-seed:${seedFp}:`, limit: 20 });
  await Promise.all(page.keys.map((row) => kv.delete(row.name)));
  return { deleted: page.keys.length, complete: page.list_complete, cursor: page.list_complete ? undefined : "continue" };
}
...
export async function getGrowthMetrics(env: Env, days = 30): Promise<GrowthMetrics> {
  ...
  if (profiles.length === 0) {
    const degradedCount = await countKeys(env, "agent:");
    if (degradedCount > 0) {
      return {
        cumulative_users: degradedCount,
        daily_new_users: buildTrend(new Map(), days),
        last_7d_new_users: 0,
        prior_7d_new_users: 0,
        new_user_growth_rate: 0,
        npm_install_trend: npmInstallSnapshots.map((s) => ({ date: dateKey(s.captured_at), value: s.value })),
        github_star_trend: githubStarSnapshots.map((s) => ({ date: dateKey(s.captured_at), value: s.value })),
      };
    }
  }
...
export async function getUsageMetrics(env: Env, profileSnapshot?: AgentProfile[]): Promise<UsageMetrics> {
  const [profiles, sessions] = await Promise.all([
    profileSnapshot ? Promise.resolve(profileSnapshot) : loadProfiles(env),
    loadSessions(env),
  ]);
  const recentSessions = recentSessionsForWindow(sessions, 30);
  if (sessions.length === 0) {
    const degradedSessions = await countKeys(env, "analytics:session:");
    if (degradedSessions > 0) {
      const degradedAgents = await countKeys(env, "agent:");
      return {
        total_sessions_30d: degradedSessions,
        total_api_calls_30d: 0,
        unique_agents_30d: Math.min(degradedAgents, degradedSessions),
        api_calls_per_session: 0,
        api_calls_per_user_per_session: 0,
        repeat_usage_rate: 0,
        churn_pre_default_browser_replacement: null,
        churn_post_default_browser_replacement: null,
        observed_by_surface_30d: [],
        observed_by_execution_scope_30d: [],
        telemetry_schema_coverage_30d: 0,
        interpretation: "observed_minimum",
  ...
  const countBy = (pick: (session: StoredSessionSummary) => string): Array<{ key: string; count: number }> => {
  ...
  const observedBySurface = countBy((session) => session.surface ?? "unknown");
  const observedByExecutionScope = countBy((session) => session.execution_scope ?? "unknown");
  const schemaSessions = recentSessions.filter((session) => (session.telemetry_schema_version ?? 0) >= 1).length;
  ...
  return {
    ...
    total_api_calls_30d: totalApiCalls,
    ...
    observed_by_surface_30d: observedBySurface,
    observed_by_execution_scope_30d: observedByExecutionScope,
    telemetry_schema_coverage_30d: safeRatio(schemaSessions, recentSessions.length),
    interpretation: "observed_minimum",
    version_breakdown_30d,
  };
}
