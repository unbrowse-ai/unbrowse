...
/** Structural: key names an HTTP header *map* (name→value). Port of client isHeaderMapKey. */
function isHeaderMapKey(key: string): boolean {
  const k = key.trim();
  return (
...
/** Empty every nested header map in place (server re-seal for index wire). */
function stripHeaderMapsDeep(node: unknown): void {
  if (node == null || typeof node !== "object") return;
  if (Array.isArray(node)) {
    for (const item of node) stripHeaderMapsDeep(item);
    return;
  }
...
export function sanitizeForPublish<T extends SanitizableEndpoint>(endpoints: T[]): T[] {
  return endpoints.map((endpoint) => {
    ...
    // ALL header maps empty on the wire (not a name denylist). proven_recipe.headers
    // is a second copy of the cookie jar; example_request.headers same class.
    const proven = (clean as { proven_recipe?: { headers?: Record<string, string>; body?: unknown } }).proven_recipe;
    if (proven && typeof proven === "object") {
      proven.headers = {};
    }
    stripHeaderMapsDeep(clean);
