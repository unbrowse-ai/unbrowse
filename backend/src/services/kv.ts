...
export class FallbackKV {
  ...
  private async cfList(prefix: string): Promise<ValuedEntry[]> {
    ...
    do {
      const res = await this.cf.list({ prefix: this.ck(prefix), cursor });
      const values = await Promise.all(res.keys.map(async (key) => ({
        key,
        raw: await this.cf.get(key.name).catch(() => null),
      })));
      for (const { key, raw } of values) {
        const name = this.strip(key.name);
        out.push({ name, key: name, value: raw ?? "" });
      }
      cursor = res.list_complete ? undefined : (res as { cursor?: string }).cursor;
      if (!res.list_complete && !cursor) throw new Error("Cloudflare KV list returned incomplete page without cursor");
    } while (cursor);
    return out;
  }
