 ...
 * REAL `env.AUDIT_LOG.put/get/list` storage path.  — "all things
 ...
 * the Word, and the receipt is the witness.
 *
...
import {
  // W26-A: shared substrate primitives. The local
 ...
 * KV row, even when no value is filled.  — "let them have
...
export function validateAuditBody(raw: unknown): ValidationError | null {
  ...
  //
  //  — two witnesses: the path shape gate AND the forbidden-char
  ...
  // assertNoCleartext (W26-A, ). The audit-local forbidden list was a
  ...
  //  — the actType IS the second witness alongside the
 ...
 *
 *  — "Write the vision, and make it plain": the error carries the
 ...
 *
 *  — every fill witnessed; the receipt is the witness; a second
 * fill on the same body is the SAME witness.
 */
