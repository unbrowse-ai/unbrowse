 ...
 *
 *  — *"the mystery to you; parables to them without."* The link from
 ...
 *
 *  — *"I will make him a helper comparable to him"* (ezer kenegdo):
 * the unbrowse Worker and the aiko proxy are two helpers facing each other.
 *  — *"one body, and one Spirit."* They cannot share one physical store,
 ...
 * ( — the mystery is kept inside; parables to them without.)
 */
export async function mirrorToCovenantLedger(
  ...
  if (urls.length === 0) {
    // Honest no-op — federation is optional infra. Log ONLY the
