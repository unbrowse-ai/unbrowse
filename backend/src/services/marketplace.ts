...
// Registry-category second-level labels. Under a two-letter ccTLD these form a
// PUBLIC SUFFIX, not a registrable name: nobody owns "gov.sg", so "data.gov.sg"
// and "health.gov.sg" are different organisations that merely share a suffix.
//
// This is an enumeration on purpose, and it is the narrow exception the repo's
// standing rule allows ("a true protocol constant"): a public-suffix boundary is
// external registry policy and cannot be derived from the shape of a hostname.
// It is kept structural rather than per-site — a category list plus a ccTLD
// length test, not a list of domains — so a new country or a new site needs no
// entry here. The full PSL is deliberately not shipped into a CF Worker.
const REGISTRY_CATEGORY_LABELS = new Set([
  "co", "com", "net", "org", "gov", "edu", "ac", "mil", "sch", "nic", "id",
]);

/**
 * Base (registrable) domain: the part a single owner actually controls.
 *
 *   api.github.com     -> github.com
 *   bank.example.com   -> example.com
 *   alpha.co.uk        -> alpha.co.uk    (co.uk is a public suffix)
 *   guide.data.gov.sg  -> data.gov.sg    (gov.sg is a public suffix)
 *
 * The backend's other extractor (services/validator.ts) takes the last two
 * labels unconditionally, which collapses alpha.co.uk and beta.co.uk to the
 * same "co.uk" — two unrelated publishers. That is safe there only because the
 * validator uses it as one input to a stricter descendant check; it is NOT safe
 * as a cache-sharing key, so this function does not reuse it.
 */
export function baseDomain(host: string): string {
  const parts = host.toLowerCase().split(".").filter(Boolean);
  if (parts.length <= 2) return parts.join(".");
  const [secondLevel, tld] = parts.slice(-2);
  // A two-letter TLD with a registry-category label under it => public suffix
  // spans two labels, so the registrable name is the last THREE.
  if (tld.length === 2 && REGISTRY_CATEGORY_LABELS.has(secondLevel)) {
    return parts.slice(-3).join(".");
  }
  return parts.slice(-2).join(".");
}

/** Two hosts belong to the same owner when they share a base domain. */
export function sameBaseDomain(a: string, b: string): boolean {
  const left = baseDomain(a);
  return left.length > 0 && left === baseDomain(b);
}

// Find the most-recently-updated published skill whose domain shares a BASE
// DOMAIN with the requested host. Used to serve `unbrowse.ai/<domain>` as a
// single SKILL.md per domain.
//
// This is the public, shared, cached read path: whatever it returns is rendered
// as SKILL.md, cached 300s in KV plus 120s at the edge, and reused by every
// other agent that asks. So the matching rule decides who may share a cache
// entry with whom, and the boundary is OWNERSHIP — the base domain.
//
// The previous rule matched `d.endsWith("."+target) || target.endsWith("."+d)`.
// That is not the same thing: it walked the hostname tree by string suffix, so
// a request for "gov.sg" was answered with a skill for "data.gov.sg", and a
// request for "co.uk" would collect whichever "*.co.uk" skill was published
// most recently — across unrelated organisations. Suffix-walking crosses
// ownership boundaries; base-domain equality does not.
//
// Subdomain and parent DO share an entry (bank.example.com and example.com are
// the same owner) — that is the intended policy, not an oversight. Note it is
// looser than publish admission, which additionally refuses to walk up; a
// credential is never carried through here (endpoints are scrubbed
// server-side at publish and the rendered markdown only describes HOW to fetch
// your own cookie), but the served endpoints are what an agent will send its
// own cookies to, which is why the owner boundary is enforced at all.
export async function getSkillByDomain(env: Env, domain: string): Promise<SkillManifest | null> {
  ...
  const candidates = all.filter((s) => {
    ...
    return sameBaseDomain(s.domain ?? "", target);
  });
