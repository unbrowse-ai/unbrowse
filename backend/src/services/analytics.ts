...
export async function getEngagement(env: Env, profileSnapshot?: AgentProfile[]): Promise<EngagementMetrics> {
  const profiles = profileSnapshot ?? await loadProfiles(env);
...
export async function getRetention(env: Env, days = 30, profileSnapshot?: AgentProfile[]): Promise<RetentionCohort[]> {
  const profiles = profileSnapshot ?? await loadProfiles(env);
  const activityByDate = buildActivityIndex(profiles);
