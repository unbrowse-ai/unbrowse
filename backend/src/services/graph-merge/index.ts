/**
 * graph-merge — the CRYPTOGRAPHICALLY GATED CRDT MERGE into the shared route graph
 * (plan node 4). SERVER-ONLY: the contributor constructs and signs a delta; this
 * service is the gate that admits it. No client (src/) file may import this
 * (enforced by scripts/delta-gate.sh's boundary check).
 *
...
import { verifyDeltaBound, type DeltaBoundClaim } from "../../../../src/values/delta-bound.js";
import {
...
/** The admission gate + LWW comparison for ONE endpoint, against its CURRENT winner (or null). This
...
export function gateAndCompare(current: RouteDelta | null, c: Contribution): AdmitResult {
  ...
  if (!verifyDeltaBound(delta, validity)) return { admitted: false, reason: "bad-validity-proof" };
