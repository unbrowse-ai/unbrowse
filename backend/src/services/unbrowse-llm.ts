...
const CONTRACT_LLM_CHAIN: ReadonlyArray<{ url: string; model: string; freeKey?: boolean }> = [
  {
    // PRIMARY: NVIDIA nemotron-nano-9b-v2 — FREE, 128k ctx (holds the full chat prompt), non-reasoning.
  ...
  },
  {
    ...
    // when the free tier is down. Cannot hold the long-context chat (too long); fine for compile.
