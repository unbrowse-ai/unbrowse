...
// GET /v1/graph/endpoint-penalties — endpoint failure penalties for the ranker.
//
// RETIRED with the Neon->IQ migration (2026-06). The penalty signal was a
// Postgres GROUP BY over the telemetry_executions ledger (fed by
// POST /telemetry/execute, also retired). SQL aggregation has no efficient KV
// form, so the feature was retired rather than re-architected (explicit
// non-goal). The endpoint stays so the ranker's caller keeps working — it now
// always returns the empty/unconfigured shape, which the caller already treats
// as "no penalties" and ranks unaffected.
graphRoutes.get("/graph/endpoint-penalties", bearerAuth, async (c) => {
  ...
  const endpointIds = rawIds.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
