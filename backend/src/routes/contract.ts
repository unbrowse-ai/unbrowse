...
import { verifyBinding, type CredBinding, type CredProof } from "../services/declare-binding";
...
export interface DeclareRequest {
  ...
  /**
   * OPTIONAL credential binding (src/values/cred-binding.ts scheme, verified by
   * `declare-binding.ts`). When present, the declare additionally proves
   ...
   * backend verifyBinding()s it and REJECTS (400 invalid_cred_binding) on failure.
   *
   * Backward-compat: when ABSENT, the existing Ed25519 declare_signature path is
   * UNCHANGED — resolve/execute/publish auto-declares keep working with no
   * binding. The binding is additive, never mandatory. (Payloads from the
   * retired zero-knowledge scheme's `zk_binding` field are ignored.)
   */
  cred_binding?: {
    ...
    sig: string;
    /** Proof of knowledge {sig, ctx}. ctx must equal the canonical body. */
    proof: { sig: string; ctx: string };
...
async function executeDeclare(
  ...
  try {
    ...
    // ─── Optional credential-binding gate ─────────────────────────
    //
    // ADDITIVE. When `cred_binding` is present, the declare additionally
    // proves knowledge of a wallet-bound credential, bound to THIS
    // declare body (ctx = the canonical declare bytes — the same bytes
    // declare-signature canonicalizes). verifyBinding (derived-key POK
    // sig + ed25519 wallet-sig over y) must hold, and the proof's ctx must
    // equal the canonical body. On any failure → 400 invalid_cred_binding.
    //
    // When ABSENT, this block is a no-op: the existing Ed25519
    // declare_signature path is unchanged (backward-compat — auto-declares
    // from resolve/execute/publish keep working with no binding).
    if (req.cred_binding) {
      ...
      const binding: CredBinding = {
        y: req.cred_binding.y,
        root: req.cred_binding.root ?? req.wallet_identity ?? "",
        sig: req.cred_binding.sig,
      };
      const proof: CredProof = req.cred_binding.proof;
      const ctxMatches = proof.ctx === expectedCtxHex;
      const ok = ctxMatches && (await verifyBinding(binding, proof));
      if (!ok) {
        return c.json(
          {
            error: "invalid_cred_binding",
            detail: ctxMatches
              ? "credential proof did not verify against the binding"
              : "credential proof ctx does not equal the canonical declare body",
          },
          400,
        );
      }
...
contractRoutes.post("/contract/declare", async (c) => {
  ...
  // ─── Creation Order: the word lands first ───
  //
 ...
 * to the SAME process do see each other (unlike a per-request
 * ephemeral ledger). This is enough for the unblock signal and
