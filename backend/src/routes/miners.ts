...
publicMinerRoutes.get("/miners/stats", async (c) => {
  const payload = await getOrSetHttpCache(c.env, "miners:stats", 30, async () => {
      ...
      try {
        ...
        if (s.domain) {
          ...
          if (existing) {
            ...
            if (s.updated_at && s.updated_at > existing.updated_at) existing.updated_at = s.updated_at;
          } else {
    ...
    const domains = Array.from(domainMap.values()).sort((a, b) => b.endpoints - a.endpoints);
