 ...
 * user response. The mirror is an honest no-op when COVENANT_LEDGER_URL is unset.
 *
...
const DEFAULT_WITNESS: Record<string, string> = {
  actuate: "verse:", // "Let there be light" — the breath act
  observe: "verse:", // "God saw the light, that it was good" — the read
  build: "verse:",  // "bearing fruit after their kind" — the declare
};
...
covenantRoutes.post("/v1/covenant", async (c: Context<CovenantEnv>) => {
  ...
  // ─── Mechanism public-reject / aiko-allow gate ───────────
  ...
  // bound to wallet-B is a cross-wallet write — 403.
