...
import { recordDomainQuery } from "../services/domain-stats.js";
...
searchRoutes.post("/search/domain", indexContributorAuth, requireSignedClient, async (c) => {
  ...
  try {
    ...
    // Domain observability, off the hot path — aggregate hit/miss per domain
    // only (recordDomainQuery cannot carry the agent or the intent; privacy is
    // in the signature).
    schedule(c, recordDomainQuery(c.env, domain, (results.results?.length ?? 0) > 0));
    return c.json(results);
  } catch (err) {
...
searchRoutes.post("/search/resolve", indexContributorAuth, requireSignedClient, async (c) => {
  ...
  try {
    ...
    // the hit-rate up.  — sufficient unto the day.
    //
    ...
    // Presentation-time ordering is relevance order (the internal canonical-
    // anchor organ is withheld from the public tree). The relevance cache above
    // is untouched.
    const value = cacheResult.value;
    ...
    // Domain observability, off the hot path — aggregate hit/miss per domain
    // only, recorded for anonymous callers too (nothing caller-identifying is
    // passed; a miss on an unindexed domain is the demand signal we monitor).
    if (domain) {
      const v = value as { domain_results?: unknown[]; global_results?: unknown[] };
      const hit = (v.domain_results?.length ?? 0) + (v.global_results?.length ?? 0) > 0;
      schedule(c, recordDomainQuery(c.env, domain, hit));
    }
    return c.json(value);
  } catch (err) {
