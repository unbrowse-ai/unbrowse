...
async function handleAuditPost(c: Context<AuditEnv>, expectedVariant: AuditVariant) {
  ...
  // Variant must match the route mounted.  — two witnesses:
...
// breath-act — Day-6 Dominion: non-value-bearing breath
...
//
//  — *"dominion."* Every eval read (snap/markdown/text/cookies) under
...
auditRoutes.get("/v1/audit/verify/:receiptId", async (c) => {
  ...
  try {
    ...
    // verbatim.  — sufficient unto the day.  — the
