...
import { getDomainStats } from "../services/domain-stats.js";
...
// GET /v1/stats/domains — every domain being indexed, used, and queried for,
// as monitorable per-domain counters. Aggregate-only by construction (see
// services/domain-stats.ts): no agent ids, wallets, or intent text — a reader
// learns which domains have coverage/demand, never who asked.
publicStatsRoutes.get("/stats/domains", async (c) => {
  const payload = await getOrSetHttpCache(c.env, "stats:domains", 60, async () => {
    return await getDomainStats(c.env);
  });
