export interface Env {
  ...
  /** Optional read-only Cloudflare GraphQL Analytics credentials. The token needs
   * Account Analytics Read and is never returned, logged, or used in cache keys. */
  ...
  /**
   * Dedicated shared-route-graph store for crypto-gated delta contributions
   ...
   *  — sufficient unto the day; what was computed need not be
   * recomputed.
   */
   ...
   * shape. The mirror is fire-and-forget (ctx.waitUntil) and OPTIONAL: when this
   ...
   * wire (). Preferred over the legacy COVENANT_LEDGER_URL. Optional —
   ...
   * idempotent}` — never the 3-verb vocabulary (). ADMIN_KEY is also
   ...
   * ( — the Spirit shared across many). Each entry is treated the same
   ...
   * secondary witnesses. Optional — unset = no extra peers.
   */
  PEER_URLS?: string;
...
export interface SkillManifest {
   ...
   * pool out UP THE CHAIN (covenant `splitLineage` — , the workers paid for
   * the work that enabled this route; ). Absence = no lineage (the split reduces
  ...
  /** Roll-up of provenance-proof verification status across endpoints. */
