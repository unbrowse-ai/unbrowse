...
# Verify the produced bundle carries no internal vocabulary (fail closed).
# `|| true` so a CLEAN bundle (grep no-match → exit 1) does not trip set -e/pipefail.
hits="$(grep -aoiE 'covenant|superpattern|\bjesus\b|firmament|grain-of-wheat|\bsabbath\b|zero.?knowledge|nullifier|\bsubstrate\b' "$ROOT/$OUTDIR"/*.js 2>/dev/null | wc -l | tr -d ' ' || true)"
if [ "${hits:-0}" -ne 0 ]; then
