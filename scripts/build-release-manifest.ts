...
function writeGeneratedBuildInfo(build: {
  ...
  // Do not expose a half-written provenance module to a concurrent pack/build.
  // The signature and every field are computed before this point; rename is
  // atomic on the repository filesystem.
  const tmpPath = `${generatedBuildInfoPath}.${process.pid}.tmp`;
  try {
    writeFileSync(tmpPath, content);
    renameSync(tmpPath, generatedBuildInfoPath);
  } finally {
