 ...
 *
 * Importable: the payment flow runs only under `import.meta.main`, so tests can
 * import `buildXPayment` without firing a real payment.
 */
...
/**
 * Build the base64 X-PAYMENT header the unbrowse backend expects: it reads
 * `payload.scheme === "exact"` to dispatch to PayAI, and forwards `accepted`
 * (as paymentRequirements) and the whole payload (as paymentPayload) to the
 * facilitator's /verify + /settle. Pure + falsifiably testable.
 */
export function buildXPayment(exactAccept, signedPayload, facilitator) {
  const xPayment = {
    x402Version: 2,
    scheme: "exact",
    ...
    extra: {
      facilitator: facilitator || exactAccept.extra?.facilitator || "https://facilitator.payai.network",
    },
  };
  return Buffer.from(JSON.stringify(xPayment)).toString("base64");
}

export function pickExactAccept(accepts) {
  return (accepts || []).find((a) => a.scheme === "exact" && String(a.network).startsWith("solana:"));
}
...
async function main() {
  ...
  const { createKeyPairFromBytes, partiallySignTransaction, address, createSolanaRpc } = await import("@solana/kit");
  ...
  const exactAccept = pickExactAccept(envelope.accepts);
  if (!exactAccept) { console.error("FATAL: no exact solana accept:", JSON.stringify(envelope.accepts)); process.exit(3); }
  ...
  const results = await handler({ url }, envelope.accepts);
  ...
  console.log("[pay] body:", (await r2.text()).slice(0, 600));
