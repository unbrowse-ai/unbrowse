...
# Papers reflection (privacy + economy story on FE + CLI)
...
bash scripts/cli-papers-docs-check.sh >/dev/null 2>&1 && ok "CLI eval stats docs block (privacy + x402)" || bad "cli-papers-docs-check.sh"

# binding-invariant / hole signals (secrets wallet-bound client-side, no raw id leaves)
run_root tests/holeify-attribution.test.ts   "holes: attribution ids → wallet-bound commitments (no raw id leaves)"
run_root tests/bind-forge.test.ts             "holes: forged/raw-in-bind rejected; real commitment admitted"
run_root tests/backfill-no-raw-secret.test.ts "holes: backfill wire body carries zero raw secrets, only commitments"
run_be tests/bind-admission.test.ts           "holes: backend residual-secret gate admits commitment, rejects raw id"
bash scripts/binding-invariant-gate.sh >/dev/null 2>&1 && ok "binding-invariant gate (secrets wallet-bound client-side)" || bad "binding-invariant-gate.sh"
