...
for f in "${DOCS[@]}"; do
  ...
  doc_filtered=$(awk '/<!-- validator:skip-routes-below -->/{skip=1; next} /<!-- \/validator:skip-routes-below -->/{skip=0; next} !skip' "$f" | grep -viE 'GREENFIELD|needs new|requires new|proposed|do(es)? NOT exist|NEW backend|new backend|coming soon|not built|not yet|no [^.]*(route|POST)|stays local|local-only|refuse(s)? to ship|skipped entirely')
