...
#   1. privacy/moat frontier green   — scripts/binding-gate.sh (wallet-bound backend
...
echo "=== GATED 1/4 — privacy/moat frontier (binding-gate) ==="
if ! bash scripts/binding-gate.sh >/tmp/replace-binding.log 2>&1; then
  echo "[replace-gate] FAIL — binding-gate red:"; tail -6 /tmp/replace-binding.log; fail=1
else
  echo "  ok — backend-is-the-harness frontier green ($(grep -c '  built' /tmp/replace-binding.log) nodes)"
fi
