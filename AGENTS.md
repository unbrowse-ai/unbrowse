...
- Do not hand-sync public skill docs. Use `scripts/sync-skill.sh` so `SKILL.md`, package contents, and `docs/whitepaper/` stay aligned and push to the checked-out public-skill branch instead of assuming `main`.
...
- Prefer harness/agent-accessible decision-making over hard heuristics for endpoint selection. Use heuristics mainly for safety rails, junk filtering, and obvious vetoes; do not let heuristics dominate semantic endpoint choice when the harness/agent can inspect plausible candidates directly.
...
- For product claims, count only CLI/orchestrator runs through the canonical eval stack: `bun run eval:core` for public confidence, `bun run eval:full` when auth behavior matters. `eval:core` is the task-shaped product path: fixture-backed marketplace retrieval regression + product-success + stable public WebArena-style multistep workflow checks. Treat live marketplace probes, auth-demo WebArena runs, and debug/breadth lanes like `eval:codex`, `eval:codex:stress`, `eval:codex:many-domains:*`, and repeatability reports as support/regression signal, not primary product truth.
