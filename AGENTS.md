...
- Rank/capture/retrieve fixes must stay general: use structural signals that improve indexing and reverse-engineering across sites. Do not specialize on one URL, domain, vendor, probe, or named benchmark case unless the site exposes a documented standard protocol.
- Cleanup/refactor work must actively remove existing non-general rank/capture/retrieve code and redundant scaffolding: replace domain/probe/vendor/url-shaped branches with reusable structural primitives, delete duplicate logic, and keep the smallest truthful implementation justified by real bench artifacts.
