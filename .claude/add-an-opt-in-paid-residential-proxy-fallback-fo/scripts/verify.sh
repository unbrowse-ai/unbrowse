#!/bin/bash
# verify.sh - runs the declared verify_gate for plan "add-an-opt-in-paid-residential-proxy-fallback-fo".
...
echo "[verify:$PLAN] gate: agent-browser published-artifact check (length preflight)"
echo "[verify:$PLAN] command:"
echo "  wc -w .claude/$PLAN.draft.md | awk '$1 >= 30 && $1 <= 4000 {exit 0} {exit 1}' && { echo '[validation_channel=agent-browser] length preflight ok. Declare the real outcome check as verify_command in the"
set +e
