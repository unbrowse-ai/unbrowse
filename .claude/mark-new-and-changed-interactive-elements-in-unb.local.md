...
plan_text: "Mark new and changed interactive elements in unbrowse snap output after an action: when snap runs following a click fill or submit in the same browse session, prefix elements that appeared since the previous snapshot with a delta marker so the agent sees what changed without re-reading the whole accessibility tree. Best practice source_id deepwiki:browser-use/browser-use (new-element star indicator). Scoped to one upgrade. Verified by a bun test asserting the delta marker appears only on elements absent from the prior snapshot."
...
verify_gate: "project sniff (package.json:scripts.test)"
verify_command: |
    npm test
...
bound_contracts:
  ...
  # - add-a-flash-minimal-resolve-output-mode-to-unbro   # organ-mate candidate, overlap 0.306
  # - add-structured-next-action-hints-to-unbrowse-mcp   # organ-mate candidate, overlap 0.306
  # - honor-path-extract-and-limit-in-unbrowse-mcp-exe   # organ-mate candidate, overlap 0.245
  # - make-unbrowse-banger-a-meta-contract-that-writes   # organ-mate candidate, overlap 0.204
  ...
  #   ~ unbrowse-improvement-loop   overlap 0.184
  #   ~ unbrowse-self-build   overlap 0.143
  #   ~ agent-factory-mvp-improvement-loop   overlap 0.122
  #   ~ unbrowse-improvement-fleet   overlap 0.122
...
status: "shipped: markNewSnapElements + unbrowse_snap delta marking added (src/api/browse-snap-detail-levels.ts + src/mcp.ts handler, commit 74939f24, branch feat/unbrowse-banger-flash-snap). verify green: tests/mcp-snap-new-element-marker.test.ts 7/7"
---
...
```
.claude/mark-new-and-changed-interactive-elements-in-unb/
├── scripts/
...
```bash
bash .claude/mark-new-and-changed-interactive-elements-in-unb/scripts/iterate.sh
```
