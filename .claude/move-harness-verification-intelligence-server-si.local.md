---
plan: move-harness-verification-intelligence-server-si
plan_text: "move harness verification intelligence server-side as MCP tools: the verify.sh Python field extractor (type parity audit comparing EndpointDescriptor fields across backend/src/types.ts + src/types/skill.ts + packages/sdk/src/contracts.ts) runs locally now but must become an MCP tool unbrowse_type_audit exposed by src/mcp.ts so the local agent harness calls MCP instead of running Python. Contract: any harness verify gate that computes intelligence (field diffs, type coverage, structural analysis) calls an MCP tool that returns raw evidence. The MCP tool is the server-side source of truth; the local verify.sh is a thin caller that reads the MCP output and surfaces it for agent judgment."
...
verify_gate: "MCP tool unbrowse_type_audit exists in src/mcp.ts + returns field diff evidence via MCP call"
verify_command: |
...
bound_contracts:
  ...
  # - test-the-unbrowse-agent-experience-end-to-end-an   # organ-mate candidate, overlap 0.316
  # - build-end-to-end-funnel-tracking-for-unbrowse-ev   # organ-mate candidate, overlap 0.281
  # - audit-the-unbrowse-capture-enrichment-resolve-ra   # organ-mate candidate, overlap 0.263
  # - drive-every-bug-class-surfaced-by-the-mcp-gate-r   # organ-mate candidate, overlap 0.263
  # - port-the-meta-harness-contract-dag-design-patter   # organ-mate candidate, overlap 0.263
  #
  ...
  #   ~ autonomous-build-harness   overlap 0.14
  #   ~ meta-harness   overlap 0.14
  #   ~ unbrowse-self-build   overlap 0.14
  #   ~ self-build   overlap 0.123
...
# move harness verification intelligence server-side as MCP tools: the verify.sh Python field extractor (type parity audit comparing EndpointDescriptor fields across backend/src/types.ts + src/types/skill.ts + packages/sdk/src/contracts.ts) runs locally now but must become an MCP tool unbrowse_type_audit exposed by src/mcp.ts so the local agent harness calls MCP instead of running Python. Contract: any harness verify gate that computes intelligence (field diffs, type coverage, structural analysis) calls an MCP tool that returns raw evidence. The MCP tool is the server-side source of truth; the local verify.sh is a thin caller that reads the MCP output and surfaces it for agent judgment.
...
```
.claude/move-harness-verification-intelligence-server-si/
├── scripts/
...
```bash
bash .claude/move-harness-verification-intelligence-server-si/scripts/iterate.sh
```
