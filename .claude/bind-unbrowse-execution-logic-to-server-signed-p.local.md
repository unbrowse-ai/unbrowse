---
plan: bind-unbrowse-execution-logic-to-server-signed-p
plan_text: "Bind unbrowse execution logic to server-signed per-call HMAC tokens so a reverse-engineered or patched binary cannot run resolve/execute/capture/publish without a valid server handshake. The server holds the canonical signing secret (RELEASE_MANIFEST_SIGNING_SECRET reused). The CLI MUST acquire a per-call signed challenge from /v1/session/exec-token bound to {agent_id, build_sha, deployed_at, intent_hash, nonce, expires_at} before executing the local capture/extraction/ranking path. Server rejects token issuance for unsigned builds (no /v1/version match) or tampered identity vars. Local code refuses to commit captured results to the marketplace OR write to local cache OR return results to the agent without a valid token. Fail-closed semantics: missing token = error_code=unsigned_build with actionable next_step. Bind the gate deeply enough that lifting the binary, running it standalone, or stripping x402 economics also strips the ability to use the marketplace index at all."
...
created: 2026-05-22
last_iterated: ""
status: shipped-waves-1-3-anti-RE-gate-live-enforce-flip-is-the-only-remaining-step
---

# Bind unbrowse execution logic to server-signed per-call HMAC tokens so a reverse-engineered or patched binary cannot run resolve/execute/capture/publish without a valid server handshake. The server holds the canonical signing secret (RELEASE_MANIFEST_SIGNING_SECRET reused). The CLI MUST acquire a per-call signed challenge from /v1/session/exec-token bound to {agent_id, build_sha, deployed_at, intent_hash, nonce, expires_at} before executing the local capture/extraction/ranking path. Server rejects token issuance for unsigned builds (no /v1/version match) or tampered identity vars. Local code refuses to commit captured results to the marketplace OR write to local cache OR return results to the agent without a valid token. Fail-closed semantics: missing token = error_code=unsigned_build with actionable next_step. Bind the gate deeply enough that lifting the binary, running it standalone, or stripping x402 economics also strips the ability to use the marketplace index at all.
...
```
.claude/bind-unbrowse-execution-logic-to-server-signed-p/
├── scripts/
...
```bash
bash .claude/bind-unbrowse-execution-logic-to-server-signed-p/scripts/iterate.sh
```
