...
plan_text: "Honor path extract and limit in unbrowse MCP execute with a jq truncation recipe: make the execute tool in src/mcp.ts apply the caller-supplied path, extract and limit before the ~25KB wire budget truncates, and when the response still exceeds the budget emit a claude-code-style truncation hint carrying a concrete jq recipe and a maxResultSizeChars override pointer instead of a dead-end cut. Best practice source_id deepwiki:anthropics/claude-code (large-output handling, format-specific recipes). Scoped to one upgrade. Verified by a bun test on a response over 25KB asserting path and limit narrow the body and the truncation hint with jq recipe is present."
...
bound_contracts:
  ...
  # - add-structured-next-action-hints-to-unbrowse-mcp   # organ-mate candidate, overlap 0.345
  # - drive-every-bug-class-surfaced-by-the-mcp-gate-r   # organ-mate candidate, overlap 0.19
  # - audit-the-unbrowse-capture-enrichment-resolve-ra   # organ-mate candidate, overlap 0.155
  # - build-a-proxy-mcp-server-in-front-of-unbrowse-mc   # organ-mate candidate, overlap 0.155
  # - make-unbrowse-banger-a-meta-contract-that-writes   # organ-mate candidate, overlap 0.155
  #
  ...
  #   ~ unbrowse-improvement-loop   overlap 0.138
  #   ~ autonomous-build-harness   overlap 0.103
  #   ~ integrate-anything   overlap 0.103
  #   ~ boldsign   overlap 0.086
...
```
.claude/honor-path-extract-and-limit-in-unbrowse-mcp-exe/
├── scripts/
...
```bash
bash .claude/honor-path-extract-and-limit-in-unbrowse-mcp-exe/scripts/iterate.sh
```
