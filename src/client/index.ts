...
/** Website login URL that pairs the local CLI and lands on a safe in-app path. */
export function buildDashboardPairingUrl(opts: {
  frontendUrl: string;
  localBaseUrl: string;
  pairToken: string;
  nextPath?: string;
}): string {
  const next = opts.nextPath && opts.nextPath.startsWith("/") && !opts.nextPath.startsWith("//")
    ? opts.nextPath
    : "/dashboard";
  const url = new URL(`${opts.frontendUrl.replace(/\/+$/, "")}/login`);
  url.searchParams.set("local", opts.localBaseUrl.replace(/\/+$/, ""));
  url.searchParams.set("pair", opts.pairToken);
...
/** True while the one-shot pairing file still exists (not yet consumed or expired). */
export function isDashboardPairingTokenPending(token: string): boolean {
  const safe = safePairingToken(token);
  if (!safe || safe !== token) return false;
  return existsSync(join(getPairingDir(), `${safe}.json`));
}
...
/**
 * Whether the operator has explicitly authorised unbrowse to install software on
 * this machine in response to an HTTP status.
 *
 * Off unless set, and "0"/"false"/"" are off — an opt-in that a stray empty env
 * var can flip to on is not consent. This is deliberately a separate variable
 * from UNBROWSE_UPDATE_COMMAND: the command says WHAT would run, this says
 * WHETHER anything may run, and the leak was that setting the former implied
 * the latter.
 */
export function autoUpdateOptedIn(): boolean {
  const v = process.env.UNBROWSE_AUTO_UPDATE?.trim().toLowerCase();
  return v != null && v !== "" && v !== "0" && v !== "false" && v !== "no";
}

/** Process-lifetime gate so a multi-call resolve path does not spam 3 warn lines N times. */
let _clientUpdateWarned = false;
function warnClientUpdateOnce(errCode: string, suggested: string): void {
  if (_clientUpdateWarned) return;
  _clientUpdateWarned = true;
  console.warn(`\n[unbrowse] Client attestation mismatch (${errCode}). Local paths still run.`);
  console.warn(`[unbrowse] To refresh the shipped binary: ${suggested}`);
  console.warn("[unbrowse] (set UNBROWSE_AUTO_UPDATE=1 to auto-apply once; source checkouts often hit this)");
}
/** Test hook */
export function _resetClientUpdateWarnForTests(): void {
  _clientUpdateWarned = false;
}
...
async function apiRequest<T = unknown>(
  ...
  // Handle 426 — client outdated or verification failed.
  //
  // This used to run `curl -fsSL … | bash` followed by `pkill -9 -f 'unbrowse|kuri'`
  // the moment a server answered 426: an HTTP response chose what shell command
  // executed on the user's machine, with no prompt, no signature check and no
  // interactive guard. Three ways that goes wrong, all reachable today:
  //   - anything that can answer for the API (a proxy, DNS, a compromised or
  //     merely mistaken backend) gets arbitrary local code execution;
  //   - UNBROWSE_UPDATE_COMMAND replaces the command wholesale, so any process
  //     able to set an env var picks what runs;
  //   - `pkill -9 -f 'unbrowse|kuri'` is a substring match over every process on
  //     the box — it killed a test runner mid-suite on a developer machine, and
  //     the backend currently returns 426 for source checkouts, so this was not
  //     a hypothetical path.
  //
  // Installing software is the user's decision, so the default is now to say what
  // to run and refuse to run it. The failure branch below already did exactly
  // that; it is now the only branch unless the operator opts in explicitly.
  if (res.status === 426 && !opts?.skipAutoUpdate) {
    ...
    if (errCode === "client_update_required" || errCode === "client_verification_failed") {
      // The command shown to a human may come from the server — it is only ever
      // printed. The command we would EXECUTE never does.
      // Prefer npm @latest for people who installed via npm (the common path);
      // fall back to install.sh when the server supplies a custom command.
      const suggested = (typeof (data as Record<string, unknown>).update_command === "string"
        ? String((data as Record<string, unknown>).update_command)
        : "npm install -g unbrowse@latest");

      if (!autoUpdateOptedIn()) {
        // Soft path: marketplace/backend may 426 on source checkouts or
        // slightly stale npm builds. Warn once, then throw so callers fall
        // through to local direct-document / capture — do not spam 3 lines
        // per concurrent API hop (live HN get printed this 6+ times).
        warnClientUpdateOnce(String(errCode), suggested);
        throw new Error(`Client update required. Run: ${suggested}`);
      }

      console.warn(`\n[unbrowse] Server requires a client update (${errCode}).`);
      console.warn("[unbrowse] UNBROWSE_AUTO_UPDATE is set — attempting automatic update...");
      try {
        ...
        console.warn(`[unbrowse] Please update manually: ${suggested}`);
        throw new Error(`Client update required. Run: ${suggested}`);
      }
    }
  }
...
// SECURITY: skill_id is server-generated by nanoid() but the marketplace
// response is fully untrusted (custom UNBROWSE_BACKEND_URL, MITM, compromised
// origin) and the value flows straight into a filesystem path. A skill_id of
// `../../../.zshrc` would let a malicious upstream overwrite arbitrary files
// next time we cachePublishedSkill. nanoid() emits the same charset minus
// the underscore — we accept a slightly broader [a-z0-9_-]{1,64} so legacy
// admin-seeded skill IDs continue to load.
...
function skillCachePath(skillId: string): string {
  if (!isSafeSkillId(skillId)) {
    throw new Error(`unsafe skill_id: ${JSON.stringify(skillId).slice(0, 80)}`);
  }
...
function writeSkillCache(skill: SkillManifest, scopeId?: string): void {
  if (!isSafeSkillId(skill.skill_id)) {
  ...
  try {
    ...
    // Commitment input-censoring at the persistence firmament: sensitive write-body
...
export async function publishSkill(
  ...
  if (options.idempotencyKey) proofHeaders["X-Unbrowse-Idempotency-Key"] = options.idempotencyKey;
  if (proofCount > 0) {
    proofHeaders["X-Unbrowse-Provenance-Proof-Count"] = String(proofCount);
  }
  ...
  // Last-line client seal: even if a caller bypassed marketplace sanitize,
  // nothing secret-shaped may POST to /v1/skills. Headers of the HTTP request
  // itself are only Unbrowse API Bearer + proof metadata — never site cookies.
  const publishDraft = sanitizeManifestForPublish(withRef);
  assertWirePayloadClean(publishDraft, "client.publishSkill→POST /v1/skills");
...
export async function updateEndpointSchema(
  _skillId: string,
  _endpointId: string,
  _schema: import("../types/index.js").ResponseSchema,
): Promise<never> {
  // Learned schema changes are publication mutations. They must be folded into
  // a sanitized manifest and authorized by a fresh lifecycle permit.
  throw new Error("schema_publish_requires_manifest_permit");
}
...
export async function recordAnalyticsSession(payload: AnalyticsSessionPayload): Promise<void> {
  if (isLocalOnly() || !getTelemetryConfig().enabled) return;
...
export async function publishGraphEdges(
  _domain: string,
  _node: { endpoint_id: string; method: string; url_template: string },
  _edges: Array<{ target_endpoint_id: string; kind: string; confidence: number }>,
): Promise<never> {
  // Graphs are part of the sanitized, artifact-fingerprinted manifest. A second
  // transport could bypass consent, lifecycle proof, and URL sanitization.
  throw new Error("graph_publish_requires_manifest_permit");
}
