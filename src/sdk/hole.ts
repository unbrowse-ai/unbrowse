 ...
 * holder — the wallet-bound hole: the request carries a wallet attestation, and only the holder
...
import { mergedAuthHeaders } from "./auth.js";
...
/** A wallet attestation attached when the hole is sealed (the wallet-bound hole). */
...
function makeDefaultTransport(clientOpts?: UnbrowseClientOptions): HoleTransport {
  // The published zero-argument quickstart must be real: when the caller did
  // not provide either auth mechanism, use the SDK-exported lazy local-wallet
  // signer. Construction stays synchronous; wallet creation/signing happens on
  // the first request.
  const client = new Unbrowse(
    clientOpts?.apiKey || clientOpts?.walletSigner
      ? clientOpts
      : { ...clientOpts, walletSigner: () => mergedAuthHeaders() },
  );
  return async (req: HoleRequest): Promise<Omit<HoleResult, "seal">> => {
    ...
    const res = (req.act
      ...
      : await client.search({
          intent: req.intent,
          ...(hostOf(req.url) ? { domain: hostOf(req.url) } : {}),
...
/** Construct a hole. With `wallet`, it is the wallet-protected hole. */
