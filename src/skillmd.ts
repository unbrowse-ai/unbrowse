 ...
 * (the hole-template / wallet-bind pattern): prove you hold the credential
 * without the skill or marketplace ever seeing it. This lists WHICH holes a
 * caller must be able to fill, not the values.
 */
export function credentialHoles(skill: SkillManifest): CredentialHole[] {
  ...
  const add = (name: string, location: string) => {
    ...
    if (!holes.has(key)) holes.set(key, { name, location, fill: "wallet:private-key" });
  };
 ...
 * format's wallet-filled holes connect to the real bind/prove/verify round-trip
 * (bound-hole.ts). Credentials are secrets filled from the wallet-backed
...
export function exportSkillMdLocal(skill: SkillManifest): string | null {
  ...
  // over OFFLINE when both are set.
