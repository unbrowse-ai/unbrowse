...
// one verb per surface (CLI / MCP / route). MCP tool names hit
// dispatchByKind, which routes by op_kind to the matching v7
// handler (build/act/eval) and returns a structured result. When
...
function listUserContextResources(): ResourceDefinition[] {
  ...
  // BEFORE invoking resolve/go. the platform uses this state internally
  // at capture time; exposing it as resources lets the calling LLM make
  // the routing decision instead of the platform guessing.
  //
  ...
  return [
    {
      ...
      read: async () => {
        ...
        const claudePath = path.join(os.homedir(), "(internal)");
        try {
        ...
        // fresh key. platform-faithful: probe collects status_code +
...
export function maybePostProcessResult(result: Record<string, unknown>, args: Record<string, unknown>): unknown {
  ...
  // platform-internal: resolveSkillId / executeResolvedEndpoint already
...
export function addGoNextStepHints(
  ...
  // tools/list now exposes. the platform reports what is callable; it
...
const tools: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        skill_id: { type: "string", description: "Optional. The skill_id you just executed against. When present, the platform applies a Bayesian-smoothed reliability update to (skill_id, endpoint_id)." },
  ...
  {
    name: "unbrowse_snap",
    description: "Get the current accessibility snapshot with stable element refs like e12. Use during a browse session (after unbrowse_go) to see what's on page before interacting. Defaults to detail_level=\"minimal\" (under 1KB); pass \"summary\" for landmark breakdown or \"full\" for the raw tree. Pass session_id from the unbrowse_go response when multiple browse sessions are concurrently live (parallel agents); the platform raises session_id_required if more than one session exists and no id is given.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  // by tests/mcp-fetch-resilience.test.ts. platform enables (env-gated
 ...
 *
 * platform-correct behaviour: detect, warn loudly with the exact
 ...
 * decides "the user actually wants this dead" baked into the platform
