...
export type GraphQLOperationSemantics = "query" | "mutation" | "unknown";

/** Classify an endpoint from the actual GraphQL document, never POST alone. */
export function classifyGraphQLOperation(endpoint: EndpointDescriptor): GraphQLOperationSemantics {
  if (endpoint.method !== "POST") return "unknown";
  const body = endpoint.body as unknown;
  let document = "";
  if (typeof body === "string") {
    try {
      const parsed = JSON.parse(body) as { query?: unknown };
      document = typeof parsed.query === "string" ? parsed.query : body;
    } catch { document = body; }
  } else if (body && typeof body === "object") {
    const query = (body as Record<string, unknown>).query;
    if (typeof query === "string") document = query;
  }
  // The document is stronger evidence than a URL suffix: several GraphQL APIs
  // accept POST at `/`. Strip comments before inspecting the leading operation.
  // Shorthand `{ ... }` is a query by the grammar; ambiguous/persisted ops stay unknown.
  const normalized = document.replace(/#[^\n\r]*/g, "").trim();
  if (/^mutation\b/i.test(normalized)) return "mutation";
  if (/^(?:query\b|\{)/i.test(normalized)) return "query";
  return "unknown";
}

export function endpointIsConfirmedReadOnly(endpoint: EndpointDescriptor): boolean {
  return endpoint.method === "GET"
    || endpoint.idempotency === "safe"
    || endpoint.idempotency === "read"
    || classifyGraphQLOperation(endpoint) === "query";
}
...
export function getEndpointPolicy(endpoint: EndpointDescriptor): EndpointPolicyDescriptor | undefined {
  ...
  if (endpointIsConfirmedReadOnly(endpoint)) return undefined;
