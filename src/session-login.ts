...
/**
 * Standard browser headers that are NOT auth-related.
 * We exclude these and capture EVERYTHING else — any non-standard header
 * is potentially auth/session related. This is a blocklist approach:
 * instead of guessing which headers ARE auth (and missing some),
 * we just exclude known noise.
 */
const STANDARD_BROWSER_HEADERS = new Set([
  // Content negotiation
  ...
  "if-modified-since", "if-unmodified-since", "etag", "expires",
  // Connection
  "connection", "keep-alive", "host", "origin", "referer", "referrer",
  // Browser fingerprint (auto-set, not auth)
  ...
  // Other standard
  "upgrade-insecure-requests", "dnt", "te", "expect", "range",
  "transfer-encoding",
 ...
 *
 * Strategy: capture ALL non-standard headers. Any header the site's frontend
 * sends that isn't a standard browser header is potentially auth-related
 * (csrf tokens, session IDs, API keys, custom auth schemes, etc.).
 * Downstream systems (provenance, replay) decide what to use.
 */
function extractAuthHeaders(
  ...
  // From captured requests — grab everything that's not standard browser noise
  for (const entry of captured) {
    for (const [name, value] of Object.entries(entry.headers)) {
      const lower = name.toLowerCase();
      if (!STANDARD_BROWSER_HEADERS.has(lower) && !lower.startsWith(":")) {
        authHeaders[lower] = value;
      }
    }
  }
