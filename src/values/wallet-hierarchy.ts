 ...
 * parent SIGNS the child's pubkey (the vertical covenant). This turns the single
 ...
 * packet wallet, each covenanted to the one above.
 *
 * Verification walks the vertical covenant chain from the root: every layer's parent
 ...
 * "ownership" is provable: the parent re-derives the exact child it covenanted.)
 */
...
export function childSeed(parentSeed: Uint8Array, layer: string): Uint8Array {
  return new Uint8Array(hkdfSync("sha256", parentSeed, Buffer.from("covenant-layer-v1"), Buffer.from(`child:${layer}`), 32));
}
...
export interface LayerWallet {
  ...
  covenantSig: string;  // parent's Ed25519 signature over this child's pubkey (hex)
}
 ...
 * child seed (ownership) and signs the child pubkey (covenant). Returns only
 * pubkeys + covenant signatures — no child seed ever leaves.
 */
 ...
 *  zeroed; only pubkeys + covenant signatures returned). */
...
/** Walk the vertical covenant chain from `rootPub`: every layer's parent is the one
 *  above and the parent's covenant signature over the child verifies. Fails closed. */
export function verifyLayerTree(tree: LayerWallet[], rootPub: string): boolean {
  ...
  for (const w of tree) {
    ...
    try {
      if (!verifyEd25519(Buffer.from(w.parent, "hex"), Buffer.from(w.pubkey, "utf8"), Buffer.from(w.covenantSig, "hex"))) {
        return false;
      }
    } catch {
      return false;
