...
/**
 * U-3: resolves through getUnbrowseHome() so UNBROWSE_HOME relocates it, as
 * `runtime/paths.ts` promises it does "everywhere".
 *
 * This one mattered more than the other five. It backs the CLI's FAST PATH
 * (`cli.ts:841` — "a fresh resolution-cache hit short-circuits BEFORE the
 * in-process" resolve), so while it read the caller's REAL home a run with an
 * isolated UNBROWSE_HOME was still answered from the developer's warm cache:
 * `cache_hit:true`, `mode:resolution_cache`, and identical total_ms, without
 * executing anything. Every "cold" measurement taken through this path was
 * measuring the developer's history, not a cold call.
 */
export function defaultResolutionCacheDir(): string {
  return join(getUnbrowseHome(), "resolution-cache");
}
