/**
 * cardinality — the net-vs-fish guard (, "the spreading of nets;
...
/** Exact result count explicitly requested by the caller, if any. */
export function requestedResultCount(intent?: string): number | null {
  const match = (intent ?? "").match(/\b(?:exactly|first|top|return|show|list|get|fetch)\s+(\d{1,3})\b/i);
...
// (): a net is satisfiable only by many fish. Every resolve→execute
...
/** Structural collection signal (the net, ): do these link hrefs
...
export function isConcreteResourceLeaf(leaf: string): boolean {
  return (
    !!leaf &&
    !/(?:list|search|results?|index|home|feed|catalog|collection)(?:\.[a-z0-9]+)?$/.test(leaf) &&
...
/**
 * True when an array is a coherent answer for this URL + intent. This is kept
 * stateless: admission depends only on the request, never on a remembered route.
 * A plural collection leaf with collection query controls is the important API
 * case (`/issues?state=open&per_page=100`); a concrete detail leaf remains closed
 * (`/issues/123?state=open`).
 */
export function requestExpectsCollection(url: string | undefined, intent?: string): boolean {
  if (!url) return isListLikeIntent(intent) || requestedResultCount(intent) !== null;
  try {
    const parsed = new URL(url);
    const segments = parsed.pathname.split("/").filter(Boolean);
    const leaf = decodeURIComponent(segments.at(-1) ?? "").toLowerCase();
    if (!leaf || !isConcreteResourceLeaf(leaf) || urlPathLooksListLike(url)) return true;

    // Nested plural resources are collections when the caller asks for a list.
    const pluralLeaf = /(?:s|ies)$/.test(leaf) && !/^(status|news)$/.test(leaf);
    if (pluralLeaf && (isListLikeIntent(intent) || requestedResultCount(intent) !== null)) return true;

    // Filter/pagination controls turn a plural API resource into a collection.
    const collectionQuery = [...parsed.searchParams.keys()].some((key) =>
      /^(?:q|query|search|keyword|keywords|state|status|filter|sort|order|page|per_page|limit|offset|cursor|size)$/i.test(key),
    );
    return pluralLeaf && collectionQuery;
  } catch {
    return false;
  }
}
