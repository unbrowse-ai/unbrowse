 ...
 * and COVENANTED (signed). So a packet-layer action is signed by the packet wallet,
 ...
 *   covenantSig — the PARENT's signature over this layer's pubkey (vertical ownership)
...
export interface LayerSignedRecord {
  ...
  covenantSig: string; // parent's signature over this layer's pubkey (vertical ownership)
...
/** Verify all three bindings from the root: vertical ownership covenant, each
...
export function verifyLayerWalletDescent(chain: LayerSignedRecord[], rootPub: string): boolean {
  ...
  for (let seq = 0; seq < chain.length; seq++) {
    ...
    try {
      // vertical: the parent owns (covenanted) this layer's wallet
      if (!verifyEd25519(Buffer.from(r.parent, "hex"), Buffer.from(r.pubkey, "utf8"), Buffer.from(r.covenantSig, "hex"))) return false;
