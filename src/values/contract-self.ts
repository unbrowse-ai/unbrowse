...
import { FDRY_MINT, FDRY_VAULT_PDA, fdryNativePortrait, isFdryNativeEnabled } from "./fdry-native.js";

export { FDRY_MINT, FDRY_VAULT_PDA };

export interface SelfPortrait {
  economy: {
    /** FDRY BONDS (stake/security), never settles usage — , no two masters. */
...
export function contractSelfPortrait(): SelfPortrait {
  const fdry = fdryNativePortrait();
  return {
    economy: {
      stakeToken: { symbol: "FDRY", mint: FDRY_MINT, role: fdry.role },
      ...
      tithe: isFdryNativeEnabled() ? fdry.tithe : "usage revenue routes to the vault → stFDRY NAV; holders who abide earn it, nobody else",
    },
    security: {
