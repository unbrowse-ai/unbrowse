...
function rejectionReason(skill: SkillManifest, endpoint: EndpointDescriptor): PublishAdmissionReason | null {
  ...
  // Discovery/capture is evidence that a route exists, not evidence that it
  // can be replayed. Only a successful replay promotes an endpoint to the
  // marketplace's viable set.
  if (endpoint.verification_status !== "verified") return "not_replay_verified";
