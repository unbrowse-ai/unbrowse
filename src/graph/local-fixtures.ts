...
export function buildLocalHarnessFixtures(): { skills: SkillManifest[]; cases: LocalHarnessCase[] } {
  ...
  const agentmail = baseSkill("fixture-agentmail", "agentmail.to", [
    endpoint("agentmail-create-inbox", "POST", "https://api.agentmail.to/inboxes", {
      action_kind: "create",
      resource_kind: "inbox",
      description_in: "Optional username, domain, display_name, client_id",
      description_out: "Creates a new email inbox and returns inbox_id and email address",
      response_summary: "inbox_id, email, display_name, created_at",
      example_request: { username: "unbrowse-example", display_name: "Unbrowse Agent" },
      example_response_compact: { inbox_id: "abc123", email: "unbrowse-example@agentmail.to" },
      example_fields: ["inbox_id", "email", "display_name", "created_at"],
      requires: [],
      provides: [
        { key: "inbox_id", semantic_type: "inbox_identifier", source: "response", example_value: "abc123" },
        { key: "email", semantic_type: "email_address", source: "response", example_value: "unbrowse-example@agentmail.to" },
      ],
      negative_tags: [],
      confidence: 0.95,
      auth_required: true,
    }),
    endpoint("agentmail-list-inboxes", "GET", "https://api.agentmail.to/inboxes", {
      action_kind: "list",
      resource_kind: "inbox",
      description_in: "Optional limit, page_token",
      description_out: "Returns all email inboxes with their ids, emails, and creation dates",
      response_summary: "inboxes[].inbox_id, inboxes[].email, inboxes[].display_name",
      example_request: {},
      example_response_compact: { inboxes: [{ inbox_id: "abc123", email: "unbrowse-example@agentmail.to" }] },
      example_fields: ["inboxes[].inbox_id", "inboxes[].email", "inboxes[].display_name"],
      requires: [],
      provides: [
        { key: "inbox_id", semantic_type: "inbox_identifier", source: "response", example_value: "abc123" },
      ],
    ...
    endpoint("agentmail-list-messages", "GET", "https://api.agentmail.to/inboxes/{inbox_id}/messages", {
      action_kind: "list",
      resource_kind: "message",
      description_in: "Requires inbox_id. Optional limit, labels, page_token",
      description_out: "Returns messages in an inbox with sender, subject, and body",
      response_summary: "messages[].message_id, messages[].from, messages[].subject, messages[].text",
      example_request: { inbox_id: "abc123" },
      example_response_compact: { messages: [{ message_id: "m1", from: "noreply@example.com", subject: "Verify" }] },
      example_fields: ["messages[].message_id", "messages[].from", "messages[].subject", "messages[].text"],
      requires: [{ key: "inbox_id", semantic_type: "inbox_identifier", required: true, source: "url_template" }],
      provides: [
        { key: "message_id", semantic_type: "message_identifier", source: "response", example_value: "m1" },
        { key: "thread_id", semantic_type: "thread_identifier", source: "response", example_value: "t1" },
      ],
      negative_tags: [],
      confidence: 0.95,
      auth_required: true,
    }),
    endpoint("agentmail-get-message", "GET", "https://api.agentmail.to/inboxes/{inbox_id}/messages/{message_id}", {
      action_kind: "detail",
      resource_kind: "message",
      description_in: "Requires inbox_id and message_id",
      description_out: "Returns full message details including body text, html, extracted content, and attachments",
      response_summary: "message_id, from, subject, text, html, extracted_text, extracted_html",
      example_request: { inbox_id: "abc123", message_id: "m1" },
      example_response_compact: { message_id: "m1", from: "noreply@example.com", subject: "Your code", text: "123456" },
      example_fields: ["message_id", "from", "subject", "text", "html", "extracted_text"],
      requires: [
        { key: "inbox_id", semantic_type: "inbox_identifier", required: true, source: "url_template" },
        { key: "message_id", semantic_type: "message_identifier", required: true, source: "url_template" },
      ],
      provides: [],
      negative_tags: [],
      confidence: 0.95,
    ...
    endpoint("agentmail-send-message", "POST", "https://api.agentmail.to/inboxes/{inbox_id}/messages", {
      action_kind: "create",
      resource_kind: "message",
      description_in: "Requires inbox_id, to, subject, text. Optional html, cc, bcc, attachments",
      description_out: "Sends an email and returns message_id and thread_id",
      response_summary: "message_id, thread_id",
      example_request: { inbox_id: "abc123", to: ["user@example.com"], subject: "Hello", text: "Hi there" },
      example_response_compact: { message_id: "m2", thread_id: "t1" },
      example_fields: ["message_id", "thread_id"],
      requires: [{ key: "inbox_id", semantic_type: "inbox_identifier", required: true, source: "url_template" }],
      provides: [
        { key: "message_id", semantic_type: "message_identifier", source: "response", example_value: "m2" },
      ],
      negative_tags: [],
      confidence: 0.95,
      auth_required: true,
    }),
    endpoint("agentmail-list-threads", "GET", "https://api.agentmail.to/inboxes/{inbox_id}/threads", {
      action_kind: "list",
      resource_kind: "thread",
      description_in: "Requires inbox_id. Optional limit, labels, page_token",
      description_out: "Returns email threads with subject, senders, message count, and last message",
      response_summary: "threads[].thread_id, threads[].subject, threads[].message_count, threads[].senders",
      example_request: { inbox_id: "abc123" },
      example_response_compact: { threads: [{ thread_id: "t1", subject: "Verify", message_count: 2, senders: ["noreply@example.com"] }] },
      example_fields: ["threads[].thread_id", "threads[].subject", "threads[].message_count", "threads[].senders"],
      requires: [{ key: "inbox_id", semantic_type: "inbox_identifier", required: true, source: "url_template" }],
      provides: [{ key: "thread_id", semantic_type: "thread_identifier", source: "response", example_value: "t1" }],
      negative_tags: [],
      confidence: 0.95,
      auth_required: true,
    }),
  ...
  const skills = [discord, github, marketplace, linkedin, formDriven, agentmail];
