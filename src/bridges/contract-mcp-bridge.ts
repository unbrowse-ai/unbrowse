...
// ~/(internal) or a Rust binary on PATH). This
...
//
// platform principle: this bridge surfaces what the CLI already declares;
      ...
      instructions:
        binPath
          ? `Bridge resolved /contract bin at ${binPath}. ` +
            ...
            "The bridge owns no state; the CLI's ledger is the platform."
          : "No /contract bin detected. Set $CONTRACT_BIN, or install Claude Code (provides ~/(internal)).",
    });
    return;
  }
      ...
      rpcResult(id, {
        ...
        content: [{
          ...
          text:
            ...
            "Tried: $CONTRACT_BIN, `which contract`, ~/.cargo/bin/contract, ~/(internal)\n" +
