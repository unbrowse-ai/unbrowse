...
import { isObscuraSession, obscuraClient, isObscuraNull, closeObscuraPage } from "../../obscura/live-page.js";
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (isObscuraSession(rec)) {
    // Chrome-free backend: read the parked URL with one evaluate, then end the
    // broker — that IS the teardown (no target/context to close, no Chrome to
    // signal). The parked row below is backend-agnostic, so a session parked on
    // obscura restores on obscura.
    try {
      const href = await obscuraClient(rec).evaluate("location.href");
      targetUrl = isObscuraNull(href) ? "" : href.trim();
    } catch {
      // honest no-op — the parked URL falls back to empty
    }
    chromeKilled = closeObscuraPage(rec.sessionId) || tryKillProcess(rec.chromePid);
  } else {
  try {
