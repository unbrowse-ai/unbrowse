...
import { obscuraBackendSelected } from "../../obscura/live-page.js";
...
export interface AuthCaptureCapability {
  available: boolean;
  browser_family: "chromium";
  credential_store: "macos-keychain";
  reason?: "unsupported_platform" | "login_keychain_not_found";
}

/**
 * Auth capture is an end-to-end capability: a visible Chromium window alone is
 * not enough. We must also be able to seal the captured values. Keep this
 * preflight side-effect free so callers never ask a user to log in and only
 * then disclose that persistence is unavailable.
 */
export function getAuthCaptureCapability(): AuthCaptureCapability {
  if (platform() !== "darwin") {
    return {
      available: false,
      browser_family: "chromium",
  ...
  if (!existsSync(LOGIN_KEYCHAIN_PATH)) {
    return {
      ...
      browser_family: "chromium",
      credential_store: "macos-keychain",
      reason: "login_keychain_not_found",
    };
  }
  return { available: true, browser_family: "chromium", credential_store: "macos-keychain" };
}
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  // Chrome-free backend: this verb opens a HEADED window for a human to log in
  // by hand, and obscura has no UI surface at all — there is no window to show.
  // That is not a loss here, because unbrowse already has a stronger path to the
  // same outcome: the user is ALREADY logged in somewhere, so lift the session
  // out of their real browser (extractBrowserCookies / findBestBrowserSession)
  // instead of asking them to log in a second time inside a robot.
  if (obscuraBackendSelected()) {
    emit(
      {
        ok: false,
        error: "unsupported_on_obscura:breath_auth_capture",
        subcommand: "breath auth-capture",
        op_kind: meta.op_kind,
        backend: "obscura",
        domain: targetDomain,
        reason: "obscura has no UI, so an interactive login window cannot be shown",
        alternatives: [
          `unbrowse eval auth-inventory --domain ${targetDomain}`,
          "auth is imported from your existing browser session automatically on capture",
        ],
        terminal: true,
        retryable: false,
      },
    ...
    emit(
      {
        ...
        subcommand: "breath auth-capture",
        op_kind: meta.op_kind,
        error: "auth_capture_unavailable",
        reason: capability.reason,
        browser_family: capability.browser_family,
        credential_store: capability.credential_store,
        next_step: "Use `unbrowse auth <login-url>` for Chromium browser-cookie import and interactive vault storage.",
      },
      opts,
    );
