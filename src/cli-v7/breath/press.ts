...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  // page; the URL comes from a single evaluate, read here (pre-press) so the
  // audit binds to the URL the keypress happened on, same as the CDP path.
  ...
  try {
    if (viaObscura) {
      try {
        const href = await obscuraClient(rec).evaluate("location.href");
  ...
  try {
    if (viaObscura) {
      // Chrome-free backend: the broker composes down+up itself from the DOM
      // `key` value — the same key the CDP path dispatches.
      const res = await obscuraClient(rec).press(desc.key);
      if (/not found|no such element|failed/i.test(res)) {
        throw new Error(`press_failed:${keyName}`);
      }
    } else {
