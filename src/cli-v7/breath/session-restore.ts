/**
 * `unbrowse breath session-restore <session-id>` — restore a parked session.
 *
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  try {
    ...
    if (obscuraBackendSelected()) {
      // Chrome-free backend: a restore is a fresh broker navigated to the parked
      // URL — the same shape as respawning Chrome and re-creating the target,
      // minus the Chrome. The parked sessionId is preserved below exactly as on
      // the CDP path, so the agent's existing references stay valid.
      const opened = await openObscuraPage(row.targetUrl);
      chromeWsUrl = opened.record.chromeWsUrl;
      newTargetId = opened.record.targetId;
      newContextId = opened.record.contextId;
      rec = { ...opened.record, sessionId: row.sessionId, createdAt: Date.now() };
    } else {
  ...
  // W24.2 — emit a sig-keyed `session-restore` breath-act receipt.
