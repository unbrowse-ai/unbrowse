...
import {
  ...
  parseActTimeoutMs,
  withActDeadline,
  type ActDeadlineSpec,
} from "./_act-deadline.js";
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "breath select",
      {
        ...
        usage: "unbrowse breath select <selector> <pointer-or-value> [--session <id>] [--by value|label|index] [--arg key=value] [--argScope <json>] [--timeout <ms>]",
        positional: [
  ...
  // Every crossing below goes to a target that may already be gone. A stale
  // lease answers no CDP frame, and un-bounded that is the ISSUE-4 hang. One
  // shared bound (_act-deadline.ts), re-labelled per step so the failure names
  // which crossing stalled rather than just "select timed out".
  const deadline: ActDeadlineSpec = {
    subcommand: "breath select",
    opKind: meta.op_kind,
    waitingOn: `CDP attach to target ${rec.targetId}`,
    timeoutMs,
    opts,
  ...
  // ── Attach to CDP ─────────────────────────────────────────────────────────
  // Chrome-free backend: no CDP attach — the obscura broker already holds the
  // page, and the URL comes from a single evaluate (still bounded: the broker
  // hop is a network crossing too).
  const viaObscura = isObscuraSession(rec);
  let conn: Awaited<ReturnType<typeof attach>> | undefined;
  let targetSessionId: string = "";
  let currentUrl: string;
  try {
    if (viaObscura) {
      const href = await guardAct(
        { ...deadline, waitingOn: `obscura evaluate location.href (session ${rec.sessionId})` },
        () => obscuraClient(rec).evaluate("location.href"),
      );
      currentUrl = isObscuraNull(href) ? "" : href.trim();
    } else {
    const attached = await guardAct(deadline, async () => {
      const c = await attach(rec.chromeWsUrl);
      const target = await attachToTarget(c, rec.targetId);
      const hist = await call<
        Record<string, never>,
        { currentIndex: number; entries: Array<{ url: string }> }
      >(c, "Page.getNavigationHistory", {}, target.sessionId);
      return {
        conn: c,
    ...
    conn = attached.conn;
    targetSessionId = attached.targetSessionId;
    currentUrl = attached.currentUrl;
    }
  } catch (err) {
    emitErr(err, opts);
    process.exit(EX_CDP);
  ...
  if (!isPointer) {
    try {
      await guardAct(
        { ...deadline, waitingOn: `Runtime.callFunctionOn setting <select> ${selector}` },
        async () => {
          if (viaObscura) {
            // Chrome-free backend: obscura resolves the selector and picks the
            // option itself — one broker call replaces the callFunctionOn recipe.
            const res = await obscuraClient(rec).selectOption(selector, pointerArg);
            if (/not found|no such element|failed/i.test(res)) {
              throw new Error(`selector_not_found:${selector}`);
            }
            return;
          }
          if (!conn) throw new Error("cdp_connection_missing");
          return setSelectValue(conn, targetSessionId, selector, pointerArg, by);
        },
      );
    } catch (err) {
      emitErr(err, opts);
    ...
    // blocks (A5).
    const breathAudit = await guardAct(
      { ...deadline, waitingOn: `breath-act audit receipt for select ${selector}` },
      () =>
  ...
  try {
    try {
      ...
      try {
        // `withActDeadline` (rejects), NOT `guardAct` (emits + process.exit):
        // exiting here would skip the `finally` chain below and leave the
        // resolved bytes un-zeroed. The timeout is reported after safeZero,
        // in the catch — the bound must not cost the secret-hygiene invariant.
        await withActDeadline(
          async () => {
            if (viaObscura) {
              // Chrome-free backend: one broker call replaces the
              // callFunctionOn recipe; the string still lives only in this
              // stack frame and the bytes are zeroed in the finally below.
              const res = await obscuraClient(rec).selectOption(selector, valueAsString);
              if (/not found|no such element|failed/i.test(res)) {
                throw new Error(`selector_not_found:${selector}`);
              }
              return;
            ...
            if (!conn) throw new Error("cdp_connection_missing");
          return setSelectValue(conn, targetSessionId, selector, valueAsString, by);
          },
          timeoutMs,
          `Runtime.callFunctionOn setting <select> ${selector}`,
        );
      } finally {
    ...
    // Value bytes are zeroed and disposed by here, so it is now safe to take
    // the exiting path.
    if (err instanceof TargetLostError) {
      emitTargetLost({ ...deadline, waitingOn: err.waitingOn }, err);
  ...
  try {
    const res = await guardAct(
      { ...deadline, waitingOn: `POST ${auditUrl} (audit receipt for select ${selector})` },
      () =>
        fetch(auditUrl, {
          method: "POST",
