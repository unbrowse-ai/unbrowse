...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  // Chrome-free backend: the slot enumeration is a plain Runtime.evaluate of a
  // self-contained expression, so the SAME expression runs in obscura's V8 and
  // returns the same JSON. No CDP attach, no Chrome.
  ...
  try {
    if (viaObscura) {
      ...
      const raw = await client.evaluate(formSlotEnumExpression(formSelector));
      const parsedSlots = JSON.parse(raw) as { error?: string; slots?: FormSlot[] };
      if (parsedSlots.error) throw new Error(parsedSlots.error);
      slots = Array.isArray(parsedSlots.slots) ? parsedSlots.slots : [];
    } else {
  ...
  for (const slot of populated) {
    ...
    try {
      ...
      try {
        if (viaObscura) {
          // Deliberately per-slot, NOT obscura's batch browser_fill_form: a batch
          // call would require every slot's plaintext in memory at once, which
          // breaks this file's contract of one value in scope at a time, zeroed
          // immediately. One field per call keeps that contract intact.
          const res = await obscuraClient(rec).fill(slot.selector, valueAsString);
          if (/not found|no such element|failed/i.test(res)) {
            throw new Error(`selector_not_found:${slot.selector}`);
          }
          filled = true;
        } else {
          if (!conn) throw new Error("cdp_connection_missing");
