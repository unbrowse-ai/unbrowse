...
import { isObscuraSession, closeObscuraPage } from "../../obscura/live-page.js";
import {
  EX_GENERIC,
  emit,
  emitErr,
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (isObscuraSession(rec)) {
    // Chrome-free backend: there is no target/context to close and no Chrome to
    // signal — tearing down the broker process ends the page. closeObscuraPage
    // kills it and drops the broker record; the session record is deleted below
    // on the shared path, so the envelope stays identical.
