...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  // Chrome-free backend: obscura scrolls the page itself (its browser_scroll
  // reports {x,y,max_y,viewport_h}), so there is no box-model round-trip and no
  // wheel-coordinate to compute — the anchor is reported as the scroll delta's
  // origin rather than invented pixel coordinates.
  if (isObscuraSession(rec)) {
    try {
      const client = obscuraClient(rec);
      await client.scroll({ dx: amount.dx, dy: amount.dy });
