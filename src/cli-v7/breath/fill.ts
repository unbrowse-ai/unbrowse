...
import { resolveFillTarget } from "../../cdp/dom/ref.js";
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
        ...
        positional: [
          {
            name: "selector",
            description:
              "CSS selector (e.g. `#password`) OR an `eN` accessibility ref from `eval snap` (`e52`, `@e52`, `[e52]`). " +
              "Refs are ordinals into the accessibility tree and shift when an earlier fill adds text to the page — " +
              "re-snap between fills, or prefer a CSS selector, which does not shift.",
            required: true,
          },
  ...
  // ── Attach to the page, read current URL for contextHash binding ─────────
  // Chrome-free backend: no CDP attach — the obscura broker already holds the
  // page, and the URL comes from a single evaluate. The value-handling contract
  // below (one string boundary, zeroed in finally, no commitment on failure) is
  ...
  try {
    try {
      // Resolve the selector → backendNodeId → focus → insertText. The
      // selector may be CSS *or* an `@eN` accessibility ref from `eval snap`
      // (the help text has always promised both). Ref resolution lives in
      // src/cdp/dom/ref.ts because the interesting part is a refusal, not a
      // lookup: an `eN` ref is an ORDINAL into the AX tree, and an earlier
      // fill shifts it — which is how `fill <password-ref>` used to reach
      // `DOM.focus` holding the <label>'s DOM #text node and die on Chrome's
      // opaque "Node is not an Element". resolveFillTarget recovers a slid
      // ref only when exactly one control is unambiguously named by what the
      // ref landed on, and otherwise fails with a message that names the ref
      // and points at the CSS fallback.
      let backendNodeId: number | undefined;
      if (!viaObscura) {
        // The non-obscura path always attached above; bind it to a const so the
        // nested closure below narrows (a reassignable `let` does not).
        if (!conn) throw new Error("cdp_connection_missing");
        const cdp = conn;
        try {
          const target = await resolveFillTarget(
            <TParams, TResult>(method: string, params: TParams) =>
              call<TParams, TResult>(cdp, method, params, targetSessionId),
            selector,
          );
          backendNodeId = target.backendNodeId;
          await call(cdp, "DOM.focus", { backendNodeId }, targetSessionId);
        } catch (rawErr) {
      ...
      try {
        if (viaObscura) {
          // obscura resolves the selector and sets the value itself — one call
          // replaces resolveFillTarget + DOM.focus + Input.insertText. The
          // string still lives only in this stack frame; the bytes are zeroed
          // in the finally below exactly as on the CDP path.
          const res = await obscuraClient(rec).fill(selector, valueAsString);
