...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "breath click",
      {
        ...
        usage: "unbrowse breath click <selector> [--session <id>] [--button left|right|middle] [--click-count 1|2|3] [--modifiers shift,alt,ctrl,meta] [--timeout <ms>]",
        positional: [
  ...
  // Everything past this line crosses to Chrome. A stale lease/target accepts
  // the CDP frame and never answers, so each crossing is bounded — see
  // _act-deadline.ts. Timing out reports `target_lost`; it never returns a
  // click that did not happen.
  const deadline = {
    subcommand: "breath click",
    opKind: meta.op_kind,
    waitingOn: `CDP click on selector ${selector} (target ${rec.targetId})`,
    timeoutMs,
    opts,
  ...
  let currentUrl = "";
  try {
    await guardAct(deadline, async () => {
      if (isObscuraSession(rec)) {
        // Chrome-free backend: obscura clicks BY SELECTOR (it resolves the node
        // and dispatches the event itself), so there is no box-model round-trip
        // and no coordinates to report — x/y stay 0 in the envelope rather than
        // being invented.
        const client = obscuraClient(rec);
        const res = await client.click(selector);
  ...
  // gesture (A5). Bounded too: the receipt POST is a second network
  // crossing, and an un-timed one reintroduces the hang after the
  // gesture already landed.
  const breathAudit = await guardAct(
    { ...deadline, waitingOn: `breath-act audit receipt for click ${selector}` },
    () =>
