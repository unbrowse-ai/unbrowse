/**
 * `unbrowse breath proxy-rotate [--country=US]` — rotate the residential proxy.
 *
...
import { obscuraBackendSelected, openObscuraPage } from "../../obscura/live-page.js";
import {
  EX_GENERIC,
  emit,
  emitErr,
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  const meta = lookupKindMap("breath", "proxy-rotate")!;
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "breath proxy-rotate",
      {
        ...
        usage: "unbrowse breath proxy-rotate [--country <iso2>] [--session <id>]",
        positional: [],
        flags: [
  ...
  let target;
  try {
    // Chrome-free backend: obscura takes --proxy at process start, so a rotation
    // is a fresh broker on the new sticky IP — the same shape as respawning
    // Chrome with a new --proxy-server, minus the Chrome. Emits the same
    // session record so a following `breath go <url>` uses the rotated egress.
    if (obscuraBackendSelected()) {
      // obscura's --proxy takes a full URL, so the country-locked credentials
      // ride IN the URL. That closes a gap the Chrome path still has: Chrome
      // rejects embedded creds in --proxy-server and needs a
      // Fetch.continueWithAuth wiring that is not landed, so the rotated egress
      // only actually authenticates on this backend. Creds stay in-process and
      // never reach the envelope (KEEP-LOCAL invariant above).
      const authed = proxy.server.replace(
        "://",
        `://${encodeURIComponent(proxy.username)}:${encodeURIComponent(proxy.passwordWithCountry)}@`,
      );
      const opened = await openObscuraPage("about:blank", { proxy: authed });
      await writeSessionRecord(opened.record);
      emit(
        {
          ok: true,
          subcommand: "breath proxy-rotate",
          op_kind: meta.op_kind,
          session_id: opened.record.sessionId,
          backend: "obscura",
    ...
    // `breath go <url>` to navigate through.
