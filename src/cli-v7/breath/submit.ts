/**
 * `unbrowse breath submit [selector]` — form submission.
 *
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "breath submit",
      {
        ...
        usage: "unbrowse breath submit [selector] [--session <id>] [--wait] [--timeout <ms>]",
        positional: [
          {
            name: "selector",
  ...
  if (isObscuraSession(rec)) {
    // Chrome-free backend: the SAME submit expression runs in obscura's V8.
    // obscura returns an evaluate result as text, so the structured
    // SubmitResult is parsed back out of it (a non-JSON reply means the page
    // returned nothing usable, which is `no_result` exactly as on CDP).
    try {
      ...
      const raw = await client.evaluate(buildSubmitExpression(selector));
      try {
        result = JSON.parse(raw) as SubmitResult;
      } catch {
        result = isObscuraNull(raw) ? { ok: false, error: "no_result" } : { ok: true };
      }
    } catch (err) {
      emitErr(err, opts);
      process.exit(EX_CDP);
