...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  // below (one string boundary, zeroed in finally) is identical on both backends.
  ...
  if (!isPointerPath) {
    try {
      if (viaObscura) {
        // HONEST GAP, verified live against obscura v0.1.11: `breath type` types
        // into whatever holds focus, and obscura has no focus model — it rejects
        // a `:focus` selector ("Element not found: :focus") and `.focus()`
        // followed by `document.activeElement` yields nothing, so there is no
        // element to address. Rather than a wire that always fails opaquely,
        // refuse with the selector-addressed alternative that DOES work here.
        throw new Error(
          "unsupported_on_obscura:breath_type — obscura has no focused-element " +
          "model; use `breath fill <selector> <pointer>` (CSS selector or an eN " +
          "ref from `eval snap`), which addresses the element directly",
        );
      } else {
  ...
  try {
    try {
      ...
      if (viaObscura) {
        // Same honest gap as the literal path above (no focus model on obscura).
        // Thrown INSIDE the try so the finally still zeroes the resolved bytes.
        throw new Error(
          "unsupported_on_obscura:breath_type — obscura has no focused-element " +
          "model; use `breath fill <selector> <pointer>` instead",
        );
      } else {
        if (!conn) throw new Error("cdp_connection_missing");
