...
const ADAPTERS: Partial<Record<V7OpKind, ArgAdapter>> = {
  ...
  // breath get <intent> --url <url> [--header ... | --bearer-token ...]
  // MCP uses snake_case while the CLI's public spelling is kebab-case.
  // Normalize here so both surfaces reach the same direct read path.
  "breath:get": (a) => {
    const positional: string[] = [];
    if (typeof a.intent === "string") positional.push(a.intent);
    const flags: Record<string, string | boolean> = {};
    if (typeof a.url === "string") flags.url = a.url;
    if (typeof a.header === "string") flags.header = a.header;
    else if (a.header && typeof a.header === "object") flags.header = JSON.stringify(a.header);
    const bearer = a.bearer_token ?? a["bearer-token"];
    if (typeof bearer === "string") flags["bearer-token"] = bearer;
    if (a.fresh === true || a.no_cache === true) flags.fresh = true;
    if (a.no_browse === true) flags["no-browse"] = true;
    return { positional, flags };
  },
