 ...
 *   2. The route platform (~/Projects/route) — every KindSpec
...
export interface DispatchResult {
   ...
   * is a platform error the caller should surface, not a v6 fall-through.
   */
 ...
 *
 *    — *"the waters gathered... and the dry land appear."*
 *   The 37 named internet ops are the dry land (the product surface);
 *   the mechanism kinds (`context_fetch`, `route_populate`,
 *   `route_rank`, `rank_train`, `route_witness`, `identity_rules`,
 *   `discovery_query`, `recall_route`, ...) are the deep — they
 *   have no row here, so they are unreachable from this public surface
 *   (`.planning/v7-rip/INTERNET_LAYER_ROUTE_SURFACE.md` §5).
 *
 ...
 * mechanism is reachable only aiko-side (the sovereign route binary +
 ...
 * already serial per session; route fire-and-await is serial per
  ...
  switch (verb) {
    ...
    case "act": {
      const [go, fill, type, click, press, select, scroll, submit, execute, authCapture, proxyRotate, close] = await Promise.all([
        ...
        import("../act/click.js"),
        import("../act/press.js"),
        import("../act/select.js"),
        import("../act/scroll.js"),
        import("../act/submit.js"),
        import("../act/execute.js"),
        import("../act/auth-capture.js"),
        import("../act/proxy-rotate.js"),
        import("../act/close.js"),
      ]);
      return {
        go: go.handler,
...
export async function dispatchByKind(
  ...
  // Mechanism kinds (context_fetch / route_populate / route_rank /
  // route_witness / identity_rules / discovery_query / ...) fail
  // CLOSED here — structured `unknown_op`, no handler load, the route
  ...
  if (!entry) {
    return {
      ...
      // vocabulary or the mechanism-kind names.
