...
async function loadVerb(verb: string): Promise<Record<string, unknown>> {
  switch (verb) {
    ...
    case "eval": {
      const [snap, resolve, status, version, trace, markdown, screenshot, text, cookies, stats, skills, skill, sessions, earnings, settings, feedback, reflect, authInventory, browsers, spec, explain, search, inspect] = await Promise.all([
        import("../eval/snap.js"),
