 ...
 *
 * The verb routers (build/index.ts, act/index.ts, eval/index.ts) own
...
export async function dispatchVerb(
  ...
  switch (parsed.verb) {
    ...
    case "act": {
      const { router } = await import("./act/index.js");
      return router.dispatch(parsed, opts);
    }
    case "eval": {
...
export async function routeSub<HandlerArgs>(
  ...
  if (parsed.wantsHelp && !parsed.sub) {
    helpExit(
      verbName,
      {
        summary: `${verbName} — ${verbName === "build" ? "declare patterns" : verbName === "act" ? "animate the runtime" : "observe the runtime"}`,
        ...
        mcp_tool: null,
        verb: verbName as "build" | "act" | "eval",
      },
      opts,
    );
  }
