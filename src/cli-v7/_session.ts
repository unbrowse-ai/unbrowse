...
export interface BrowseSessionRecord {
  ...
  /**
   * The tab id the BROKER reports, when it reports one.
   *
   * Optional and back-compatible: a record without it (CDP, or written before
   * this field existed) reads exactly as before. It exists because obscura
   * sessions were persisted with a hard-coded placeholder id while the broker
   * listed a real one, so every rehydrated session failed liveness with
   * `tab_not_in_discover` and `run-js` answered `no_active_session` for a page
   * that was loaded and evaluable the whole time. Identity is asked for, not
   * assumed.
   */
  readonly tabId?: string;
  /** The settled URL, so a rehydrated session carries the identity liveness needs. */
  readonly url?: string;
  /**
