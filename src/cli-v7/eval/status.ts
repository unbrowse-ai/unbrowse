 ...
 * vs error).
 *
 * Substrate surfaces (W24.6 — stateless is the sole path):
...
export async function handler(
  ...
  try {
    ...
    const statelessAuditReady = post.ok === true && !post.bindingMissing;
    envelope.preflight = {
      schema: "unbrowse.runtime-preflight.v1",
      compatible: statelessAuditReady,
      runtime: {
        engine: typeof Bun !== "undefined" ? "bun" : "node",
        platform: process.platform,
        arch: process.arch,
        version: PACKAGE_VERSION,
        build_sha: RUNTIME_GIT_SHA,
      },
      browser_contract: {
        supported: ["chromium"],
      ...
      checks: {
        wallet_signer: { ok: true },
        local_session_store: { ok: true, sessions: sessions.length },
        stateless_audit: {
          ok: statelessAuditReady,
