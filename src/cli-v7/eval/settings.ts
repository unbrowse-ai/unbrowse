...
/**
 * The unbrowse DATA ROOT for these files.
 *
 * The previous version treated `UNBROWSE_HOME` as a home directory and appended
 * `.unbrowse` to it, so under an isolated run settings landed in
 * `$UNBROWSE_HOME/.unbrowse/` while `getUnbrowseHome()` — which every other
 * module uses, including `cli-v7/_session.ts` for the sibling `tmp/<sigHash>/`
 * layout — resolved `$UNBROWSE_HOME` as the root itself. Two readings of one
 * env var, so isolation was half-applied and invisible unless you looked:
 *
 *     getUnbrowseHome()  -> /tmp/ISO          _session tmp -> /tmp/ISO/tmp
 *     stashRoot()        -> /tmp/ISO/.unbrowse/tmp
 *
 * An explicit argument is still a HOME (the shape callers pass), and the
 * default is the shared data root. The uncustomised path is unchanged:
 * `~/.unbrowse/settings.json` either way.
 */
function unbrowseRoot(home?: string): string {
  return home ? join(home, ".unbrowse") : getUnbrowseHome();
}

function legacySettingsPath(home?: string): string {
  return join(unbrowseRoot(home), "settings.json");
}

function stashRoot(home?: string): string {
  return join(unbrowseRoot(home), "tmp");
}
