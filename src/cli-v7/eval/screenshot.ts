...
import { isObscuraSession } from "../../obscura/live-page.js";
import {
  EX_GENERIC,
  EX_USAGE,
  emit,
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  try {
    ...
    // Chrome-free backend: pixels are OUT OF SCOPE, by obscura's design and by
    // our own choice. obscura parses a DOM and runs JS but has no layout or
    // paint pipeline (its source refuses Page.captureScreenshot for exactly this
    // reason), so there is no image to capture — not a missing API, an absent
    // subsystem. Rather than silently returning an empty PNG, refuse and name
    // the two readers that DO carry page truth on this backend.
    if (isObscuraSession(rec)) {
      emit(
        {
          ok: false,
          error: "unsupported_on_obscura:eval_screenshot",
          subcommand: "eval screenshot",
          op_kind: meta.op_kind,
          session_id: rec.sessionId,
          backend: "obscura",
          reason: "obscura has no layout or paint engine, so a page cannot be rasterised",
          alternatives: ["eval snap", "eval text", "eval markdown"],
          terminal: true,
          retryable: false,
        },
        opts,
