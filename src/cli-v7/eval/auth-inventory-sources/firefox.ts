...
/**
 * Firefox profiles root for the host platform — the single best one, native
 * first. Discovery itself lives in `src/auth/browser-profile-roots.ts`: a
 * Flatpak/Snap Firefox is invisible to a hardcoded `~/.mozilla/firefox`, and
 * this reader having its own copy of that path is why the last fix left
 * auth-inventory reporting `sources_scanned: []`.
 */
...
/**
 * EVERY Firefox profiles root that exists — native, Flatpak (both layouts),
 * Snap. An inventory wants all of them: the roots are different browsers'
 * worth of profiles, and the logged-in session may live in any of them.
 */
export function defaultFirefoxRoots(): string[] {
  return existingBrowserProfileRoots(FIREFOX_PROFILE_TARGET);
}
