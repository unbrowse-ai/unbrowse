...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  try {
    ...
    // Same expression on both backends so the shared turndown path below
    // yields an identical markdown shape regardless of the driver.
    const expression = "document.body ? document.body.outerHTML : ''";

    let html: string;
    let currentUrl = "";

    if (isObscuraSession(rec)) {
      // Chrome-free backend: the same outerHTML expression runs in obscura's
      // V8 through the session broker, then feeds the shared htmlToMarkdown
      // path. No CDP socket, no Chrome.
      const client = obscuraClient(rec);
      const rawHtml = await client.evaluate(expression);
      html = isObscuraNull(rawHtml) ? "" : rawHtml;
      try {
      ...
      html = typeof evalResult.result?.value === "string"
