...
export async function handler(
  ...
  if (parsed.wantsHelp) {
    helpExit(
      ...
      {
        summary:
          "Your ledger + the network's: `you` (cost saved, time saved, contributions, payouts — local impact log + your dashboard) and `network` (everyone, from /v1/stats/summary). With --domains: every domain being indexed, used, and queried for, as per-domain counters (from /v1/stats/domains; aggregate-only, no caller identity).",
        usage: "unbrowse eval stats [--domains] [--fresh] [--json]",
        flags: [
          {
            name: "--domains",
            description:
              "Per-domain view: indexed (skills/endpoints), used (executions), queried (hits/misses incl. unindexed demand).",
          },
          {
            name: "--fresh",
  ...
  const domainsView = parsed.flags.domains === true;

  try {
    const base = resolveApiBase();
    const url = domainsView ? `${base}/v1/stats/domains` : `${base}/v1/stats/summary`;
    ...
    // The domains view is network-wide observability; the local "you" ledger
    // does not apply there.
    const you = domainsView ? undefined : await buildYouView();
    ...
    emit(
      {
        ...
        // Network), and credentials that never leave your device: wallet-bound
        // one-way commitments, revealed only under signature (Crypto Was All You
        // Needed). Public-tier pointers — the WHAT, read the papers for the rest.
        docs: {
          ...
          privacy: "credentials never leave your device — bound to your wallet as one-way commitments, revealed only under signature — paper: Crypto Was All You Needed",
