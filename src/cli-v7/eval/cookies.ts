...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  try {
    ...
    let rawCookies: ReadonlyArray<Record<string, unknown>>;
    let currentUrl = "";

    if (isObscuraSession(rec)) {
      // Chrome-free backend: the session broker returns the whole jar as
      // JSON text. Parse defensively — a broker hiccup must read as "no
      // cookies", never crash or echo the raw payload (which carries
      // values) — then feed the SAME redaction boundary below.
      const client = obscuraClient(rec);
      let jar: unknown = [];
      const rawJar = await client.getCookies();
      if (!isObscuraNull(rawJar)) {
        try { jar = JSON.parse(rawJar); } catch { jar = []; }
      }
      const list: unknown[] = Array.isArray(jar)
        ? jar
        : Array.isArray((jar as { cookies?: unknown[] } | null)?.cookies)
          ? (jar as { cookies: unknown[] }).cookies
          : [];
      // The urls filter is CDP-only; approximate it with RFC 6265
      // domain-matching on cookie metadata (never `value`).
      const host = domainArg
        ? domainArg.replace(/^https?:\/\//, "").replace(/[/:?#].*$/, "")
        : "";
      rawCookies = (host
        ? list.filter((c) => {
            const d = String((c as Record<string, unknown> | null)?.domain ?? "").replace(/^\./, "");
            return d !== "" && (d === host || host.endsWith(`.${d}`));
          })
        : list) as ReadonlyArray<Record<string, unknown>>;
      try {
        const href = await client.evaluate("location.href");
        if (!isObscuraNull(href)) currentUrl = href.trim();
      ...
      rawCookies = (result.cookies ?? []) as unknown as ReadonlyArray<Record<string, unknown>>;
    ...
    // SECRET REDACTION at the boundary. The raw cookie array (CDP
    // `result.cookies` or the parsed obscura jar) MUST NOT escape this
    // scope. `redactCookies` returns brand-new objects with no `value`
    // field.
    const redacted = redactCookies(rawCookies);
