 ...
 * metadata (Chrome/Chromium/Dia + Firefox cookies, history, bookmarks).
 *
 * Profile roots are NEVER computed here. They come from
 * `src/auth/browser-profile-roots.ts`, which is the one place that knows about
 * Flatpak and Snap layouts. This handler owning its own copy of
 * `~/.mozilla/firefox` is precisely why a previous Flatpak fix left
 * `sources_scanned: []` here while `act go` had started working.
 *
 ...
 *     "sources_scanned": ["chrome:/path", "chromium:/path", "dia:/path", "firefox:/path", ...],
 ...
 *   }
 *
 * Likely-logged-in / site-priority heuristic (cookie ≫ bookmark ≫ history):
 *   +0.60  domain has ≥1 cookie whose NAME matches the auth-pattern regex
 *          AND that cookie's expires_utc > now() + 1 hour
 *   +0.15  any cookie present (jar exists; weak session signal)
 *   +0.20  domain is bookmarked (explicit preference beats raw history)
 *   +0.05  last_visit within 30 days
 *   +0.05  visit_count > 50 (history volume is a tie-break only)
 *   cap    0.95 (we never claim 1.0 without an actual auth attempt)
 *
 * Order is structural: cookies dominate, then bookmarks, then history.
 */
...
} from "../../auth/browser-profile-roots.js";
import { AUTH_COOKIE_NAME_RE } from "../../auth/browser-cookies.js";

/** Cookie names contributing the +0.6 score lift — the shared auth-name rule. */
const AUTH_COOKIE_NAME_REGEX = AUTH_COOKIE_NAME_RE;
...
export function composeInventory(args: {
  ...
  for (const domain of allDomains) {
    ...
    // cookie ≫ bookmark ≫ history — see header comment for weights.
    let score = 0;
    if (fresh_cookie) score += 0.6;
    else if (has_cookie) score += 0.15;
    if (bookmarked) score += 0.2;
    if (last_visit_unix > 0 && nowUnix - last_visit_unix <= THIRTY_DAYS_S) {
      score += 0.05;
    }
    if (visit_count > 50) score += 0.05;
...
export async function runInventory(opts?: {
  ...
  // Chrome, Chromium and Dia all read Chrome's schema; only the source label
  // they carry into `sources_scanned` differs.
  const absorbChromiumProfile = async (profile: string, label: string): Promise<void> => {
    const result: ChromeProfileResult = await readChromeProfile(profile);
    sources_scanned.push(`${label}:${profile}`);
    ...
    if (result.locks.cookies === "locked") {
      locked_sources.push(`${label}:${profile}/Cookies`);
    }
    if (result.locks.history === "locked") {
      locked_sources.push(`${label}:${profile}/History`);
    }
  };

  const profilesUnderRoots = async (roots: string[]): Promise<string[]> => {
    const out: string[] = [];
    for (const root of roots) {
      for (const profile of await listChromeProfiles(root)) {
  ...
  // Chrome. Roots come from the shared resolver — native first, then Flatpak
  // (both layouts) and Snap. `defaultChromeRoot()` stays at the head of the
  // list so this walk can never see less than it saw before.
  ...
  if (!opts?.skipChrome) {
    if (opts?.chromeProfileOverride) {
      ...
      const roots: string[] = [];
      const nativeRoot = defaultChromeRoot();
      if (nativeRoot) roots.push(nativeRoot);
      for (const root of existingBrowserProfileRoots(CHROME_PROFILE_TARGET)) {
        if (!roots.includes(root)) roots.push(root);
      }
      chromeProfiles = await profilesUnderRoots(roots);
    }
  }
  for (const profile of chromeProfiles) await absorbChromiumProfile(profile, "chrome");

  // Chromium: Chrome's schema, a different browser — and on a Flatpak-only
  // machine (SteamOS, Silverblue) frequently the only one installed. Distinct
  // prefix so provenance stays readable, same host-only privacy boundary.
  let chromiumProfiles: string[] = [];
  if (!opts?.skipChrome && !opts?.chromeProfileOverride) {
    const found = await profilesUnderRoots(existingBrowserProfileRoots(CHROMIUM_PROFILE_TARGET));
    chromiumProfiles = found.filter((p) => !chromeProfiles.includes(p));
  }
  for (const profile of chromiumProfiles) await absorbChromiumProfile(profile, "chromium");
  ...
  for (const profile of diaProfiles) await absorbChromiumProfile(profile, "dia");
  ...
  if (!opts?.skipFirefox) {
    if (opts?.firefoxProfileOverride) {
      ...
      // Every existing root, native first: a machine can run a native Firefox
      // and a Flatpak one, and only the second may hold the live session.
      for (const root of defaultFirefoxRoots()) {
        for (const profile of await listFirefoxProfiles(root)) {
          if (!firefoxProfiles.includes(profile)) firefoxProfiles.push(profile);
        }
      }
    }
  }
