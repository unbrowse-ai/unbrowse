/**
 * CLI-side stateless adapter — the LUMINARY (Gen 1:14 — *"lights in the
 * firmament... for signs"*). Day-4 worker B deliverable, 2026-05-28.
 *
 * Every CLI primitive that crosses the firmament routes through
 ...
 *      backend's `deriveCacheKey` (services/stateless-substrate.ts §215).
 ...
 *   - Substrate enables; it never prescribes (contract ee1f5409). The
 ...
 *     (W6 invariant carried forward to every cross-firmament POST).
 *
    ...
    if (FORBIDDEN_FIELDS_LC.has(f.toLowerCase())) {
      throw new Error(
        `canonicalizeSignedFragment: forbidden field "${f}" — stateless ` +
          `substrate carries pointers/hashes only`,
      );
    }
    const v = body[f];
 ...
 * `deriveCacheKey` in `services/stateless-substrate.ts`. The cross-system
 ...
 * covenant-emission surface on the CLI side (Day-4 firmament).
 *
 * Behavior:
