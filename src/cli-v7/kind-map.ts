 ...
 * pairs); the aiko-side mapping from an op to its route verb lives OUT
 * of this bundle. `strings $(which unbrowse)` shows `act:navigate`,
 * `eval:snap`, `build:skill` — generic ops — never route-mechanism
 * names.
 *
 ...
 * surprise.
 */
export type V7Verb = "build" | "act" | "eval";
 ...
 * These are generic, abstract op-class names — NOT route-mechanism
...
export interface KindMapEntry {
  /** Full CLI surface form, e.g. "act fill". */
   ...
   * shaped `<verb>:<action>` (e.g. `act:navigate`, `eval:snap`,
   * `build:skill`). Generic ops, no route-mechanism prefix. This is
