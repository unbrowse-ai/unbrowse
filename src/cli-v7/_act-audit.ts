/**
 * Day-6 Dominion worker 1 (2026-05-28) — shared act-act audit emitter
 * for the 6 non-value-bearing act handlers (click, press, scroll,
 * submit, type-literal-path, select-literal-path).
 *
 *  — *"let them have dominion."*
 * A2 invariant: every act produces a sig-keyed KV row, even when no
 * value is filled. The act-act variant of the audit log
 * (backend/src/services/audit.ts) is the platform row; this module
 * is the CLI-side route emitter.
 *
 ...
 *   - Mirrors the structure of `act/fill.ts` + `act/select.ts`
 ...
 * `BREATH_ACT_TYPES`. Closed enum; adding a new act handler means
 ...
 * the remaining act + build kinds. New entries:
 *   navigate              — act/go.ts (Page.navigate landed)
 *   teardown              — act/close.ts (target/context closed)
 *   auth-capture-start    — act/auth-capture.ts (HEADED Chrome spawned)
 *   auth-capture-finish   — act/auth-capture.ts (keychain write landed)
 *   proxy-rotate          — act/proxy-rotate.ts (new proxied Chrome up)
 *   session-restore       — act/session-restore.ts (KV row → fresh tab)
...
/**
 * Derive the act-act contextHash. Mirrors fill.ts:deriveContextHash
...
/**
 * Emit a act-act audit row. Pointer-only + selector-hashed — the
 ...
 *
 *  — every act witnessed; the receipt is the witness.
 */
export async function emitBreathActStateless(
  ...
  // Selector is sha256-hashed BEFORE crossing the boundary. The
  ...
  // AuditFillBody shape as fill.ts / select.ts but with the act-act
  ...
  if (result.bindingMissing) {
    //  — uncertain trumpet: a missing binding surfaces as a
    ...
    try {
      ...
      process.stderr.write(
        `unbrowse: audit binding missing (${result.bindingMissing}); act-act receipt skipped, gesture succeeded\n`,
      );
    } catch {
