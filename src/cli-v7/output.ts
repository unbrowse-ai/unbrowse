 ...
 *   0   success
 *   64  EX_USAGE          — user gave bad args (--help exits 0: it is a successful self-description request)
 *   70  EX_SOFTWARE       — handler not yet implemented (v7 stub state)
 *   1   generic failure
 */
import { contractVerdictFromEnvelope } from "../values/contract-shape.js";
import { buildErrorEnvelope, codeFromMessage } from "../values/error-envelope.js";
...
export function emit(data: unknown, opts: OutputOptions = {}): void {
  ...
  // substrate-enriched `_contract` themselves; this fills it for every other primitive and
...
export function emitErr(err: unknown, opts: OutputOptions = {}): void {
  ...
  // Machine-routable code, ADDITIVE to the legacy shape (error/name keep their
  // exact values — regression greps stay green). Handlers conventionally lead
  // the message with a snake_case reason token ("intent_required: usage …");
  // surface it as `code` so agents route on a stable field instead of parsing
  // prose. `retryable` defaults false — emitErr carries usage/terminal errors;
  // transient paths carry their own envelopes.
  // Built through the ONE envelope source of truth (src/values/error-envelope.ts)
  // rather than hand-rolled here. The flat CLI reinvented this shape with
  // different nesting and no `retryable`; both now construct the same object.
...
/**
 * Render a help block for a subcommand and exit 0.
 * Always machine-readable; the agent reading stdout gets a typed JSON
 * description of the surface, not human prose. Help is a SUCCESSFUL
 * request for self-description, not a usage error — `cmd --help && …`
 * chains and doc generators must not read it as failure. (EX_USAGE
 * stays reserved for real usage errors; EX_SOFTWARE 70 still marks
 * not-yet-implemented stubs via notImplementedExit.)
 */
 ...
 * --help" (0) from "I haven't built this yet" (70).
 */
