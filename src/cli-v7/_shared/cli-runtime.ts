...
export async function api(method: string, path: string, body?: unknown, opts?: { timeoutMs?: number; signal?: AbortSignal }): Promise<unknown> {
  ...
  if (process.env.UNBROWSE_URL) {
    ...
    const controller = (timeoutMs && timeoutMs > 0) || opts?.signal ? new AbortController() : undefined;
    ...
    try {
      ...
      if ((err as { name?: string }).name === "AbortError") {
        if (opts?.signal?.aborted) return { error: "cli_interrupted", message: "API request was cancelled." };
  ...
  const abortWait = opts?.signal
    ...
    : undefined;
  const waits: Array<Promise<typeof injectP extends PromiseLike<infer R> ? R : never> | Promise<null>> = [Promise.resolve(injectP)];
  if (timeoutMs && timeoutMs > 0) waits.push(new Promise<null>((resolve) => setTimeout(() => resolve(null), timeoutMs)));
  if (abortWait) waits.push(abortWait);
  const res = await Promise.race(waits);
  if (abortHandler) opts?.signal?.removeEventListener("abort", abortHandler);
  if (res === null) {
    if (opts?.signal?.aborted) return { error: "cli_interrupted", message: "In-process API request was cancelled." };
