...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  const meta = lookupKindMap("act", "fill")!;
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act fill",
      {
        ...
        usage: "unbrowse act fill <selector> <pointer> [--session <id>] [--arg key=value] [--argScope <json>]",
        positional: [
