/**
 * `unbrowse act close [session-id]` — close one browse session.
 *
 * 1:1 mapping (kind-map.ts row "act close"):
 *   CLI subcommand  : act close
 ...
 *
 * Per `feedback_close_is_antipattern_persistent_session.md` — `act close`
 * destroys the pointer-of-pointer chain that the user paid the browser-open
 * cost to acquire. Replacement: `act session-park` (KV-persisted,
...
const DEPRECATION_NOTICE =
  "[deprecation] unbrowse act close is being replaced by session-park (KV-persisted). v7.3 removes the close alias. Use 'unbrowse act session-park' for forward-compat.";
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act close",
      {
        ...
        usage: "unbrowse act close [session-id] [--session <id>]",
        positional: [
  ...
  try {
    ...
    if (!explicitId && (err as Error & { code?: string }).code === "no_active_session") {
      emit(
        { ok: true, subcommand: "act close", op_kind: meta.op_kind, idempotent_noop: true },
        opts,
      );
      process.exit(0);
    }
  ...
  // W24.2 — emit a sig-keyed `teardown` act-act receipt.
