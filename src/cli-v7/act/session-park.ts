/**
 * `unbrowse act session-park [session-id]` — replace `act close`.
 *
 * 1:1 mapping (kind-map.ts row "act session-park"):
 *   CLI subcommand  : act session-park
 *   MCP tool        : unbrowse_session_park
 *   Op kind   : act:session_park
 *   Verb            : act
 *
 ...
 * chain IS the persistent treasure that `act close` was destroying
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act session-park",
      {
        ...
        usage:
          "unbrowse act session-park [session-id] [--session <id>]",
        positional: [
          {
            name: "session-id",
  ...
  // session file's targetUrl was the navigation URL at `act go`, but
  ...
  const bodyWithoutSig: Omit<SessionParkBody, "signature" | "signatureScheme"> = {
    ...
    // session record (populated by a preceding `act auth-capture` under
  ...
  // per the §E boundary boundary (NOT a throw — graceful degraded mode).
  //
  ...
  // v7-cli-act-session-park.test.ts.
  ...
  if (result.bindingMissing) {
    ...
    process.stderr.write(
      `[binding-missing] ${result.bindingMissing}: stateless platform inert, fell back to local tmp stash\n`,
    );
