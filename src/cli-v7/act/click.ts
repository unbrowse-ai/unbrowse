/**
 * `unbrowse act click <selector>` —
 * compose DOM.querySelector + DOM.getBoxModel + Input.dispatchMouseEvent (×2).
 *
 * 1:1 mapping (kind-map.ts row "act click"):
 *   CLI subcommand  : act click
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act click",
      {
        ...
        usage: "unbrowse act click <selector> [--session <id>] [--button left|right|middle] [--click-count 1|2|3] [--modifiers shift,alt,ctrl,meta]",
        positional: [
  ...
  try {
    ...
    // Read current URL AFTER the click so the act-act audit binds to
  ...
  emit(
    {
      ...
      x,
      y,
      // act-act audit sub-state — always emitted (W24.6).
      audit: {
        ok: breathAudit.ok,
