/**
 * `unbrowse act fill-form [selector]` — the end-to-end form-fill pipe.
 *
 *  — *"breathed into his nostrils the act of life; and man became
 * a living soul."* The form (dust) becomes filled/alive.
 *
 * 1:1 mapping (kind-map.ts row "act fill-form"):
 *   CLI subcommand  : act fill-form
 *   MCP tool        : unbrowse_fill_form
 *   Op kind         : act:fill_form  (op_class: actuate, action: fill_form)
 *   Verb            : act
 *
...
/** Wallet identity the route binary records the diffusion under. */
 ...
 * public bundle; W33-C/aiko owns the op→route translation). This is the
 ...
 * indirection so the route-specific kind string is NOT a contiguous
...
export function sealDiffusionReceipt(
  ...
  intent: string,
  binPath: string = join(homedir(), ".local", "bin", "route"),
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act fill-form",
      {
        ...
        usage:
          "unbrowse act fill-form [selector] [--intent <text>] [--dry-run] [--session <id>] [--arg key=value] [--argScope <json>]",
        positional: [
          {
            name: "selector",
  ...
  // ── 3. DIFFUSION — seal via the route binary (dogfood) + read engine ───
  // The route `slots` param: name+type+pointer("unknown" so the engine
  ...
  const binPathOverride = process.env.UNBROWSE_ROUTE_BIN;
