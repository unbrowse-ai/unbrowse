/**
 * `unbrowse act scroll [selector] <dx,dy>` — page or element scroll.
 *
 * 1:1 mapping (kind-map.ts row "act scroll"):
 *   CLI subcommand  : act scroll
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  const meta = lookupKindMap("act", "scroll")!;
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act scroll",
      {
        ...
        usage: "unbrowse act scroll [selector] <dx,dy> [--session <id>]",
        positional: [
          {
            name: "selector?",
