/**
 * `unbrowse act type <text-or-pointer>` — value-bearing text input.
 *
 * 1:1 mapping (kind-map.ts row "act type"):
 *   CLI subcommand  : act type
 ...
 *
 * Secret-redaction invariants on the POINTER PATH mirror act/fill.ts:
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  const meta = lookupKindMap("act", "type")!;
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act type",
      {
        ...
        usage: "unbrowse act type <text-or-pointer> [--session <id>] [--arg key=value] [--argScope <json>]",
        positional: [
          {
  ...
  if (!isPointerPath) {
    ...
    // Day-6 Dominion: the literal-text path emits a act-act
    // receipt (variant=act-act, actType=type-literal). The
