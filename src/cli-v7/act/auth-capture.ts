/**
 * `unbrowse act auth-capture <login-url>` — interactive auth flow.
 *
 * 1:1 mapping (kind-map.ts row "act auth-capture"):
 *   CLI subcommand  : act auth-capture
 *   MCP tool        : unbrowse_auth_capture
 *   Op kind   : act:auth_capture
 *   Verb            : act
 *
 * Behavior:
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  const meta = lookupKindMap("act", "auth-capture")!;
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act auth-capture",
      {
        ...
        usage:
          "unbrowse act auth-capture <login-url> [--domain <domain>] [--timeout <ms>] [--settle <ms>]",
        positional: [
  ...
  try {
    ...
    emit({ error: "invalid_url", subcommand: "act auth-capture", url: loginUrl }, opts);
    process.exit(EX_GENERIC);
  }
  ...
  try {
    ...
    // Persist a session record so subsequent act/eval calls can attach.
    ...
    // — the session is now usable by downstream `act` commands via the
    // recorded chromeWsUrl. The user/agent calls `act close` when done.
    void target;
    void conn;
  }
}
