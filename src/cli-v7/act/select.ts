/**
 * `unbrowse act select <selector> <pointer-or-value>` —
 * set a <select> element to a chosen <option>.
 *
 * 1:1 mapping (kind-map.ts row "act select"):
 *   CLI subcommand  : act select
 ...
 * Same security invariants as act/fill.ts (see that file for the spec).
 *
...
export async function handler(parsed: ParsedV7Args, opts: OutputOptions): Promise<void> {
  ...
  if (parsed.wantsHelp) {
    helpExit(
      "act select",
      {
        ...
        usage: "unbrowse act select <selector> <pointer-or-value> [--session <id>] [--by value|label|index] [--arg key=value] [--argScope <json>]",
        positional: [
  ...
  if (!isPointer) {
    ...
    // Day-6 Dominion: the literal-select path emits a act-act
    // receipt (variant=act-act, actType=select-literal). Selector
