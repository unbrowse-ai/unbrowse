 ...
 * (build / breath / eval) with one subcommand each, then a small
...
export interface ParsedV7Args {
  /** First positional, e.g. "build" | "breath" | "eval" | "help" | "--help" | "" */
  verb: string;
...
export function parseV7Args(argv: string[]): ParsedV7Args {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if (tok.startsWith("--")) {
      ...
      const next = rest[i + 1];
      if (BOOLEAN_ONLY_FLAGS.has(body)) {
        flags[body] = true;
      } else if (VALUE_EXPECTED_FLAGS.has(body) && next !== undefined) {
        flags[body] = next;
        i++;
