...
import type { ProvenanceProof, ProofCommitment } from "../types/proof.js";
...
export type { ProvenanceProof, ProofCommitment, ProofVerificationResult } from "../types/proof.js";
export { isNotaryAvailable, createNotaryClient } from "./notary.js";
export type { NotaryClient } from "./notary.js";

/**
 * Check if provenance proof generation is enabled.
...
/**
 * Generate a provenance proof for a captured HTTP request.
 *
 * Currently supports:
...
export async function generateProofsForCapture(
  ...
  domain: string,
): Promise<Map<string, ProvenanceProof>> {
  const proofs = new Map<string, ProvenanceProof>();
