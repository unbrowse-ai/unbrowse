...
/**
 * Pointer to another contract's id in the /contract platform.
 * A ContractRef is the string id of a cell contract whose satisfaction
 * is the precondition for this field's truth claim. See
 * ~/(internal) — "pointer principle". Wave-1
 * carrier of the runtime↔platform isomorphism (organ b9c8a64d stage 1).
...
export interface OperationBinding {
   ...
   * walker resolves dependency by querying the platform ledger rather
...
export interface ExecutionTrace {
   ...
   *  mode against the contextUrl. platform emits truth; agent judges. */
