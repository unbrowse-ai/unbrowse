...
export interface EndpointCorroboration {
  ...
  /** Legacy wire name — counts verified provenance proofs (no zk crypto involved). */
...
export interface OperationBinding {
  ...
  observed_at?: string;
  /**
   * Seal commitment (sha256 of the PLAINTEXT) for a credential/storage hole
...
export interface EndpointDescriptor {
  ...
  /** Evidence-backed schema-less protobuf request replay. Bindings are emitted
   * only when a captured URL/query value uniquely equals one top-level string
   * field. Ambiguous/nested fields are deliberately unsupported. */
  protobuf_request?: {
    body_base64: string;
  ...
  zk_proof?: import("./proof.js").ProvenanceProof;
  /**
...
export interface ExecutionOptions {
  ...
  client_scope?: string;
  /** Internal: result-cache hit — creativity-economy act recorder uses this. */
  cache_hit?: boolean;
  ...
  __walkDepth?: number;
  /** Cancels this invocation and any transport tier that supports AbortSignal. */
  signal?: AbortSignal;
  /** Internal test/process seam for a shared durable route-lifecycle store. */
  __route_lifecycle_file?: string;
...
export interface OrchestrationTiming {
  ...
  get_skill_ms: number;
  execute_ms: number;
  /** True only when this task actually attempted to open a Chromium tab/session. */
  browser_opened: boolean;
  total_ms: number;
  source: "marketplace" | "live-capture" | "dom-fallback" | "route-cache" | "browser-action" | "exa" | "direct-fetch" | "direct-document" | "execute" | "result-cache";
  cache_hit: boolean;
