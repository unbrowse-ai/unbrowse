...
export interface AnalyticsSessionPayload {
  ...
  success?: boolean;
  source?: string;
  /** Entry surface that observed this session. Never inferred from agent identity. */
  surface?: "cli" | "mcp" | "sdk" | "local-http" | "unknown";
  /** Whether this action required an Unbrowse cloud request or completed locally. */
  execution_scope?: "local" | "cloud" | "mixed" | "unknown";
  /** Best-effort delivery diagnostics from the client-side telemetry transport. */
...
export function buildAnalyticsSessionPayload(
  ...
  observation?: Pick<AnalyticsSessionPayload, "surface" | "execution_scope">,
  ...
  return {
    ...
    source,
    surface: observation?.surface ?? "local-http",
    execution_scope: observation?.execution_scope ?? (cacheLike ? "mixed" : "local"),
    telemetry_schema_version: 1,
