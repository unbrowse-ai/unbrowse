...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/intent/resolve", async (req, reply) => {
    const { intent, params, context, projection } = req.body as {
      intent: string;
    ...
    try {
      const result = await resolveAndExecute(intent, params ?? {}, context, projection);
  ...
  app.post("/v1/skills/:skill_id/execute", async (req, reply) => {
    const { skill_id } = req.params as { skill_id: string };
    const { params, projection } = req.body as {
      params?: Record<string, unknown>;
      projection?: ProjectionOptions;
    };
    const skill = getSkill(skill_id);
    if (!skill) return reply.code(404).send({ error: "Skill not found" });
    try {
      const execResult = await executeSkill(skill, params ?? {}, projection);
