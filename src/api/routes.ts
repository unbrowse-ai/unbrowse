  ...
  app.post("/v1/browse/submit", async (req, reply) => {
    ...
    const nextStep = result.ok
      ? (result.capture_sync?.background_publish_queued
          ...
          : "Capture synced locally for this step. If more UI steps remain, continue the flow, then run `unbrowse close` when you're done to save auth and queue final publish.")
