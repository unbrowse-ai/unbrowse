...
export async function registerRoutes(app: FastifyInstance) {
  ...
  // POST /v1/auth/agent-mail — autonomous email-based login/registration via AgentMail
  app.post("/v1/auth/agent-mail", { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } }, async (req, reply) => {
    const { domain, action, timeout_ms } = req.body as {
      domain: string;
      action?: "create" | "wait-otp" | "wait-link" | "wait-email";
      timeout_ms?: number;
    };
    if (!domain) return reply.code(400).send({ error: "domain required" });

    try {
      const { tryAgentMailAuth } = await import("../auth/agent-mail.js");
      const session = await tryAgentMailAuth(domain);
      if (!session) {
        return reply.code(503).send({ error: "AgentMail not available — set AGENTMAIL_API_KEY" });
      }

      // Default: just create inbox and return email
      if (!action || action === "create") {
        return reply.send({ email: session.email, inbox_id: session.inboxId, domain });
      }

      const timeout = timeout_ms ?? 90_000;
      if (action === "wait-otp") {
        const otp = await session.waitForOtp(timeout);
        return reply.send({ email: session.email, otp, domain, found: Boolean(otp) });
      }
      if (action === "wait-link") {
        const link = await session.waitForLink(timeout);
        return reply.send({ email: session.email, link, domain, found: Boolean(link) });
      }
      if (action === "wait-email") {
        const email = await session.waitForEmail(timeout);
        return reply.send({ email: session.email, verification: email, domain, found: Boolean(email) });
      }
  ...
  // POST /v1/auth/autonomous — full end-to-end autonomous login (AgentMail + Kuri browser)
  app.post("/v1/auth/autonomous", { config: { rateLimit: { max: 5, timeWindow: "1 minute" } } }, async (req, reply) => {
    const { url, domain } = req.body as { url: string; domain?: string };
    if (!url) return reply.code(400).send({ error: "url required" });

    try {
      const { autonomousLogin } = await import("../auth/autonomous-login.js");
      const result = await autonomousLogin(url, domain);
      return reply.send(result);
    } catch (err) {
      return reply.code(500).send({ error: (err as Error).message, success: false });
    }
  });

  // POST /v1/email/inbox — create or get an AgentMail inbox
  app.post("/v1/email/inbox", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    const { domain } = req.body as { domain?: string };
    try {
      const { getOrCreateSiteInbox, isAgentMailAvailable } = await import("../auth/agent-mail.js");
      if (!isAgentMailAvailable()) return reply.code(503).send({ error: "AGENTMAIL_API_KEY not set" });
      const inbox = await getOrCreateSiteInbox(domain ?? "general");
  ...
  // POST /v1/email/send — send an email from an AgentMail inbox
  app.post("/v1/email/send", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    const { domain, to, subject, body } = req.body as { domain?: string; to: string; subject: string; body: string };
    ...
    try {
      ...
      if (!isAgentMailAvailable()) return reply.code(503).send({ error: "AGENTMAIL_API_KEY not set" });
      const session = await autonomousEmailLogin(domain ?? "general");
      const result = await session.sendEmail(to, subject, body ?? "");
      return reply.send({ ...result, from: session.email });
    } catch (err) {
      return reply.code(500).send({ error: (err as Error).message });
    }
  });

  // POST /v1/email/check — check inbox for new messages
  app.post("/v1/email/check", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (req, reply) => {
    const { domain, from_domain, timeout_ms } = req.body as { domain?: string; from_domain?: string; timeout_ms?: number };
    try {
      const { autonomousEmailLogin, waitForVerificationEmail, isAgentMailAvailable } = await import("../auth/agent-mail.js");
      if (!isAgentMailAvailable()) return reply.code(503).send({ error: "AGENTMAIL_API_KEY not set" });
      const session = await autonomousEmailLogin(domain ?? "general");
      if (from_domain) {
        const email = await waitForVerificationEmail(session.inboxId, from_domain, timeout_ms ?? 30_000);
        return reply.send({ email: session.email, message: email, found: Boolean(email) });
      }
      // No from_domain — just return inbox info
      return reply.send({ email: session.email, inbox_id: session.inboxId });
    } catch (err) {
      return reply.code(500).send({ error: (err as Error).message });
    }
  });
