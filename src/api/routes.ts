...
import type { HarEntry } from "./har-entry.js";
import { captureResolveVisualContext } from "../browser/resolve-visual-context.js";
...
import type { EndpointDescriptor, ExecutionTrace, OrchestrationTiming, ProjectionOptions, SkillManifest } from "../types/index.js";
import { routeIntentCompatibility } from "../lib/ranking-core/signals/intent-yield.js";
...
import { sanitizeManifestForPublish } from "../publish/sanitize.js";
import { getRegistrableDomain } from "../domain.js";
import { generateLocalDescription, writeSkillSnapshot, readSkillSnapshot, buildResolveCacheKey, getDomainReuseKey, domainSkillCache, persistDomainCache, scopedCacheKey, snapshotPathForCacheKey, invalidateRouteCacheForDomain, summarizeSchema, extractSampleValues, routeIdentityForUrl } from "../orchestrator/index.js";
...
import { queuePassiveSkillPublish } from "../orchestrator/passive-publish.js";
...
import { recordCreativityAct, recordCreativityActFromExecute } from "../values/creativity-economy.js";
...
import { ObscuraBrowseSessionClient, OBSCURA_SINGLE_TAB } from "../obscura/browse-session-client.js";
import { obscuraBackendSelected, isObscuraSession } from "../obscura/live-page.js";
import { existsSync as existsSync_, readdirSync as readdirSync_, readFileSync as readFileSync_ } from "node:fs";
...
import { createPermissionContext, type PermissionEffect } from "../runtime/permission-gate.js";
...
export function canRecoverSingleEndpointForIntent(
  endpoint: EndpointDescriptor | undefined,
  intent?: string,
  contextUrl?: string,
): boolean {
  return assessSingleEndpointRecovery(endpoint, intent, contextUrl).allowed;
}

export function assessSingleEndpointRecovery(
  endpoint: EndpointDescriptor | undefined,
  intent?: string,
  contextUrl?: string,
): { allowed: boolean; reason?: "route_intent_mismatch" | "cardinality_mismatch" } {
  if (!endpoint) return { allowed: true };
  if (routeIntentCompatibility(endpoint, intent, contextUrl) === "incompatible") {
    return { allowed: false, reason: "route_intent_mismatch" };
  }
  if (!cardinalityMatches(intent, { kind: "route", route: endpoint }, { contextUrl })) {
    return { allowed: false, reason: "cardinality_mismatch" };
  }
  return { allowed: true };
}
...
export function buildAnalyticsSessionPayload(
  ...
  return {
    ...
    source,
    surface: "local-http",
    execution_scope: source === "marketplace" ? "mixed" : "local",
...
export function passiveIndexFromRequests(
  ...
  // The identity the route cache will ask for on read — port-qualified for local
  // targets. The port stays OUT of the auth key below: browsers scope cookies by
  // host, not by port.
  const routeDomain = routeIdentityForUrl(pageUrl, domain);
  ...
  return (async () => {
    try {
      ...
      const existingSkill = findExistingSkillForDomain(routeDomain, intent);
      ...
      const skill: SkillManifest = {
        ...
        // the route-matching identity, port-qualified for local targets
        domain: routeDomain,
      ...
      queueBackgroundIndex({
        ...
        contextUrl: pageUrl,
        cacheKey,
        publishAfterIndex: options.publishAfterIndex !== false,
      });
 ...
 */
/**
 * Import obscura browse sessions from the cli-v7 session store.
 *
 * There are TWO session stores, and they did not meet: obscura `go` writes a
 * cli-v7 record (tmp/<hash>/<id>.json, targetId "obscura", chromeWsUrl
 * http://127.0.0.1:PORT/mcp) while the whole /v1/browse/* surface — including
 * eval, which backs `run-js` — reads sessions.jsonl. So an obscura session was
 * invisible to eval and `run-js` answered `no_active_session` against a session
 * that was open and healthy.
 *
 * That was MASKED until the session store started honouring UNBROWSE_HOME: eval
 * used to find sessions only because it was reading hundreds of foreign ones.
 * Removing the leak revealed the gap rather than causing it.
 *
 * Read-side on purpose. Making obscura `go` write both stores would duplicate
 * session truth in two places; importing at rehydrate keeps one writer per store
 * and one reader that knows about both. Recognition is by SHAPE
 * (`isObscuraSession`), so a session stays importable after the env that created
 * it is gone.
 */
function importObscuraSessions(sessions: Map<string, BrowseSession>): number {
  let imported = 0;
  try {
    const root = pathJoin(getUnbrowseHome(), "tmp");
    if (!existsSync_(root)) return 0;
    for (const dir of readdirSync_(root)) {
      let files: string[];
      try { files = readdirSync_(pathJoin(root, dir)); } catch { continue; }
      for (const f of files) {
        if (!f.endsWith(".json")) continue;
        try {
          const rec = JSON.parse(readFileSync_(pathJoin(root, dir, f), "utf8")) as {
            sessionId?: string; targetId?: string; chromeWsUrl?: string;
            tabId?: string; url?: string;
          };
          if (!rec?.sessionId || sessions.has(rec.sessionId)) continue;
          if (!isObscuraSession({ targetId: rec.targetId, chromeWsUrl: rec.chromeWsUrl } as never)) continue;
          // The broker port lives in the record's own endpoint — the client is
          // resolved from it by brokerForSession, not stored here.
          let brokerPort: number | undefined;
          try { brokerPort = Number(new URL(String(rec.chromeWsUrl)).port) || undefined; } catch { /* leave undefined */ }
          sessions.set(rec.sessionId, {
            sessionId: rec.sessionId,
            // The id the BROKER reported at open, when it reported one. The
            // placeholder is the fallback for builds with no tab model, not
            // the default — recording it unconditionally is what made every
            // rehydrated obscura session fail liveness as `tab_not_in_discover`.
            tabId: rec.tabId || OBSCURA_SINGLE_TAB,
            url: rec.url ?? "",
            domain: "",
            harActive: false,
            brokerPort,
          ...
          imported++;
        } catch { /* unreadable record — skip it, never fail rehydrate */ }
      }
    }
  } catch { /* store unreadable — import nothing */ }
  return imported;
}
...
export function rehydrateBrowseSessions(): { restored: number } {
  ...
  // Obscura sessions live in the other store; import them so eval/run-js can see
  // a session that `go` genuinely opened.
  restored += importObscuraSessions(browseSessions);
  return { restored };
}
const inspectedHarEntries = new Map<string, HarEntry[]>();

function rememberInspectedHarEntries(sessionId: string, entries: HarEntry[]): void {
  if (entries.length === 0) return;
  const existing = inspectedHarEntries.get(sessionId) ?? [];
  inspectedHarEntries.set(sessionId, [...existing, ...entries]);
}

function drainInspectedHarEntries(sessionId: string): HarEntry[] {
...
/**
 * Is this session client actually obscura-backed?
 *
 * Recognised by SHAPE, mirroring `isObscuraSession`, so a client stays correctly
 * identified after the environment that created it is gone. Used only to detect
 * a backend mismatch and say so out loud — never to change which client runs.
 */
function isObscuraBackedClient(client: unknown): boolean {
  return client instanceof ObscuraBrowseSessionClient;
}

function brokerForSession(session: BrowseSession | undefined): kuri.KuriClient {
  if (session?.client) {
    // A cached/rehydrated client outranks the backend selection below. That is
    // deliberate — an open session must stay drivable by whatever opened it —
    // but it must never be SILENT: with UNBROWSE_BROWSER_BACKEND=obscura set, a
    // session carrying a kuri client quietly serves Chrome under obscura's name.
    //
    // Measured: `go` opened a real obscura session (obscura proc up, no new
    // Chrome), then `run-js` in a fresh process rehydrated it, took this branch,
    // started kuri and used Chrome — with no error and no warning. Anyone
    // benchmarking "obscura" through that path measures Chrome and cannot tell.
    if (obscuraBackendSelected() && !isObscuraBackedClient(session.client)) {
      console.error(
        "[backend-mismatch] UNBROWSE_BROWSER_BACKEND=obscura but this session carries a "
        + "Chrome/CDP client — serving it as-is. Measurements from this call are NOT obscura.",
      );
    }
    return session.client as kuri.KuriClient;
  }
  // Chrome-free backend: serve the whole /v1/browse/* surface from an obscura
  // broker instead of a kuri+Chrome one. The client covers every method these
  // handlers call; the two obscura genuinely cannot do (screenshot, and typing
  // into focus) throw `unsupported_on_obscura:*` rather than returning a blank
  // result. Default is unchanged when the env is unset.
  if (obscuraBackendSelected()) {
    return new ObscuraBrowseSessionClient({
      port: session?.brokerPort,
    }) as unknown as kuri.KuriClient;
  }
...
/**
 * The broker port of the ONE open obscura session, if exactly one is open.
 *
 * Deliberately refuses to guess with several: silently picking the most recent
 * would leak one caller's tab into another's, which is the same reasoning that
 * makes listLiveBrowseSessions demand an explicit id when several are live.
 * Undefined then, and the caller surfaces "which session?" rather than acting on
 * the wrong one.
 */
function soleObscuraBrokerPort(): number | undefined {
  const ports = [...browseSessions.values()]
    // Keyed on the broker port, NOT on a tab id: the tab id is now whatever the
    // broker reports, so matching a placeholder here would have silently
    // stopped selecting real sessions the moment identity became honest.
    .filter((s) => typeof s.brokerPort === "number")
    .map((s) => s.brokerPort as number);
  return new Set(ports).size === 1 ? ports[0] : undefined;
}

export function selectBrowseBrokerClient(requestedSessionId?: string): kuri.KuriClient {
  // Honour the selected backend BEFORE any kuri pool logic. Every path below
  // this point returns a kuri client, so with UNBROWSE_BROWSER_BACKEND=obscura
  // an unnamed request got a Chrome broker and probed Chrome for liveness.
  //
  // Measured: an open, healthy obscura session was declared dead with
  // reason "tab_not_in_discover", tab_id "obscura-page", against
  // discovered_tab_ids ["22947BE3D4C9D6DDEDBD6BA916D013F5"] — CDP tab ids from
  // the wrong browser entirely. `run-js` then answered `no_active_session` for a
  // session `go` had just opened.
  //
  // Mirrors brokerForSession, which already does this; this is its sibling and
  // was missed. A named session with an existing client still wins below, so an
  // open Chrome session stays drivable.
  if (obscuraBackendSelected()) {
    // Resolve the session's OWN broker port. Passing undefined attaches to a
    // different obscura broker with a fresh blank page — measured: `go` landed
    // defillama on the broker at :36479, then run-js evaluated location.href as
    // "about:blank" because it had connected somewhere else entirely. An unnamed
    // request must still find the open session, not invent a new one.
    const named = requestedSessionId ? browseSessions.get(requestedSessionId) : undefined;
    const port = named?.brokerPort ?? soleObscuraBrokerPort();
    return new ObscuraBrowseSessionClient({ port }) as unknown as kuri.KuriClient;
  }
...
function stablePermissionValue(value: unknown): string {
  if (value === null || typeof value !== "object") return JSON.stringify(value) ?? String(value);
  if (Array.isArray(value)) return `[${value.map(stablePermissionValue).join(",")}]`;
  const record = value as Record<string, unknown>;
  return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${stablePermissionValue(record[key])}`).join(",")}}`;
}

/** Classify the effect of the endpoint which will actually be dispatched. */
export function isTrustedPermissionCaller(address: string | undefined): boolean {
  if (!address) return false;
  const normalized = address.trim().toLowerCase().replace(/^\[|\]$/g, "");
  return normalized === "127.0.0.1" || normalized === "::1" || normalized === "::ffff:127.0.0.1";
}

export function classifyExecutePermissionEffect(endpoint: EndpointDescriptor, dryRun = false): PermissionEffect {
  if (dryRun) return "read";
  const action = endpoint.semantic?.action_kind?.trim().toLowerCase() ?? "";
  const resource = endpoint.semantic?.resource_kind?.trim().toLowerCase() ?? "";
  if (endpoint.pay_provider || /^(pay|payment|purchase|checkout|subscribe)$/.test(action)) return "payment";
  if (/^(publish|publication|share)$/.test(action)) return "publication";
  if (endpoint.oauth_plan || /^(auth|authenticate|authentication|login|logout|oauth|session|token)$/.test(action)) return "authentication";
  if (/^(challenge|captcha|verify|verification)$/.test(action)) return "challenge";
  if (/^(hook|webhook)$/.test(action) || /^(hook|webhook)$/.test(resource)) return "hook";
  if (["POST", "PUT", "PATCH", "DELETE"].includes(endpoint.method.toUpperCase())) return "mutation";
  return "read";
}

export function executeOperationFingerprint(input: {
  skillId: string;
  endpoint: EndpointDescriptor;
  params?: Record<string, unknown>;
}): string {
  const canonical = stablePermissionValue({
    skill_id: input.skillId,
    endpoint_id: input.endpoint.endpoint_id,
    method: input.endpoint.method.toUpperCase(),
    url_template: input.endpoint.url_template,
    params: input.params ?? {},
  });
  return `sha256:${createHash("sha256").update(canonical).digest("hex")}`;
}

const executePermissionContext = createPermissionContext();

/** Host/UI seam. This is deliberately not an HTTP tool an invoking agent can self-call. */
export function issueHostExecuteApproval(input: {
  operation_fingerprint: string;
  effect: PermissionEffect;
  ttl_ms?: number;
}): { token: string; id: string; expires_at_ms: number; max_uses: number } {
  return executePermissionContext.issueApproval({ ...input, max_uses: 1 });
}

export async function registerRoutes(app: FastifyInstance) {
  const permissionContext = executePermissionContext;
  ...
  app.get("/v1/settings", async (_req, reply) => {
    ...
    return reply.send({
      ...
      mutation_policy: getMutationPolicySettings(),
      contribution: {
  ...
  app.post("/v1/settings", async (req, reply) => {
    const body = (req.body ?? {}) as {
      ...
      mutation_policy?: MutationPolicyMode;
      mutation_whitelist?: string[];
    };
    ...
    const mutationPolicy = updateMutationPolicySettings({
      mode: body.mutation_policy,
      whitelist: Array.isArray(body.mutation_whitelist) ? body.mutation_whitelist : undefined,
    });
  ...
  app.post("/v1/intent/resolve", { config: { rateLimit: ROUTE_LIMITS["/v1/intent/resolve"] } }, async (req, reply) => {
    ...
    try {
      ...
      if (visual_context && context?.url) {
        const visual = await captureResolveVisualContext(context.url, kuri);
        if (visual) (res as Record<string, unknown>).visual_context = visual;
      }
  ...
  // Backend marketplace listing (auth-gated) stays at beta-api.unbrowse.ai/v1/skills.
  ...
  app.post("/v1/skills/from-routes", async (req, reply) => {
    ...
    // This route is populated passively by browser integrations. Preserve the
    // local index unconditionally, but never publish capture-time evidence.
    // Remote publication requires a later independent replay permit.
    const publishedAt: string | undefined = undefined;
    const publishStatus: "indexed" = "indexed";
  ...
  app.post("/v1/skills/:skill_id/execute", { config: { rateLimit: ROUTE_LIMITS["/v1/skills/:skill_id/execute"] } }, async (req, reply) => {
    const clientScope = clientScopeFor(req);
    const { skill_id } = req.params as { skill_id: string };
    const { params, projection, confirm_unsafe, confirm_third_party_terms, payment_verified, approval_token, dry_run, intent, context_url, session_id, auth_headers } = req.body as {
      params?: Record<string, unknown>;
      projection?: ProjectionOptions;
      confirm_unsafe?: boolean;
      confirm_third_party_terms?: boolean;
    ...
    const selectedEndpoint = skill.endpoints.find((endpoint) => endpoint.endpoint_id === (execParams as Record<string, unknown>).endpoint_id)
      ?? (skill.endpoints.length === 1 ? skill.endpoints[0] : undefined);
    const mutationCandidates = skill.endpoints.filter((endpoint) => ["POST", "PUT", "PATCH", "DELETE"].includes(endpoint.method.toUpperCase()));
    if (!selectedEndpoint && mutationCandidates.length > 0 && !dry_run) {
      return reply.code(409).send({
        ok: false,
        error: "mutation_endpoint_selection_required",
        permission: {
          decision: "ask",
          reason: "mutation_endpoint_selection_required",
          bypass_immune: true,
        },
        mutation_guard: {
          mode: "ask",
          user_confirmation_required: true,
          candidate_endpoint_ids: mutationCandidates.map((endpoint) => endpoint.endpoint_id),
        },
      });
    }
    // The existing mutation policy remains authoritative; it is composed into the
    // centralized gate as deny/ask rules rather than being replaced or weakened.
    const mutationDecision = selectedEndpoint
      ? decideMutationPolicy(selectedEndpoint.method, selectedEndpoint.url_template, confirm_unsafe === true)
      : { allowed: true as const, mode: "safe" as const, reason: "endpoint_selected_during_execution" };
    const effect = selectedEndpoint ? classifyExecutePermissionEffect(selectedEndpoint, dry_run === true) : "read";
    const operationFingerprint = selectedEndpoint
      ? executeOperationFingerprint({ skillId: skill_id, endpoint: selectedEndpoint, params: execParams })
      : `sha256:${createHash("sha256").update(stablePermissionValue({ skill_id, params: execParams })).digest("hex")}`;
    const termsRequired = selectedEndpoint?.policy?.requires_third_party_terms_confirmation === true;
    const termsMissing = termsRequired && confirm_third_party_terms !== true;
    const hasMutationMethod = dry_run !== true && selectedEndpoint != null && ["POST", "PUT", "PATCH", "DELETE"].includes(selectedEndpoint.method.toUpperCase());
    const mutationBlocked = hasMutationMethod && !mutationDecision.allowed && mutationDecision.mode !== "ask";
    const mutationNeedsConfirmation = hasMutationMethod && !mutationDecision.allowed && mutationDecision.mode === "ask";
    const rules = mutationBlocked
      ? { deny: true, reason: mutationDecision.reason }
      : mutationNeedsConfirmation
        ? { ask: true, reason: mutationDecision.reason }
        : termsMissing
          ? { ask: true, reason: "third_party_terms_confirmation_required" }
          : { allow: true, reason: "execute_policy_satisfied" };

    // Explicit local confirmations may mint an operation-bound, one-shot grant.
    // Payment is deliberately excluded: caller input is not a settlement receipt.
    // A signal for different params/endpoint can never approve this call.
    // Confirmation flags shape policy and the typed ask, but cannot mint their
    // own authority. Only a host/UI-issued opaque token can cross this boundary.
    const permission = permissionContext.evaluate({
      operation_fingerprint: operationFingerprint,
      effect,
      workspace_trusted: isTrustedPermissionCaller(req.ip ?? req.socket.remoteAddress),
      approval_token: approval_token,
      rules,
    });
    if (permission.decision !== "allow") {
      return reply.code(permission.decision === "deny" ? 403 : 409).send({
        ok: false,
        ...
        permission: {
          ...
          operation_fingerprint: operationFingerprint,
          confirmation: {
            ...(effect !== "read" ? { host_approval_token_required: true } : {}),
            ...(effect === "payment" ? { verified_payment_receipt_required: true } : {}),
            ...(termsRequired ? { confirm_third_party_terms: true } : {}),
          },
        },
        ...(hasMutationMethod ? {
          mutation_guard: {
            mode: mutationDecision.mode,
            method: selectedEndpoint?.method,
            url_template: selectedEndpoint?.url_template,
            whitelist: getMutationPolicySettings().whitelist,
            user_confirmation_required: permission.decision === "ask",
          },
        } : {}),
      });
    }
    const effectiveConfirmUnsafe = confirm_unsafe === true || (adhocWrite && mutationDecision.allowed);
    ...
    try {
      ...
      // connection open without completing the body. Bound the whole dispatch and
      // return a structured execution_timeout the agent can act on, instead of an
      // open-ended hang. Generous default (covers replay + probe + one browser
      // recovery); override with UNBROWSE_EXECUTE_TIMEOUT_MS.
      const executeDeadlineMs = Number(process.env.UNBROWSE_EXECUTE_TIMEOUT_MS) || 60_000;
      ...
      if (cacheKey) {
        ...
        if (cachedResult) {
          ...
          recordCreativityActFromExecute(response, {
            intent,
            skill_id,
            endpoint_id: cachedResult.trace.endpoint_id,
          });
      ...
      const cancelDisconnectedExecution = () => {
        if (!reply.raw.writableEnded) disconnectController.abort(new Error("client_disconnected"));
      };
      req.raw.once("aborted", cancelDisconnectedExecution);
      reply.raw.once("close", cancelDisconnectedExecution);
      let execResult: Awaited<ReturnType<typeof executeSkill>>;
      try {
        execResult = await withExecuteDeadline<Awaited<ReturnType<typeof executeSkill>>>(
        (signal) => executeSkill(skill!, execParams, projection, { confirm_unsafe: effectiveConfirmUnsafe, confirm_third_party_terms, payment_verified: false, dry_run, intent, contextUrl: context_url, client_scope: clientScope, authHeaders: auth_headers, signal }),
        executeDeadlineMs,
        () => {
          ...
          return {
            trace: {
              trace_id: nanoid(),
              skill_id,
              endpoint_id: endpointIdForTrace,
              started_at: now,
              completed_at: now,
              success: false,
            ...
            result: {
              error: "execution_timeout",
              message: `Execution exceeded ${executeDeadlineMs}ms and was abandoned to avoid an indefinite hang.`,
              skill_id,
              endpoint_id: endpointIdForTrace,
              hint: "The endpoint replay stalled (often a stale browser cookie attached to a public endpoint, or a server holding the connection). Retry, or re-capture the route with `unbrowse run`/`unbrowse go`.",
            },
          };
        },
        ...
        req.raw.removeListener("aborted", cancelDisconnectedExecution);
        reply.raw.removeListener("close", cancelDisconnectedExecution);
      }
      ...
      if (
        ...
        if (!recovered) {
          ...
          if (fromResolve && fromResolve.endpoint_id !== want) {
            ...
            const recovery = assessSingleEndpointRecovery(onlyEp, intent, context_url);
            if (recovery.allowed) {
              console.log(`[exec] D7 single-endpoint skill: rewriting endpoint_id ${want} → ${only}`);
              execParams.endpoint_id = only;
              recovered = true;
            } else {
              console.log(`[exec] D7 single-endpoint skill: only endpoint ${only} is incompatible (${recovery.reason}) — not forcing`);
              execResult = {
                trace: {
                  ...execResult.trace,
                  success: false,
                  error: "no_relevant_route",
                },
                result: {
                  error: "no_relevant_route",
                  reason: recovery.reason,
                  message: `The cached endpoint exists but cannot satisfy the requested intent (${recovery.reason}).`,
                  requested_endpoint_id: want,
                  available_endpoints: onlyEp ? [{
                    endpoint_id: onlyEp.endpoint_id,
                    method: onlyEp.method,
                    url_template: onlyEp.url_template,
                    description: onlyEp.description,
                    action_kind: onlyEp.semantic?.action_kind,
                    resource_kind: onlyEp.semantic?.resource_kind,
                  }] : errResult.available_endpoints,
                },
              };
            }
          }
          ...
          try {
            ...
            execResult = {
              ...
              result: {
                ...
                message: "No cached sibling endpoint satisfies the requested intent and response cardinality.",
                requested_endpoint_id: execParams.endpoint_id,
                available_endpoints: skill.endpoints.map((endpoint) => ({
                  endpoint_id: endpoint.endpoint_id,
                  method: endpoint.method,
                  url_template: endpoint.url_template,
      ...
      recordCreativityAct({
        text: [
          ...
          `skill:${skill_id}`,
          execResult.trace.endpoint_id ? `endpoint:${execResult.trace.endpoint_id}` : null,
        ].filter(Boolean).join(" "),
        route: execResult.trace.endpoint_id ?? skill_id,
        cacheHit: false,
      });
      if (execResult.trace.success) {
        ...
        // Canonical action DAG: a successful direct replay settles publication
        // before returning. share_pointers=false and per-domain policy remain
        // authoritative opt-outs inside queuePassiveSkillPublish.
  ...
  app.post("/v1/skills/:skill_id/auth", async (req, reply) => {
    ...
    //
    // `publishSkill` sanitizes `draft.endpoints` and NOTHING ELSE, so this route used to
    // push `operation_graph[].page_metadata.localStorage` — the page's real localStorage,
    // where sites park access tokens — straight at the public index. Route the manifest
    // through the boundary instead.
    //
    // Then re-cache the UNSANITIZED manifest: `publishSkill` caches whatever it is handed,
    // and the local copy must keep the captured material local replay depends on. Publish
    // is the only place the sanitized form belongs.
    if (!skill.auth_profile_ref) {
      // Adding local credentials is not publication consent. Keep the vault
      // reference in the local cache; an explicit publish can sanitize/share it
      // later without this auth helper becoming a hidden publish path.
      const patched: SkillManifest = { ...skill, auth_profile_ref: ref };
      cachePublishedSkill(patched);
    }
  ...
  async function spoolBrowseCapture(
    ...
    let harEntries: HarEntry[] = drainInspectedHarEntries(session.sessionId);
    if (session.harActive) {
      try {
        const broker = brokerForSession(session);
        const stopResult = await broker.harStop(session.tabId);
        ...
        async (session) => submitBrowseForm(
          {
            ...
            session,
            flushCapture: async (session) => await flushBrowseCapture(session, { queuePublish: false }),
