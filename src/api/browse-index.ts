...
export interface CaptureDiagnostic {
  ...
  // CLAUDE.md's "platform enables; does not prescribe" rule these are
...
export function shouldIndexDomBrowseFallback(params: {
  ...
  // Heuristic admission gates REMOVED 2026-05-18 per platform-enables
...
export async function cacheBrowseRequests(params: {
  ...
  if (rawEndpoints.length > 0) {
    ...
    if (!existingSkill || mergedEndpoints.length >= existingSkill.endpoints.length) {
      ...
      // (NEBIUS_API_KEY / OPENAI_API_KEY). platform-correct quality
  ...
  try {
    ...
    const endpoint: EndpointDescriptor = {
      ...
      // usage data existed. platform-correct shape: start at 0.5 and let
