...
export function harEntriesToRawRequests(entries: HarEntry[], baseUrl?: string): RawRequest[] {
  return entries
...
export function mergeBrowseRequests(intercepted: RawRequest[], harEntries: HarEntry[], baseUrl?: string): RawRequest[] {
...
export async function cacheBrowseRequests(params: {
  ...
  // The identity the route cache asks for on read — port-qualified for local
  // targets, unchanged for public hosts. Both skill manifests below (API and DOM)
  // must be stored under it or they can never be replayed.
  const routeDomain = routeIdentityForUrl(sessionUrl, domain);
  ...
  if (rawEndpoints.length > 0) {
    const existingSkill = findExistingSkillForDomain(routeDomain);
  ...
  try {
    ...
    const existing = findExistingSkillForDomain(routeDomain);
