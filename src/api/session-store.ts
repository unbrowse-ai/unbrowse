...
//
// Measured before this: a run with a FRESH UNBROWSE_HOME logged "rehydrated 245
// browse session(s) from disk" — the developer's real sessions, inherited whole.
// Among them were stale Chrome-brokered sessions, which is how an isolated
// obscura run ended up starting a kuri broker on port 7800 and attaching Chrome.
// An isolated run that inherits 245 foreign sessions is not isolated, and every
// cold-vs-warm measurement taken through it is suspect.
const DEFAULT_PATH = path.join(getUnbrowseHome(), "sessions.jsonl");
