...
/**
 * `<data root>/queue/capture-pending` — sibling of queue/pending, sharing the
 * same .heartbeat / dead / quarantine conventions.
 *
 * The parameter is the DATA ROOT (`~/.unbrowse`, or `$UNBROWSE_HOME`), not a
 * home directory. It used to be a home that this function appended
 * `.unbrowse` to, which gave one function two path vocabularies and made the
 * default bypass U-3 — an isolated run spooled captures into the caller's real
 * queue. One meaning now, defaulted from the same helper everything else uses.
 */
export function getCaptureSpoolDir(dataRoot: string = getUnbrowseHome()): string {
  return join(dataRoot, "queue", "capture-pending");
}
