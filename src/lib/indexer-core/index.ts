...
import { validateManifest, cachePublishedSkill } from "../../client/index.js";
import { publishSkill, type MarketplacePublishAuthorization } from "../../marketplace/index.js";
...
import { writeJob, listJobs, heartbeatAgeMs, tryAcquireWorkerSlot, sanitizeDomain } from "./queue-store.js";
import {
  acknowledgeIndexCompletion, cleanupAcknowledgedIndexJobs,
  createDurableIndexEnvelope, indexJobStore, pendingIndexCompletions,
} from "./durable-index-jobs.js";
import type { IndexJobId, TerminalJob } from "../../runtime/job-store.js";

// U-3, and a split-brain closed: src/orchestrator/index.ts computes this SAME
// logical directory as `join(getUnbrowseHome(), "skill-snapshots")`. Computing
// it from $HOME here meant that under UNBROWSE_HOME the orchestrator wrote
// snapshots to the isolated root while this module read the real one — two
// halves of one mechanism disagreeing about where their data lives.
const SKILL_SNAPSHOT_DIR = process.env.UNBROWSE_SKILL_SNAPSHOT_DIR
  ?? join(getUnbrowseHome(), "skill-snapshots");

function getQueueDir(): string {
  return join(getUnbrowseHome(), "queue", "pending");
}
 ...
 *
 * ROUTE IDENTITY, NOT COOKIE SCOPE — and the PROPAGATION path for the
 * 2026-08 data.gov.sg defect. `candidate.domain` and `domain` are both
 * capture CONTEXTS (the page host a capture ran on; see the manifest built
 * in src/browser/index.ts, `domain: new URL(url).hostname`), so this is
 * route identity and is host-scoped through the one shared helper
 * {@link sameRouteScope}. Under the old `getRegistrableDomain` comparison a
 * capture on `guide.data.gov.sg` merged its GitBook endpoints straight into
 * the `data.gov.sg` manifest — which re-poisons selection through the back
 * door even after the selection-side fix, because the endpoint then lives
 * inside a manifest whose own `domain` IS `data.gov.sg`. It also disagreed
 * with `getDomainReuseKey` (host-scoped) used by persistIndexedSkillState
 * below; the two now share one alphabet.
 *
 * Deliberately NOT tightened: the ENDPOINTS being merged are never checked
 * against the host — `mergeEndpoints` accumulates `api.example.com` URLs
 * into an `example.com` skill on purpose (endpoint ownership, registrable-
 * wide). `sameRouteScope` also keeps `www.`/geo-variant siblings merging.
 */
export function findAndMergeDomainSnapshot(
  ...
  for (const entry of readdirSync(snapshotDir)) {
    ...
    try {
      ...
      if (!sameRouteScope(candidate.domain, domain)) continue;
 ...
 * job completes.
 */
export function queueBackgroundIndex(job: BackgroundIndexJob): IndexJobId | undefined {
  if (shouldRunInline()) {
  ...
  let durable: ReturnType<typeof createDurableIndexEnvelope>;
  try {
    durable = createDurableIndexEnvelope(queueDir, job);
  } catch (err) {
    console.error(`[capture-pipeline] durable queue admission failed: ${(err as Error).message}`);
    return undefined;
  }
  const { envelope } = durable;
  writeJob(queueDir, envelope)
    ...
    .catch(err => {
      // Admission is already durable. Preserve a terminal notification rather
      // than leaving an unobservable running record if the legacy write fails.
      try { indexJobStore(queueDir).fail(durable.id, envelope.durable!.ownerId, err); } catch {}
      console.error(`[capture-pipeline] queue write failed: ${(err as Error).message}`);
    });
  return durable.id;
}

/** Idempotent completion delivery for the local index-queue owner. */
export function pendingBackgroundIndexCompletions(limit?: number): TerminalJob[] {
  return pendingIndexCompletions(getQueueDir(), limit);
}

/** Owner receipt is the GC barrier; repeated acknowledgement is harmless. */
export function acknowledgeBackgroundIndexCompletion(id: IndexJobId): TerminalJob {
  return acknowledgeIndexCompletion(getQueueDir(), id);
}

export function cleanupAcknowledgedBackgroundIndexJobs(olderThanMs = 0, limit = 100): IndexJobId[] {
  return cleanupAcknowledgedIndexJobs(getQueueDir(), olderThanMs, limit);
}
...
export async function publishIndexedSkill(
  indexed: IndexedSkillState,
  ...
  if (!authorization) {
    ...
    console.warn(`[capture-pipeline] legacy publish denied for ${domain}: lifecycle_publish_permit_required`);
  ...
  const selectedGraph = skill.operation_graph ? {
  ...
  const draft: SkillManifest = {
    ...skill,
    endpoints: sanitized,
    ...(selectedGraph ? { operation_graph: selectedGraph } : {}),
    indexer_id: getLocalAgentId(),
  };
  let validation;
  try {
  ...
  let published;
  try {
    published = await publishSkill(draft, authorization);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
  ...
  if (published.published_remotely !== true) {
    ...
    console.warn(`[capture-pipeline] remote publish not committed for ${domain}; keeping ${skill.skill_id} indexed locally`);
    return { published: false, publishStatus: "indexed" };
  }
  ...
  // The selected DAG is included in the permit-bound manifest; never emit a
  // second graph transport outside that authorization boundary.
 ...
 *   1. `share_pointers` (explicit-consent flag): false → local indexing only;
 *      no manifest is sent to the marketplace (including private storage).
...
export function shouldPublishAfterIndex(
  ...
  if (!contribution.share_pointers) {
    return {
      ...
      reason: "share_pointers=false: indexed locally only; explicit marketplace consent is required before remote publication.",
    };
  }
