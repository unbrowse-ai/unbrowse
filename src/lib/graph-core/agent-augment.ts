...
import { stripAllHeaderMapsInPlace } from "../../publish/sanitize.js";
...
function buildEndpointPayload(endpoint: EndpointDescriptor) {
  const semantic = resolveEndpointSemantic(endpoint);
  // Graph wire: never send captured header maps (any name→value under headers*).
  const sample_request = compact(semantic.example_request);
  if (sample_request && typeof sample_request === "object") {
    stripAllHeaderMapsInPlace(sample_request);
  }
  return {
