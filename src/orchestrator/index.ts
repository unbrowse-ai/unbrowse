 ...
 * execute should be able to use it. platform honors the contract.
 *
 ...
 * web-search highlights ABOUT the topic.
 */
...
export async function resolveAndExecute(
  ...
  function buildDeferral(
    ...
    // locally, but the root signature-user online-learned confidence is projected
    ...
    if ((epRanked.length === 0 || allNegative) && hostMatches) {
      ...
      // platform-faithful: page_fetch is a STRUCTURAL primitive (one per
      ...
      const pageFetchShortlistEntry = {
        ...
        agent_warning: "page_fetch fallback (platform's structural floor): fetches the rendered page + extracts inline. Pick this when none of the ranker candidates look intent-relevant.",
      };
  ...
  if (context?.url && !forceCapture && !agentChoseEndpoint) {
    ...
    if (raceOutcome.winner) {
      ...
      // 6bb821ff;  — prove all things, hold fast the good):
            ...
            if (!directDocument.rejected) {
              ...
              if (directDocument.reason === "interstitial_detected") {
                try {
                  ...
                  // policy (route sha256:65714387c8c9f6...): direct-egress
  ...
  if (!learned_skill && !trace.success) {
    ...
    // failure is NOT a platform-tagged auth_required (which has its own
