...
} from "../execution/anti-bot-rescue.js";
import { walkDocumentEscalationLayers } from "../values/layer-adapters.js";
import { egressFetchWithBlockCheck, type EgressFetch } from "../execution/egress-chain.js";
...
const proxiedWebFetch: EgressFetch = (input, init) =>
...
import { cardinalityMatches, routeLooksLikeSingleItem, isListLikeIntent, isConcreteResourceLeaf, urlHasConcreteResourcePath, pathCoherent, requestExpectsCollection, requestedResultCount } from "../values/cardinality.js";
export { schemaLooksLikeSingleItem } from "../values/cardinality.js";
import { credentialFromAuthContext, principalScope } from "../runtime/principal-scope.js";
import { getRouteLifecycle, issueRoutePublishPermit, transitionRouteLifecycle, type RouteLifecycleIdentity, type RouteLifecycleStoreOptions } from "../runtime/route-lifecycle.js";
...
import { passivePublishArtifactFingerprint, queuePassiveSkillPublish } from "./passive-publish.js";
import { getContributionConfig } from "../config/contribution.js";
import { decideCheckpointPublish } from "../settings.js";
import { isEvidenceBackedReadEndpoint } from "./action-dag.js";
...
import { existsSync, writeFileSync, readFileSync, mkdirSync, readdirSync, statSync, unlinkSync } from "node:fs";
...
import { intentRequiresEvaluatedJavascript } from "../runtime/browser-access.js";
import { captureSession, isBrowserAccessAvailable } from "../capture/index.js";
import { noteCdpRequired } from "../capture/engine.js";
import { shouldFallbackToBrowser, fallbackImproved, carryBrowserOpened, markBarrenPage, exportBarrenPages, hydrateBarrenPages } from "./browser-fallback.js";
import { getUnbrowseHome } from "../runtime/paths.js";
import { passiveIndexFromRequests } from "../api/routes.js";

const CONFIDENCE_THRESHOLD = 0.3;

// ── Background API Discovery ─────────────────────────────────────────────────
// After a successful direct-document (HTML) extraction, fire a background
// browser capture to discover and index internal API routes (XHR/fetch).
// The HTML result is returned immediately; the next call finds cached API
// routes (structured JSON, <200ms) instead of re-parsing HTML.
// Per-domain dedup: only one background capture per domain at a time.
const inflightApiCaptures = new Set<string>();
/** Per-domain cooldown between background discovery attempts. Survives process
 *  exit via a dated marker file, because the CLI is one process per call. */
const BG_CAPTURE_COOLDOWN_MS = Number(process.env.UNBROWSE_BG_API_CAPTURE_COOLDOWN_MS ?? 6 * 60 * 60 * 1000);
/** Max background discovery browsers alive at once, across ALL processes. */
const BG_CAPTURE_MAX_CONCURRENT = Number(process.env.UNBROWSE_BG_API_CAPTURE_MAX ?? 2);

function queueBackgroundApiCapture(url: string, intent?: string): void {
  let domain: string;
  try { domain = new URL(url).hostname; } catch { return; }

  // Skip if already capturing this domain
  if (inflightApiCaptures.has(domain)) return;

  // Skip if browser is not available (proxy-only / headless-only configs)
  try {
    if (!isBrowserAccessAvailable()) return;
  } catch { return; }

  // Skip if explicitly disabled
  if (process.env.UNBROWSE_BG_API_CAPTURE === "0") return;

  // Cross-process dedup. `inflightApiCaptures` is per-process and the CLI runs ONE
  // PROCESS PER CALL, so on its own it deduped nothing — every call would spawn
  // another browser for the same domain. A dated marker under the (U-3-respecting)
  // home makes the cooldown survive process exit, which is where it has to live.
  const markerDir = join(getUnbrowseHome(), "bg-capture");
  const marker = join(markerDir, `${domain.replace(/[^a-z0-9.-]/gi, "_")}.json`);
  try {
    const prev = statSync(marker).mtimeMs;
    if (Date.now() - prev < BG_CAPTURE_COOLDOWN_MS) return;
  } catch { /* no marker yet — first discovery for this domain */ }

  // Global concurrency cap. Each spawn is a real browser in its own process, and
  // one process per CLI call means nothing else bounds them: a 100-site run would
  // settle ~half from HTML and try to launch a browser for every one of them.
  // Live pids are counted (and dead ones reaped) so a killed child never leaks a
  // slot permanently.
  const activeDir = join(getUnbrowseHome(), "bg-capture", "active");
  let active = 0;
  try {
    ...
    for (const f of readdirSync(activeDir)) {
      ...
      if (alive) active++;
      else { try { unlinkSync(join(activeDir, f)); } catch { /* raced with another reaper */ } }
    }
  } catch { /* unreadable dir — fall through and allow the spawn */ }
  if (active >= BG_CAPTURE_MAX_CONCURRENT) {
    console.log(`[bg-api-capture] ${domain} skipped — ${active} discovery job(s) already running`);
    return;
  }

  inflightApiCaptures.add(domain);

  // Spawn DETACHED and unref. The previous in-process promise could not finish:
  // the parent returns the HTML immediately and exits, taking a multi-second
  // browser capture with it. Witnessed on defillama.com — "queued" logged, no
  // completion line, no route-cache written, warm call re-scraped the same HTML.
  try {
    mkdirSync(markerDir, { recursive: true });
    writeFileSync(marker, JSON.stringify({ url, intent, at: new Date().toISOString() }));
    const entry = process.argv[1] ?? "";
    if (!entry) return;
    const child = spawn(process.execPath, [entry, "__bg-capture", url, ...(intent ? [intent] : [])], {
      detached: true,
      stdio: "ignore",
      env: { ...process.env, UNBROWSE_BG_API_CAPTURE: "0" }, // the child must never re-arm this
    });
    child.unref();
    // Claim the slot with the CHILD's pid, so liveness is checked against the
    // process doing the work rather than this one, which is about to exit.
    if (child.pid) {
      try { writeFileSync(join(activeDir, `${child.pid}.pid`), String(child.pid)); } catch { /* best-effort */ }
    }
    console.log(`[bg-api-capture] ${url} queued — discovering API routes in background`);
  } catch (err) {
    console.log(`[bg-api-capture] ${domain} spawn failed: ${err instanceof Error ? err.message : String(err)}`);
  }
}
// ── End Background API Discovery ─────────────────────────────────────────────
...
export function shouldAutoWalk(
  ...
  if (!topUrl || !isWalkCandidateUrl(topUrl)) return false;
...
/** Admit only URLs that can represent a real browser destination. Search providers
 * occasionally return citation punctuation as an encoded relative-looking path
 * (for example `/%EF%BC%89%7C` => `/）|`). A merely non-root pathname must not
 * make that garbage outrank an actual result. Unicode letters and numbers are
 * deliberately accepted so internationalized routes remain first-class. */
export function isWalkCandidateUrl(candidate: string | null | undefined): boolean {
  if (!candidate) return false;
  try {
    const url = new URL(candidate);
    if ((url.protocol !== "http:" && url.protocol !== "https:") || !url.hostname) return false;
    let decodedPath: string;
    try { decodedPath = decodeURIComponent(url.pathname).replace(/\/+$/g, ""); }
    catch { return false; }
    if (!decodedPath) return true;
    if (/[\p{L}\p{N}]/u.test(decodedPath)) return true;
    return [...url.searchParams.keys()].some((key) => /[\p{L}\p{N}]/u.test(key));
  } catch {
    return false;
  }
}
...
const captureInFlight = new Map<
  string,
  Promise<{ learned_skill?: SkillManifest; trace: ExecutionTrace; result: unknown; parity_baselines?: Record<string, unknown> }>
>();
...
>();
// U-3: these resolve through getUnbrowseHome() so UNBROWSE_HOME relocates them,
// as `runtime/paths.ts` promises it does "everywhere". They previously read
// process.env.HOME directly, which made that promise false for exactly the
// caches that decide cold-vs-warm. Measured: a run with UNBROWSE_HOME set to a
// temp dir put logs/, secrets/ and config.json there, and still wrote
// route-cache.json, domain-skill-cache.json and barren-pages.json into the
// caller's REAL ~/.unbrowse — so an "isolated" cold call was served by the
// developer's own warm route cache. Default behaviour is unchanged: with no
// override, getUnbrowseHome() is ~/.unbrowse, the same path as before.
const ROUTE_CACHE_FILE = join(getUnbrowseHome(), "route-cache.json");
const SKILL_SNAPSHOT_DIR = process.env.UNBROWSE_SKILL_SNAPSHOT_DIR
  ?? join(getUnbrowseHome(), "skill-snapshots");
...
const DOMAIN_CACHE_FILE = join(getUnbrowseHome(), "domain-skill-cache.json");
// Barren pages survive process exit for the same reason every other cache does:
// the CLI is one process per call, so an in-memory-only set is always empty at startup.
const BARREN_CACHE_FILE = join(getUnbrowseHome(), "barren-pages.json");
...
// VALUE ( — the binding around the hole, that it not be torn; sign the resolved blob, not
...
/**
 * Barren-page memory across processes.
 *
 * `browser-fallback.ts` keeps the set in memory and stays pure (the permutation
 * matrix exhausts it as a pure function), so persistence lives here. Without it
 * the guard was dead weight for the CLI: one process per call meant the set was
 * always empty and the ~14s barren capture was re-paid every single invocation.
 */
function loadBarrenPages(): void {
  if (!LOCAL_CACHES_ENABLED || ISOLATED_SKILL_SNAPSHOT_MODE) return;
  try {
    if (!existsSync(BARREN_CACHE_FILE)) return;
    const raw = JSON.parse(readFileSync(BARREN_CACHE_FILE, "utf-8")) as Array<[string, number]>;
    if (Array.isArray(raw)) hydrateBarrenPages(raw);
  } catch { /* a corrupt cache is not worth failing a resolve over */ }
}

function persistBarrenPages(): void {
  if (!LOCAL_CACHES_ENABLED || ISOLATED_SKILL_SNAPSHOT_MODE) return;
  try {
    const dir = dirname(BARREN_CACHE_FILE);
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
    writeFileSync(BARREN_CACHE_FILE, JSON.stringify(exportBarrenPages()), "utf-8");
  } catch { /* best effort */ }
}
...
export function invalidateRouteCacheForDomain(domain: string): void {
  let deleted = 0;
  // Keys carry the ROUTE SCOPE HOST in their domain segment now, so match that
  // form too (superset of the previous literal/www-stripped needles — this only
  // ever adds matches, never removes one).
  const needles = new Set([`:${domain}:`, `:${domain.replace(/^www\./, "")}:`]);
  const scope = routeScopeHost(domain);
  if (scope) needles.add(`:${scope}:`);
  for (const [k] of skillRouteCache) {
    if ([...needles].some((needle) => k.includes(needle))) {
...
export function writeSkillSnapshot(cacheKey: string, skill: SkillManifest): string | undefined {
  ...
  // Precedence: STATELESS wins over OFFLINE.
...
function compositeSnapshotDir(): string {
  return process.env.UNBROWSE_COMPOSITE_DIR
    ?? join(getUnbrowseHome(), "composites");
}
...
export function findBestLocalDomainSnapshot(
  ...
  // Route identity, not cookie scope: a snapshot captured on guide.data.gov.sg is
  // not a candidate for a data.gov.sg request. The context URL is the more
  // specific truth about what was requested, so it wins over `requestedDomain`
  // (which a caller may already have collapsed to an eTLD+1).
  const targetScope = routeScopeHost(contextUrl) ?? routeScopeHost(requestedDomain);
  ...
  for (const entry of readdirSync(SKILL_SNAPSHOT_DIR)) {
    ...
    try {
      ...
      if (!sameRouteScope(candidate.domain, targetScope)) continue;
...
/* ---------------------------------------------------------------------------
 * ROUTE IDENTITY HOST SCOPE — "route identity is not cookie scope"
 *
 * Cookies are legitimately scoped to the registrable domain (eTLD+1): that is
 * how browsers actually work, and `src/auth/*` is right to use
 * `getRegistrableDomain` for credential/profile/vault scoping. ROUTE identity is
 * a different question with a different right answer. Two hosts under one
 * registrable domain are routinely two entirely different APPLICATIONS with
 * entirely different APIs.
 *
 * The defect this fixes, reproduced end to end: a route captured on
 * `guide.data.gov.sg` (a GitBook-hosted developer guide) was stored under domain
 * `gov.sg`, so it was selected for `https://data.gov.sg/datasets` with intent
 * "list the datasets" — and returned GitBook's UI *translation file* as "the
 * datasets", with trace.success true. Every host under `gov.sg` shared one route
 * namespace. Note that a correct public-suffix table does NOT fix this: with
 * `gov.sg` as a public suffix both hosts still share the registrable domain
 * `data.gov.sg`. The scope itself was wrong, not the suffix table.
 *
 * The equivalence chosen — structural, no per-site list (repo standing rule):
 *   route scope = the FULL hostname label path, with two normalizations that are
 *   protocol/DNS conventions rather than application boundaries:
 *     1. a leading `www` (or `wwwN`) label is a DNS alias for the apex, so it is
 *        stripped — `www.example.com` and `example.com` stay one route scope;
 *     2. a geo-variant registrable suffix is the same application in another
 *        country (`airbnb.com` <-> `airbnb.com.sg`), so brand equality is still
 *        honoured — but ONLY when the subdomain path in FRONT of the registrable
 *        domain is identical. That extra requirement is the actual fix:
 *        `isSameBrandDomain` on its own maps BOTH `data.gov.sg` and
 *        `guide.data.gov.sg` to the brand label "gov" and collides them again.
 *   Ports are preserved for port-sensitive hosts (localhost / literal IPs), where
 *   the port is the application boundary.
 *
 * Deliberately NOT widened/narrowed here (different questions):
 *   - cookie / credential / browser-profile scoping in `src/auth/*` — genuinely
 *     eTLD+1, untouched;
 *   - "does this ENDPOINT belong to the site its skill was captured from"
 *     (`hasUsableEndpoints`, the weak-relevance on-domain fallback) — a site's
 *     own XHRs legitimately live on `api.` / `cdn.` / `gateway.` subdomains, so
 *     that comparison stays registrable-domain/brand wide. The two questions are
 *     told apart structurally, not by label names: route identity compares two
 *     REQUEST/CAPTURE CONTEXTS, endpoint ownership compares an ENDPOINT URL
 *     against its own owning skill.
 * ------------------------------------------------------------------------- */

/** Canonical route-scope host for a URL, host, or bare domain string. This is the
 *  cache-key alphabet and the unit of route identity. Returns null on empty
 *  input. Pure + exported for unit tests. */
export function routeScopeHost(input?: string | null): string | null {
  if (!input) return null;
  const raw = String(input).trim().toLowerCase().replace(/\.+$/, "");
  if (!raw) return null;
  let hostname = raw;
  let port = "";
  // `raw` may already be a URL, or a bare host ("data.gov.sg", "localhost:3000").
  // Try it as-is first, then with a scheme; `new URL("localhost:3000")` parses as
  // scheme "localhost" with an EMPTY hostname, hence the truthiness check.
  for (const candidate of [raw, `https://${raw}`]) {
    try {
      ...
      /* try the schemeless form next */
    }
  }
  hostname = hostname.replace(/\.+$/, "");
  if (!hostname) return null;
  // localhost / literal IPs: the PORT is the application boundary.
  if (isPortSensitiveHostname(hostname)) return port ? `${hostname}:${port}` : hostname;
  return hostname.replace(/^www\d*\./, "");
}

/**
 * The identity a learned skill must be stored under so the route cache can find
 * it again — i.e. exactly what {@link cachedSkillHostMatchesContext} will ask for
 * on read.
 *
 * Every skill-manifest builder used to derive this itself as
 * `new URL(pageUrl).hostname`, which silently DROPS the port. For localhost and
 * literal IPs the port is the application boundary (see {@link routeScopeHost}),
 * so the read side compared "localhost" against "localhost:35737" and always
 * failed: routes were learned for local targets and then never replayed, on
 * every call, forever. One shared derivation instead of N ad-hoc ones, so a new
 * builder gets this right for free.
 *
 * Deliberately narrow: it only widens when the scope is exactly
 * `<hostname>:<port>`. Public hosts keep the domain they already have, so the
 * `www.`-stripping that {@link routeScopeHost} also does cannot silently
 * re-key every existing skill.
 */
export function routeIdentityForUrl(pageUrl: string | undefined, hostname: string): string {
  if (!pageUrl) return hostname;
  const scope = routeScopeHost(pageUrl);
  return scope && scope.startsWith(`${hostname}:`) ? scope : hostname;
}

/** The labels in front of the registrable domain — the part of the host that says
 *  WHICH application: "guide.data.gov.sg" -> "guide.data", "data.gov.sg" -> "data",
 *  "airbnb.com" / "airbnb.com.sg" -> "". */
function routeScopeSubdomainPath(host: string): string {
  const registrable = getRegistrableDomain(host);
  const hostLabels = host.split(".");
  const depth = hostLabels.length - registrable.split(".").length;
  return depth > 0 ? hostLabels.slice(0, depth).join(".") : "";
}

/** Route-identity equivalence: same application, therefore same route namespace.
 *  Equality of {@link routeScopeHost}, plus the geo-variant allowance (same
 *  subdomain path + same brand). `data.gov.sg` !== `guide.data.gov.sg`;
 *  `www.example.com` === `example.com`. Pure + exported for unit tests. */
export function sameRouteScope(a?: string | null, b?: string | null): boolean {
  const scopeA = routeScopeHost(a);
  const scopeB = routeScopeHost(b);
  if (!scopeA || !scopeB) return false;
  if (scopeA === scopeB) return true;
  // The geo-variant allowance below is a fact about REGISTRABLE DOMAINS
  // (airbnb.com ~ airbnb.com.sg). It must never reach localhost or a literal IP,
  // where the port IS the application and there is no brand family to belong to.
  // Found by the permutation matrix: `getRegistrableDomain("127.0.0.1:3000")` is
  // "0.1:3000", whose subdomain path and brand both match "0.1:4000" — so a
  // route learned from one local service replayed against a DIFFERENT one on
  // another port. For a port-sensitive host, scope equality above is the only
  // acceptable test.
  if (isPortSensitiveHostname(stripScopePort(scopeA)) || isPortSensitiveHostname(stripScopePort(scopeB))) {
    return false;
  }
  return (
    routeScopeSubdomainPath(scopeA) === routeScopeSubdomainPath(scopeB) &&
    isSameBrandDomain(scopeA, scopeB)
  );
}

/** `"127.0.0.1:3000"` -> `"127.0.0.1"`; leaves a portless scope untouched. */
function stripScopePort(scope: string): string {
  return scope.replace(/:\d+$/, "");
}

/** Domain-level skill-cache key. Host-scoped (see the ROUTE IDENTITY block above):
 *  `guide.data.gov.sg` and `data.gov.sg` must not share a cache slot. */
export function getDomainReuseKey(input?: string | null): string | null {
  return routeScopeHost(input);
}
 ...
 *  be replayed for a requested URL in the SAME ROUTE SCOPE as the cached skill —
 *  a github.com skill must never answer a reddit.com request, and (the 2026-08
 *  data.gov.sg defect) a guide.data.gov.sg skill must never answer a data.gov.sg
 *  request. (endpointMatchesContextOrigin above is a localhost-only guard: it is
 *  a no-op for public hosts, so without this a mismatched — or poisoned —
 *  public-host cache entry replays freely. This is the structural recognition:
 *  compare hosts, not an allowlist.) Returns true when there is no requested URL
 *  (nothing to anchor to); otherwise defers to {@link sameRouteScope}, so `www.`
 *  and geo-variant siblings are kept but a different subdomain application is
 *  not. Pure + exported for unit tests. */
export function cachedSkillHostMatchesContext(
  skillDomain: string | null | undefined,
  contextUrl?: string,
): boolean {
  if (!contextUrl) return true;
  const ctxScope = routeScopeHost(contextUrl);
  if (!ctxScope) return true;
  const skillScope = routeScopeHost(skillDomain);
  if (!skillScope) return true;
  return sameRouteScope(skillScope, ctxScope);
}
...
/** Resolve cache key. The domain segment is the ROUTE SCOPE HOST, not the
 *  registrable domain: callers may hand us either a full host or an already
 *  eTLD+1-collapsed string (`context.domain` is caller-supplied and was literally
 *  "gov.sg" in the data.gov.sg defect), so the URL's own host — the more specific
 *  truth about what was actually requested — wins when present. Without this the
 *  key segment reads "gov.sg" for both data.gov.sg and guide.data.gov.sg and the
 *  sibling-key scans collapse two different applications into one namespace. */
export function buildResolveCacheKey(domain: string | null, intent: string, url?: string): string {
  const scope = routeScopeHost(url) ?? routeScopeHost(domain) ?? "global";
  return `${scope}:${intent.trim().toLowerCase()}:${normalizeRouteContext(url)}`;
}
...
/**
 * Could this route actually be executed again?
 *
 * Structural, not a source allowlist. The previous test named four sources
 * (`marketplace` / `route-cache` / `live-capture` / `direct-fetch`), which
 * silently excluded `dom-fallback` — so a browser rescue that landed via DOM
 * extraction never promoted a learned skill, and the browser re-opened on every
 * single call. Measured against a live public host: three consecutive calls,
 * three browser launches, nothing learned.
 *
 * "Which code path produced this" was never the real question. "Does it carry an
 * endpoint we can replay" is, and it answers itself: a concrete absolute
 * `url_template` plus a method. Sources that legitimately have nothing to replay
 * (an `exa` search hit carries no endpoints) are excluded by the same test
 * without being named, and a source added tomorrow is handled for free.
 */
export function carriesReplayableEndpoint(skill: Pick<SkillManifest, "endpoints"> | null | undefined): boolean {
  return (skill?.endpoints ?? []).some(
    (endpoint) =>
      typeof endpoint?.url_template === "string" &&
      /^https?:\/\//i.test(endpoint.url_template) &&
...
function persistWalkedRoute(scope: string, cacheKey: string, walked: OrchestratorResult): void {
  try {
    ...
    if (carriesReplayableEndpoint(skill)) {
...
export function isCachedSkillRelevantForIntent(
  ...
  // NOTE (route-identity split): this stays REGISTRABLE-DOMAIN wide on purpose.
  // It compares an ENDPOINT's URL against the request context — the "does this
  // API belong to this site" question — and a site's own XHRs legitimately live
  // on api./cdn./gateway. subdomains. Route IDENTITY (which cached route may
  // answer which request) is host-scoped instead; see routeScopeHost.
...
export interface OrchestratorResult {
  ...
  skill: SkillManifest;
  timing: OrchestrationTiming;
  /** Canonical per-task browser-open truth; false for cache/HTTP-only paths. */
...
/**
 * Structural URL-shape signal for "this path is probably a JSON API leaf"
 * — used when HEAD/probe content-type is missing or failed (DNS/TLS flake on
 * HEAD, servers that only speak GET). Implementation lives in values/url-shape
 * so execution/public-api derivation can share it without host allowlists.
 */
import { urlLooksLikeJsonApi } from "../values/url-shape.js";
import { settleJsonApiStructuralMirrors } from "../values/json-api-mirror.js";
export { urlLooksLikeJsonApi, structuralJsonApiMirrorUrls } from "../values/url-shape.js";
export { settleJsonApiStructuralMirrors } from "../values/json-api-mirror.js";
 ...
 */
/**
 * Free curl-impersonate rescue for tryDirectJsonFetch.
 * Used when native fetch throws OR returns non-2xx/3xx (e.g. CF 525 origin
 * TLS error, fingerprint blocks). Never arms paid egress (forceDirect).
 */
async function tryDirectJsonViaCurl(
  url: string,
  timeoutMs: number,
  curlFallback: typeof tryCurlImpersonateFetch | undefined,
): Promise<{ data: unknown; content_type: string } | null> {
  try {
    const curl = await (curlFallback ?? tryCurlImpersonateFetch)({ url, timeoutMs, forceDirect: true });
    ...
    /* curl unavailable / non-JSON — fall through to null */
  }
  return null;
}
...
export async function tryDirectJsonFetch(
  ...
  try {
    ...
    // Non-ok (525/403/5xx): still try free curl-impersonate before giving up.
    // Plain fetch often loses on CF fingerprint/origin TLS where chrome JA4 wins.
    if (!res.ok) {
      return tryDirectJsonViaCurl(url, timeoutMs, opts?.curlFallback);
    }
...
async function withAbortableOpTimeout<T>(
  ...
  try {
    return await Promise.race([
      work(signal),
      new Promise<never>((_, reject) => {
        if (signal.aborted) return reject(signal.reason ?? new Error(`${label}_timeout:${ms}`));
        signal.addEventListener(
          "abort",
          () => reject(signal.reason ?? new Error(`${label}_timeout:${ms}`)),
          { once: true },
        );
      }),
    ]);
...
function isAcceptableIntentResult(result: unknown, intent: string): boolean {
  ...
  // Cardinality guard (): a list/search intent must not be
...
async function fetchLiteralConcreteBody(
  url: string,
  intent?: string,
): Promise<{ result: unknown; content_type: string; success: boolean } | null> {
  try {
    ...
    if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      try {
        ...
        if (parsed !== null && typeof parsed === "object") return { result: parsed, content_type: ct, success: true };
      } catch { /* not JSON — fall to text */ }
    }
    if (/text\/html|application\/xhtml\+xml/i.test(ct)) {
      const document = buildBloombergDirectDocumentResult(url, body, ct, intent);
      if (document.rejected) {
        const blocker = document.reason === "challenge_html" || document.reason === "interstitial_detected"
          ? "challenge_page"
          : undefined;
        return {
          result: {
      ...
      return { result: document, content_type: ct, success: document.task_ok };
    }
    return {
      result: { url, content_type: ct, text: body, source_url: url, task_ok: true, extraction: { source: "direct-fetch", rejected: false } },
      content_type: ct,
      success: true,
    };
  } catch (e) {
...
function stableLifecycleJson(value: unknown, field = ""): string {
  if (Array.isArray(value)) return `[${value.map((child) => stableLifecycleJson(child, field)).join(",")}]`;
  if (value && typeof value === "object") {
    return `{${Object.entries(value as Record<string, unknown>)
      .filter(([key]) => !/value|example|token|cookie|authorization|timestamp|generated_at|observed_at/i.test(key))
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([key, child]) => `${JSON.stringify(key)}:${stableLifecycleJson(child, key)}`).join(",")}}`;
  }
  // DAG identity keeps only topology/schema labels. Runtime/request values never enter it.
  if (/^(?:operation_id|endpoint_id|from|to|source|target|kind|method|action_kind|resource_kind|key)$/i.test(field)) {
    return JSON.stringify(value) ?? "null";
  }
  return JSON.stringify(typeof value) ?? "null";
}

function routeDagFingerprint(skill: SkillManifest): string | undefined {
  if (!skill.operation_graph) return undefined;
  return `sha256:${createHash("sha256").update(stableLifecycleJson(skill.operation_graph)).digest("hex")}`;
}

function canonicalEndpointShape(endpoint: SkillManifest["endpoints"][number]): string {
  try {
    const url = new URL(endpoint.url_template);
    const queryKeys = [...url.searchParams.keys()].sort();
    return `${endpoint.method} ${url.origin}${url.pathname}${queryKeys.length ? `?${queryKeys.join("&")}` : ""}`;
  } catch {
    return `${endpoint.method} ${endpoint.url_template.split("?")[0]}`;
  }
}

function lifecycleBaselineFingerprint(intent: string, result: unknown): string {
  const rows = normalizeParityRows(result, intent);
  const evidence = rows.length
    ? rows.map(parityFingerprint).filter(Boolean).sort()
    : [compactParityValue(result)];
  return `sha256:${createHash("sha256").update(JSON.stringify(evidence)).digest("hex")}`;
}

function lifecycleIntentShape(intent: string): string {
  const shape = intent.toLowerCase()
    .replace(/https?:\/\/\S+/g, "<url>")
    .replace(/["'`][^"'`]+["'`]/g, "<value>")
    .replace(/\b(?:\d+[a-z]*|[0-9a-f]{8,}|[a-z0-9_-]{20,})\b/gi, "<value>")
    .replace(/\s+/g, " ").trim();
  return `sha256:${createHash("sha256").update(shape).digest("hex")}`;
}

async function routeLifecycleIdentityFor(
  skill: SkillManifest,
  endpointId: string | undefined,
  intent: string,
  options?: ExecutionOptions,
): Promise<RouteLifecycleIdentity | undefined> {
  if (!skill.endpoints || skill.endpoints.length === 0) return undefined;
  const endpoint = skill.endpoints.find((candidate) => candidate.endpoint_id === endpointId);
  if (!endpoint || endpoint.dom_extraction || endpoint.method === "WS") return undefined;
  let domain = skill.domain;
  try { domain = new URL(endpoint.url_template).hostname; } catch { /* keep manifest domain */ }
  const cookies = await getStoredAuth(domain).catch(() => null);
  const principal = principalScope(credentialFromAuthContext(options?.authHeaders, cookies));
  const dag = routeDagFingerprint(skill) ?? "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
  return {
    principal_scope: principal,
    skill_id: skill.skill_id,
    endpoint_fingerprint: `sha256:${createHash("sha256").update(`${canonicalEndpointShape(endpoint)}\0${dag}`).digest("hex")}`,
    intent_shape_hash: lifecycleIntentShape(intent),
  };
}

async function queueValidatedLifecyclePublication(
  skill: SkillManifest,
  identity: RouteLifecycleIdentity,
  store: RouteLifecycleStoreOptions,
): Promise<void> {
  // Consent and validation are independent durable gates. Issuing a permit must
  // not move an opted-out route into publication state.
  if (!getContributionConfig().contribution.share_pointers) return;
  if (!decideCheckpointPublish(skill.domain).publishQueued) return;
  const artifactFingerprint = passivePublishArtifactFingerprint(skill);
  const permit = await issueRoutePublishPermit(identity, artifactFingerprint, store);
  if (!permit) return;
...
async function persistRouteLifecycleOutcome(
  intent: string,
  output: OrchestratorResult,
  options?: ExecutionOptions,
): Promise<void> {
  if (!output.skill || !output.trace) return;
  const identity = await routeLifecycleIdentityFor(output.skill, output.trace.endpoint_id, intent, options);
  if (!identity) return;
  const store = options?.__route_lifecycle_file ? { file: options.__route_lifecycle_file } : {};
  const dag_fingerprint = routeDagFingerprint(output.skill) ?? "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";
  const current = await getRouteLifecycle(identity, store);
  if (output.timing.browser_opened === true) {
    const browserBaseline = (output as OrchestratorResult & { __browser_baseline?: unknown }).__browser_baseline;
    if (output.trace.success && browserBaseline !== undefined) {
      const baseline_fingerprint = lifecycleBaselineFingerprint(intent, browserBaseline);
      await transitionRouteLifecycle(identity, {
        type: "browser_observed", baseline_fingerprint,
        ...(dag_fingerprint ? { dag_fingerprint } : {}),
      }, store);
    }
  } else if (current.state === "validation_pending") {
    const baseline_fingerprint = lifecycleBaselineFingerprint(intent, output.result);
    const parity = output.trace.success
      && baseline_fingerprint === current.baseline_fingerprint
      && (!current.dag_fingerprint || dag_fingerprint === current.dag_fingerprint);
    const next = await transitionRouteLifecycle(identity, parity ? {
      type: "api_validation_succeeded", baseline_fingerprint,
      ...(dag_fingerprint ? { dag_fingerprint } : {}),
    } : { type: "api_validation_failed" }, store);
    if (parity && next.state === "validated") {
      await queueValidatedLifecyclePublication(output.skill, identity, store);
    }
  }
}
...
async function resolveAndExecuteInternal(
  intent: string,
  params: Record<string, unknown> = {},
  ...
  const t0 = Date.now();
  // Per-task primitive only; no browser/session handle enters telemetry.
  ...
  /**
   * The status the REQUESTED origin actually returned, as observed by the probe:
   * a real HTTP status, or 0 meaning "nothing answered" (no DNS record, refused
   * connection, CDN could not reach its backend). Null until something observed
   * it.
   *
   * Function-scoped because the two Exa-fallback emit sites sit in sibling
   * blocks, and BOTH used to stamp a hard-coded `status_code: 200` on a request
   * that had received no response at all. This carries the observed truth to
   * them so neither has to invent one.
   */
  let observedOriginStatus: number | null = null;
  ...
  function finalize(
    ...
    timing.browser_opened = browserOpened;
  ...
  async function openBrowseSessionHandoff(url: string, tabId?: string): Promise<OrchestratorResult | null> {
    ...
    if (!handoffTabId) {
      ...
      if (handoffTabId) browserOpened = true;
    }
  ...
  function canAutoExecuteEndpoint(endpoint: SkillManifest["endpoints"][number]): boolean {
    ...
    return isEvidenceBackedReadEndpoint(endpoint);
  }
  ...
  async function tryAutoExecute(
    ...
    // N-API-2: compulsory internal-API precedence — when the shortlist mixes real APIs
    // and document/page-fetch artifacts, the API must rank first. Documents remain
    // shortlistable as fallbacks (doc-only skills unchanged) but are clamped below
    // the best internal API so a high BM25 on the page-fetch cannot outrank an API
    // that matches the intent. Real API = !dom_extraction && method !== WS and not
    // document-shaped (checked via isPageFetchEndpoint / isDocumentLikeCandidate).
    if (hasInternalApiCandidate) {
      const isDocEndpoint = (candidate: RankedCandidate) =>
        !!candidate.endpoint.dom_extraction ||
        isPageFetchEndpoint(candidate.endpoint) ||
        isDocumentLikeCandidate(candidate, context?.url);
      const apiScores = epRanked
        .filter((candidate) => !isDocEndpoint(candidate) && candidate.endpoint.method !== "WS")
        .map((candidate) => candidate.score);
      const bestApiScore = apiScores.length > 0 ? Math.max(...apiScores) : Number.NEGATIVE_INFINITY;
      if (Number.isFinite(bestApiScore)) {
        let clamped = false;
        epRanked = epRanked.map((candidate) => {
          if (!isDocEndpoint(candidate)) return candidate;
          if (candidate.score < bestApiScore) return candidate;
          clamped = true;
          return { ...candidate, score: bestApiScore - 1 };
        });
        if (clamped) epRanked.sort((a, b) => b.score - a.score);
        if (epRanked.length > 0 && isDocEndpoint(epRanked[0]!)) {
          const apis = epRanked.filter((candidate) => !isDocEndpoint(candidate));
          const docs = epRanked.filter((candidate) => isDocEndpoint(candidate));
          if (apis.length > 0) epRanked = [...apis, ...docs];
        }
      }
    }
        ...
        if (stillUnbound.length > 0 && prereqOrder.length > 0) {
          ...
          if (chainSteps.length > 0) {
            ...
            if (chainSteps.every((s) => s.ok) && canAutoExecuteEndpoint(candidate.endpoint)) {
              ...
              // A newly attached composite remains local; top-level publication is allowlisted.
              void composedNewlyAttached;
  ...
  if (context?.url && !forceCapture && !agentChoseEndpoint) {
    ...
    // Early JSON-API settle (before budget race / Exa). When the caller's URL
    // looks like a JSON API by PATH SHAPE, fetch it now — do not wait for a
    // probe content-type that HEAD may never deliver (swapi DNS, spacex 525 on
    // HEAD, etc.). Miss falls through unchanged.
    //
    // KNOWN RESIDUAL: this settles BEFORE the route cache is read, so for
    // API-shaped URLs a route learned by the browser fallback is not replayed
    // and the browser can re-open on every call. Preferring a learned route here
    // was tried twice (localhost and a live public host) and did not fire either
    // time — the rescued route is not landing in a reusable form, so the fix
    // belongs at the promotion site, not here. Left unshipped rather than
    // shipped-unproven; see `persistWalkedRoute`'s `isReplayableRoute`.
    if (urlLooksLikeJsonApi(raceContextUrl)) {
      const earlyJson = await tryDirectJsonFetch(raceContextUrl, { timeoutMs: Math.min(12_000, budgetMs + 4000) });
      if (earlyJson) {
        const trace: ExecutionTrace = {
          trace_id: nanoid(),
        ...
        console.log(`[direct-fetch] ${raceContextUrl} url-shape JSON-API early path — direct fetch before race (ct=${earlyJson.content_type})`);
        return {
          result: earlyJson.data,
          trace,
          source: "direct-fetch" as const,
          skill: undefined as any,
          timing: finalize("direct-fetch", earlyJson.data, "direct-fetch", undefined as any, trace),
        };
      }
      console.log(`[direct-fetch] ${raceContextUrl} url-shape JSON-API early path miss — continuing race`);
    }
    ...
    const raceOutcome = await Promise.race([
      ...
      // `probeObserved?: undefined` keeps this deadline shape assignable to the
      // same union the race returns, so readers need no narrowing. A timeout
      // observed nothing about the origin, which is exactly what undefined says.
      new Promise<{ winner: null; tried: []; ms: number; probeObserved?: undefined }>((resolve) =>
    ...
    // Record what the origin actually said (0 = it never answered) so the Exa
    // fallbacks below can report it instead of fabricating a 200. Read from
    // `probeObserved`, NOT the winner: a status-0 probe fails `isValid` and so
    // never wins, which is precisely the case that was being lost.
    if (raceOutcome.probeObserved) observedOriginStatus = raceOutcome.probeObserved.status;
    ...
    if (raceOutcome.winner) {
      ...
      if (
        ...
        const literal = await fetchLiteralConcreteBody(raceContextUrl, queryIntent);
        if (literal) {
          const trace: ExecutionTrace = {
            trace_id: nanoid(), skill_id: "direct-fetch", endpoint_id: "direct-fetch",
            started_at: new Date(t0).toISOString(), completed_at: new Date().toISOString(), success: literal.success,
          };
      ...
      // A route WE indexed for this host outranks scraping the page.
      //
      // Any skill that exposes a real internal API (at least one non-document
      // endpoint — not bare `page_fetch` / `direct-document`) outranks scraping
      // the same page as HTML. Previously this only exempted `local-skill`,
      // so a marketplace skill that already covered a real API was still thrown
      // away in favor of re-scraping the literal URL. Measured on sites100,
      // 53 of 84 successful settles were HTML while learned skills sat unused.
      // When NO real API exists for the host, `direct-document` remains the
      // honest cold-start fallback; background discovery is queued so the next
      // call wins as API.
      //
      // The concrete-resource guard directly above still runs first either way,
      // so a skill that is not path-coherent with a concrete URL has already
      // been rejected before here.
      const indexedInternalApiOutranksDocument = skillCoversRealApi(w.skill);
      if (
        (w.kind === "marketplace" || w.kind === "local-skill") &&
        !agentChoseEndpoint &&
        raceContextUrl &&
        !indexedInternalApiOutranksDocument &&
        !isSearchLikeIntent(queryIntent, raceContextUrl)
      ) {
        const directDoc = await fetchDirectDocument(raceContextUrl, queryIntent);
      ...
      // Probe content-type OR url-shape: either is enough to prefer direct JSON
      // over Exa (url-shape covers HEAD-less / CT-missing APIs).
      if (w.kind === "probe" && (probeLooksLikeDirectJsonApi(w) || urlLooksLikeJsonApi(raceContextUrl))) {
      ...
      if (w.kind === "probe" && probeLooksLikeFetchableHtmlDocument(w)) {
        ...
        // Anti-bot rescue then deep browser capture via layered semantic-KV walk (N2b).
        // Cheapest-first: free rescue → paid rescue → browser. Free rungs never arm paid egress.
        // Opt out of deep capture with UNBROWSE_DEEP_CAPTURE=0 for fast coverage budgets.
        const ddReason = directDoc?.rejected ? (directDoc as { reason?: string }).reason : undefined;
        const escalatable = isRescueableDirectDocumentReason(ddReason);
        if (escalatable) {
          try {
            const antiBotDeadlineAt = t0 + budgetMs;
            const walked = await walkDocumentEscalationLayers(
              {
                intent: queryIntent,
                key: raceContextUrl,
                context: { url: raceContextUrl, target: raceContextUrl },
              },
              {
                freeHtml: async () => {
                  const rescued = await rescueBlockedPageFree({ url: raceContextUrl });
                  if (rescued.html && rescued.reason === "ok") {
                    return {
                      html: rescued.html,
                      via: rescued.via ?? "rescue_free",
                      bytes: rescued.bytes,
                    };
                  }
                  console.log(
                    `[anti-bot-rescue] free ${raceContextUrl} ${rescued.reason}${rescued.via ? ` via=${rescued.via}` : ""}`,
                  );
                  return null;
                },
                paidHtml: async () => {
                  const paid = await rescueBlockedPagePaid({ url: raceContextUrl });
                  if (paid.html && paid.reason === "ok") {
                    return {
                      html: paid.html,
                      via: paid.via ?? "rescue_paid",
                      bytes: paid.bytes,
                    };
                  }
                  if (paid.reason !== "no_payment" && paid.reason !== "still_blocked") {
                    console.log(
                      `[anti-bot-rescue] paid ${raceContextUrl} ${paid.reason}${paid.via ? ` via=${paid.via}` : ""}`,
                    );
                  }
                  return null;
                },
                browserHtml: isDeepCaptureEnabled()
                  ? async () => {
                      browserOpened = true;
                      noteCdpRequired("rendered page HTML for anti-bot rescue");
                      const cap = await captureSession(raceContextUrl, undefined, undefined, intent);
                      if (cap?.html) {
                        return {
                          html: cap.html,
                          via: "browser-render",
                          bytes: cap.html.length,
                        };
                      }
                      return null;
                    }
                  : undefined,
                accept: (html) => {
                  const rendered = buildBloombergDirectDocumentResult(
                    raceContextUrl,
                    html,
                    "text/html",
                    intent,
                  );
                  return rendered.rejected ? null : rendered;
                },
              },
              { deadlineAt: antiBotDeadlineAt },
            );

            if (walked.ok && walked.value) {
              const via = walked.value.via;
              const isBrowser = walked.layer === "browser";
              const trace: ExecutionTrace = {
                trace_id: nanoid(),
                skill_id: isBrowser ? "deep-capture" : "anti-bot-rescue",
                endpoint_id: via,
                started_at: new Date(t0).toISOString(),
                completed_at: new Date().toISOString(),
                success: true,
                decision_trace: [
                  {
                    step: isBrowser ? "deep_capture_layer_walk" : "anti_bot_rescue_layer_walk",
                    via,
                    layer: walked.layer,
                    bytes: walked.value.bytes,
                    cost_paid: walked.costPaid,
                    path: walked.path,
                  },
                ],
              };
              const source = isBrowser ? ("live-capture" as const) : ("direct-document" as const);
              console.log(
                `[${isBrowser ? "deep-capture" : "anti-bot-rescue"}] ${raceContextUrl} via ${via} layer=${walked.layer} cost=${walked.costPaid}: ${walked.value.bytes}B`,
              );
              return {
                result: walked.value.document,
                trace,
                source,
                skill: undefined as any,
                timing: finalize(source, walked.value.document, source, undefined as any, trace),
              };
            }
            if (walked.skippedFailure) {
              console.log(`[anti-bot-rescue] ${raceContextUrl} skipped via failure-cache`);
            }
          } catch (e) {
            console.log(`[anti-bot-rescue] layer walk failed: ${e instanceof Error ? e.message : String(e)}`);
          }
        }
      ...
      // Free residual: when the requested URL is JSON-API-shaped and the origin
      // early-fetch already missed (DNS dead / CF 525), try structural path
      // mirrors from pre-domain-anchor search hits (path transplant onto
      // off-origin candidates). No host allowlist — tryDirectJsonFetch fails
      // closed on non-JSON. Settles as direct-fetch (strict usable), not soft
      // browser-capture / exa highlights.
      if (walkDepth === 0 && urlLooksLikeJsonApi(raceContextUrl) && exaHits.length > 0) {
        const mirrorSettle = await settleJsonApiStructuralMirrors({
          requestedUrl: raceContextUrl,
          hits: exaHits,
          tryFetch: tryDirectJsonFetch,
          timeoutMs: Math.min(8_000, Math.max(2_000, budgetMs - raceOutcome.ms)),
          maxMirrors: 5,
        });
        if (mirrorSettle) {
          const trace: ExecutionTrace = {
          ...
          console.log(
            `[direct-fetch] ${raceContextUrl} free JSON-API structural mirror ${mirrorSettle.mirror_url} — settle over soft shell (ct=${mirrorSettle.content_type})`,
          );
          return {
            result: {
              ...(typeof mirrorSettle.data === "object" && mirrorSettle.data !== null
                ? (mirrorSettle.data as Record<string, unknown>)
                : { data: mirrorSettle.data }),
              mirrored_from: mirrorSettle.mirror_url,
              origin_url: mirrorSettle.origin_url,
              walked_from: "structural-json-api-mirror",
            },
            trace,
            source: "direct-fetch" as const,
            skill: undefined as any,
            timing: finalize("direct-fetch", mirrorSettle.data, "direct-fetch", undefined as any, trace),
          };
        }
      }
      ...
      // Note: free JSON-API structural mirrors already ran above on pre-anchor hits.
      ...
      if (walkDepth === 0 && exaHits.length > 0) {
        ...
        // The probe already knows whether the REQUESTED origin answered, and
        // `ProbeResult.status` is 0 exactly when it did not. Stamping a
        // hard-coded 200 here reported an HTTP status that was never received:
        // measured against `swapi.dev` (no DNS record), unbrowse returned
        // success:true + status_code:200 carrying ANOTHER domain's marketing
        // blurb as the answer. Carry what was observed; fabricate nothing.
        // `observedOriginStatus`, not `w.status`: a status-0 probe fails isValid
        // and never becomes the winner, so reading `w` here could never see it.
        const originUnreachable = observedOriginStatus === 0;
        const exaTrace: ExecutionTrace = {
          ...
          started_at: new Date(t0).toISOString(),
          completed_at: new Date().toISOString(),
          // Web search found POINTERS; it did not fulfil the intent against the
          // requested target. When that target never answered, this is a miss —
          // and `shouldFallbackToBrowser` excludes it from the browser floor,
          // so honesty here does not buy a second identical DNS failure.
          success: !originUnreachable,
          ...(typeof observedOriginStatus === "number" && observedOriginStatus > 0
            ? { status_code: observedOriginStatus }
            : {}),
          // Canonical marker (not the raw message, which varies by layer) so the
          // ONE shared classifier in values/origin-health.ts recognises it.
          // A BARE token, not prose. Every other error the orchestrator emits is a
          // stable snake_case code (auth_required, no_cached_match, payment_required,
          // x402_required); this one shipped as "origin_down (no response from origin)"
          // and was the only compound string, which is exactly why the shared
          // classifier has to substring-match instead of comparing a code.
          ...(originUnreachable ? { error: "origin_down" } : {}),
        };
  ...
  if (!forceCapture && !agentChoseEndpoint) {
    ...
    if (cachedResult) {
      // A browser-derived snapshot is not independent replay evidence. Bypass it while
      // validation is pending (and after drift/staleness); validated routes may reuse
      // the API result snapshot without reopening a browser.
      let lifecycleRequiresReplay = false;
      try {
        const identity = await routeLifecycleIdentityFor(
          cachedResult.skill,
          cachedResult.endpointId ?? cachedResult.trace.endpoint_id,
          queryIntent,
          options,
        );
        if (identity) {
          const record = await getRouteLifecycle(
            identity,
            options?.__route_lifecycle_file ? { file: options.__route_lifecycle_file } : {},
          );
          lifecycleRequiresReplay = record.browser_observations > 0;
        }
      } catch { /* lifecycle evidence is an accelerator, never a resolution dependency */ }
      if (
        lifecycleRequiresReplay ||
  ...
  if (!forceCapture && !agentChoseEndpoint && requestedDomain) {
    ...
    // sameRouteScope, NOT isSameBrandDomain: the bare brand label maps BOTH
    // data.gov.sg and guide.data.gov.sg to "gov", which is precisely how a
    // GitBook developer-guide route got handed a data.gov.sg request. The geo
    // allowance survives; the sibling-subdomain collision does not.
    if (!domainCached && domainKey) {
      for (const [k, v] of domainSkillCache) {
        if (sameRouteScope(k, domainKey)) {
          domainCached = v;
          break;
        }
      }
  ...
  if (!forceCapture && !options?.local_skills_only) {
    ...
    // When a target domain is specified, only accept skills in that ROUTE SCOPE
    // (host-anchored, not eTLD+1 — see the ROUTE IDENTITY block near
    // routeScopeHost). The context URL wins over `requestedDomain` because a
    // caller may have already collapsed the latter to a registrable domain.
    const targetScopeHost = routeScopeHost(context?.url) ?? routeScopeHost(requestedDomain);
    for (const c of candidates) {
      ...
      if (targetScopeHost && !sameRouteScope(skill.domain, targetScopeHost)) continue;
    ...
    if (context?.url) {
      ...
      try {
        ...
        if (directRes.ok) {
          ...
          if (ctSaysJson) {
            ...
            if (!directDocument.rejected) {
              const trace: ExecutionTrace = {
                ...
                started_at: new Date().toISOString(),
                completed_at: new Date().toISOString(),
                success: directDocument.task_ok,
              };
              ...
              // TRY to index. This is the most-travelled direct-document settle,
              // and it was the ONLY one that never asked for API discovery — the
              // other three (literal-URL, probe-winner fast path, curl_cffi
              // bypass) all queue it. Measured consequence on sites100: 53 of 81
              // successful settles came back as scraped HTML, `live-capture`
              // fired ONCE across 100 sites, and cold and warm were identical
              // (53/53) because nothing was ever learned between the two calls.
              // The HTML result still returns immediately; this only asks the
              // background to discover the site's real XHR routes so the NEXT
              // call can replay an internal API instead of re-parsing a page.
              queueBackgroundApiCapture(context.url, queryIntent);
              return {
                ...
                try {
                  ...
                  if (cffi?.html && cffi.html.length > 1024 && cffi.status >= 200 && cffi.status < 400) {
                    ...
                    if (cffiArtifact.endpoint && cffiArtifact.result) {
                      ...
                      queueBackgroundApiCapture(context.url, intent);
          ...
          // Challenge solver: detect vendor from 403/429 body and try
          // the matching solver (cf/px/akamai/kasada) before falling
          // through to the browser ladder.
          let terminalVendor: string | undefined;
          try {
            const blockBody = await directRes.text();
            const blockHeaders: Record<string, string | string[] | undefined> = {};
            directRes.headers.forEach((v: string, k: string) => { blockHeaders[k] = v; });
            const { classifyExecuteFailure } = await import("../execution/index.js");
            const classified = classifyExecuteFailure({ status: directRes.status, body: blockBody, headers: blockHeaders });
            terminalVendor = classified.kind === "vendor_blocked" ? classified.vendor : undefined;
            const { tryChallengeSolver } = await import("./challenge-solver-bridge.js");
            const solverResult = await tryChallengeSolver({
              url: context.url,
              body: blockBody,
              status: directRes.status,
              headers: blockHeaders,
            });
            if (solverResult && solverResult.html.length > 1024) {
              const solverArtifact = buildPageArtifactCapture(context.url, queryIntent, solverResult.html, false);
              if (solverArtifact.endpoint && solverArtifact.result) {
                console.log(`[challenge-solver] ${context.url} ${solverResult.vendor} solved — ${solverResult.html.length} bytes`);
                const solverTrace: ExecutionTrace = {
                  trace_id: nanoid(),
                  skill_id: "challenge-solver",
                  endpoint_id: solverArtifact.endpoint.endpoint_id,
                  started_at: new Date().toISOString(),
                  completed_at: new Date().toISOString(),
                  success: true,
                  decision_trace: [{ step: "challenge_solver_success", vendor: solverResult.vendor, html_bytes: solverResult.html.length }],
                };
                queueBackgroundApiCapture(context.url, queryIntent);
                return {
                  result: artifactResultWithShortlist(solverArtifact, "challenge-solver", context.url),
                  trace: solverTrace,
                  source: "live-capture" as any,
                  skill: undefined as any,
                  timing: finalize("live-capture", solverArtifact.result, "challenge-solver", undefined as any, solverTrace),
                };
              }
            }
          } catch (solverErr) {
            console.log(`[challenge-solver] ${context.url} error: ${solverErr instanceof Error ? solverErr.message : String(solverErr)}`);
          }
          if (terminalVendor) {
            const now = new Date().toISOString();
            const terminalTrace: ExecutionTrace = {
              trace_id: nanoid(),
              skill_id: "direct-fetch",
              endpoint_id: "vendor-blocked",
              started_at: now,
              completed_at: now,
              success: false,
              status_code: directRes.status,
              error: `${terminalVendor}_blocked`,
              decision_trace: [{ step: "vendor_recovery_terminal", vendor: terminalVendor, retryable: false, max_attempts: 2 }],
            };
            return {
              result: {
                error: `${terminalVendor}_blocked`,
                blocker: terminalVendor,
                terminal: true,
                retryable: false,
                recovery_attempts: 2,
                status_code_observed: directRes.status,
                message: `Direct impersonation and the ${terminalVendor} solver were exhausted without target content.`,
              },
              trace: terminalTrace,
              source: "direct-fetch",
              skill: undefined as any,
              timing: finalize("direct-fetch", null, "vendor-blocked", undefined as any, terminalTrace),
            };
          }
        }
      } catch (err) {
    ...
    if (walkDepth === 0 && viable.length === 0 && exaResults?.length) {
      ...
      if (richHit) {
        ...
        // Same fabricated-trace fix as the probe-fallback site above, and this is
        // the one the live repro hit: for `swapi.dev` (no DNS record) THIS branch
        // returned success:true / status_code:200 with swapi.INFO's blurb as the
        // answer. The race winner carries the only observed status; status 0 is
        // the probe's own "the origin never answered" signal.
        const exaProbeStatus = observedOriginStatus;
        const exaOriginUnreachable = exaProbeStatus === 0;
        const exaTrace: ExecutionTrace = {
          trace_id: nanoid(),
          skill_id: "exa-web-search",
          ...
          started_at: new Date().toISOString(),
          completed_at: new Date().toISOString(),
          success: !exaOriginUnreachable,
          ...(typeof exaProbeStatus === "number" && exaProbeStatus > 0
            ? { status_code: exaProbeStatus }
            : {}),
          ...(exaOriginUnreachable ? { error: "origin_down" } : {}),
        };
  ...
  if (!bypassLiveCaptureQueue) {
    ...
    if (existingCapture) {
      ...
      const parityBaseline = waited.parity_baselines;
      timing.execute_ms = 0;
  ...
  let parityBaseline: Record<string, unknown> | undefined;
  ...
  try {
    if (bypassLiveCaptureQueue) {
      ...
      parityBaseline = out.parity_baselines;
    } else {
      const capturePromise = withDomainCaptureLock(captureDomain, async () => {
        ...
        return {
          ...
          result: out.result,
          learned_skill: out.learned_skill,
          parity_baselines: out.parity_baselines,
        };
      });
  ...
  function queuePassivePublishIfExecuted(
    skill: SkillManifest,
    orchestratorResult: OrchestratorResult,
    browserBaselines?: Record<string, unknown>,
  ): void {
    if (!orchestratorResult.trace.success || !orchestratorResult.trace.endpoint_id) return;
    const browserBaseline = browserBaselines?.[orchestratorResult.trace.endpoint_id];
    if (browserBaseline !== undefined) {
      Object.defineProperty(orchestratorResult, "__browser_baseline", {
        value: browserBaseline,
        configurable: true,
        enumerable: false,
      });
    }
    // Legacy capture-time publication has no lifecycle permit and remains fail-closed.
    // Publication is queued only after persistRouteLifecycleOutcome validates replay.
  }
  ...
  if (!learned_skill && !trace.success) {
    ...
    if (!isAuthRequired && context?.url && blockingStatus === 402) {
      // Payment configuration is not per-call consent. Never auto-spend merely because
      // a wallet exists; surface a typed, bypass-immune approval gate instead.
      ...
      return {
        result: {
          ...
          message: "Target returned 402 Payment Required. Explicit payment approval is required before retrying.",
          permission: {
            decision: "ask",
            effect: "payment",
            reason: "target_payment_required",
            bypass_immune: true,
          },
          url: context.url,
        },
    ...
    if (!isAuthRequired && context?.url) {
      ...
      // Challenge solver: when empty_capture occurs (0 traffic in browser),
      // the site is likely behind an anti-bot vendor that blocked the browser
      // before any JS ran. Re-fetch to get the challenge page body, detect the
      // vendor, and try the matching solver before falling through to camoufox.
      if (context?.url && !isAuthRequired) {
        try {
          const { fetchAndTryChallengeSolver } = await import("./challenge-solver-bridge.js");
          const solverResult = await fetchAndTryChallengeSolver({
            url: context.url,
            timeoutMs: 12_000,
          });
          if (solverResult && solverResult.html.length > 1024) {
            const solverArtifact = buildPageArtifactCapture(context.url, queryIntent, solverResult.html, false);
            if (solverArtifact.endpoint && solverArtifact.result) {
              console.log(`[orchestrator] challenge_solver_success: ${solverResult.html.length} bytes via ${solverResult.solver} (${solverResult.vendor})`);
              const solverTrace: ExecutionTrace = {
                trace_id: nanoid(),
                skill_id: captureSkill?.skill_id ?? "challenge-solver",
                endpoint_id: solverArtifact.endpoint.endpoint_id,
                started_at: new Date().toISOString(),
                completed_at: new Date().toISOString(),
                success: true,
              ...
              if (captureSkill) {
                recordRoutingStep("live-capture", captureSkill, solverTrace, solverArtifact.result, {
                  candidateCount: 1,
                  selectedEndpointId: solverArtifact.endpoint.endpoint_id,
                  didStepUnlockNextStep: true,
                  userOverride: false,
                  requiredRecovery: true,
                });
              ...
              queueBackgroundApiCapture(context.url, queryIntent);
              return {
                result: artifactResultWithShortlist(solverArtifact, captureSkill?.skill_id ?? "challenge-solver", context.url),
                trace: solverTrace,
                source: "live-capture",
                skill: captureSkill,
                timing: finalize("live-capture", solverArtifact.result, captureSkill?.skill_id ?? "challenge-solver", captureSkill, solverTrace),
              };
            }
          }
        } catch (solverErr) {
          console.log(`[orchestrator] challenge_solver_error: ${solverErr instanceof Error ? solverErr.message : String(solverErr)}`);
        }
      }

      // Final JS-challenge rung BEFORE the vendor terminal: camoufox stealth Firefox.
      // SSR-fastpath + curl_cffi have no JS engine, so they cannot clear a Cloudflare
      // "Just a moment…" JS challenge — camoufox (fingerprint-spoofed Firefox +
      // solve_cloudflare loop, yoinked from Scrapling's StealthyFetcher) can, and
      // does (verified: stackoverflow → real "Newest Questions" DOM). This rung was
      // previously unreachable — the vendor-block terminal returned before it. Now it
      // runs first: success short-circuits before the terminal, failure falls through
      // to the terminal so the block is still reported honestly. Returns null when the
      // dev venv is absent or the page is still blocked, so it degrades cleanly.
      if (context?.url && !isAuthRequired) {
        try {
      ...
      if (hasVendorBlockSignal) {
        const vendor = (capturedBlockSignals as string[])
          .find((signal) => signal.startsWith("vendor:"))
          ?.slice("vendor:".length) ?? "antibot";
        const terminalTrace: ExecutionTrace = {
          ...trace,
          success: false,
          error: `${vendor}_blocked`,
          completed_at: new Date().toISOString(),
          decision_trace: [
            ...((trace as ExecutionTrace & { decision_trace?: Array<Record<string, unknown>> }).decision_trace ?? []),
            { step: "vendor_recovery_terminal", vendor, retryable: false, max_attempts: 3 },
          ],
        };
        return {
          result: {
            ...
            retryable: false,
            recovery_attempts: 3,
            message: `The bounded ${vendor} recovery ladder was exhausted without target content.`,
          },
          trace: terminalTrace,
          source: "live-capture",
          skill: captureSkill,
          timing: finalize("live-capture", null, captureSkill?.skill_id, captureSkill, terminalTrace),
        };
      }
      // Paid unblocker/CAPTCHA solvers are not part of the canonical agent path.
      // A still-blocked challenge is surfaced for human action below.
    }
  ...
  if (!learned_skill) {
    ...
    if (trace.success && context?.url) {
      try {
        ...
        if (evalTabId) browserOpened = true;
        if (evalTabId) {
          try {
    ...
    if (context?.url) {
      ...
      if (!trace.success || looksBlocked(capturedText, 512)) {
        ...
        // Paid unblocker/CAPTCHA solvers are intentionally not auto-dispatched.
      }
    }
  ...
  if (agentChoseEndpoint && learned_skill) {
    ...
    if (execOut.trace.success && isAcceptableIntentResult(execOut.result, queryIntent)) {
      queuePassivePublishIfExecuted(learned_skill, outcome, parityBaseline);
    }
    return outcome;
  }
  const deferred = await buildDeferralWithAutoExec(
...
function carriesEvaluatedJavascriptTruth(value: unknown, depth = 0): boolean {
  if (!value || typeof value !== "object" || depth > 4) return false;
  const record = value as Record<string, unknown>;
  if (record.truth_mode === "javascript_evaluated") return true;
  for (const key of ["truth", "browser_truth", "result", "data", "artifact", "extraction"]) {
    if (carriesEvaluatedJavascriptTruth(record[key], depth + 1)) return true;
  }
  return false;
}

/** One terminal truth judgment shared by every resolution/capture return path. */
export function enforceEvaluatedJavascriptTruth(intent: string, output: OrchestratorResult): OrchestratorResult {
  if (!output || !output.trace) return output as OrchestratorResult;
  if (!intentRequiresEvaluatedJavascript(intent) || carriesEvaluatedJavascriptTruth(output.result)) return output;
  const error = "javascript_evaluation_required";
  return {
    ...output,
    result: {
      error,
      blocker: error,
      message: "This intent requires evaluated Chromium evidence; static, synthetic, or unproved DOM artifacts cannot satisfy it.",
      required_truth_mode: "javascript_evaluated",
      observed_truth_mode: "static_unevaluated",
    },
    trace: { ...output.trace, success: false, error },
  };
}

function carriesAuthenticatedEvidence(value: unknown, depth = 0): boolean {
  if (!value || typeof value !== "object" || depth > 5) return false;
  if (Array.isArray(value)) return value.some((item) => carriesAuthenticatedEvidence(item, depth + 1));
  const record = value as Record<string, unknown>;
  if (record.auth_ok === true || record.auth_outcome === "authenticated") return true;
  for (const key of ["result", "data", "truth", "browser_truth", "extraction", "diagnostic"]) {
    if (carriesAuthenticatedEvidence(record[key], depth + 1)) return true;
  }
  return false;
}

function contextRequiresAuthenticatedEvidence(url?: string): boolean {
  if (!url) return false;
  try {
...
/** Shared terminal task-truth gate for every orchestrator return path. */
export function enforceIntentResultTruth(
  intent: string,
  output: OrchestratorResult,
  context?: { url?: string; domain?: string },
): OrchestratorResult {
  if (!output || !output.trace) return output as OrchestratorResult;
  if (!output.trace.success) return output;

  if (contextRequiresAuthenticatedEvidence(context?.url) && !carriesAuthenticatedEvidence(output.result)) {
    const error = "authenticated_evidence_required";
    return {
      ...output,
      result: {
        error,
  ...
  const assessed = assessIntentResult(output.result, intent);
  const projected = assessed.projected;
  if (urlHasConcreteResourcePath(context?.url) && !requestExpectsCollection(context?.url, intent) && Array.isArray(projected)) {
    const error = "response_shape_mismatch";
    return {
      ...output,
      result: {
        error,
        message: "A concrete detail URL was answered by a collection route.",
        observed_cardinality: projected.length,
        task_ok: false,
        intent_applied: true,
        intent_fulfilled: false,
      },
  ...
  const requested = requestedResultCount(intent);
  if (requested !== null && Array.isArray(projected)) {
    if (projected.length < requested) {
      const error = "response_shape_mismatch";
      return {
        ...output,
        result: {
          error,
          message: `Requested exactly ${requested} results but only ${projected.length} were returned.`,
    ...
    return { ...output, result: projected.slice(0, requested) };
  }

  return output;
}

/** Project mutable runtime observation into a stateless, public task verdict. */
export function withBrowserOpenedTruth(output: OrchestratorResult): OrchestratorResult {
  if (!output || !output.timing) return output as OrchestratorResult;
  return { ...output, browser_opened: output.timing.browser_opened === true };
}

/**
 * Record what the browser fallback did on the trace's decision ledger, so a
 * fallback that ran and still failed is visible rather than silently folded
 * back into the original verdict.
 */
function withFallbackStep(
  output: OrchestratorResult,
  step: string,
  reason: string,
): OrchestratorResult {
  if (!output || !output.trace) return output as OrchestratorResult;
  const existing = output.trace.decision_trace ?? [];
  return { ...output, trace: { ...output.trace, decision_trace: [...existing, { step, reason }] } };
}

async function resolveAndExecuteWithLifecycleFallback(
  intent: string,
  params: Record<string, unknown> = {},
  context?: { url?: string; domain?: string },
  projection?: ProjectionOptions,
  ...
  const output = await resolveAndExecuteInternal(intent, params, context, projection, options);
  options?.signal?.throwIfAborted();
  const judged = enforceIntentResultTruth(intent, enforceEvaluatedJavascriptTruth(intent, output), context);

  // The index is an accelerator, not a dependency. When the fast paths produced
  // nothing usable — an empty shortlist, or a guessed page of the wrong shape —
  // the floor is to OPEN THE BROWSER, learn the route, and index it so the next
  // call is a fast index hit. Without this the shipped path treats a failed
  // verdict as terminal: against an empty index it guesses a URL, mismatches on
  // shape, and stops, having never looked at the page it was asked about.
  //
  // Bounded by construction: only on a browser-fixable error, only when no
  // browser was opened already, only with a URL, and only ONCE.
  options?.signal?.throwIfAborted();
  const fallback = shouldFallbackToBrowser(judged, context, options?.__walkDepth, intent);
  if (fallback.escalate && isBrowserAccessAvailable()) {
    console.log(`[browser-fallback] ${context?.url} — ${fallback.reason}; opening the browser rather than returning the miss`);
    try {
      // force_capture clears this domain's route/result caches and skips the
      // marketplace, so the retry genuinely goes to the browser instead of
      // replaying the same miss. Capture indexes what it learns on the way
      // back out, which is what makes the NEXT call a fast index hit — the
      // "use search again later once there's something there" half.
      const retried = enforceIntentResultTruth(
        intent,
        enforceEvaluatedJavascriptTruth(
          intent,
          await resolveAndExecuteInternal(intent, params, context, projection, {
            ...(options ?? {}),
            force_capture: true,
          }),
        ),
      ...
      if (!retried) {
        console.log(`[browser-fallback] escalation produced no result for ${context?.url} — treating as exhausted`);
        return withBrowserOpenedTruth(withFallbackStep(judged, "browser_fallback_exhausted", fallback.reason));
      }
      // Only adopt the retry when it actually beat the verdict it was sent to
      // fix; otherwise keep the original, more precise diagnosis.
      if (fallbackImproved(judged, retried)) {
        // Index the rescued route under the OUTER key so the next identical call
        // replays it instead of re-deriving the same miss and re-opening the
        // browser. Without this the fast path keeps losing to its own bad guess
        // and every call pays for a capture — measured: call 2 escalated again.
        // Same move the exa→candidate walk makes with its own result.
        try {
          const scope = options?.client_scope ?? "global";
          const domain = context?.domain ?? registrableHost(context?.url ?? "") ?? null;
          persistWalkedRoute(
            scope,
            scopedCacheKey(scope, buildResolveCacheKey(domain, intent, context?.url)),
            retried,
          );
        } catch {
          /* caching is best-effort — never blocks the rescued answer */
        }
        return withBrowserOpenedTruth(withFallbackStep(retried, "browser_fallback_rescued", fallback.reason));
      }
      // The rescue failed. Remember that this page had nothing for this intent,
      // so the next call does not pay for another full capture to learn the same
      // thing — measured at ~14s against a page with no API calls. TTL-bounded,
      // so a site that ships an API later is still picked up.
      if (context?.url && retried.timing?.browser_opened === true) {
        markBarrenPage(intent, context.url);
        persistBarrenPages();
      }
      // A browser really did open during this call, and the public verdict must
      // say so — reporting browser_opened:false would understate what it did.
      return withBrowserOpenedTruth(
        withFallbackStep(
          carryBrowserOpened(judged, retried),
          "browser_fallback_exhausted",
          fallback.reason,
        ),
      );
    } catch (err) {
      ...
      console.log(`[browser-fallback] escalation failed: ${message}`);
      return withBrowserOpenedTruth(withFallbackStep(judged, "browser_fallback_error", message));
    }
  }

  // A skip we chose deliberately should be visible, not an absence. Only the
  // barren-page case is recorded: the other skips ("succeeded", "no url") are
  // self-evident from the verdict, while "we already looked at this page and it
  // had nothing" explains an otherwise puzzling fast failure.
  if (fallback.reason === "page_known_barren") {
    return withBrowserOpenedTruth(withFallbackStep(judged, "browser_fallback_skipped", fallback.reason));
  }

  return withBrowserOpenedTruth(judged);
}
...
export async function resolveAndExecute(
  ...
  options?.signal?.throwIfAborted();
  const output = await resolveAndExecuteWithLifecycleFallback(intent, params, context, projection, options);
  options?.signal?.throwIfAborted();
  if (!output) return output as OrchestratorResult;
  try {
    await persistRouteLifecycleOutcome(intent, output, options);
  } catch (error) {
    console.warn(`[route-lifecycle] evidence persistence failed: ${error instanceof Error ? error.message : String(error)}`);
  }
  if (output && typeof output === "object") {
    delete (output as OrchestratorResult & { __browser_baseline?: unknown }).__browser_baseline;
  }
  return output;
}
...
export async function getOrCreateBrowserCaptureSkill(): Promise<SkillManifest> {
  ...
  // Route the manifest through the credential boundary, not just its endpoints.
  // `publishSkill` sanitizes `draft.endpoints` only, so `operation_graph[].page_metadata
  // .localStorage` (raw captured localStorage — access tokens live there) has no boundary
  // between it and the public index on any publish path that does not do this.
  //
  // On THIS path the manifest is a hardcoded meta-skill with no endpoints and no graph, so
  // there is nothing to strip today; the call is here so the boundary is a property of the
  // publish site rather than of what this particular literal happens to contain. `skill` is
  // returned unsanitized on purpose — the caller keeps the local manifest.
  // The browser meta-skill is cached locally. Automatic remote publication is
  // forbidden without an independent lifecycle validation permit.
  return skill;
}

/** Reject skills where no endpoint returns structured data or a replayable canonical document route. */
/** True when the skill exposes at least one real internal-API endpoint (not bare page_fetch). */
export function skillCoversRealApi(skill: SkillManifest | undefined | null): boolean {
  if (!skill || !skill.endpoints || skill.endpoints.length === 0) return false;
  return skill.endpoints.some((ep) => !isPageFetchEndpoint(ep as any));
}

export function hasUsableEndpoints(skill: SkillManifest | undefined | null): boolean {
  if (!skill || !skill.endpoints || skill.endpoints.length === 0) return false;
  return skill.endpoints.some((ep) => {
    try {
...
/** Which marketplace skills to spend the (bounded) hydration budget fetching.
 *  Deliberately still registrable-domain wide: this is a FETCH-ORDERING heuristic
 *  over a capped list, not a correctness gate — the route-identity filter that
 *  decides which skill may actually answer runs downstream on the hydrated skill
 *  (`sameRouteScope(skill.domain, targetScopeHost)`). Keeping it broad here costs
 *  at most a wasted fetch; narrowing it would only lose recall. */
