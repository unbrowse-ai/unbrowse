...
export function upsertDagEdgesFromOperationGraph(skill: SkillManifest): void {
  scheduleWrite(skill.skill_id, () => {
    ...
    // Remote graph publication is folded into the permit-bound manifest path.
  });
}

/**
 * Standalone graph publication is intentionally disabled. Graphs leave the
 * machine only inside the sanitized, consented, lifecycle-permitted manifest.
 */
export function publishEdgesToBackend(
  _domain: string,
  _graph: { operations: SkillOperationNode[]; edges: SkillOperationEdge[] },
): never {
