...
export type RunPlanStepKind = "resolve" | "discover" | "execute" | "publish" | "subsequent_direct" | "index" | "browse";
...
export type PublishOutcome = { status?: "published" | "queued" | "skipped" | "failed"; reason?: string };
...
export type RunPlannerDeps = {
  ...
  publish?: (args: { skill_id: string; endpoint_id: string; url: string; intent: string }) => Promise<PublishOutcome>;
...
export async function runPlanner(
  ...
  const settlePublish = async (skillId: string, endpointId: string): Promise<void> => {
    if (!policy.publish_to_marketplace) {
      run_plan.push({ id: "publish", depends_on: ["execute"], step: "publish", status: "skipped", reason: "explicit_opt_out" });
      return;
    }
    run_plan.push({ id: "publish", depends_on: ["execute"], step: "publish", status: "started", mode: "marketplace_default" });
    const step = run_plan[run_plan.length - 1];
    if (!deps.publish) {
      step.status = "queued";
      step.reason = "host_execution_path_owns_publish";
      return;
    }
    try {
      const outcome = await deps.publish({ skill_id: skillId, endpoint_id: endpointId, url: input.url, intent: input.intent });
      step.status = outcome.status ?? "queued";
  ...
  if (isResolveHit(resolved)) {
    ...
    if (skillId && endpoint) {
      ...
      if (endpointIsSafeToAutoExecute(endpoint, hasParams)) {
        ...
        if (ok) await settlePublish(skillId, endpoint.endpoint_id);
        return {
          status: ok ? "ok" : "error",
          result: exec,
          run_plan,
