...
export interface RunResolveRaceResult {
  ...
  ms: number;
  /**
   * What the probe OBSERVED about the requested origin, win or lose.
   *
   * `winner` only carries a probe that passed `isValid` (2xx/3xx with a
   * content-type), so the single most diagnostic outcome — `status: 0`, meaning
   * nothing answered at all — was computed and then dropped on the floor. That
   * is how a host with no DNS record reached the Exa fallback indistinguishable
   * from a healthy site with a boring page, and got reported as a success.
   *
   * Undefined when no probe ran (no URL, or the racer was cancelled in flight).
   */
  probeObserved?: RaceWinnerProbe;
}
...
export async function runResolveRace(args: RunResolveRaceArgs): Promise<RunResolveRaceResult> {
  ...
  // Captured whether or not this racer wins — see `probeObserved`.
  let probeObserved: RaceWinnerProbe | undefined;
  racers.push({
    name: "probe",
    start: async () => {
      const t = Date.now();
