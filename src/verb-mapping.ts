/**
 * Route-shape mapping for every unbrowse tool surface.
 *
 ...
 *     ATTACHES route fields to them via `name` lookup.
 ...
 *   - A future bridge into `~/Projects/route/kinds.ts` will project each
 *     unbrowse verb as a KindSpec, so any LLM dispatching route kinds
 *     can reach the unbrowse surface via the platform.
 *
 * Composes with rules:
 ...
 *     — when a route kind needs structured web data, prefer the unbrowse
 ...
 *
 * Witness  — *after their kind*. Each tool seeds a KindSpec entry of
 ...
 * (covered by `tests/verb-mapping-coverage.test.ts`).
 *
 ...
 *   - **act** (days 2–6) — runtime animation, web actuation, capture:
 ...
 *     review.
 */
export const ROUTE_MAP: Record<string, RouteShape> = {
	...
	// ─── act (days 2–6 — runtime animation, web actuation) ────────────────
	unbrowse_resolve: {
...
// Konmari: routeShape / routeToolNames / ProjectedKindSpec / projectKindSpec
// / allProjectedKindSpecs were future-bridge scaffolding with zero callers (each
// verified by grep before removal) — let go. ROUTE_MAP (read via
// ROUTE_MAP[name] or Object.keys) is the one load-bearing surface; its
// consumer is src/route-seed.ts.
