...
export interface EgressRequest {
  ...
  body?: string | null;
  timeoutMs?: number;
  /** Cancels the whole ladder; an abort never advances to another tier. */
  signal?: AbortSignal;
}
...
export async function egressChain(req: EgressRequest, opts: EgressOpts = {}): Promise<EgressOutcome> {
  ...
  const tierSignal = (): AbortSignal => req.signal
    ? AbortSignal.any([req.signal, AbortSignal.timeout(timeoutMs)])
    : AbortSignal.timeout(timeoutMs);
  const rethrowCancellation = (): void => req.signal?.throwIfAborted();
  ...
  if (opts.allowServer !== false && !authBearing) {
    try {
      ...
      rethrowCancellation();
      /* server tier unavailable — advance */
    }
  }
    ...
    if (proxyUrl) {
      try {
        const { response } = await proxiedFetchOnce(
          req.url,
          { method, headers: req.headers, body: req.body ?? undefined, signal: tierSignal() },
          proxyUrl,
        );
...
export const egressFetch: EgressFetch = async (input, init) => {
  ...
  try {
    ...
    /* throttle / block — fall to the server → proxy tiers */
  }
  ...
  const out = await egressChain({ url, method, headers, body, timeoutMs: 12_000, signal: init?.signal ?? undefined }, { skipLocal: true });
...
export function egressFetchWithBlockCheck(
  ...
  return async (input, init) => {
    ...
    const out = await egressChain(
      { url, method, headers, body, timeoutMs: 12_000, signal: init?.signal ?? undefined },
