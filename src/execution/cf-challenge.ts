/**
 * Cloudflare challenge bundle detection + retry helpers (plan-v13 Tier 2A).
 *
 * This module exposes the surface needed by the executor to solve a
 * Cloudflare JS challenge ("jsd" bundle) and retry the original request
 * with the resulting clearance cookies.
 *
 ...
 *     challenge runtime.
 *   - `solveCfAndRetry` — REAL implementation. Capzy-first path → bundle URL
 *     extraction → bundle fetch → `runBundleReplay` → `cf_clearance`
 *     verification → cookie merge → armed retry fetch.
 */
...
// ---------------------------------------------------------------------------
// solveCfAndRetry — REAL: Capzy-first → bundle-replay → cf_clearance → retry
// ---------------------------------------------------------------------------
