...
export async function startUnbrowseServer(options: StartServerOptions = {}): Promise<RunningUnbrowseServer> {
  ...
  const activeMcpRequests = new WeakSet<object>();
  app.addHook("onRequest", async (req, _reply) => {
    lastActivityTs = Date.now();
    activeRequests.add(req);
    inflightRequestCount++;
    if (String(req.headers["x-unbrowse-mcp-bridged"] ?? "") === "1") {
      activeMcpRequests.add(req);
      inflightMcpBridgedCount++;
    }
  });
  const settleRequest = (req: object) => {
    // Fastify invokes onError and then onResponse for failed requests. Settle
    // each request exactly once or one failure can erase another live request
    // from the counter and let the idle reaper close the server mid-task.
    if (!activeRequests.delete(req)) return;
    inflightRequestCount = Math.max(0, inflightRequestCount - 1);
    if (activeMcpRequests.delete(req)) {
  ...
  if (Number.isFinite(idleMs) && idleMs > 0) {
    idleTimer = setInterval(() => {
      ...
      if (getInflightRequestCount() > 0) return;
