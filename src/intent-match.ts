...
export function projectIntentData(data: unknown, intent?: string): unknown {
  ...
  if (/\b(package|packages|crate|crates)\b/.test(lower)) {
    if (/\bsearch\b/.test(lower)) {
      ...
      const rawCrates = isRecord(unwrapped) && Array.isArray(unwrapped.crates) ? unwrapped.crates : [];
      const rawExactCrate = rawCrates.find((row) => isRecord(row) && row.exact_match === true);
      if (rawExactCrate) return rawExactCrate;
      const exactCrate = normalizeCratesPackageSearchResults(unwrapped).find((row) => row.exact_match === true);
      if (exactCrate) return exactCrate;
...
export function assessIntentResult(data: unknown, intent?: string): {
  ...
  if (isRecord(projected)) {
    ...
    if (/\b(company|companies|organization|organisations|business|person|people|profile|profiles|member|members|user|users|repo|repository|repositories|project|projects|package|packages|crate|crates|doc|docs|documentation|question|questions|recipe|recipes|course|courses|definition|dictionary|meaning|product|products|item|items|stock|stocks|ticker|tickers|quote|quotes|channel|channels|server|servers|guild|guilds|workspace|workspaces)\b/.test(lower)) {
      const classified = classifyRows([projected], intent ?? "");
      return { ...classified, projected };
    }
  }
