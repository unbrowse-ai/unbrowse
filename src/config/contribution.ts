/**
 * Marketplace contribution config (explicit opt-in + review-gated).
 *
 * Lives at `~/.unbrowse/config.json`. Marketplace sharing is fail-closed:
 * `share_pointers` and `auto_review` both default to false. Local capture and
 * indexing continue normally; a user must explicitly opt in via setup, mode,
 * or settings before any automatic path may publish remotely.
 *
...
export interface ContributionConfig {
  contribution: {
     ...
     * descriptions are accepted as-is. Default false: heuristic descriptions
     * never imply consent to publish.
     */
    auto_review: boolean;
    /**
...
const DEFAULT: ContributionConfig = {
  // Remote marketplace publication always requires an explicit opt-in.
  // Passive indexing remains enabled because it only updates the local index.
  contribution: { share_pointers: false, auto_review: false, passive_index: true, set_via: "default" },
