...
//    metadata only — values are never decrypted). That probe only globs
//    NATIVE paths, so on a Flatpak/Snap machine its "no" is blindness,
//    not a verdict — see `probeBlindToSandboxedProfiles` below.
//
...
import { hasSandboxedBrowserProfileRoots } from "./browser-profile-roots.js";
...
export function applyHarvestFallback(
  ...
  opts?: {
    /**
     * The python probe only globs NATIVE roots (`~/.mozilla/firefox`,
     * `~/.config/<browser>`). On a machine whose browsers are Flatpaks or
     * Snaps its "no cookie for this host" is absence of evidence, not evidence
     * of absence — so a definite negative from it is downgraded to a question
     * the harvest (which resolves sandboxed roots) gets to answer.
     */
    probeBlindToSandboxedProfiles?: boolean;
  },
): CookieFreshness {
  const probeIsBlind =
    opts?.probeBlindToSandboxedProfiles === true &&
    !base.fresh &&
    (base.source === "chrome" || base.source === "firefox");
  if (base.source !== "none" && !probeIsBlind) return base;
...
export function hasFreshCookieForHost(
  ...
  opts?: {
    ...
    /**
     * Injectable answer to "does this machine hold Flatpak/Snap browser
     * profiles the python probe cannot see?" (default: the shared resolver).
     */
    sandboxed_profiles_present?: boolean;
  },
): CookieFreshness {
  ...
  // The probe is native-path-only; a Flatpak/Snap-only machine makes its
  // verdict blind rather than authoritative. Resolved once per call — a
  // handful of existsSync calls, and never on darwin/win32.
  const probeBlindToSandboxedProfiles =
    opts?.sandboxed_profiles_present ?? hasSandboxedBrowserProfileRoots();
  ...
  try {
    ...
    // before bouncing them to an interactive login. Same sweep when the probe
    // said "no" from a native-only view of a Flatpak/Snap machine.
    return applyHarvestFallback(base, host, scan, { probeBlindToSandboxedProfiles });
  } catch (err) {
