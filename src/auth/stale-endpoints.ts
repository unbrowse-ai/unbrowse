...
//
// This platform persists the (domain, endpoint_id) pairs that returned
 ...
 *     primitive the platform already calls from /v1/browse/go.
 ...
 *  if they prefer their own browser. the platform doesn't try to
 ...
 *
 *  Inputs are bring-your-own-cookies — the platform doesn't reach
...
export function markCookieExpiry(
  ...
  const expiredAuthLike = cookies.some((c) => {
    ...
    // Only count cookies likely to be the auth platform — http_only OR
