...
const PERIMETERX_TEXT = /perimeterx|px-captcha|press\s*&\s*hold|access to this page has been denied/i;
const LOGGED_OUT_SHELL = /LoggedOutShell|log in to x|sign in to x|create an account|join (?:x|twitter) today/i;
const AUTH_REJECTION_TEXT = /couldn[’']t authenticate you|bad credentials|invalid (?:username|password|credentials)|authentication failed|session (?:has )?expired|you must be (?:signed|logged) in|please (?:sign|log) in|private (?:page|resource).*(?:sign|log) in|access denied.*(?:sign|log) in/i;
const LOGIN_FORM_TEXT = /(?:forgot (?:your )?password|password\s*\n\s*(?:sign|log) in|username or email address\s*\n\s*password)/i;
const AUTHENTICATED_ONLY_TEXT = /(?:password and authentication|ssh and gpg keys|change username|your repositories|account settings|for you\s+following|account information|log out|sign out)/i;
...
/**
 * Whether local browser cookie jars may be imported into kuri/CDP sessions.
 *
 * Default ON. Disable with either alias:
 *   UNBROWSE_IMPORT_BROWSER_COOKIES=0
 *   UNBROWSE_COOKIE_IMPORT=0
 * (also false/no/off). Explicit "1"/"true" on either re-enables when the other is unset.
 */
export function shouldImportBrowserCookies(): boolean {
  const primary = process.env.UNBROWSE_IMPORT_BROWSER_COOKIES?.trim();
  const alias = process.env.UNBROWSE_COOKIE_IMPORT?.trim();
  // Prefer the primary flag when set; otherwise honor the shorter alias.
...
/** Authentication is an observed target outcome, not merely cookie presence. */
export function classifyAuthenticatedPage(input: {
  pageText?: string;
  responseStatus?: number;
  hadPresentedCredentials: boolean;
  credentialsExpired?: boolean;
  currentUrl?: string;
  targetUrl?: string;
  authenticatedEvidence?: boolean;
}): "authenticated" | "auth_required" | "session_expired" | "cloudflare" | "perimeterx" | "unknown" {
  const text = input.pageText ?? "";
  if (CLOUDFLARE_TEXT.test(text)) return "cloudflare";
  if (PERIMETERX_TEXT.test(text)) return "perimeterx";
  let redirectedToLogin = false;
  if (input.currentUrl) {
    try {
      redirectedToLogin = LOGIN_PATHS.test(new URL(input.currentUrl).pathname);
    } catch { /* malformed observation cannot prove auth */ }
  }
  const rejected = input.responseStatus === 401
    || input.responseStatus === 403
    || LOGGED_OUT_SHELL.test(text)
    || AUTH_REJECTION_TEXT.test(text)
    || LOGIN_FORM_TEXT.test(text)
    || redirectedToLogin;
  if (rejected) return input.hadPresentedCredentials || input.credentialsExpired ? "session_expired" : "auth_required";
  const independentlyAuthenticated = input.authenticatedEvidence === true || AUTHENTICATED_ONLY_TEXT.test(text);
  return independentlyAuthenticated ? "authenticated" : "unknown";
}
...
export function assessInteractiveLoginState(input: {
  ...
  if (input.pageText && PERIMETERX_TEXT.test(input.pageText)) return { status: "blocked", reason: "perimeterx" };
  if (input.pageText && (LOGGED_OUT_SHELL.test(input.pageText) || AUTH_REJECTION_TEXT.test(input.pageText) || LOGIN_FORM_TEXT.test(input.pageText))) {
    return { status: "pending", reason: "auth_required" };
  }
  ...
  try {
    ...
    while (Date.now() - startTime < effectiveTimeout) {
      ...
      try {
        ...
        // Cookie presence is not page authentication. Always inspect the rendered
        // target so a logged-out shell or rejection overrides a stale auth cookie.
        const pageText = await kuri.getText(tabId).catch(() => "");
