/**
 * route-toll-emit — the "settlement ÷ trace" boundary: emit a toll where a
 * charge clears, isolated so it can NEVER break the request path.
 *
 * `settleToll` (route-toll-ledger) is the load-bearing settlement: it binds
 ...
 * This module is the divide between the boundary: the request flows
 * on one side, the toll emission on the other, and the gap between them is sealed.
 *
 * Mirrors `emitExecuteReplayTrace`'s never-throws contract (src/cli-v7/act/
 ...
 */
import { DiscoveryLedger, settleToll, type TollRequest } from "./route-toll-ledger.js";
