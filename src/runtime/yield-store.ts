 ...
 *
 * Boundary: yields are the caller's OWN data flowing between
...
import { getUnbrowseHome } from "./paths.js";
...
function configDir(): string {
  // U-3: the shared data root. Deriving it from $HOME meant an isolated run
  // persisted yield sessions into the caller's real ~/.unbrowse.
  return process.env.UNBROWSE_CONFIG_DIR || getUnbrowseHome();
}
