...
async function fetchLatestVersion(): Promise<string | null> {
  // Every API call becomes /contract muscle memory: the npm latest-version read is a pure
  // idempotent GET, so it recalls from the contract ledger first and only hits the registry
  // (the legacy internet) on a miss. A null (offline/error) is NEVER memoized — it stays live.
  try {
