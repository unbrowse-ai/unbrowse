...
// Binary-path resolution only — deliberately NOT the browser client. This file
// used to import all three from `kuri/client.ts`, dragging 2,543 lines of
// browser control into setup just to locate a file on disk.
import { findKuriBinary, getKuriBinaryCandidates, getKuriSourceCandidates } from "../kuri/resolve-paths.js";
...
export type SetupReport = {
  ...
  // server is attached by the caller (cli.ts) after runSetup.
  // started:false + in_process:true for the in-proc surface that setup actually
  // brings up (no daemon). started:true is only truthful for `unbrowse serve`.
  server?: {
    started: boolean;
    skipped?: boolean;
    in_process?: boolean;
...
function writeOpenCodeCommand(scope: SetupScope, cwd: string): SetupReport["opencode"] {
  ...
  return {
    ...
    action,
    scope: resolvedScope,
    // command_file kept for callers that read it (cli.ts info line); the
    // SetupReport type is widened in this file to carry it without tsc noise.
    command_file: commandFile,
  } as SetupReport["opencode"] & { command_file?: string };
}

export async function ensureBrowserEngineInstalled(): Promise<SetupReport["browser_engine"]> {
  // `findKuriBinary()` reports a miss as null (a returned string is always a
  // path that existed at resolution time — see src/kuri/client.ts). A miss must
  // NOT abort setup: it is the ordinary state of a kuri-less machine, and the
  // whole point of what follows is to build one from source. So null falls
  // through to source discovery exactly as an absent path used to, and only the
  // "no source either" branch reports failure.
  ...
  if (!sourceDir) {
    return {
      ...
      action: "failed",
      message: binary === null
        ? `Kuri binary not found. Checked ${getKuriBinaryCandidates().join(", ")}`
        : `Kuri binary not found. Checked ${binary}`,
    };
  }
  ...
  try {
    ...
    if (builtBinary !== null && existsSync(builtBinary)) {
      return {
        installed: true,
        action: "installed",
    ...
    return {
      installed: false,
      action: "failed",
      message: builtBinary === null
        ? `Kuri build completed but no binary was created in ${getKuriBinaryCandidates().join(", ")}`
        : `Kuri build completed but ${builtBinary} was not created`,
    };
  } catch (error) {
...
export async function runSetup(options?: {
  ...
  if (flexEscrow.skipped || flexSessionKey.skipped) {
    console.log("");
    console.log("[unbrowse setup] ✓ Ready — runtime + Agent Skill installed.");
    console.log("  Wallet is optional (sponsored usage works without one).");
    console.log("  Pair later to earn: npx @crossmint/lobster-cli setup");
    console.log("  Dashboard:         unbrowse dashboard");
    // Concrete first command is printed by cmdSetup (printSetupNextSteps).
  }
