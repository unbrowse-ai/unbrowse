/**
 * route-seed — the minimal internet primitive (UNBROWSE.md, the 10 atoms).
 *
 * The whole of unbrowse, reduced to "only the required level of primitives to
 * expose the internet to an agent." Composes the existing route surface
 * (ROUTE_MAP is the load-bearing map; this is its consumer) rather than
 ...
 *   node   → RouteNode (six interrogatives)
 *   verb   → verbOfKind / verbOfTool (build/act/eval, the three-verb river)
 ...
 *
 * Mechanism-compatible with backend/src/services/route-core.ts: insertion-order
 * JSON → sha256 → `sha256:` pointer (NOT sorted-key — would break route-binary
...
import { ROUTE_MAP, type Verb } from "./verb-mapping.js";
...
const VERB_BY_BASE: Record<string, Verb> = {
	build: "build", // declare / commit a persistent claim (Father)
	meter: "build", // toll: commit a billable claim — the 402 event (Father)
	actuate: "act", // route / act on the web (Spirit)
	observe: "eval", // read-only query (Son)
};

/** verb of a `base:action` kind. Unknown base → act (the routing default). */
export function verbOfKind(kind: string): Verb {
	return VERB_BY_BASE[kind.split(":")[0] ?? ""] ?? "act";
}

/** verb of a declared MCP tool, straight from the one map. */
export function verbOfTool(toolName: string): Verb | undefined {
	return ROUTE_MAP[toolName]?.verb;
}
...
function canonicalBody(node: RouteNode): string {
...
/** MEMOIZE: the `sha256:`-prefixed content address of a node (route receipt ptr). */
export async function receiptPtr(node: RouteNode): Promise<string> {
...
export async function sealNode(
	...
	const sealed: RouteNode = { ...node, identity };
 ...
 */
export async function verifyNode(node: RouteNode): Promise<boolean> {
	try {
...
/**
 * Pick the minimal fetch rung — Chromium is the LAST resort, not the platform:
...
// The 402 event as one route node of kind `meter:toll`, sealable through the
...
/**
 * tollNode — the 402 event as a sealable route node. `who` = the paying agent;
...
export function tollNode(
	...
	return {
		...
		identity: agent,
		witness: " — the laborer is worthy of his wages",
	};
}
