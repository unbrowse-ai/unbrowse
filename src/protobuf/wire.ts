...
export interface ProtobufRequestContract {
  body_base64: string;
  bindings: Array<{ param: string; field_number: number; captured_value: string }>;
  unsupported_reason?: string;
}

type WireSegment = { fieldNumber: number; wireType: number; start: number; valueStart: number; end: number; text?: string };

function requestBytes(body: string): Uint8Array | null {
  const trimmed = body.trim();
  const encoded = trimmed.match(/^(?:unbrowse:)?base64[:,](.+)$/i)?.[1];
  if (!encoded) return null;
  const bytes = decodeBase64(encoded);
  return bytes.length ? bytes : null;
}

function topLevelSegments(bytes: Uint8Array): WireSegment[] | null {
  ...
  try {
    while (offset < bytes.length) {
      ...
      const wireType = Number(tag.value) & 7;
      if (fieldNumber < 1) return null;
      let valueStart = offset;
      let text: string | undefined;
      if (wireType === 0) offset = readVarint(bytes, offset, bytes.length).offset;
      else if (wireType === 1) offset += 8;
      else if (wireType === 5) offset += 4;
      else if (wireType === 2) {
        const len = readVarint(bytes, offset, bytes.length);
        const size = Number(len.value);
        valueStart = len.offset;
        offset = valueStart + size;
        if (offset > bytes.length) return null;
        text = decodeUtf8IfPrintable(bytes.slice(valueStart, offset)) ?? undefined;
      } else return null;
      if (offset > bytes.length) return null;
      out.push({ fieldNumber, wireType, start, valueStart, end: offset, text });
    }
    return out;
  } catch { return null; }
}

/** Infer only bindings directly witnessed by captured scalar values. */
export function inferProtobufRequestContract(body: string, evidence: Record<string, string>): ProtobufRequestContract | null {
  const bytes = requestBytes(body);
  if (!bytes) return null;
  const segments = topLevelSegments(bytes);
  if (!segments) return { body_base64: Buffer.from(bytes).toString("base64"), bindings: [], unsupported_reason: "invalid_or_unsupported_wire_shape" };
  const bindings: ProtobufRequestContract["bindings"] = [];
  for (const [param, capturedValue] of Object.entries(evidence)) {
    if (!capturedValue) continue;
    const matches = segments.filter((segment) => segment.wireType === 2 && segment.text === capturedValue);
    if (matches.length === 1) bindings.push({ param, field_number: matches[0].fieldNumber, captured_value: capturedValue });
    else if (matches.length > 1) return { body_base64: Buffer.from(bytes).toString("base64"), bindings: [], unsupported_reason: `ambiguous_field:${param}` };
  }
  return {
    body_base64: Buffer.from(bytes).toString("base64"),
    bindings,
    ...(bindings.length === 0 ? { unsupported_reason: "no_evidence_backed_bindings" } : {}),
  };
}

export function encodeProtobufRequest(contract: ProtobufRequestContract, params: Record<string, unknown>): Uint8Array {
  if (contract.unsupported_reason || contract.bindings.length === 0) throw new Error(`protobuf_parameterization_unsupported:${contract.unsupported_reason ?? "no_bindings"}`);
  const bytes = new Uint8Array(Buffer.from(contract.body_base64, "base64"));
  const segments = topLevelSegments(bytes);
  if (!segments) throw new Error("protobuf_parameterization_unsupported:invalid_wire_shape");
  const replacements = new Map<number, Uint8Array>();
  for (const binding of contract.bindings) {
    if (params[binding.param] == null) continue;
    const candidates = segments.filter((segment) => segment.fieldNumber === binding.field_number && segment.text === binding.captured_value);
    if (candidates.length !== 1) throw new Error(`protobuf_parameterization_unsupported:binding_drift:${binding.param}`);
    replacements.set(candidates[0].start, new TextEncoder().encode(String(params[binding.param])));
  }
  const chunks: Uint8Array[] = [];
  for (const segment of segments) {
    const replacement = replacements.get(segment.start);
    if (!replacement) chunks.push(bytes.slice(segment.start, segment.end));
    else chunks.push(new Uint8Array([segment.fieldNumber << 3 | 2, ...writeVarint(replacement.length), ...replacement]));
  }
...
function writeVarint(value: number): number[] {
  const out: number[] = [];
  let next = value >>> 0;
  while (next >= 128) { out.push((next & 127) | 128); next >>>= 7; }
  out.push(next);
  return out;
}
