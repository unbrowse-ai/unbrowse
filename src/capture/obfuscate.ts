 ...
 *
 * Wallet-bound credential proofs are the provability upgrade (prove the obfuscated credential is bound
 * to the wallet without revealing it). This primitive is the redaction +
 * commitment layer the binding proofs strengthen; it is correct on its own.
 */
...
export interface ObfuscateOpts {
   ...
   *  (header values, body leaves, query params). The binding pass mints a binding
   *  over precisely these, so its sha16 keys match the `[bound:<sha16>]`
   *  placeholders. Pass `new Set()`, then `bindKnownSecrets([...sink], wallet)`. */
