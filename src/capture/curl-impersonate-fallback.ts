...
/** Directory holding this module, or undefined where import.meta is unavailable. */
...
/**
 * Locate a python helper that ships beside the runtime.
 *
 * The old code resolved this against `process.cwd()` alone, which meant the
 * rung only ever worked when the CLI happened to be invoked from the repo root.
 * In an npm install it resolved to a path that does not exist, so every call
 * returned null and the entire impersonation ladder was dead without saying so.
 * Search order puts the explicit overrides first, then the packaged location
 * (beside the bundled runtime), then the repo layout, then the historical
 * cwd-relative path so existing dev invocations keep working.
 */
function resolveHelperScript(fileName: string, envVar: string): string | null {
  const here = moduleDir();
  const candidates = [
    process.env[envVar],
    here ? resolve(here, fileName) : undefined,
    here ? resolve(here, "scripts", fileName) : undefined,
    here ? resolve(here, "..", "..", "scripts", fileName) : undefined,
    here ? resolve(here, "..", "..", "..", "scripts", fileName) : undefined,
...
/**
 * A python interpreter that can actually `import curl_cffi`, or null.
 *
 * Probing matters because "python3 exists" and "python3 has curl_cffi" are
 * different facts, and only the second one makes this rung work. The result
 * (including the negative) is cached for the process, and the negative is
 * logged ONCE with the command that fixes it — the previous behaviour was a
 * silent null on every call, which is indistinguishable from "the site blocked
 * us" and hid a dead ladder rung.
 */
let interpreterProbe: Promise<string | null> | null = null;

function probeInterpreter(bin: string): Promise<boolean> {
...
export function resolveCurlImpersonatePython(): Promise<string | null> {
  if (interpreterProbe) return interpreterProbe;
  interpreterProbe = (async () => {
    const candidates = [
      process.env.UNBROWSE_PYTHON,
      // A venv provisioned next to the helper wins over the system interpreter,
      // because that is where `unbrowse setup` can install curl_cffi without
      // touching a distro-managed python.
      moduleDir() ? resolve(moduleDir()!, ".venv", "bin", "python") : undefined,
      resolve(process.cwd(), "scripts", ".curl-impersonate-venv", "bin", "python"),
      "python3",
      "python",
    ].filter((c): c is string => typeof c === "string" && c.length > 0);
    for (const bin of candidates) {
      if (await probeInterpreter(bin)) return bin;
    }
    log(
      "curl-impersonate",
      "TLS-impersonation rung unavailable: no python interpreter with curl_cffi " +
        "(tried UNBROWSE_PYTHON, a local venv, python3, python). Anti-bot hosts that " +
        "need a Chrome TLS fingerprint will fall through to the browser ladder. " +
        "Fix: pip install curl_cffi (or set UNBROWSE_PYTHON to an interpreter that has it).",
    );
    return null;
  })();
...
/** Test seam: forget the cached interpreter probe. */
export function resetCurlImpersonateProbe(): void {
  interpreterProbe = null;
  rungUnavailableLogged = false;
}

/** One line per process, so a dead rung is visible without spamming per fetch. */
let rungUnavailableLogged = false;
function logRungUnavailableOnce(reason: string): void {
  if (rungUnavailableLogged) return;
  rungUnavailableLogged = true;
  log("curl-impersonate", `TLS-impersonation rung unavailable: ${reason}`);
}
...
export async function tryCurlImpersonateFetch(opts: CurlCffiOptions): Promise<CurlCffiResult | null> {
  const scriptPath =
    opts.scriptPath ??
    resolveHelperScript("curl-impersonate-fetch.py", "UNBROWSE_CURL_IMPERSONATE_SCRIPT");
  if (!scriptPath) {
    logRungUnavailableOnce(
      "helper script curl-impersonate-fetch.py not found beside the runtime, in the repo, or under $PWD/scripts " +
        "(override with UNBROWSE_CURL_IMPERSONATE_SCRIPT)",
    );
    return null;
  }
  const python = await resolveCurlImpersonatePython();
  ...
  return await new Promise<CurlCffiResult | null>((resolveP) => {
    ...
    const childEnv = opts.forceDirect
      ...
      : process.env;
    const child = spawn(python, args, { stdio: ["ignore", "pipe", "pipe"], env: childEnv });
    let stdout = "";
    const timer = setTimeout(() => {
      killed = true;
      try { child.kill("SIGKILL"); } catch { /* best-effort */ }
...
export async function tryCamoufoxFetch(opts: CurlCffiOptions): Promise<CurlCffiResult | null> {
  // Same packaging bug as the curl_cffi rung: a cwd-relative helper path only
  // resolves when the CLI is invoked from the repo root. The camoufox venv stays
  // cwd/dev-scoped (it is a heavy optional install, never packaged), but it is
  // now overridable so a provisioned venv elsewhere is usable.
  // Intentionally NOT resolved via resolveHelperScript/moduleDir: the venv is
  // ~350 MB with compiled Firefox, dev-only, never shipped in the npm package.
  // UNBROWSE_CAMOUFOX_PYTHON overrides it for provisioned installs elsewhere.
  const scriptPath =
    opts.scriptPath ?? resolveHelperScript("camoufox-fetch.py", "UNBROWSE_CAMOUFOX_SCRIPT");
  if (!scriptPath) return null;
  const pythonPath =
    process.env.UNBROWSE_CAMOUFOX_PYTHON ??
    resolve(process.cwd(), "scripts/.camoufox-venv/bin/python");
  // Honest unavailable: no interpreter at the expected path → null, not throw.
  // Only gate on file existence when the value looks like a path; a bare
  // binary name (e.g. "python3") is resolved via PATH at spawn time.
  if (pythonPath.includes("/") && !existsSync(pythonPath)) return null;
