...
// ---------------------------------------------------------------------------
// Warm-tab pool: persistent authenticated browser tabs per origin.
//
// Problem: executeInBrowser opens a fresh tab on every call, navigates to the
// origin cold, and resets/releases at the end. For sites whose auth or token
// signing logic lives in a JS wrapper on window (Twitter ads api, Shopify
// admin, LinkedIn voyager), a cold tab misses the runtime context entirely —
// the tokens are only computed once the page hydrates, and only for requests
// the page itself fires.
//
// Solution: keep one authenticated tab alive per origin. On demand, execute
// target requests via window.fetch() INSIDE that tab so the call inherits all
// of the site's runtime state: cookies, service-worker interceptors,
// fetch-level monkey-patches, CSRF headers injected by the site's HTTP
// client, etc. The tab is reused across calls until TTL expiry or a dead-tab
// error.
//
// This closes the gap between pure server-fetch (fast but misses runtime
// context) and triggerAndIntercept (slow and only works for requests the
// page fires on navigation).

const WARM_TAB_TTL_MS = 10 * 60 * 1000; // 10 minutes
interface WarmTabEntry { tabId: string; openedAt: number; lastUsed: number }
const warmTabsByOrigin = new Map<string, WarmTabEntry>();
const warmTabAcquiring = new Map<string, Promise<string>>();

/**
 * Acquire a warm authenticated tab for the given origin. Returns a tabId
 * that the caller may use for `kuri.executeInPageFetch`, `kuri.evaluate`,
 * etc. Callers MUST NOT call `resetTab` / `releaseTabSlot` on it — the pool
 * owns the lifecycle.
 */
export async function acquireWarmTab(
  origin: string,
  cookies?: Array<{ name: string; value: string; domain: string; path?: string; secure?: boolean; httpOnly?: boolean; sameSite?: string; expires?: number }>,
): Promise<string> {
  if (!isBrowserAccessAvailable()) {
  ...
  const now = Date.now();
  const existing = warmTabsByOrigin.get(origin);
  if (existing && now - existing.openedAt < WARM_TAB_TTL_MS) {
    existing.lastUsed = now;
    return existing.tabId;
  }

  // Dedupe concurrent acquires for the same origin
  const inflight = warmTabAcquiring.get(origin);
  if (inflight) return inflight;

  const p = (async () => {
    await kuri.start();
    await kuri.discoverTabs();
    const tabId = await openFreshCaptureTab();
    activeTabRegistry.add(tabId);
    ...
    try {
      ...
      await kuri.navigate(tabId, origin);
      await waitForReadyState(tabId, 5000);
      warmTabsByOrigin.set(origin, { tabId, openedAt: Date.now(), lastUsed: Date.now() });
      return tabId;
    } catch (err) {
      // Failed to warm up — release the slot we acquired
      try { await resetTab(tabId); } catch { /* ignore */ }
      releaseTabSlot(tabId);
      activeTabRegistry.delete(tabId);
      throw err;
    }
  })();

  warmTabAcquiring.set(origin, p);
  try {
    return await p;
  } finally {
    warmTabAcquiring.delete(origin);
  }
}

/**
 * Execute a request inside a warm tab for the given URL's origin. Uses
 * `window.fetch` inside the tab so the request inherits the site's full
 * runtime context (cookies, service worker, fetch interceptors, CSRF).
 *
 * On dead-tab errors (tab crashed or was closed externally), the pool entry
 * is evicted and a single retry with a fresh warm tab is attempted.
 */
export async function executeInWarmTab(
  url: string,
  method: string,
  ...
  const attempt = async (): Promise<{ status: number; data: unknown; trace_id: string }> => {
    const tabId = await acquireWarmTab(origin, cookies);
    const result = await kuri.executeInPageFetch(tabId, url, method, headers, body);
    return { ...result, trace_id: nanoid() };
  };
  ...
  try {
    ...
    // Dead-tab signals: evict and retry once with a fresh warm tab
    const message = err instanceof Error ? err.message : String(err);
    const isDeadTab = /tab.*closed|target closed|session.*destroyed|cdp.*connection|websocket.*closed/i.test(message);
    if (!isDeadTab) throw err;

    log("warm-tab", `dead tab for ${origin}: ${message} — evicting and retrying`);
    evictWarmTab(origin);
    return await attempt();
  }
}

/** Evict a cached warm tab for an origin. Call on 401/403 so the next acquire
 *  re-navigates with fresh auth. */
export function evictWarmTab(origin: string): void {
  const entry = warmTabsByOrigin.get(origin);
  if (!entry) return;
  warmTabsByOrigin.delete(origin);
  void (async () => {
    try { await resetTab(entry.tabId); } catch { /* ignore */ }
    try { releaseTabSlot(entry.tabId); } catch { /* ignore */ }
    activeTabRegistry.delete(entry.tabId);
  })();
}

/** Reap expired warm tabs. Called opportunistically by execute to keep the
 *  pool bounded. Safe to call at any time. */
export function reapWarmTabs(): void {
  const now = Date.now();
  for (const [origin, entry] of warmTabsByOrigin) {
    if (now - entry.openedAt > WARM_TAB_TTL_MS) {
      evictWarmTab(origin);
    }
  }
}

/** Testing hook — close all warm tabs. */
export function _clearWarmTabsForTests(): void {
  for (const origin of [...warmTabsByOrigin.keys()]) evictWarmTab(origin);
}
