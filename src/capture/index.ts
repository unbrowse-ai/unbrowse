...
import { hasAuthenticatedSession } from "../auth/browser-cookies.js";
...
import { browserRecovery, DEFAULT_BROWSER_ACCESS } from "../runtime/browser-access.js";
import { planSafeExploration, type ExplorationAction } from "./exploration.js";
...
// Leave teardown/envelope headroom inside the public 90s operation budget.
//
// Overridable because the default is sized for an anonymous page load, and an
// AUTHENTICATED capture of a heavy SPA is strictly more work — x.com/messages
// signed in loads a real inbox and blew the 75s budget mid-`extractCookies`,
// where the same capture signed out fit easily. Same env-override shape as
// UNBROWSE_LIVE_CAPTURE_TIMEOUT_MS in the orchestrator.
const CAPTURE_TIMEOUT_MS = Number(process.env.UNBROWSE_CAPTURE_TIMEOUT_MS) || 75_000;
...
export interface CaptureResult {
  ...
  domain: string;
  /**
   * True when the capture deadline fired but the traffic collected up to that
   * point was kept and returned rather than discarded. The result is usable —
   * just truncated — so callers can index it and, if they want more, re-run
   * with a larger `UNBROWSE_CAPTURE_TIMEOUT_MS`.
   */
  partial?: boolean;
  ...
  zk_proofs?: Map<string, import("../types/proof.js").ProvenanceProof>;
  ...
  exploration?: {
    ...
    action_types: string[];
    causal_api_deltas: number;
    query_hooks: QueryHookEvent[];
...
function requestCorrelationKey(request: Pick<RawRequest, "method" | "url" | "request_body">): string {
  return `${String(request.method).toUpperCase()} ${request.url}\n${request.request_body ?? ""}`;
}

export function applyActionCorrelations(
  requests: RawRequest[],
  correlations: Map<string, { step: number; action: string; ref: string }>,
): void {
  for (const request of requests) {
    const provenance = correlations.get(requestCorrelationKey(request));
    if (!provenance) continue;
    request.triggered_by_step = provenance.step;
    request.triggered_by_action = provenance.action;
    request.triggered_by_ref = provenance.ref;
  }
}
...
/** Convert untrusted extension timestamps without allowing one malformed
 * observer row to discard an otherwise valid HAR capture. */
export function normalizeCaptureTimestamp(value: unknown, fallback = new Date().toISOString()): string {
  let candidate: unknown = value;
  if (typeof candidate === "number" && Number.isFinite(candidate) && candidate > 0 && candidate < 10_000_000_000) {
    candidate *= 1000; // extension APIs may report Unix seconds
  }
  const date = new Date(candidate as string | number | Date);
  return Number.isFinite(date.getTime()) ? date.toISOString() : fallback;
}

/** CDP Network.getResponseBody may return textual JSON as base64. Decode at
 * ingestion so every downstream capture/reveng path sees the wire body. */
export function decodeCdpResponseBody(result: { body?: unknown; base64Encoded?: unknown } | null | undefined): string | undefined {
  if (typeof result?.body !== "string" || result.body.length === 0) return undefined;
  if (result.base64Encoded !== true) return result.body;
  try {
...
export function isBlockedAppShell(html?: string): boolean {
  ...
  return (
    ...
    || /LoggedOutShell|log in to x|sign in to x/i.test(html)
    || /perimeterx|px-captcha|press\s*&\s*hold/i.test(html)
  );
}

export type CaptureBlocker = "cloudflare" | "perimeterx" | "auth_required" | "empty_capture";

/** Classify rendered failures without turning WAFs or logged-out shells into auth guesses. */
export function classifyCaptureBlocker(input: { html?: string; requestCount: number; responseBodyCount: number }): CaptureBlocker | undefined {
  const html = input.html ?? "";
  if (/Cloudflare|cf\.errors\.css|cf-error-details|just a moment/i.test(html)) return "cloudflare";
  if (/perimeterx|px-captcha|press\s*&\s*hold/i.test(html)) return "perimeterx";
  if (/LoggedOutShell|log in to x|sign in to x/i.test(html)) return "auth_required";
  if (input.requestCount === 0 && input.responseBodyCount === 0) return "empty_capture";
  return undefined;
}

export interface CaptureRecoveryPlan {
  blocker: CaptureBlocker | "chromium_unavailable" | "chromium_cdp_unavailable" | "kuri_sandbox_unavailable";
  retryable: boolean;
  max_attempts: 0 | 1;
  deadline_ms: number;
  next_step?: string;
...
/**
 * Terminal, bounded recovery policy. Vendor challenges are observations, not a
 * promise that stealth or a retry can bypass them. A single empty-capture retry
 * is allowed only when the verified CDP runtime is actually available.
 */
export function captureRecoveryPlan(
  blocker: CaptureRecoveryPlan["blocker"],
  ...
  const boundedDeadline = Math.max(1_000, Math.min(deadlineMs, CAPTURE_TIMEOUT_MS));
  const recovery = browserRecovery(blocker, facts);
  if (blocker === "cloudflare" || blocker === "perimeterx") {
    return { blocker, retryable: false, max_attempts: 0, deadline_ms: boundedDeadline };
  }
  return {
    blocker,
    retryable: recovery.retryable,
    max_attempts: recovery.retryable ? 1 : 0,
    deadline_ms: boundedDeadline,
...
/** Feed/timeline-shaped collection hints. An intent about posts/tweets/threads
 *  wants many items even when the phrasing lacks a list keyword ("top posts
 *  with text and metrics"). Derived through the shared hint interface
 *  (deriveIntentHints), so zero per-host entries — the standing rule. */
const FEED_SHAPE_HINTS = new Set([
  "tweet", "timeline", "status", "trend", "explore", "topic", "thread", "feed",
]);

/** Capture-stimulus decision (scroll + explore). Lazy listing XHRs only fire
 *  on scroll or a driven action, so stimulate whenever the intent or the
 *  context-URL path is collection-shaped: a list/search intent, a list-shaped
 *  URL path, or a feed/timeline-shaped intent (a feed is a net that wants many
 *  fish — the shared cardinality signal, NOT a `search|explore` URL allowlist
 *  or an enumerated entity-word list; CLAUDE.md "generalize — never a hard
 *  filter"). Pure + exported so the trigger is deterministically witnessable
 *  without kuri. */
export function shouldScrollStimulate(captureUrl?: string, intent?: string): boolean {
  if (isListLikeIntent(intent)) return true;
  if (urlPathLooksListLike(captureUrl)) return true;
  return deriveIntentHints(captureUrl, intent).some((hint) => FEED_SHAPE_HINTS.has(hint));
}
...
export function mergePassiveCaptureData(
  ...
  for (const entry of extensionEntries) {
    ...
    seen.set(dedupKey, {
      ...
      timestamp: normalizeCaptureTimestamp(entry.timestamp),
    });
  }
...
async function waitForContentReady(
  ...
  try {
    ...
    if (hasCf) {
      ...
      const cleared = await kuri.waitForCloudflare(tabId);
      if (cleared) {
  ...
  if (captureUrl && responseBodies && shouldScrollStimulate(captureUrl, intent)) {
    try {
      // Lazy listing XHRs fire as the page loads each successive window of a
      // virtualized feed (X ProfileTimeline, infinite-scroll grids). A single
      // 2-viewport jump settles above the fold — the first page of items is
      // often SSR'd straight into the HTML — and never reaches the load-more
      // sentinel, so page-2+ API calls stay unobserved. Drive to the bottom in
      // bounded steps, letting the app render and fire its XHRs between steps;
      // stop after two consecutive no-growth rounds (feed exhausted). Then
      // return to the top (newest-first feeds load the next page up top).
      const SCROLL_HEIGHT_JS =
      ...
      for (let step = 0; step < MAX_SCROLL_STEPS; step++) {
        const beforeHeight = await kuri.evaluate(tabId, SCROLL_HEIGHT_JS).catch(() => 0);
        await kuri.evaluate(tabId, "window.scrollTo(0, document.body.scrollHeight)");
        await new Promise((r) => setTimeout(r, 800));
        const afterHeight = await kuri.evaluate(tabId, SCROLL_HEIGHT_JS).catch(() => 0);
...
export async function captureSession(
  ...
  if (options?.signal?.aborted) {
    releaseTabSlot("cancelled-before-tab");
    options.signal.throwIfAborted();
  }
  ...
  // Set when the capture deadline fired but real traffic was already collected
  // and returned anyway (see the salvage branch). Surfaced on the result so a
  // caller can tell a complete capture from a truncated-but-usable one.
  let capturePartial = false;
  const abortController = new AbortController();
  const abortFromCaller = () => {
    abortController.abort(options?.signal?.reason ?? new Error("capture_cancelled"));
  ...
  try {
    ...
    const cdpRequestMap = new Map<string, { method: string; url: string; headers: Array<{name: string; value: string}>; postData?: string; timestamp: string }>();
    ...
    const cdpPendingPostData = new Map<number, string>(); // msgId -> requestId
            ...
            try {
              ...
              if (msg.method === "Network.requestWillBeSent") {
                ...
                cdpRequestMap.set(p.requestId, {
                  ...
                  timestamp: typeof p.wallTime === "number"
                    ? new Date(p.wallTime * 1000).toISOString()
                    : new Date().toISOString(),
                });
                // Chromium may omit request.postData from requestWillBeSent even when
                // hasPostData is true (notably fetch/XHR requests serviced through a
                // worker). Ask CDP for the exact bytes before the request disappears;
                // without them a learned POST route is structurally unreplayable.
                if (!p.request?.postData && p.request?.hasPostData) {
                  const id = ++cdpMsgId;
                  cdpPendingPostData.set(id, p.requestId);
                  cdpWs!.send(JSON.stringify({ id, method: "Network.getRequestPostData", params: { requestId: p.requestId } }));
                }
              ...
              } else if (msg.id && cdpPendingPostData.has(msg.id)) {
                const requestId = cdpPendingPostData.get(msg.id)!;
                cdpPendingPostData.delete(msg.id);
                const req = cdpRequestMap.get(requestId);
                if (req && typeof msg.result?.postData === "string" && msg.result.postData.length > 0) {
                  req.postData = msg.result.postData;
                }
                ...
                const decodedBody = decodeCdpResponseBody(msg.result);
                if (decodedBody) {
                  cdpResolvedBodies.set(reqUrl, decodedBody);
                }
              }
    ...
    if (cookies && cookies.length > 0) {
      ...
      // Skip vault injection only when the browser ALREADY HOLDS a session:
      // its own cookies are fresher, and injecting stale vault cookies (e.g. a
      // dead JSESSIONID) causes HTTP 400.
      //
      // "Holds a session" is read off the live cookie jar, not off the URL. The
      // previous test — landed on the right host and the path does not look like
      // /login — is blind to every site that serves guests a normal page: x.com
      // and reddit never redirect an anonymous visitor, so a guest tab looked
      // authenticated, the real vault cookies were never injected, and the
      // capture recorded an anonymous session (an x-guest-token instead of the
      // logged-in timeline). `hasAuthenticatedSession` is the same auth-shape
      // rule the cookie extractor ranks jars with.
      const postOriginUrl = await phase("checkOriginUrl", () => kuri.getCurrentUrl(tabId));
      let browserAlreadyAuthed = false;
      try {
        const originHost = new URL(origin).hostname;
        if (new URL(postOriginUrl).hostname === originHost) {
          const live = await phase("readBrowserCookies", () =>
            kuri.getCookies(tabId).catch(() => [] as Awaited<ReturnType<typeof kuri.getCookies>>),
          );
          const forOrigin = live.filter((c) => {
            const d = (c.domain ?? "").replace(/^\./, "");
            return d.length > 0 && (originHost === d || originHost.endsWith(`.${d}`));
          });
          browserAlreadyAuthed = hasAuthenticatedSession(forOrigin);
        }
      } catch { /* unreadable jar or bad URL — inject, the safe direction */ }

      if (browserAlreadyAuthed) {
        log("capture", `browser holds a session for ${postOriginUrl} — skipping vault cookie injection`);
      } else {
        log("capture", `browser has no session for ${postOriginUrl} — injecting ${cookies.length} vault cookie(s)`);
    ...
    // Bounded action-driven exploration. Passive load traffic is a baseline,
    // not proof that a route responds to an agent action. For list/search
    // intents, exercise at most one reversible filter, pagination control, and
    // detail link while capture remains live, then correlate only request IDs
    // that appeared after each action.
    const actionCorrelations = new Map<string, { step: number; action: string; ref: string }>();
    const queryHooks: QueryHookEvent[] = [];
    const attemptedLabels = new Set<string>();
    const actionTypes: string[] = [];
    let actionsAttempted = 0;
    let actionsSucceeded = 0;
    let explorationStoppedReason: string | undefined;
    if (process.env.UNBROWSE_DISABLE_AUTO_EXPLORE !== "1" && shouldScrollStimulate(url, intent)) {
      const startDomain = pageDomain;
      const desiredKinds: ExplorationAction["kind"][] = ["filter", "pagination", "detail"];
      for (let stepIndex = 0; stepIndex < desiredKinds.length; stepIndex++) {
        if (abortController.signal.aborted) { explorationStoppedReason = "capture_timeout"; break; }
        const kind = desiredKinds[stepIndex];
        const snapshot = await phase(`explore:snapshot:${kind}`, () => kuri.snapshot(tabId, "interactive").catch(() => ""));
        const action = planSafeExploration(snapshot, {
          maxActions: 3,
          maxFilters: kind === "filter" ? 3 : 0,
          maxPagination: kind === "pagination" ? 3 : 0,
          maxDetails: kind === "detail" ? 3 : 0,
        }).find((candidate) => candidate.kind === kind && !attemptedLabels.has(`${kind}:${candidate.label.toLowerCase()}`));
        if (!action) continue;
        attemptedLabels.add(`${kind}:${action.label.toLowerCase()}`);
        actionsAttempted++;
        actionTypes.push(action.kind);
        const baselineCdpIds = new Set(cdpRequestMap.keys());
        const baselineIntercepted = await collectInterceptedRequests(tabId).catch(() => []);
        const baselineInterceptedKeys = new Set(baselineIntercepted.map((entry) =>
          requestCorrelationKey({ method: entry.method, url: entry.url, request_body: entry.request_body })
        ));
        try {
          await phase(`explore:${kind}`, () => kuri.action(tabId, action.action, action.ref));
          await new Promise((resolve) => setTimeout(resolve, 1500));
          const observedUrl = await kuri.getCurrentUrl(tabId).catch(() => url);
          let sameDomain = false;
          try { sameDomain = getRegistrableDomain(new URL(observedUrl).hostname) === startDomain; } catch {}
          if (!sameDomain) {
            explorationStoppedReason = "domain_drift";
            await kuri.navigate(tabId, url).catch(() => {});
            break;
          }
          actionsSucceeded++;
          const triggered: Array<{ url: string; method: string }> = [];
          for (const [requestId, request] of cdpRequestMap) {
            if (baselineCdpIds.has(requestId)) continue;
            const key = requestCorrelationKey({ method: request.method, url: request.url, request_body: request.postData });
            actionCorrelations.set(key, { step: stepIndex, action: action.action, ref: action.ref });
            triggered.push({ url: request.url, method: request.method });
          }
          const afterIntercepted = await collectInterceptedRequests(tabId).catch(() => []);
          for (const entry of afterIntercepted) {
            const key = requestCorrelationKey({ method: entry.method, url: entry.url, request_body: entry.request_body });
            if (baselineInterceptedKeys.has(key)) continue;
            actionCorrelations.set(key, { step: stepIndex, action: action.action, ref: action.ref });
            if (!triggered.some((item) => item.url === entry.url && item.method === entry.method)) {
              triggered.push({ url: entry.url, method: entry.method });
            }
          }
          queryHooks.push({
            ...
            selector: action.ref,
            requests_triggered: triggered,
            timestamp: Date.now(),
          });
          log("capture", `explore ${kind} ${action.ref}: ${triggered.length} causal request(s)`);
          // Detail navigation is a branch. Return to the original result page so
          // the capture remains scoped and later extraction sees the collection.
          if (kind === "detail" && observedUrl !== url) {
            await kuri.navigate(tabId, url).catch(() => {});
            await kuri.waitForLoad(tabId, 8_000).catch(() => {});
          }
        } catch (error) {
          log("capture", `explore ${kind} ${action.ref} failed: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
    ...
    applyActionCorrelations(requests, actionCorrelations);
    ...
    //
    // Best-effort ON PURPOSE: by this line the passive traffic is already
    // merged into `requests`, so cookies are enrichment. Racing them fatally
    // against the capture deadline meant a heavy authenticated page (LinkedIn
    // JD, x.com/messages) could collect hundreds of real requests and then
    // die with `capture phase 'extractCookies' timed out`, discarding all of
    // it. A late cookie read is worth less than the capture it was killing.
    let rawCookies: CapturedCookie[] = [];
    try {
      rawCookies = await phase("extractCookies", () => extractCookiesFromPage(tabId, url));
    } catch (err) {
      log("capture", `cookie extraction skipped (${err instanceof Error ? err.message : String(err)}) — keeping ${requests.length} captured request(s)`);
    }
    ...
    if (captureTimedOut) {
      // Salvage: the deadline bounds how long we COLLECT, not whether we keep
      // what we collected. Everything passive is already merged into
      // `requests` above, and nothing below this point can abort (no further
      // `phase()` calls), so a deadline that fires on a page which produced
      // real traffic is a STOP signal, not a failure. Throwing here is what
      // made capture look flaky: an identical URL succeeded or returned
      // `endpoints_discovered: 0` purely on whether the site crossed the
      // wall-clock line, discarding hundreds of captured requests either way.
      // `noProgressBail` still throws — zero network activity means there is
      // genuinely nothing to salvage, and it routes to the SSR fast-path.
      if (!noProgressBail && requests.length > 0) {
        capturePartial = true;
        log("capture", `deadline fired after ${CAPTURE_TIMEOUT_MS}ms — salvaging partial capture (${requests.length} request(s), ${responseBodies.size} bodies) instead of discarding it`);
      } else if (noProgressBail) {
        ...
        // Deadline fired with nothing captured — a real failure.
    ...
    if (
      ...
      if (options?.forceEphemeral) {
        const blocker = classifyCaptureBlocker({ html, requestCount: requests.length, responseBodyCount });
        throw Object.assign(new Error(blocker ?? "blocked_app_shell"), {
          code: blocker ?? "auth_required",
          login_url: final_url || url,
          reason: blocker ?? "blocked_app_shell",
        });
      }
      retryFreshTab = true;
      ...
      return {
        ...
        har_lineage_id,
        domain,
        partial: capturePartial || undefined,
        ...
        exploration: {
          ...
          causal_api_deltas: queryHooks.reduce((sum, hook) => sum + hook.requests_triggered.length, 0),
          query_hooks: queryHooks,
          ...(explorationStoppedReason ? { stopped_reason: explorationStoppedReason } : {}),
        },
      };
    }
  } catch (error) {
  ...
  if (retryFreshTab && !options?.forceEphemeral) {
    return captureSession(url, authHeaders, cookies, intent, { forceEphemeral: true, signal: options?.signal });
  }
