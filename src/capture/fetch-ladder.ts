...
import { tryPatchrightFetch } from "./patchright-fetch.js";
...
const FETCH_LADDER: readonly LadderRung[] = [
  ...
  {
    // LAST because it is the most expensive rung: a real, headed browser.
    // The impersonate rungs above clear the TLS-fingerprint-only class; this one
    // exists for the JS-challenge class they explicitly do not reach. Measured:
    // 18/100 corpus sites are blocked to both plain HTTP and obscura
    // (bench/sites100/CHALLENGE-RATE-FINDINGS.md). Optional — returns null when
    // patchright is not installed, so the ladder just ends.
    name: "patchright-headed",
    run: async (url, cookies) => {
      const r = await tryPatchrightFetch({
        url,
        timeoutMs: 45_000,
        proxy: resolveProxyUrl() ?? undefined,
        cookies,
      });
      return r && r.html ? { text: r.html, bytes: r.bytes, status: r.status } : null;
    },
