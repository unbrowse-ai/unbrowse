/**
 * surface-projector — the answer to "is the CLI architecture-shaped, or do we expose
 * only the verbs we need each time?" It is BOTH, at two altitudes:
 *
 *   - cli-surface.ts is the MODEL: the complete architecture/route verb tree
 *     (every command = one verb build/act/eval × one interrogative + its holes+auth,
 *     no route/logic/secret). That is the route shape (~/Projects/route: three
 ...
 *     to the architecture — it is the architecture, projected for context.
 *
...
/**
 * Project the full architecture surface down to only the verbs+holes the moment needs.
