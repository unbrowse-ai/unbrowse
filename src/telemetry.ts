...
function getTraceDir(): string {
  // U-3: the data root, not $HOME. Traces written straight to the caller's real
  // ~/.unbrowse under an isolated run are the same class of leak that made a
  // "cold" benchmark read the developer's warm route cache.
  return process.env.UNBROWSE_TRACES_DIR ?? join(getUnbrowseHome(), "traces");
}
