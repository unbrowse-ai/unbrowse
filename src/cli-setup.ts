 ...
 *
 * Three options:
 *   [1] Private            — captures cached locally, never published
 *   [2] Share (default)    — pointers contributed to the marketplace
...
export async function promptContributionMode(
  ...
  log("");
  log("  [1] Private            — cache locally, never publish");
  log("  [2] Share (default)    — contribute pointers to the marketplace");
  log("  [3] Share + earn       — contribute and add a wallet for x402 rev-share");
  log("");

  const reader = opts.readChoice ?? defaultReadChoice;
  const choice = await reader([1, 2, 3], 2);
