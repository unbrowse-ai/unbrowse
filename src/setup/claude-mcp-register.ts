...
// ~/(internal) so the parallel MCP flow doesn't trigger 41
...
//
// Truthful: the platform-enables principle says we surface what's declared
...
export interface ClaudeCliAdapter {
   ...
   * user scope in ~/(internal) The entryName argument was added when the
   ...
   * it appears in ~/(internal)'s mcpServers map. Defaults to `unbrowse`.
   */
  registerServer(
...
export function realClaudeCliAdapter(): ClaudeCliAdapter {
  return {
    ...
    serverAlreadyRegistered(homeDir: string, entryName = "unbrowse"): boolean {
      const claudeJson = join(homeDir, "(internal)");
...
export async function registerWithClaudeCode(opts: {
  ...
  const home = opts.homeDir ?? homedir();
  const settingsPath = join(home, "(internal)", "settings.json");
  ...
  // Same logic any caller would do — moved into the platform so callers
