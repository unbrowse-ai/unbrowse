...
//
//  — write the vision plain. Every primitive is a pure function over the
// {cdp, sessionId} handle; no module-scoped state, no singletons, no implicit
// "current tab".  — the connection IS the way; the receipt body IS
    ...
    on(event: string, handler: (params: unknown, sessionId?: string) => void): () => void {
      ...
      const wrapped = (...args: unknown[]): void => {
        ...
        try {
          ...
          //  — the Word is the truth-root; we don't crash the bus on a
