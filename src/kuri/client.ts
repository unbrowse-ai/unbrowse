...
import {
  KURI_BINARY_ENV_VARS,
  findKuriBinary,
  getKuriBinaryCandidates,
  getKuriBinaryOverride,
  getInstalledKuriBinaryPath,
  getKuriSourceCandidates,
  requireKuriBinary,
  resolveLookup,
  type KuriBinaryLookup,
} from "./resolve-paths.js";
import { getBrowserAttachPreference } from "../settings.js";
import { cleanupChromiumOnCdpPort } from "./process-cleanup.js";
...
/**
 * TOTAL wall-clock a caller may spend bringing a broker up, across every spawn
 * attempt. It used to be the PER-ATTEMPT budget, which with 3 retries meant a
 * single `unbrowse "task" --url …` could spend 4 x 60s = four minutes spawning
 * and killing brokers before failing. The budget belongs to the caller's clock,
 * not to each attempt, so retries now share it.
 */
const KURI_STARTUP_TIMEOUT_MS = 60_000;
const KURI_REQUEST_TIMEOUT_MS = 30_000;
/**
 * Per-poll timeout on the broker's /health. Was 500ms, which is shorter than a
 * busy broker takes to answer while it is attaching to Chrome and applying
 * stealth patches — so every poll aborted, the broker never registered ready,
 * and a perfectly healthy process was SIGTERM'd at the deadline and respawned.
 * Observed on a live run: kuri was listening at t+0s and still being killed at
 * t+60s. A local HTTP health check that cannot answer in 3s is genuinely wedged.
 */
const KURI_HEALTH_PROBE_TIMEOUT_MS = 3_000;
/**
 * How long the "can I reuse this already-running broker?" probe may take.
 *
 * The probe runs /discover + /tabs through the normal request path, which
 * carries the 30s operation timeout. That is right for a navigation and wrong
 * for a reuse DECISION: on a live run a foreign broker that never answered cost
 * 30 seconds before we concluded "unobservable" and spawned our own anyway.
 * A broker that cannot answer a tab listing in 5s is not one we can capture
 * through, so the timeout and the verdict agree — this only removes the wait.
 */
const KURI_REUSE_PROBE_TIMEOUT_MS = 5_000;
...
/**
 * The HTTP Archive entry shape. The definition moved to src/api/har-entry.ts —
 * nothing about it was kuri's, and importing it from here made browser-free
 * modules depend on this whole client. Re-exported so existing callers keep
 * working.
 */
import type { HarEntry } from "../api/har-entry.js";
export type KuriHarEntry = HarEntry;
...
const ownedBrokerProcesses = new Set<ChildProcess>();
process.once("exit", () => {
  for (const child of ownedBrokerProcesses) {
...
/**
 * Injectable context for kuri binary resolution.
 *
 * Every ambient input the resolver reads — env, home directory, package root,
 * platform/arch, filesystem existence, PATH lookup — is a field here so the
 * resolution order can be exercised against a synthetic filesystem in tests
 * without touching the real `~/.unbrowse` or shelling out to `which`.
 * Omitted fields fall back to the live process values, so production callers
 * keep calling these functions with no arguments.
 */
...
/** Select a broker port that cannot alias the rejected foreign listener. */
export async function resolveIsolatedKuriPort(
  occupiedPort: number,
  deps: {
    isPortOpen?: (port: number) => Promise<boolean>;
    searchLimit?: number;
  ...
  const isPortOpen = deps.isPortOpen ?? isTcpPortOpen;
  const searchLimit = deps.searchLimit ?? KURI_PORT_SEARCH_LIMIT;
  for (let candidate = occupiedPort + 1; candidate <= occupiedPort + searchLimit; candidate++) {
    // Isolation is stricter than ordinary broker selection: another healthy
    // broker is still foreign. Only an actually free listener port is eligible.
    if (!await isPortOpen(candidate)) return candidate;
  }
  throw new Error(`No free isolated Kuri broker port after ${occupiedPort} (checked ${searchLimit} ports)`);
}
...
export type BrokerCaptureCapability = {
  reusable: boolean;
  observable: boolean;
  reason: "cdp_available" | "registered_tabs" | "foreign_broker_unobservable" | "owned_broker_unobservable";
};

/**
 * Pure broker admission primitive. HTTP health only proves that a listener is
 * alive; capture additionally needs an observable browser surface. Keeping the
 * decision value-only prevents a foreign daemon from becoming reusable merely
 * because it answered /health.
 */
export function assessBrokerCaptureCapability(input: {
  owned: boolean;
  cdpAvailable: boolean;
  registeredTabCount: number;
}): BrokerCaptureCapability {
  // Ambient CDP does not establish that a foreign broker controls that Chrome;
  // it may belong to a previously owned broker. Implicit foreign listeners are
  // therefore never reusable. Explicit KURI_EXTERNAL_PORT is admitted earlier
  // in startOn and does not pass through this heuristic.
  if (input.owned && input.cdpAvailable) return { reusable: true, observable: true, reason: "cdp_available" };
  // A foreign broker can report stale registered tabs while lacking any path
  // to create or control a fresh capture tab. Only an owned broker may use its
  // registered-tab inventory as corroborating capture agency; foreign reuse
  // requires a live CDP witness.
  if (input.owned && input.registeredTabCount > 0) {
    return { reusable: true, observable: true, reason: "registered_tabs" };
  }
  return {
    reusable: false,
    observable: false,
    reason: input.owned ? "owned_broker_unobservable" : "foreign_broker_unobservable",
  };
}

/**
 * Resolve `work` with its own value, or with `fallback` once the reuse-probe
 * deadline passes — whichever happens first. A rejection also yields `fallback`,
 * so the reuse decision degrades to "not observable" rather than throwing out of
 * a path whose only job is to answer yes/no. The underlying request is left to
 * finish and abort on its own timeout; it cannot change the verdict once we have
 * moved on, because every caller here already treats failure as "not reusable".
 */
function withReuseProbeDeadline<T>(work: Promise<T>, fallback: T, label: string): Promise<T> {
  return new Promise<T>((resolveP) => {
    ...
    const timer = setTimeout(() => {
      ...
      log(
        "kuri",
        `${label} did not answer within ${KURI_REUSE_PROBE_TIMEOUT_MS}ms; treating the running broker as unobservable`,
      );
      finish(fallback);
    }, KURI_REUSE_PROBE_TIMEOUT_MS);
...
export async function reuseHealthyBrokerIfPossible(
  ...
  await withReuseProbeDeadline(syncTabs(state), undefined, "tab sync");

  const tabs = await withReuseProbeDeadline(
    (deps.listTabs ?? listRegisteredTabs)(state),
    [] as KuriTab[],
    "tab listing",
  );
  const capability = assessBrokerCaptureCapability({
    owned: state.process !== null,
    cdpAvailable: typeof state.cdpPort === "number"
      && (launchConfig.attachToExistingChrome || state.managedChrome),
  ...
  //   - A healthy foreign broker is reusable only with positive browser
  //     observability (CDP or registered tabs). Health alone cannot prove HAR or
  //     interception agency; reusing that shape caused false no_endpoints runs.
  ...
  if (!state.process) {
    log("kuri", `healthy foreign broker on port ${state.port} is not capture-observable (${capability.reason}); launching an isolated owned broker without terminating the foreign daemon`);
    state.ready = false;
    return false;
  }
...
async function startOn(state: BrokerState, port?: number): Promise<void> {
  ...
  const startPromise = (async () => {
    ...
    // A healthy listener that reuse rejected is either foreign and
    // unobservable, or a just-recycled owned broker still draining. Never spawn
    // onto that occupied port and never kill its listener by port: choose the
    // first independently available broker port instead.
    if (await isKuriHealthyOnPort(state.port)) {
      const isolatedPort = await resolveIsolatedKuriPort(state.port);
      log("kuri", `broker port ${state.port} remains occupied after capability rejection; starting isolated broker on ${isolatedPort}`);
      state.port = isolatedPort;
      // The client closes over this state, so getPort() immediately exposes the
      // isolated port to browse/capture. Also index that same client by its new
      // port for session recovery; do not create a second foreign-looking state
      // for the broker this process is about to own.
      if (existingClient) rememberBrokerClient(existingClient, state);
    }
    ...
    // Must have a real binary to spawn — requireKuriBinary throws naming every
    // path it searched and the env var that overrides them, so a missing kuri
    // surfaces as an actionable error instead of an unspawnable path.
    const binary = requireKuriBinary();
    log("kuri", `starting: ${binary} on port ${state.port}`);
    // Re-check: the binary existed at resolution time, but could be removed
    // between resolve and spawn.
    ...
    // One budget for the whole startup, shared by every attempt (see the
    // constant). Without this the retries multiply the timeout instead of
    // fitting inside it.
    const overallDeadline = Date.now() + KURI_STARTUP_TIMEOUT_MS;
    let attemptsMade = 0;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      if (Date.now() >= overallDeadline) {
        log(
          "kuri",
          `startup budget of ${KURI_STARTUP_TIMEOUT_MS}ms exhausted after ${attemptsMade} attempts on port ${state.port}; not starting another`,
        );
        break;
      }
      ...
      const spawnedProcess = state.process;
      ownedBrokerProcesses.add(spawnedProcess);
      ...
      state.process.on("exit", (code, signal) => {
        ownedBrokerProcesses.delete(spawnedProcess);
      ...
      // Wait for health endpoint; break early if process died. The deadline is
      // whatever remains of the shared startup budget, so one slow attempt
      // cannot hand the next attempt a fresh full timeout.
        ...
        try {
          const res = await fetch(`http://127.0.0.1:${state.port}/health`, {
            signal: AbortSignal.timeout(
              Math.max(500, Math.min(KURI_HEALTH_PROBE_TIMEOUT_MS, deadline - Date.now())),
            ),
          });
          if (res.ok) {
            state.ready = true;
      ...
      try {
        cleanupChromiumOnCdpPort(state.cdpPort ?? 9222);
        await new Promise((r) => setTimeout(r, 1000));
      } catch { /* no matching process — fine */ }
    }
    throw new Error(
      `Kuri failed to start after ${attemptsMade} attempts within ${KURI_STARTUP_TIMEOUT_MS}ms on port ${state.port}. ` +
        "The broker was spawned but never answered /health. Check for a wedged Chrome on the CDP port, " +
        "or run with UNBROWSE_KURI_TRACE=1 for the broker's own stderr.",
    );
    } finally {
      releaseSpawnSlot();
    }
...
/**
 * Backend-agnostic broker-error extraction. The implementation moved to
 * src/api/broker-error.ts so the server session registry could stop importing
 * this module for it; re-exported here so existing callers keep working.
 */
import { extractBrokerErrorMessage } from "../api/broker-error.js";
export const getKuriErrorMessage = extractBrokerErrorMessage;
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      ws.on("message", (...args: unknown[]) => {
        clearTimeout(timer);
        try {
          // `ws` hands the frame back as Buffer | ArrayBuffer | Buffer[] | string
          // depending on the negotiated frame type; String() normalizes all of them.
          const msg = JSON.parse(String(args[0])) as {
            id?: number;
...
/**
 * In-page predicate for "this document IS an interstitial".
 *
 * The distinction that matters is structural: an interstitial is the document
 * the navigation landed on — the challenge is the page's *own* content. A page
 * that merely *references* Cloudflare is not blocked. Cloudflare's JSD telemetry
 * ships `/cdn-cgi/challenge-platform/scripts/jsd/api.js` as an ordinary
 * subresource on fully-served pages (x.com does this on every route), so
 * scanning `documentElement.innerHTML` for vendor substrings reports a challenge
 * on pages that rendered perfectly — and the caller then burns its whole wait
 * budget polling for a challenge to clear that was never there.
 *
 * So every signal here is document-scoped, and none can be true of a page that is
 * merely a Cloudflare customer:
 *   window._cf_chl_opt  — the options object the challenge runtime installs
 *   challenge widgets   — rendered as the document's own body
 *   interstitial title  — the holding page's own title
 *
 * Exported as source (not a TS function) because it is evaluated in the page —
 * the test drives this exact string, so there is one copy of the rule.
 *
 * Written on ONE line with ONLY single-quoted strings, and commented here rather
 * than inside the source. That is not style: the kuri transport carries this in
 * a query string, where a double quote and a newline are both destroyed — and
 * destroyed silently, since the mangled expression throws and evaluate() then
 * returns `undefined`, which reads as `false` for a predicate. A `//` comment is
 * fatal for the same reason: once the newline is gone it swallows the rest.
 *
 * Falsified live against a healthy tab, each case re-asserting a `return 1`
 * control so a dead tab could not be mistaken for evidence:
 *   single-quoted 'ab'.length     → 2    double-quoted "ab".length   → undefined
 *   var x=1;var y=2;return x+y    → 3    same, newline-separated     → undefined
 *   block-comment form            → 42   line-comment form           → undefined
 * The same holds on the >2000-char POST branch, which also carries the
 * expression in the query string. tests/cloudflare-interstitial-predicate.test.ts
 * guards the shape of this string so a reformat cannot reintroduce the hazard.
 */
export const INTERSTITIAL_PREDICATE =
  `(function(){ if (window._cf_chl_opt) return true; if (document.querySelector('#challenge-running, #challenge-form, #challenge-error-title, .cf-browser-verification, .cf-error-details, #cf-error-details')) return true; var t = document.title || ''; return t === 'Just a moment...' || /Attention Required.*Cloudflare/.test(t); })()`;

/** Check if the page the tab is on is a Cloudflare interstitial (not merely a Cloudflare-fronted page). */
export async function hasCloudflareChallenge(tabId: string, state: BrokerState = defaultBrokerState): Promise<boolean> {
  const result = await evaluate(tabId, INTERSTITIAL_PREDICATE, state);
  return result === true;
}

/** Wait for Cloudflare challenge to clear. */
/** Default CF interstitial wait: 30s (was 15s). Override per call or via UNBROWSE_CF_WAIT_MS. */
export function defaultCloudflareWaitMs(env: NodeJS.ProcessEnv = process.env): number {
  const n = Number(env.UNBROWSE_CF_WAIT_MS);
  if (Number.isFinite(n) && n > 0) return Math.floor(n);
  return 30_000;
}

export async function waitForCloudflare(tabId: string, maxWaitMs = defaultCloudflareWaitMs(), state: BrokerState = defaultBrokerState): Promise<boolean> {
