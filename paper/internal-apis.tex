...
% carries a real citation; claims are tagged [shipped] (in the product),
% [reference] (a runnable, tested reference implementation ships in reference/),
% or [proposed] (design only) so nobody gets sold a vibe with a footnote.
% =====================================================================
...
each layer instead of inventing one, and we label what is \textbf{[shipped]} in
the product, what is \textbf{[reference]} (a runnable, tested reference
implementation that ships with this paper in \texttt{reference/}), and what is
\textbf{[proposed]} so the reader always knows whether they are looking at a
product, running code, or a promise. The short version: internal APIs were a
...
fingerprint-faithful fetch; and the uniform \emph{signed descent through every
layer} (screen $\to$ browser $\to$ CLI $\to$ OS $\to$ kernel $\to$ packet) now ships
in production: \impl{} \texttt{src/values/signed-descent.ts} signs one wallet root
once and threads a hash-chained, per-layer signature down the whole stack, with tests
that any tampered or reordered layer fails to verify. Ownership is vertical:
\texttt{src/values/wallet-hierarchy.ts} derives each layer's wallet from its parent's
(HKDF) and the parent signs the child's key, and \texttt{src/values/layer-wallet-descent.ts}
has each layer sign with its own parent-owned wallet --- so the root wallet owns the
screen wallet owns the browser wallet, all the way down to the packet wallet. What
remains referenced rather than shipped is emitting \emph{real} signed OS/kernel/packet
operations across platforms; the signed descent record, the wallet hierarchy, and the
fingerprint-faithful HTTP emission are in the product today.
...
network (economics)  & x402 split settlement / fair three-way compensation & \cite{x402} \\
...
Validation re-execution record, each signed by a real wallet. \prop{} % gate:prop-ok
...
credential is bound, never \emph{what} it is. \impl{} --- this is the central
...
content-addressed cache that is public by default and private by proof. \impl{} for
endpoint/route caching; and the sealed-unless-revealed commitment layer now ships in
production: \impl{} \texttt{src/values/wallet-seal.ts} addresses each value by the
...
how a slow truth-resolution is paid for once and never again: \impl{}
...
(\texttt{sha256:}$\langle$hex$\rangle$, exactly the covenant \texttt{putBlob}/\texttt{resolvePointer}
...
layer, exactly like a Docker layer cache: a miss rebuilds it, a hit replays it. The
fallback that feeds it is also shipped: \impl{} \texttt{src/values/kv-fallback-pipe.ts}
walks the descent ladder highest-capable-first, content-addressing each layer's result,
so a cache hit short-circuits the whole descent. Tests confirm the warm path never
...
purpose: no unsigned action crosses the gate, at any layer, ever. \refimpl{} as a
...
ledger (the route graph already keeps signed JSONL records); \refimpl{} for the
Merkle-root checkpoint and independent-auditor layer:
...
of the descent. The fingerprint-faithful fetch is not a diagram: it ships as a
vendored uTLS proxy across four platforms (darwin/linux $\times$ amd64/arm64) driven
by \texttt{src/chrome-prim/layer1-tls.ts} and \texttt{src/cdp/proxy/utls-daemon.ts},
so the agent's TLS ClientHello and HTTP/2 settings reproduce a real browser's JA3/JA4
signature at the \emph{network interface}, not merely a spoofed User-Agent header.
\impl{} This is the packet layer of
...
$\geq 0.75$ release threshold). \refimpl{} \texttt{scripts/coverage-gate.sh} is
...
returns real results broadly. What remains out of this paper's scope is per-task
accuracy on adversarial, heavily-JavaScript-gated sites and emitting \emph{real
signed} OS/kernel/packet syscalls across platforms --- the fingerprint-faithful HTTP
emission is shipped; raw signed syscall descent is referenced, not claimed.
...
\item \impl{} The cross-layer security primitives now ship in production, each with
...
(\texttt{src/values/sealed-ledger.ts}). The backend \emph{is} the harness: the client
surfaces only the holes to fill plus wallet-sealed auth and the backend returns nothing
but structure (\texttt{src/capture/backend-reveng-endpoint.ts}), and the stateless
binary writes no local state (only pointers and signatures cross the wire).
\item \refimpl{} The runnable reference suite in \texttt{reference/} remains the
cited foundation for each primitive above and for the parts not yet in the product:
Merkle-root checkpoints with inclusion proofs (\texttt{ledger/checkpoint.py}). The
...
honestly labelled.
\item \impl{} Zero-edit drop-in replacements for the libraries an agent would
