...
% format mirrors the arXiv source; claims tagged \impl{} [shipped],
% \refimpl{} [runnable tested reference in reference/], or proposed.
% =====================================================================
...
owner are all compensated in one transaction. Across 94 live domains we measure a
...
short version: for the overwhelming majority of agent web tasks, internal APIs are
all you need.
\end{abstract}
...
We make one claim and try not to oversell it: for the overwhelming majority of
agent web tasks, the browser is unnecessary overhead, because the first-party
endpoint behind the page --- the \emph{shadow API} --- can be learned once and
replayed by any later agent. The discovery cost should be paid a single time and
amortised across all callers, not re-paid on every visit. This paper presents
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  real usage and normalises them into a shared graph. \impl
  ...
  the resulting value over x402. \impl
  ...
  isolated in a runnable reference benchmark. \refimpl
\end{enumerate}
...
and what they returned --- value off-chain, commitment hash-chained. \refimpl
...
without launching a browser. \impl
...
settles. An \emph{unpaid \texttt{execute} gets HTTP 402}. \impl\ Agents pay for the
...
transaction. \impl\ This \emph{three-way live split is shipped}. \impl
...
replay categorically skips the cold mapping path, and the harness records a measured
$\approx$95$\times$ mean speedup on the reuse micro-benchmark
(\texttt{reference/bench/bench\_reuse.py}), with a conservative $\approx$27$\times$
observed gap on a single live URL
(\texttt{reference/bench/live-measured-1url.json}). \refimpl\ The full
...
intent (coverage $=1.0$ against a $\geq 0.75$ release threshold). \refimpl\ Coverage
...
per-step success rate) the model recovers $\approx$45$\times$ faster and
$\approx$300$\times$ fewer tokens end-to-end, bracketing the field-measured
$\approx$30$\times$ faster / $\approx$90$\times$ cheaper headline. We ship this as a
deterministic, runnable reference (\texttt{reference/bench/task\_cost.py}) so the
compounding is provable arithmetic rather than a marketing multiplier. \refimpl
...
lifecycle demotes drifting routes. What the wedge buys is the common case, which is
most of the web: a structured request the site already makes, recovered once and
shared. Two questions it deliberately leaves open --- securing the descent below
HTTP, and keeping the shared graph trustworthy as it scales --- are the subjects of
...
one-off rediscovery into a shared, reusable route graph removes the tax and makes
maintenance, not discovery, the place value compounds. Internal APIs are most of
what an agent needs to read and act on the web, and for that majority they are, in
the affectionate sense of the title, all you need. They are also not the whole
stack: securing the layers below HTTP is \emph{Crypto Was All You
...
\texttt{reference/bench/bench\_reuse.py} (\refimpl) so the categorical
