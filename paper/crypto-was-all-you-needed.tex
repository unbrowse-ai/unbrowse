...
single structure --- the layered tree from which our internal tooling is built ---
and every section below is one node of it, mapped to a primitive that already
...
\hline
\end{tabular}
\caption{Each layer of the signed stack is one node of the layered tree, mapped
...
(\texttt{bench/BENCHMARKS.md}) shows a small on-device agent routed to a library of
executable tools turning tasks it fails from weights alone into tasks it solves
reliably --- every number the same on-device model, tools versus no tools:
code-correctness \textbf{25\% $\to$ 100\%} by routing to a real executor; knowledge
absent from the weights \textbf{0\% $\to$ 95\%} by retrieve-then-execute; hard reasoning
families \textbf{50\% $\to$ 92\%} by distilled routing; and applying a retrieved skill
rather than reasoning it from scratch \textbf{63\% $\to$ 93\%}. The architecture is the
