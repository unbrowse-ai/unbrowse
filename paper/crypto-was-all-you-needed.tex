...
the old hash no longer resolves, and the layer re-resolves on next access
(\texttt{src/values/resolution-ledger.ts}). Nothing is invalidated by a clock or a
manual flush; correctness falls out of the addressing, because a changed input is
\emph{by construction} a different key --- the same reason changing a base layer in a
Dockerfile invalidates the layers built on it. The fallback that feeds it walks the
descent ladder highest-capable-first (\texttt{src/values/kv-fallback-pipe.ts}),
content-addressing each layer's result, so a cache hit short-circuits the whole descent
and a changed input re-resolves at the layer it touched. Propagating one changed
pointer through a \texttt{requires}/\texttt{yields} set to recompute every dependent
layer in a single pass --- a full content-addressed rebuild DAG --- is the natural next
step and remains design work. Tests confirm the warm path never recomputes, the ledger
