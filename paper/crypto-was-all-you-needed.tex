...
the other side is keying on~\cite{ja3,ja4}. The browser, CLI, and HTTP layers and
the fingerprint-faithful fetch run in the product, and the uniform \emph{signed descent through every
layer} (screen $\to$ browser $\to$ CLI $\to$ OS $\to$ kernel $\to$ packet) ships
in production: \texttt{src/values/signed-descent.ts} signs one wallet root
...
root. No signature, no action; the stack has no anonymous side door. The
...
Invalidation is the same discipline run the other way. When an upstream value
\emph{changes}, its bytes change, so its content hash changes, so the pointer keyed on
the old hash no longer resolves --- and every downstream layer that named that pointer
in its \texttt{requires} set now misses and recomputes, cascading through the
dependency DAG (\texttt{src/orchestrator/dag-feedback.ts}) exactly as changing a base
layer in a Dockerfile rebuilds every layer above it. Nothing is invalidated by a clock
or a manual flush; correctness falls out of the addressing, because a changed input is
\emph{by construction} a different key. The fallback that feeds it walks the descent
ladder highest-capable-first (\texttt{src/values/kv-fallback-pipe.ts}),
content-addressing each layer's result, so a cache hit short-circuits the whole descent
and a changed input re-resolves only the layers genuinely affected. Tests confirm the
warm path never recomputes, the ledger is tamper-evident, and eviction reproduces the
same pointer.
