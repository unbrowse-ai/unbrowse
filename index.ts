  ...
  register(api: OpenClawPluginApi) {
    ...
    const extractMessageContentText = (content: unknown): string => {
      ...
      if (Array.isArray(content)) {
        return content.map((item) => extractMessageContentText(item)).filter(Boolean).join("\n");
      }
    ...
    const extractPromptText = (event: unknown): string => {
      if (!event) return "";
      if (typeof event === "string") return event;
      if (Array.isArray(event)) {
        return event.map((item) => extractPromptText(item)).filter(Boolean).join("\n");
      }
    ...
    const isApiIntent = (event: unknown): boolean => {
