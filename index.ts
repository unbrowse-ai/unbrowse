...
/** Generate a TypeScript API client from endpoints (for skills downloaded without scripts) */
function generateApiClient(
  ...
  const primaryAuthHeader = authType.toLowerCase().includes("bearer") ? "Authorization" : "authorization";

  return `/**
 * ${className} API Client
 * Generated by Unbrowse for skill: ${className.toLowerCase().replace(/([A-Z])/g, "-$1").slice(1)}
 */
...
export class ${className}Client {
  ...
  async request(method: string, endpoint: string, opts?: RequestOptions): Promise<unknown> {
    ...
    if (!resp.ok) throw new Error(\`\${method} \${endpoint} failed: \${resp.status} \${resp.statusText}\`);
            ...
            if (p.install) {
              try {
                ...
                if (pkg.scripts && Object.keys(pkg.scripts).length > 0) {
                  ...
                  // Generate api.ts client if not provided by publisher
                  const endpoints = extractEndpoints(pkg.skillMd);
                  const baseUrl = pkg.domain ? `https://${pkg.domain}` : "https://api.example.com";
                  const className = pkg.name.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join("");
                  const apiTs = generateApiClient(className, baseUrl, endpoints, pkg.authType || "Unknown");
