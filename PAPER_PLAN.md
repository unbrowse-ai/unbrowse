...
| 9 | Composite scoring: 40 sim / 30 reliability / 15 freshness / 15 verification | §3.3 | partial | rank deltas (51780d7e, 5c880731); Wave-1 seed at `src/ranking/index.ts`; P2 W1 weights+freshness at `src/ranking/{composite,freshness}.ts`; Wave-2 every src/ caller imports from `src/ranking/`; W3 cluster #1 bm25 at `src/ranking/signals/bm25.ts`; W3 cluster #2 intent-yield at `src/ranking/signals/intent-yield.ts`; W3 cleanup: 7 noise-filter regexes at `src/ranking/filters/noise-patterns.ts`; W3 hard-clamp cluster at `src/ranking/clamps.ts` (3 inline Math.min lifted to clampToFloor + 3 named floors) | 122 inline `score += [0-9]` hits remain outside src/ranking/ (intent-match cluster ~13 deltas is next loop's largest target); composite not yet wired into runtime ranker |
