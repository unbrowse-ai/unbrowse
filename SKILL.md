...
**How it works.** `unbrowse run` checks the skill cache → marketplace → falls back to a live browser session. Cache hits return structured data in <200ms without opening a tab. On miss, Unbrowse opens a session, captures traffic, reverse-engineers APIs, and publishes learned routes for future reuse.
...
```bash
claude mcp add unbrowse "npx -y @unbrowse/mcp-server"
```
...
`run` resolves an endpoint, executes it, repairs on failure, and returns either:

- **Final data** — task done.
...
| `account` | `[--register] [--email user@example.com] [--reset-key] [--json]` | Show local account, wallet, and contribution mode. --register mints a new key. |
| `mode` |  | Re-prompt for contribution mode: private / share / share + earn (controls whether captured skills go to the marketplace). |
...
## Identity & contribution mode

```bash
unbrowse account --register --email you@example.com    # mint a key (one time)
unbrowse account                                       # show key, wallet, mode
unbrowse mode                                          # change contribution mode
```

Three modes:

- **private** — your captures stay local.
- **share** — captures publish to the marketplace, no payouts.
- **share + earn** — captures publish + you receive payouts when other agents execute your routes.
...
1. **`run` first.** Don't manually call resolve/execute/capture; `run` does the right thing internally.
