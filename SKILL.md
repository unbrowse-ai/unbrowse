...
| `run` | `<url> "task"` | One-shot agent path: resolve a task for a URL and auto-execute the best safe read endpoint. Accepts positional task text or --intent/--task/--query. |
| `resolve` | `--intent "..." [--url "..."] [--domain "..."] [--no-execute]` | Resolve an intent against the marketplace + local cache. --task and --query are accepted aliases for --intent. Auto-executes the top safe GET endpoint by default; --no-execute returns metadata only. Pair with `unbrowse execute` when you want explicit endpoint pick. |
