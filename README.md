...
More than a faster browser, Unbrowse is the **private, fair-compensated route engine for the agentic web**: agents discover a route once and every later agent reuses it; the indexers who captured it and the domain owner earn on each paid call via an on-chain split; credentials never leave your machine (pointer-only receipts, one wallet signs every layer). When a call needs a paid upstream the platform can't avoid — a web-unblocker for a hard-protected site, an LLM proxy, a paid API — Unbrowse fronts it and resells it at the raw cost plus a single transparent **fair-compensation** rate (20% by default, [docs/HOW_UNBROWSE_PAYS.md](./docs/HOW_UNBROWSE_PAYS.md)), so an agent pays once, in one place, and never has to hold a wallet on every chain or register with every vendor.
