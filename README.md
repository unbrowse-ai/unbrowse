...
> **Note:** The previous OpenClaw plugin has been deprecated. This repo is the universal Unbrowse skill — it works with any agent (Claude Code, Cursor, Codex, Windsurf, etc.), not just OpenClaw.
...
Clone and start the server — it auto-registers as an agent, accepts ToS, and handles setup. No manual API key configuration needed:
...
Install the [SKILL.md](./SKILL.md) as a Claude Code skill. It tells Claude how to call the local server's API to capture sites, discover endpoints, and execute learned skills. The skill expects the engine at `~/.agents/skills/unbrowse`.
...
- Private credentials stay local (encrypted vault, never published)
- Shareable API contracts are published after capture
