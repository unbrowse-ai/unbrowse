 ...
 * context PROJECTION of the architecture verb tree. Proves: the projection is
...
import { projectSurface, projectedVerbs, projectedHoles } from "../src/architecture/surface-projector.js";
import { knownCommands, verifySurface } from "../src/architecture/cli-surface.js";

const cmds = (ctx: Parameters<typeof projectSurface>[0]) => projectSurface(ctx).map((s) => s.command);

describe("surface-projector — the dynamic surface over the architecture tree", () => {
  ...
  it("every projected entry exposes ONLY holes + auth + atom — no internal field leaks", () => {
    ...
    // the model itself is architecture-complete + minimal
    expect(verifySurface().ok).toBe(true);
  });

  it("verbs are the architecture atoms (build/act/eval), populated by the LLM via holes", () => {
    const verbs = projectedVerbs({ sessionOpen: false });
    expect(verbs.every((v) => ["build", "act", "eval"].includes(v))).toBe(true);
  ...
  it("LOSSLESS: the union of all projections = the whole architecture tree (nothing hidden forever)", () => {
