/**
 * W9-B — v7 CLI tests for act click / select / submit.
 *
...
describe("v7-cli kind-map bindings (W9-B)", () => {
  it("act click is bound to actuate_click + unbrowse_click", () => {
    const meta = lookupKindMap("act", "click");
    expect(meta).toBeDefined();
    expect(meta!.op_kind).toBe("act:click");
    expect(meta!.mcp_tool).toBe("unbrowse_click");
  });

  it("act select is bound to actuate_select + unbrowse_select", () => {
    const meta = lookupKindMap("act", "select");
    expect(meta).toBeDefined();
    expect(meta!.op_kind).toBe("act:select");
    expect(meta!.mcp_tool).toBe("unbrowse_select");
  });

  it("act submit is bound to actuate_submit + unbrowse_submit", () => {
    const meta = lookupKindMap("act", "submit");
    expect(meta).toBeDefined();
    expect(meta!.op_kind).toBe("act:submit");
...
describe("v7-cli click/select/submit surface (always-on)", () => {
  it("act click --help emits op_kind=actuate_click", async () => {
    const res = await runCli(["act", "click", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.help).toBe(true);
    expect(out.op_kind).toBe("act:click");
    expect(out.mcp_tool).toBe("unbrowse_click");
  });

  it("act select --help emits op_kind=actuate_select", async () => {
    const res = await runCli(["act", "select", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.help).toBe(true);
    expect(out.op_kind).toBe("act:select");
    expect(out.mcp_tool).toBe("unbrowse_select");
  });

  it("act submit --help emits op_kind=actuate_submit", async () => {
    const res = await runCli(["act", "submit", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.help).toBe(true);
    expect(out.op_kind).toBe("act:submit");
    expect(out.mcp_tool).toBe("unbrowse_submit");
  });

  it("act click without selector emits missing_positional", async () => {
    const res = await runCli(["act", "click", "--json"]);
  ...
  it("act select without args emits missing_positional", async () => {
    const res = await runCli(["act", "select", "--json"]);
    expect(res.code).toBe(1);
    const out = JSON.parse(res.stdout);
    expect(out.error).toBe("missing_positional");
    expect(out.op_kind).toBe("act:select");
...
describe.if(W5W6_READY)("v7-cli act click/select/submit e2e (W5+W6 required)", () => {
  ...
  afterAll(async () => {
    if (sessionId) {
      await runCli(["act", "close", sessionId, "--json"]).catch(() => {});
    }
    await audit?.stop();
    await clearSessions();
  });
  ...
  it("1. click '#button' dispatches mouse press+release at the element center", async () => {
    const res = await runCli(
      ["act", "click", "#button", "--session", sessionId, "--json"],
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
    expect(out.ok).toBe(true);
    expect(out.subcommand).toBe("act click");
    expect(out.op_kind).toBe("act:click");
  ...
  it("2. click '#missing' exits 65 with selector_not_found", async () => {
    const res = await runCli(
      ["act", "click", "#missing", "--session", sessionId, "--json"],
    );
  ...
  it("3. select '#country' arg://country with --arg country=US posts a valid AuditFillBody (no value)", async () => {
    ...
    const res = await runCli(
      [
        "act", "select", "#country", "arg://country",
  ...
  it("5. select '#country' \"US\" (literal value) sets the select with NO audit POST", async () => {
    const beforeCount = audit.bodies.length;
    const res = await runCli(
      ["act", "select", "#country", "US", "--session", sessionId, "--json"],
      { UNBROWSE_API_URL: audit.url },
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
  ...
  it("6. submit 'form#login' calls requestSubmit and returns navigation descriptor", async () => {
    const res = await runCli(
      ["act", "submit", "form#login", "--session", sessionId, "--json"],
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
    expect(out.ok).toBe(true);
    expect(out.subcommand).toBe("act submit");
    expect(out.op_kind).toBe("act:submit");
