...
import { holeifyEndpointAttribution } from "../src/capture/bound-hole.js";
...
// secrets — only wallet-bound commitments (`bind:` tags). Public structural fields
...
describe("backfill prep emits no raw attribution ids on the wire", () => {
  test("serialized body carries only bind: commitments, structure preserved", async () => {
    ...
    for (const id of allIds) {
      expect(id.startsWith("bind:")).toBe(true);
    }
