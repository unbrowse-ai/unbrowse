 ...
 *
 * No mocks of the substrate itself — the cloud route handlers run
 * against a real in-memory ContractLedger implementation declared in
 * this test. The substrate principle ("harness collects, agent judges")
...
describe("thin-client foundation — wire surface end-to-end", () => {
  ...
  test("thin client end-to-end: declare → iterate → status via injected fetch", async () => {
    ...
    // handler functions. This is the real cloud-side substrate; we just
    // skip the HTTP transport layer to keep the test substrate-faithful.
    const ledger = inMemoryLedger();
