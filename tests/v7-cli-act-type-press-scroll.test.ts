/**
 * W9-A — v7 CLI act {type, press, scroll} tests.
 *
 * Scope:
...
describe("v7-cli act press — pure-argv", () => {
  it("5. `press NotAKey` exits 64 (EX_USAGE) with `unknown key`", async () => {
    const res = await runCli(["act", "press", "NotAKey", "--json"]);
  ...
  it("`press` with no key exits with missing_positional", async () => {
    const res = await runCli(["act", "press", "--json"]);
  ...
  it("`press --help` prints the subcommand surface and exits 64", async () => {
    const res = await runCli(["act", "press", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.help).toBe(true);
    expect(out.subcommand).toBe("act press");
    expect(out.op_kind).toBe("act:press");
    expect(out.mcp_tool).toBe("unbrowse_press");
  });
});

describe("v7-cli act scroll — pure-argv", () => {
  it("`scroll bogus` exits 64 (EX_USAGE) with bad_amount_format", async () => {
    const res = await runCli(["act", "scroll", "bogus", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.ok).toBe(false);
  ...
  it("`scroll` with no positional exits missing_positional", async () => {
    const res = await runCli(["act", "scroll", "--json"]);
    expect(res.code).toBe(1);
    const out = JSON.parse(res.stdout);
    expect(out.error).toBe("missing_positional");
  });
  ...
  it("`scroll --help` carries the kind-map row", async () => {
    const res = await runCli(["act", "scroll", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.subcommand).toBe("act scroll");
    expect(out.op_kind).toBe("act:scroll");
    expect(out.mcp_tool).toBe("unbrowse_scroll");
  });
});

describe("v7-cli act type — pure-argv", () => {
  it("`type` with no positional exits missing_positional", async () => {
    const res = await runCli(["act", "type", "--json"]);
    expect(res.code).toBe(1);
    const out = JSON.parse(res.stdout);
    expect(out.error).toBe("missing_positional");
  });
  ...
  it("`type --help` carries the kind-map row and pointer schemes", async () => {
    const res = await runCli(["act", "type", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.subcommand).toBe("act type");
    expect(out.op_kind).toBe("act:type");
...
describe.if(W5W6_READY)("v7-cli act {type, press, scroll} (e2e — W5+W6 required)", () => {
  ...
  beforeAll(async () => {
    ...
    const res = await runCli(["act", "go", "https://example.com", "--json"]);
    if (res.code !== 0) {
  ...
  afterAll(async () => {
    if (sessionId) {
      await runCli(["act", "close", sessionId, "--json"]);
    }
    await audit?.stop();
    await clearSessions();
  });
  ...
  it("1. `type \"hello world\"` (literal) — succeeds without audit POST", async () => {
    ...
    const res = await runCli(
      ["act", "type", "hello world", "--session", sessionId, "--json"],
  ...
  it("4. `press Enter` — dispatches keyDown+keyUp, no audit POST", async () => {
    const before = audit.bodies.length;
    const res = await runCli(
      ["act", "press", "Enter", "--session", sessionId, "--json"],
      { UNBROWSE_API_URL: audit.url },
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
  ...
  it("6. `scroll 0,500` — viewport scroll, no audit", async () => {
    const before = audit.bodies.length;
    const res = await runCli(
      ["act", "scroll", "0,500", "--session", sessionId, "--json"],
      { UNBROWSE_API_URL: audit.url },
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
  ...
  it("7. `scroll body 0,200` — element-anchored scroll (`body` always exists on example.com)", async () => {
    const before = audit.bodies.length;
    const res = await runCli(
      ["act", "scroll", "body", "0,200", "--session", sessionId, "--json"],
      { UNBROWSE_API_URL: audit.url },
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
