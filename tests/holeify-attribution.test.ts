...
// replaced by a one-way, wallet-signed bind tag), and the client secret-shape gate
...
import {
  ...
  parseBoundTag,
  verifyHoleAttested,
} from "../src/capture/bound-hole.js";
...
describe("holeifyIds — attribution ids become wallet-bound commitments", () => {
  it("replaces every raw id with a valid bind commitment; no raw id survives", async () => {
    const out = await holeifyIds([RAW_PUBKEY, SHORT_ID]);
    expect(out.length).toBe(2);
    for (const tag of out) {
      expect(tag.startsWith("bind:")).toBe(true);
  ...
  it("holeifies corroboration on each endpoint, leaving non-corroboration shape intact", async () => {
    ...
    expect(corr.submitter_agent_ids.every((t) => t.startsWith("bind:"))).toBe(true);
    expect(corr.verified_release_submitter_ids.every((t) => t.startsWith("bind:"))).toBe(true);
...
describe("looksLikeSecret — admit the commitment, flag the raw id", () => {
  ...
  it("admits a bind commitment (secret-free by construction)", async () => {
    const [tag] = await holeifyIds([RAW_PUBKEY]);
