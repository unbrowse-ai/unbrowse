/**
 * W32-B — v7 CLI tests for `act fill-form`, the end-to-end form-fill pipe.
 *
 * The pipe: snap form → enumerate() candidates → route route_populate
 * (ONE dogfooded binary call) → resolve picked pointers → inject every field.
 *
 * W32.1 — the route_populate kind now surfaces its populated payload INLINE
 ...
 * wired for real). Gated off → suite passes (pattern from the sibling act
...
} from "../src/cli-v7/act/fill-form.js";
...
/**
 * Write a mock route binary that reads the route_populate op on stdin
 ...
 */
async function writeMockRouteBin(
  opLog: string,
  picks: Record<string, string | null> = {},
): Promise<string> {
  const dir = (await Bun.file(tmpdir()).exists()) ? tmpdir() : "/tmp";
  const path = join(dir, `unb7-w32b-route-${Date.now()}.sh`);
...
describe("v7-cli fill-form kind-map binding (W32-B)", () => {
  it("act fill-form → actuate_fill_form + unbrowse_fill_form", () => {
    const meta = lookupKindMap("act", "fill-form");
    expect(meta).toBeDefined();
    expect(meta!.op_kind).toBe("act:fill_form");
    expect(meta!.op_class).toBe("actuate");
    expect(meta!.action).toBe("fill_form");
    expect(meta!.mcp_tool).toBe("unbrowse_fill_form");
  });

  it("act fill-form --help emits op_kind=actuate_fill_form", async () => {
    const res = await runCli(["act", "fill-form", "--help", "--json"]);
    expect(res.code).toBe(64);
    const out = JSON.parse(res.stdout);
    expect(out.help).toBe(true);
    expect(out.op_kind).toBe("act:fill_form");
  ...
  it("sealDiffusionReceipt parses the receipt envelope AND reads picks from extra.data.populated (ONE call)", async () => {
    ...
    const bin = await writeMockRouteBin(opLog, { username: "arg://username", password: null });
    ...
    // The dogfood op carried the route_populate kind + candidates param.
    const op = JSON.parse(await readFile(opLog, "utf8"));
    expect(op.kind).toBe("route_populate");
  ...
  it("sealDiffusionReceipt records exactly ONE engine invocation (no second shell)", async () => {
    ...
    await writeFile(
      bin,
      `#!/usr/bin/env bash\ncat > /dev/null\necho call >> ${JSON.stringify(callLog)}\necho '{"ok":true,"kind":"route","route":{"day_name":"route_populate"},"receipt":"sha256:once","extra":{"data":{"ok":true,"populated":[{"name":"u","value_pointer":"arg://u"}]}}}'\n`,
      "utf8",
    );
  ...
  it("sealDiffusionReceipt returns ok=false + empty picks gracefully on a missing binary", () => {
    const seal = sealDiffusionReceipt("[]", "[]", "x", "/nonexistent/route-bin-xyz");
...
describe.if(W5W6_READY)("v7-cli act fill-form e2e (W5+W6 required)", () => {
  ...
  beforeAll(async () => {
    ...
    const res = await runCli(["act", "go", fixtureUrl, "--json"]);
    if (res.code !== 0) throw new Error(`fixture go failed: ${res.stderr}`);
    sessionId = JSON.parse(res.stdout).session_id;
  });

  afterAll(async () => {
    if (sessionId) await runCli(["act", "close", sessionId, "--json"]).catch(() => {});
    await audit?.stop();
    await clearSessions();
  ...
  it("1. fills username+password from diffusion-picked arg:// pointers (ONE binary call)", async () => {
    ...
    const bin = await writeMockRouteBin(opLog, {
    ...
    const res = await runCli(
      [
        "act", "fill-form", "form#login",
        "--intent", "login to the site",
        "--argScope", JSON.stringify({ username: "alice", password: "hunter2", nickname: "al" }),
        "--session", sessionId,
        "--json",
      ],
      { UNBROWSE_API_URL: audit.url, UNBROWSE_ROUTE_BIN: bin },
    );
    expect(res.code).toBe(0);
    const out = JSON.parse(res.stdout);
    expect(out.ok).toBe(true);
    expect(out.subcommand).toBe("act fill-form");
    expect(out.op_kind).toBe("act:fill_form");
  ...
  it("2. --dry-run proposes pointers but does NOT inject", async () => {
    const bin = await writeMockRouteBin(opLog, { username: "arg://username", password: "arg://password" });
  ...
  it("3. CANARY value is injected but NEVER leaks into any output", async () => {
    const bin = await writeMockRouteBin(opLog, { username: "arg://canary" });
    const res = await runCli(
      [
        "act", "fill-form", "form#login",
        "--intent", "login",
