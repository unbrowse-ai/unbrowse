...
// server returned a non-empty body with real data, the substrate must NOT
...
// BOTH: the body it can read and the substrate's "this drifted, may need
...
// response body but the substrate served a 200-wrapped error envelope
...
describe("execute drift -> body preservation (W1 retrieval truth-telling)", () => {
  test("breaking drift + real-data body: data is the real body, not the schema_drift envelope", async () => {
    ...
    try {
      ...
      // The substrate may still flip success=false to be coherent with
      ...
      // data (the substrate's role is to surface evidence + signals; the
  ...
  test("breaking drift on a GraphQL error envelope: still replaces body (legitimate no-data case)", async () => {
    ...
    // substrate should still surface a graphql_error_envelope wrapper so
    ...
    try {
      ...
      // so the substrate's graphql_error_envelope wrapper is the right
