/**
 * W9-C — v7 CLI tests for `act execute` / `act auth-capture` /
 * `act proxy-rotate`.
 *
...
describe("v7-cli proxy-rotate buildIproyalProxyUrl (pure)", () => {
  it("encodes country lock into password and keeps username:password separate", async () => {
    const mod = await import("../src/cli-v7/act/proxy-rotate.js");
...
describe("v7-cli act proxy-rotate — honest 503 when creds missing", () => {
  ...
  it("exits with honest 503-shaped error envelope (no silent fallback)", async () => {
    const res = await runCli(["act", "proxy-rotate", "--json"]);
