...
describe("serverProxyFallback primitive — client points to the server", () => {
  it("fails closed by default because the legacy endpoint terminates TLS", async () => {
    lastProxyBody = null;
    const out = await serverProxyFallback(
      { url: "https://example.com/private-response", method: "GET" },
  ...
  it("POSTs /v1/proxy with proxy:residential + Bearer, parses recovered body", async () => {
    const out = await serverProxyFallback(
      ...
      { apiKey: "ubr_testkey", mode: "residential", env: { UNBROWSE_API_URL: `http://127.0.0.1:${stubPort}`, UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY: "1" } as NodeJS.ProcessEnv },
    );
  ...
  it("returns null when no API key (server would 402 — honest skip)", async () => {
    const out = await serverProxyFallback(
      ...
      { apiKey: "", env: { UNBROWSE_API_URL: `http://127.0.0.1:${stubPort}`, UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY: "1" } as NodeJS.ProcessEnv },
    );
    expect(out).toBeNull();
  });
  ...
  it("returns null under UNBROWSE_DIRECT_EGRESS=1 (explicit opt-out, no proxying)", async () => {
    const out = await serverProxyFallback(
      { url: "https://example.com" },
      { apiKey: "ubr_testkey", env: { UNBROWSE_API_URL: `http://127.0.0.1:${stubPort}`, UNBROWSE_DIRECT_EGRESS: "1", UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY: "1" } as NodeJS.ProcessEnv },
    );
    expect(out).toBeNull();
  });
  ...
  it("returns null on a non-OK server envelope (graceful degrade)", async () => {
    const out = await serverProxyFallback(
      { url: "https://example.com" },
      { apiKey: "ubr_testkey", env: { UNBROWSE_API_URL: `http://127.0.0.1:${stubPort}/nope`, UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY: "1" } as NodeJS.ProcessEnv },
    );
    expect(out).toBeNull();
  });

  it("propagates caller cancellation instead of degrading into another tier", async () => {
    const ctrl = new AbortController();
    ctrl.abort(new DOMException("cancelled", "AbortError"));
    await expect(serverProxyFallback(
      { url: "https://example.com" },
      {
        apiKey: "ubr_testkey",
        signal: ctrl.signal,
        env: { UNBROWSE_API_URL: `http://127.0.0.1:${stubPort}`, UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY: "1" } as NodeJS.ProcessEnv,
      },
...
describe("executeEndpoint 429 → client relies on server iProyal fallback", () => {
  it("falls through to the server proxy and surfaces the recovered body", async () => {
    const prev = {
      ...
      terminating: process.env.UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY,
    };
    ...
    process.env.UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY = "1";
    try {
      ...
      for (const [k, v] of [["UNBROWSE_API_KEY", prev.key], ["UNBROWSE_API_URL", prev.url], ["UNBROWSE_PROXY_URL", prev.proxy], ["UNBROWSE_ALLOW_TERMINATING_SERVER_PROXY", prev.terminating]] as const) {
