 ...
 * the wallet below and covenants it vertically (src/values/wallet-hierarchy.ts).
 * Proves: each layer's wallet is the deterministic child of its parent (ownership —
 * the parent re-derives the exact child it signed); the vertical covenant chain
...
describe("wallet ownership hierarchy", () => {
  ...
  it("the vertical covenant chain verifies from the root", () => {
    const root = seed();
    expect(verifyLayerTree(buildLayerTree(root, LAYERS), rootPubkey(root))).toBe(true);
  });

  it("OWNERSHIP is provable: the parent re-derives the exact child it covenanted", () => {
    const root = seed();
    const tree = buildLayerTree(root, LAYERS);
  ...
  it("swapping any layer's wallet breaks the covenant chain", () => {
    const root = seed();
    const tree = buildLayerTree(root, LAYERS);
