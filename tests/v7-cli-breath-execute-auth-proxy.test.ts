...
import { evaluateExecuteOutcome } from "../src/cli-v7/breath/execute.js";
...
describe("v7-cli execute terminal truth", () => {
  const settled = { terminal: true, frontier: "settled" };
  const receipt = { ok: true };

  it("fails closed for invalid transport and upstream HTTP failures", () => {
    expect(evaluateExecuteOutcome({ status: 0, replayReceipt: receipt, contract: settled }))
      .toMatchObject({ error: "transport_status_invalid", blocker: "transport" });
    expect(evaluateExecuteOutcome({ status: 403, replayReceipt: receipt, contract: settled }))
      .toMatchObject({ error: "http_error", blocker: "upstream_http" });
  });

  it("fails closed for negative receipts and unsettled contracts", () => {
    expect(evaluateExecuteOutcome({
      status: 200,
      replayReceipt: { ok: false, bindingMissing: "trace" },
      contract: settled,
    })).toMatchObject({ error: "replay_receipt_invalid", blocker: "replay_receipt" });
    expect(evaluateExecuteOutcome({
      status: 200,
      replayReceipt: receipt,
      contract: { terminal: false, frontier: "adjudicate" },
    })).toMatchObject({ error: "contract_unsettled", blocker: "contract" });
  });

  it("settles only when HTTP, receipt, and contract witnesses agree", () => {
    expect(evaluateExecuteOutcome({ status: 204, replayReceipt: receipt, contract: settled })).toBeNull();
  });
});
