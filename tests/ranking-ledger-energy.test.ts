/**
 * Witness: the learned "what-worked" ledger energy is wired live into rankEndpoints.
 *
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "unbrowse-ledger-energy-"));
  process.env.UNBROWSE_TRACES = dir;
  delete process.env.UNBROWSE_LEDGER_ENERGY;
  __resetLedgerCache();
});
...
test("ledger energy: a route that always succeeded outscores one that always failed", () => {
  ...
  const good = ledgerEnergy(idx, "x.com", "good-ep");
  const bad = ledgerEnergy(idx, "x.com", "bad-ep");
  expect(good).toBeGreaterThan(0.5);
  expect(bad).toBeLessThan(0.5);
  expect(good).toBeGreaterThan(bad);
});

test("ledger energy: cold route (no history) is exactly NEUTRAL", () => {
  const idx = loadLedgerIndex(dir); // empty ledger
  expect(ledgerEnergy(idx, "never.com", "unseen-ep")).toBe(LEDGER_NEUTRAL);
  // a single observation is below MIN_EVIDENCE → still neutral (no premature opinion)
  writeTrace("one", "y.com", "lonely-ep", "success");
  expect(ledgerEnergy(loadLedgerIndex(dir), "y.com", "lonely-ep")).toBe(LEDGER_NEUTRAL);
});

test("ledger energy: disabled flag forces neutral", () => {
  for (let i = 0; i < 5; i++) writeTrace(`g${i}`, "x.com", "good-ep", "success");
  __resetLedgerCache();
  process.env.UNBROWSE_LEDGER_ENERGY = "0";
  expect(ledgerEnergyCached("x.com", "good-ep")).toBe(LEDGER_NEUTRAL);
});
...
test("ledger energy: the REAL local ledger loads and yields in-range confidences", () => {
