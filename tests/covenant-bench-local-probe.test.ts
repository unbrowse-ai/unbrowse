...
// (commit 37d7d3c0d).  + -25 — a falsifiable
...
describe("covenant bench_local_probe — firmament guard", () => {
  test.skipIf(!covenantInstalled())(
    ...
    () => {
      ...
      expect(r.covenant?.day).toBe(2); //  firmament
      expect(r.covenant?.witness?.ref).toBe("verse:");
  ...
  test("(B) kinds.ts bench_local_probe block emits no verdict token in executable fields", () => {
    ...
    expect(executableLines).toContain('default_witness: "corroborated"');
  });
});
