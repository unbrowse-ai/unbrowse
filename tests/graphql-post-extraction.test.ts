...
import { classifyGraphQLOperation, endpointIsConfirmedReadOnly } from "../src/site-policy.js";
...
describe("GraphQL operation safety semantics", () => {
  it("treats explicit and shorthand queries as confirmed reads", () => {
    for (const query of ["query Countries { countries { code } }", "{ countries { code } }"]) {
      const endpoint = { method: "POST", url_template: "https://countries.trevorblades.com/graphql", body: { query }, idempotency: "unsafe" } as never;
      expect(classifyGraphQLOperation(endpoint)).toBe("query");
      expect(endpointIsConfirmedReadOnly(endpoint)).toBe(true);
    }
  });

  it("keeps mutations and persisted/ambiguous POST operations fail-closed", () => {
    const mutation = { method: "POST", url_template: "https://example.com/graphql", body: { query: "mutation DeleteThing { deleteThing }" }, idempotency: "unsafe" } as never;
    const ambiguous = { method: "POST", url_template: "https://example.com/graphql", body: { operationName: "Thing" }, idempotency: "unsafe" } as never;
    expect(classifyGraphQLOperation(mutation)).toBe("mutation");
    expect(endpointIsConfirmedReadOnly(mutation)).toBe(false);
    expect(classifyGraphQLOperation(ambiguous)).toBe("unknown");
    expect(endpointIsConfirmedReadOnly(ambiguous)).toBe(false);
  });
});
