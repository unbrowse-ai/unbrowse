 ...
 *
 * Scope (load-bearing):
 *   1. End-to-end smoke: `act go https://example.com` writes a session
 *      file with a valid sessionId; the target is alive.
 *   2. `eval snap` after `act go` returns an AX tree containing the
 *      page title.
 *   3. `act fill` with `arg://username` + `--arg username=alice`:
 ...
 *      present.
 *   4. `act close` after `act go`: session file deleted, the Chrome
 *      process count drops.
...
import { deriveContextHash } from "../src/cli-v7/act/fill.js";
...
describe.if(W5W6_READY)("v7-cli browse loop (e2e — W5+W6 required)", () => {
  ...
  it("1. `act go` writes a session record with a valid sessionId", async () => {
  ...
  it("3. `act fill arg://username` posts a valid AuditFillBody (no value)", async () => {
    const beforeCount = audit.bodies.length;
    const res = await runCli(
      [
        "act", "fill", "body", "arg://username",
  ...
  it("4. `act close` deletes the session file", async () => {
    const res = await runCli(["act", "close", sessionId, "--json"]);
    expect(res.code).toBe(0);
    const recPath = join(SESSIONS_DIR, `${sessionId}.json`);
  ...
  it("5. CANARY: fill with a leak-detector value — value MUST NOT appear in any output", async () => {
    ...
    const openRes = await runCli(["act", "go", "https://example.com", "--json"]);
    ...
    await runCli(["act", "close", sid, "--json"]);
  }, 60_000);
});
