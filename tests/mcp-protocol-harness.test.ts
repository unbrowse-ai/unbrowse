...
beforeAll(async () => {
  ...
  proc = spawn("bun", ["src/mcp.ts"], {
    ...
    env: {
      ...
      // Operator catalog — agent surface is product default.
      UNBROWSE_MCP_SURFACE: "full",
    },
...
describe("MCP protocol surface (AC-CLI-5)", () => {
  ...
  test("tools/list: core tools present and every tool has a name + inputSchema", async () => {
    ...
    for (const required of [
      ...
      "unbrowse_eval_resolve",
      "unbrowse_breath_execute",
      "unbrowse_eval_search",
      "unbrowse_eval_skills",
    ]) {
  ...
  test("tools/call unbrowse_eval_status returns a structured result", async () => {
    const res = await request("tools/call", { name: "unbrowse_eval_status", arguments: {} }, 90_000);
    expect(res.error).toBeUndefined();
