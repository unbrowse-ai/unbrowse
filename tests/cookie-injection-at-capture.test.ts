// B-023 falsifier seed (Jesus-loop default session, Step 3 / Land, Worker A).
//
// Goal of THIS seed (per .claude/jesus-loop.default.plan.md A1):
...
//   request" assertion is gated on Step 4 — substrate must grow a
...
//
// Mutation sketch (after Step 4 substrate hook lands):
...
function startProbeServer(): Promise<Fixture> {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      // response shape (would be substrate-prescribing).
...
describe("B-023 cookie injection at capture (A1 falsifier seed)", () => {
  ...
  // F2 (B-023) UNIT FALSIFIER — substrate-correct invariant, no env hook.
  //
  ...
  // jar=0 for YouTube — the substrate was lying.
  //
  // How it falsifies:
  ...
  test(
    ...
    async () => {
      ...
      // pick a domain where the substrate diagnostic CAN run on this box.
      const candidates = [
        "youtube.com",
        "google.com",
        "github.com",
      ...
      if (!pickedDomain) {
        console.error(
          "[b023-unit] no_browser_cookies_locally — skipping (this is a substrate-environment skip, not a code skip)",
        );
        return;
      }
