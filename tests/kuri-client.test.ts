...
// Binary-path resolution lives in resolve-paths.ts, not the browser client.
import * as kuriPaths from "../src/kuri/resolve-paths.js";
...
describe("kuri client", () => {
  ...
  it("prefers a packaged Kuri binary when present", () => {
    ...
    expect(kuriPaths.findKuriBinary()).toBe(binaryPath);
    expect(kuriPaths.findKuriBinary()).toBe(binaryPath);
  });
  ...
  it("finds the vendored Kuri binary one level up when packageRoot is the bundled runtime dir", () => {
    ...
    expect(kuriPaths.getKuriBinaryCandidates()).toContain(binaryPath);
    expect(kuriPaths.findKuriBinary()).toBe(binaryPath);
  });
...
`);
  ...
  it("falls back to raw Chrome CDP tab creation when /tab/new fails", async () => {
    ...
    process.env.KURI_EXTERNAL_PORT = "7794";
    process.env.CDP_URL = "ws://127.0.0.1:9222/devtools/browser/test";
...
`);
  ...
  // A foreign broker must never be killed, but health without CDP or tabs does
  // not prove capture observability. Reject it so startOn can launch an isolated
  // owned broker on a different port.
  it("rejects (never terminates) a healthy but unobservable FOREIGN broker", async () => {
    const seen: string[] = [];
    const state = {
      process: null,
      port: 7700,
  ...
  it("admits broker capture only from explicit observable capability", () => {
    expect(kuri.assessBrokerCaptureCapability({
      owned: false,
      cdpAvailable: false,
      registeredTabCount: 0,
    })).toEqual({
      reusable: false,
      observable: false,
      reason: "foreign_broker_unobservable",
    });
    expect(kuri.assessBrokerCaptureCapability({
      owned: false,
      cdpAvailable: true,
  ...
  it("rejects a foreign broker even when ambient user Chrome CDP is restored", async () => {
    const state = {
      process: null,
      port: 7700,
      cdpPort: null,
  ...
  // U-4: stale tabs on a FOREIGN broker prove neither fresh-tab nor capture
  // agency. Reject it without killing it so startup can isolate safely.
  it("rejects (never terminates) a healthy FOREIGN broker with only stale tabs in attach mode", async () => {
    const seen: string[] = [];
    const state = {
      process: null,
      port: 7700,
  ...
  it("rejects a healthy FOREIGN broker when Chrome launch leaves a non-responsive CDP port", async () => {
    const seen: string[] = [];
    const state = {
      process: null,
      port: 7700,
    ...
    // The caller falls through to an isolated owned broker; the foreign broker
    // remains untouched.
    expect(reused).toBe(false);
    expect(state.ready).toBe(false);
    expect(state.cdpPort).toBe(null);
