...
describe("frontend API and CLI wiring", () => {
  ...
  it("keeps the public skill instructions aligned with explicit CLI registration", async () => {
    const response = await getSkillMd();
    const body = await response.text();

    expect(body).toContain("npm install -g unbrowse@preview && unbrowse setup --host mcp");
    expect(body).toContain("The skills command installs agent instructions only.");
    expect(body).toContain("unbrowse account --register");
    expect(body).toContain("unbrowse resolve --intent");
