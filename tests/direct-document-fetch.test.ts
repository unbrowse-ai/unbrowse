...
describe("direct-document seed", () => {
  ...
  test("rejects CF 525 SSL handshake origin-failure pages (soft-green quality)", () => {
    // Real shape observed on api.spacexdata.com: 200-looking HTML document
    // with title/body advertising SSL handshake failure — must not task_ok.
    const pad = "What can I do? If you're a visitor of this website: Please try again. ".repeat(80);
    const cf525 = `<!doctype html><html><head><title>spacexdata.com | 525: SSL handshake failed</title></head><body>
    ...
    if (!result.rejected) throw new Error("expected rejection");
    expect(result.reason).toBe("interstitial_detected");
    expect(String(result.evidence?.interstitial_signal ?? "").toLowerCase()).toMatch(
      /ssl handshake failed|error code\s*525/,
    );
  });
