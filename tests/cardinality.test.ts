...
describe("concrete-resource-path shape", () => {
  ...
  it("urlHasConcreteResourcePath: non-root concrete vs root/listing", () => {
    ...
    expect(urlHasConcreteResourcePath("https://www.korea.kr/news/policyNewsList.do")).toBe(false);
    expect(urlHasConcreteResourcePath("https://example.com/actions/searchResults.do")).toBe(false);
  ...
  it("admits filtered plural API collections without weakening detail paths", () => {
    expect(requestExpectsCollection("https://api.github.com/repos/unbrowse-ai/unbrowse/issues?state=open&per_page=100", "get open issues")).toBe(true);
    expect(requestExpectsCollection("https://api.example.test/issues/123?state=open", "get issue details")).toBe(false);
    expect(requestExpectsCollection("https://api.example.test/users/octocat?tab=repositories", "get user details")).toBe(false);
    expect(requestExpectsCollection("https://api.example.test/jobs", "return exactly 5 jobs")).toBe(true);
    expect(requestedResultCount("return exactly 5 jobs")).toBe(5);
  });
