...
// rejects such recoveries so the platform keeps emitting the actionable
// schema_drift_recapture_required envelope instead of overwriting it with
// a junk shell.
//
// Real-runtime: live helper functions, no mocks. platform primitive
...
describe("recoveredDataMatchesOriginalShape: platform-faithful drift recovery gate", () => {
  ...
  test("EDGE CASE: tiny schema (<= 2 keys) skips the check; confidence alone gates", () => {
    ...
    // overlap signal from. the platform must not block recovery here
