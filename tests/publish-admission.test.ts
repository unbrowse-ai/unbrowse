...
describe("selectMarketplacePublishEndpoints", () => {
  test("keeps discovered and captured routes out until replay verification succeeds", () => {
    const skill = makeSkill({
      endpoints: [
        makeEndpoint({ endpoint_id: "captured-only", verification_status: "unverified" }),
        makeEndpoint({ endpoint_id: "discovered-only", verification_status: "pending", url_template: "https://www.acme-corp.com/api/search?q={q}" }),
        makeEndpoint({ endpoint_id: "replay-verified", url_template: "https://www.acme-corp.com/api/items?page={page}" }),
      ],
    });

    const selection = selectMarketplacePublishEndpoints(skill);
    expect(selection.endpoints.map((endpoint) => endpoint.endpoint_id)).toEqual(["replay-verified"]);
    expect(selection.stats.by_reason.not_replay_verified).toBe(2);
  });
  ...
  test("drops stale, noisy, fragile, duplicate, and over-limit endpoints", () => {
    ...
    expect(selection.stats.by_reason.not_replay_verified).toBe(1);
    expect(selection.stats.by_reason.family_dedup).toBe(0);
