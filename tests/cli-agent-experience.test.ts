...
describe("cli agent experience", () => {
  it("returns a successful default get from the canonical resolve call", async () => {
    await withStubServer(async (req) => {
      const path = new URL(req.url).pathname;
      if (path === "/v1/intent/resolve") {
        return Response.json({
          trace: { success: true, skill_id: "direct-fetch", endpoint_id: "direct-fetch" },
          source: "direct-fetch",
          result: { status: "ok", title: "Example Domain" },
          next_action: { command: "unbrowse execute --skill direct-fetch --endpoint direct-fetch" },
        });
      }
      if (path === "/health") return Response.json({ status: "ok" });
      return new Response("unexpected duplicate fallback", { status: 500 });
      ...
      expect(out.body.result).toEqual({ status: "ok", title: "Example Domain" });
      expect(out.body.agent_path).toEqual({
        step: 1,
        status: "complete",
        primary: 'unbrowse "<task>" --url <url>',
      });
      expect(out.body.next_action).toBeUndefined();
  ...
  it("does not replay the canonical resolver's returned shortlist through a second execute call", async () => {
    await withStubServer(async (req) => {
      const path = new URL(req.url).pathname;
      if (path === "/v1/intent/resolve") {
        return Response.json({
          trace: { success: true, skill_id: "site-skill", endpoint_id: "site-read" },
          source: "live-capture",
          result: { status: "ok", items: [{ title: "one answer" }] },
          skill: { skill_id: "site-skill" },
          available_endpoints: [{ endpoint_id: "site-read", method: "GET" }],
        });
      }
      if (path === "/health") return Response.json({ status: "ok" });
      return new Response("default get must not execute twice", { status: 500 });
    }, async (baseUrl, requests) => {
      const out = await runCli(baseUrl, [
        "breath", "get",
        "Return the answer",
      ...
      expect(out.code).toBe(0);
      expect(out.body.result).toEqual({ status: "ok", items: [{ title: "one answer" }] });
      expect(requests.map((entry) => entry.path).filter((path) => path !== "/health")).toEqual([
        "/v1/intent/resolve",
      ]);
    });
  ...
  it("keeps the default get path on one canonical resolve call and one recovery action", async () => {
    await withStubServer(async (req) => {
      const path = new URL(req.url).pathname;
      if (path === "/v1/intent/resolve") {
        return Response.json({
      ...
      expect(out.body.next_step).toBe('unbrowse go --url "https://example.com"');
      expect(out.body.agent_path).toMatchObject({
        step: "next_step",
        status: "recovery",
        action: 'unbrowse go --url "https://example.com"',
      });
      expect(JSON.stringify(out.body)).not.toContain("suggested_commands");
      expect(requests.map((entry) => entry.path).filter((path) => path !== "/health")).toEqual([
        "/v1/intent/resolve",
      ]);
    });
  ...
  it("maps auth-required get results to the single auth recovery move", async () => {
    await withStubServer(async (req) => {
      const path = new URL(req.url).pathname;
      if (path === "/v1/intent/resolve") {
        return Response.json({
          trace: { success: false },
          result: { error: "auth_required", login_url: "https://example.com/login" },
        });
      }
      if (path === "/health") return Response.json({ status: "ok" });
      return new Response("unexpected auth side effect", { status: 500 });
    }, async (baseUrl, requests) => {
      const out = await runCli(baseUrl, [
        "breath", "get",
        "Read my account",
      ...
      expect(out.code).toBe(1);
      expect(out.body.next_step).toBe('unbrowse auth "https://example.com/login"');
      expect(out.body.agent_path).toMatchObject({ step: 2, status: "recovery" });
      expect(requests.map((entry) => entry.path).filter((path) => path !== "/health")).toEqual([
        "/v1/intent/resolve",
      ]);
    });
