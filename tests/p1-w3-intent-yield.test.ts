...
describe("P1 W3 cluster #2 — semanticIntentAdjustment numeric behavior", () => {
  it("hard-binds search, detail, and feed URL classes", () => {
    const search = { url_template: "https://crates.io/api/v1/crates?q=serde", method: "GET", semantic: { action_kind: "search" } } as never;
    const detail = { url_template: "https://crates.io/api/v1/crates/{name}", method: "GET", semantic: { action_kind: "detail" } } as never;
    const feed = { url_template: "https://crates.io/feeds/crates.xml", method: "GET" } as never;

    expect(routeIntentCompatibility(search, "get crate detail", "https://crates.io/crates/serde")).toBe("incompatible");
    expect(routeIntentCompatibility(detail, "search crates", "https://crates.io/search?q=serde")).toBe("incompatible");
    expect(routeIntentCompatibility(search, "latest crates RSS", "https://crates.io/feeds/crates.xml")).toBe("incompatible");
    expect(routeIntentCompatibility(feed, "latest crates RSS", "https://crates.io/feeds/crates.xml")).not.toBe("incompatible");
  });
