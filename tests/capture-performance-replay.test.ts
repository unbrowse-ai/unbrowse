...
import { decodeCdpResponseBody, isReplayableApiUrl, mergePassiveCaptureData, normalizeCaptureTimestamp, selectPerformanceReplayCandidates } from "../src/capture/index.js";
import { findRecordCollection } from "../src/capture/reveng-local.js";

describe("performance api replay selection", () => {
  it("fails open on malformed extension timestamps without losing captured endpoints", () => {
    const fallback = "2026-08-03T00:00:00.000Z";
    expect(normalizeCaptureTimestamp(undefined, fallback)).toBe(fallback);
    expect(normalizeCaptureTimestamp("not-a-date", fallback)).toBe(fallback);
    expect(normalizeCaptureTimestamp(1_700_000_000, fallback)).toBe("2023-11-14T22:13:20.000Z");
  });

  it("decodes base64 CDP response bodies before endpoint inference", () => {
    const json = JSON.stringify({ results: [{ id: 1, title: "A" }, { id: 2, title: "B" }] });
    expect(decodeCdpResponseBody({ body: Buffer.from(json).toString("base64"), base64Encoded: true })).toBe(json);
    expect(decodeCdpResponseBody({ body: json, base64Encoded: false })).toBe(json);
  });

  it("recognizes heterogeneous result rows that retain a stable record spine", () => {
    const jobs = Array.from({ length: 10 }, (_, index) => ({
      uuid: `job-${index}`,
      title: `Engineer ${index}`,
      ...(index % 2 === 0 ? { salary: { minimum: 5000 + index } } : {}),
      ...(index % 3 === 0 ? { schemes: [{ id: index }], metadata: { rank: index } } : {}),
      [`optional_${index}`]: true,
    }));
    const collection = findRecordCollection({ results: jobs });
    expect(collection?.path).toEqual(["results"]);
