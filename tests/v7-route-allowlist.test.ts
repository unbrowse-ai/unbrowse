 ...
 *
 *    — *"the waters gathered... and the dry land appear."* The
 ...
 *   op (`context_fetch`, etc.) fails closed with `unknown_op`, never
 *   reaching a handler/binary.
 *
 * Three surfaces under test:
 *   1. CLI/MCP dispatcher (src/cli-v7/dispatch/index.ts) — allowlist gate.
 *   2. Backend /v1/route (backend/src/routes/route.ts) — public-reject,
 *      aiko-key-allow.
...
import { routeRoutes } from "../backend/src/routes/route.js";
import {
...
// The 8 headline mechanism kinds (INTERNET_LAYER_ROUTE_SURFACE.md §1 HIDDEN).
...
describe("dispatcher allowlist — derived from KIND_MAP's 37 rows", () => {
  ...
  it("dispatchByKind('act:navigate', ...) — internet op dispatches normally", async () => {
    // act:navigate is on the allowlist → it loads its handler. We don't
    ...
    const res = await dispatchByKind("act:navigate", { url: "https://example.com" }, { json: true });
    expect(res.op_kind).toBe("act:navigate");
...
// ─── Backend /v1/route helpers (mirrors v7-route-opacity.test.ts) ──────
...
async function postRoute(app: Hono, env: Env, body: unknown, headers: Record<string, string> = {}) {
  const res = await app.fetch(
    new Request("http://test.local/v1/route", {
      method: "POST",
...
// ─── 2. Backend /v1/route — public-reject, aiko-key-allow ────────────────

describe("backend /v1/route — mechanism public-reject / aiko-allow", () => {
  test("public caller + mechanism action (observe:context_fetch) → unknown_op", async () => {
    const app = mountApp();
    const env = makeEnv();
    const { envelope } = await buildEnvelope("observe:context_fetch");

    const { status, json } = await postRoute(app, env, envelope);
    expect(status).toBe(400);
    expect(json.error).toBe("unknown_op");
    // Opacity: no mechanism vocabulary leaks in the rejection.
    const blob = JSON.stringify(json);
    expect(blob).not.toContain("context_fetch");
  ...
  test("aiko-key caller + mechanism action → allowed through (not unknown_op)", async () => {
    ...
    // `observe:recall_route` — an aiko insider may dispatch it; it falls into
    // the observe eval-read path. The key point: NOT rejected as unknown_op.
    const { envelope } = await buildEnvelope("observe:recall_route");
    const { json } = await postRoute(app, env, envelope, {
    ...
    if (json.ok === true) {
      expect(json.action).toBe("recall_route");
    }
  });
...
describe("leak-guard.sh — mechanism keyword gate", () => {
  ...
  test("planted functional leak in src/ → exit 1 + names the file", () => {
    ...
    writeFileSync(probe, 'export const probe = "context_fetch";\n');
    try {
      const { code, out } = runGuard();
      expect(code).toBe(1);
      expect(out).toContain("MECHANISM LEAK");
      expect(out).toContain("__w34b_leak_probe__.ts");
      expect(out).toContain("context_fetch");
    } finally {
  ...
  test("comment-only mention does NOT trip the guard (stripped from bundle)", () => {
    ...
    writeFileSync(probe, '// this is NOT context_fetch, just a doc note\nexport const ok = 1;\n');
    try {
      const { code } = runGuard();
      expect(code).toBe(0);
    } finally {
