...
describe("withExecuteDeadline — overall execute backstop", () => {
  ...
  test("aborts cooperative work before returning the timeout receipt", async () => {
    let observedAbort = false;
    const result = await withExecuteDeadline(
      async (signal) => await new Promise<string>((_resolve) => {
...
describe("replayRecipe — curl-impersonate rescue on native-fetch stall", () => {
  test("caller cancellation aborts a proven mutation recipe and never falls through", async () => {
    let aborted = false;
    ...
    const recipe = {
      method: "POST", headers: {}, body: { action: "delete" },
      response_signal: { status: 200 }, captured_at: new Date().toISOString(),
    } as any;
    const pending = replayRecipe(recipe, "https://x.test/mutate", [], {}, {}, 15_000, undefined, controller.signal);
    controller.abort(new Error("execution_timeout"));
    const result = await pending;
    expect(aborted).toBe(true);
    expect(result).toMatchObject({ status: 0, data: { error: "execution_cancelled" } });
  });
