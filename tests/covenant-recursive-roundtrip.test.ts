...
// Day-4 Worker-3 canary — Luke 15:4. The lost sheep here is the absence
...
// `covenant.parent` field references the parent receipt sha256.
//
// Worker-1's Day-4 canary (tests/covenant-bench-local-probe.test.ts)
...
//
// Chain under test: `correction` (no parent_param, default Prov 9:9) →
// `repent` (parent_param = "correction_receipt", default Acts 3:19).
// The covenant binary stores patch as the body; the envelope carries
// parent verbatim. Inputs are deterministic; the only non-deterministic
// fields (time, sigs) are not asserted.
//
// Falsifier intent: if any future commit to ~/.local/bin/covenant or
// /Users/lekt9/Projects/covenant/{interpreter.ts,kinds.ts} drops
...
describe("covenant recursive round-trip — lineage parent guard", () => {
  test.skipIf(!covenantInstalled())(
    "child `repent` receipt's covenant.parent references the parent `correction` receipt sha",
    () => {
      ...
      expect(parent.covenant?.day_name).toBe("correction");
      expect(parent.covenant?.parent).toBeNull(); // root of this chain
      ...
      expect(child.covenant?.day_name).toBe("repent");

      // Load-bearing assertion: the parent_param flow MUST surface the
      // parent receipt sha into covenant.parent on the child envelope.
      expect(child.covenant?.parent).toBe(parentReceipt);
