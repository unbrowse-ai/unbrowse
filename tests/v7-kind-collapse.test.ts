 ...
 * renamed the route-specific columns OUT of the public bundle:
 *   route_kind       → op_kind   (values `act:navigate`, `eval:snap`, …)
 *   route_base_kind  → op_class  (generic values kept: actuate/observe/build)
 ...
 * route-mechanism string leaks into the public surface.
 */
...
describe("W33-B op-kind — op_class + action columns", () => {
  ...
  it("op_class ↔ verb invariant: build→build, actuate→act, observe→eval", () => {
    for (const entry of KIND_MAP) {
      if (entry.op_class === "build") expect(entry.verb).toBe("build");
      if (entry.op_class === "actuate") expect(entry.verb).toBe("act");
      if (entry.op_class === "observe") expect(entry.verb).toBe("eval");
    }
  });

  it("op_kind is `<verb>:<action>` — unbrowse-native, no route prefix", () => {
    for (const entry of KIND_MAP) {
...
describe("W33-B op-kind — no route-mechanism leak in the public surface", () => {
  it("the kind-map carries NO route-mechanism strings", () => {
  ...
  it("the public cli-v7 surface (src/cli-v7/**) carries NO route-mechanism strings", () => {
    const FORBIDDEN = /route_kind|route_base_kind|context_fetch|route_populate|route_rank|actuate_|observe_/;
    ...
    for (const file of walk(root)) {
      ...
      expect(m, `${file} leaks route-mechanism string "${m?.[0]}"`).toBeNull();
    }
  });
});
