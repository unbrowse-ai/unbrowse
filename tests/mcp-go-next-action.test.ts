...
describe("addGoNextStepHints: session-open tool-availability surfacing", () => {
  test("separates navigation success from an observed logged-out auth wall", () => {
    const out = addGoNextStepHints({
      ok: true,
      session_id: "s-auth",
      page: { text: '<main id="LoggedOutShell">Sign in to X</main>' },
      cookies_injected: 0,
    }, {}) as Record<string, unknown>;
    expect(out).toMatchObject({ operational_ok: true, auth_ok: false, auth_outcome: "auth_required", auth_required: true, session_expired: false });
  });

  test("reports an observed logged-out shell as expired when credentials were presented", () => {
    const out = addGoNextStepHints({
      ok: true,
      page: { text: "Log in to X" },
      cookies_preserved: 2,
    }, {}) as Record<string, unknown>;
    expect(out).toMatchObject({ operational_ok: true, auth_ok: false, auth_outcome: "session_expired", auth_required: false, session_expired: true });
  });
  test("preserves the browser's independently adjudicated unknown outcome", () => {
    const out = addGoNextStepHints({
      ok: true,
      auth_outcome: "unknown",
      page: { text: "Keyboard shortcuts Close navigation menu" },
      cookies_preserved: 2,
    }, {}) as Record<string, unknown>;
    expect(out).toMatchObject({ operational_ok: true, auth_ok: null, auth_outcome: "unknown" });
  });
  ...
  test("surfaces the now-available session tools derived from SESSION_TOOL_NAMES (not a re-typed list)", () => {
    ...
    expect(avail).toContain("unbrowse_breath_close");
    expect(avail).toContain("unbrowse_eval_snap");
  });
  ...
  test("the close->index requirement is stated so a non-relisting agent still learns it", () => {
    ...
    expect(blob).toContain("unbrowse_breath_close");
