 ...
 * signature + signatureScheme + nonce) per Day-4 firmament rules.
 *
 ...
 *
 * Chrome-touching steps (`breath go`, `eval snap`, `breath fill`,
 * `breath session-park`, `breath session-restore`, `breath close`) are
 * honestly SKIPPED unless `UNBROWSE_W5W6_READY=1` AND a real Chrome is
 * present. Their wiring is exercised by sibling tests
 * (v7-cli-breath-session-park, v7-stateless-screenshot, etc.) — this
 ...
 * structured assertions. The Day-7 Sabbath agent reads the test output
...
describe("Day-6 Dominion: A1 + canary across the full sequence", () => {
  it("A1: $HOME/.unbrowse holds NO non-wallet/non-tmp files after the eval-surface sweep", async () => {
    ...
    try {
      ...
      // Print rows for the Day-7 Sabbath agent's witness regardless.
      if (!a1.ok) {
...
describe.if(W5W6_READY)(
  "Day-6 Dominion: full breath-loop (Chrome-required) — gated on UNBROWSE_W5W6_READY=1",
  () => {
    it("breath go → eval snap → breath session-park → kill chrome → breath session-restore → eval snap → breath close", async () => {
      // Out-of-scope for the unit test runner — exercised by the sibling
      // tests v7-cli-breath-session-park (W23 e2e), v7-stateless-screenshot,
...
describe.if(!W5W6_READY)(
  ...
  () => {
    it("records the skip reason for the Day-7 Sabbath agent", () => {
      // eslint-disable-next-line no-console
      console.log(
        "[SKIP] breath go/snap/fill/session-park/session-restore/close — " +
        "W5+W6 chrome arm not gated on; covered by sibling tests " +
        "(v7-cli-breath-session-park, v7-stateless-screenshot, v7-stateless-session-writers, " +
