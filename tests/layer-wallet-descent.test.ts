 ...
 * each layer signs its OWN action with its OWN wallet, owned + covenanted by the
 ...
 * covenant + horizontal chain verify from the root; tamper / wrong-root fail closed.
 */
...
describe("layer-wallet descent (vertical ownership ∘ signed descent)", () => {
  ...
  it("the full descent verifies (ownership covenant + per-layer action sig + chain)", () => {
    const { root, chain } = descendByLayerWallet("buy widget", OPS);
  ...
  it("splicing in a wallet the parent never covenanted fails", () => {
    const { root, chain } = descendByLayerWallet("buy widget", OPS);
