/**
 * cli-surface.test — the witness that the unbrowse CLI is shaped to the architecture
 * atoms and exposes ONLY what a client needs (src/architecture/cli-surface.ts):
 * every command maps to a verb (build/act/eval) + interrogative, surfaces only
...
import { surfaceFor, classify, verifySurface, knownCommands } from "../src/architecture/cli-surface.js";

// The canonical top-level CLI commands (from `unbrowse --help`) — the surface the
// client sees. The manifest must classify every one (architecture-complete).
const CANONICAL = [
...
describe("CLI architecture surface", () => {
  it("is architecture-complete + minimal (every command an atom, no internal leak)", () => {
  ...
  it("every command resolves to one of the three verbs", () => {
    for (const cmd of knownCommands()) {
      expect(["build", "act", "eval"]).toContain(classify(cmd)!.verb);
    }
  });
  ...
  it("canonical create/act/read commands are the only new root verbs", () => {
    ...
    expect(surfaceFor("act")).toMatchObject({ verb: "act", auth: "sealed" });
