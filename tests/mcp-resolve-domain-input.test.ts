// Day 4 (Luminaries) failing test for AC2 — platform-lie at unbrowse_resolve schema.
...
// is a platform-lie. AC2 (option b) is to remove it from the schema.
...
describe("unbrowse_resolve inputSchema — no platform-lie (AC2)", () => {
  test("inputSchema.properties does NOT advertise `domain`", async () => {
    ...
    expect(
      ...
      "unbrowse_resolve.inputSchema.properties.domain must NOT be advertised — it silently degrades to 'root' cache key (src/orchestrator/index.ts:3446,3669-3672). platform-lie per AC2.",
    ).toBe(false);
  });
});
