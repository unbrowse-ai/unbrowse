...
import { classifyAuthenticatedPage, getStoredAuth, getAuthCookies } from "../src/auth/index.js";
import { adjudicateNavigationOutcome, navigationObservation, targetRequiresAuthenticatedEvidence } from "../src/cli-v7/breath/go.js";

describe("browser navigation truth", () => {
  it("rejects about:blank as an operational success", () => {
    expect(adjudicateNavigationOutcome({ targetUrl: "https://www.linkedin.com/feed/", finalUrl: "about:blank" })).toEqual({
      operationalOk: false,
      taskOk: false,
      error: "navigation_not_committed",
    });
  });

  it("projects runtime evidence without retaining a session or browser object", () => {
    expect(navigationObservation({
      readyState: "complete",
  ...
  it("rejects a vendor challenge document", () => {
    expect(adjudicateNavigationOutcome({
      targetUrl: "https://www.zillow.com/seattle-wa/",
      finalUrl: "https://www.zillow.com/seattle-wa/",
      pageText: "Access denied. Press & Hold to confirm you are a human",
    })).toMatchObject({ operationalOk: false, taskOk: false, error: "challenge_page", blocker: "challenge" });
  });

  it("identifies protected destinations that need authenticated evidence", () => {
    expect(targetRequiresAuthenticatedEvidence("https://github.com/settings/profile")).toBe(true);
    expect(targetRequiresAuthenticatedEvidence("https://x.com/home")).toBe(true);
    expect(targetRequiresAuthenticatedEvidence("https://www.linkedin.com/feed/")).toBe(true);
    expect(targetRequiresAuthenticatedEvidence("https://github.com/openai")).toBe(false);
  });
});

describe("authenticated target outcome", () => {
  it("treats an X LoggedOutShell as rejection, not cookie success", () => {
    const pageText = '<div id="LoggedOutShell">Log in to X</div>';
    expect(classifyAuthenticatedPage({ pageText, hadPresentedCredentials: true })).toBe("session_expired");
    expect(classifyAuthenticatedPage({ pageText, hadPresentedCredentials: false })).toBe("auth_required");
  });
  ...
  it("keeps a generic X shortcuts shell unknown despite presented cookies", () => {
    expect(classifyAuthenticatedPage({
  ...
  it("requires authenticated-only target evidence before success", () => {
    expect(classifyAuthenticatedPage({
  ...
  it("treats an empty login redirect as credential rejection", () => {
    expect(classifyAuthenticatedPage({
      pageText: "",
      hadPresentedCredentials: true,
