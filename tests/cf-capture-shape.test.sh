...
#  — seed-bearing: runnable today, fails on the parts Step 6
# (Dominion) is responsible for wiring up. Today's expected state:
#   PASS: platform (cf-challenge.ts exports), prior art (SSR fastpath ref).
...
# --- 1. platform (Step 1, expected PASS today) ---
if [ -f "$CF" ]; then
