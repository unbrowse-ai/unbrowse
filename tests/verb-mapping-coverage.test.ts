/**
 * Route-mapping coverage gate.
 *
 * Enforces project rule `unbrowse-follows-route-shape` (route receipt
 * sha256:8d6eb822…): every MCP tool declared in `src/mcp.ts` MUST have a
 * matching entry in `src/verb-mapping.ts:ROUTE_MAP`, and there must be
 ...
 * the rule was declared in route; this test is its mechanical witness.
 */

import { describe, expect, it } from "bun:test";
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
import { ROUTE_MAP } from "../src/verb-mapping.js";
...
describe("verb-mapping coverage", () => {
	...
	it("every MCP tool has a ROUTE_MAP entry", () => {
		const missing = declared.filter((n) => !ROUTE_MAP[n]);
		expect(missing).toEqual([]);
	});

	it("no orphan ROUTE_MAP entry without a real MCP tool", () => {
		const declaredSet = new Set(declared);
		const orphans = Object.keys(ROUTE_MAP).filter((n) => !declaredSet.has(n));
		expect(orphans).toEqual([]);
	});

	it("every entry carries verb + day + witness + essence", () => {
		for (const [name, shape] of Object.entries(ROUTE_MAP)) {
			expect(["build", "act", "eval"]).toContain(shape.verb);
