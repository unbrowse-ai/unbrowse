...
import { registrableHost, shouldAutoWalk, pickWalkTarget, anchorHitsToDomain, isWalkCandidateUrl } from "../src/orchestrator/index.js";
...
describe("isWalkCandidateUrl — reject search-provider punctuation artifacts", () => {
  it("rejects an encoded punctuation-only pathname", () => {
    expect(isWalkCandidateUrl("https://www.mycareersfuture.gov.sg/%EF%BC%89%7C")).toBe(false);
  });
  it("keeps meaningful search routes and Unicode paths", () => {
    expect(isWalkCandidateUrl("https://www.mycareersfuture.gov.sg/search?sortBy=new_posting_date")).toBe(true);
    expect(isWalkCandidateUrl("https://example.jp/%E6%B1%82%E4%BA%BA/%E6%96%B0%E7%9D%80")).toBe(true);
    expect(isWalkCandidateUrl("https://example.com/?q=jobs")).toBe(true);
  });
  it("rejects non-web schemes and malformed input", () => {
    expect(isWalkCandidateUrl("javascript:alert(1)")).toBe(false);
    expect(isWalkCandidateUrl("https://example.com/%zz%7C")).toBe(false);
    expect(isWalkCandidateUrl("not a url")).toBe(false);
  });
});
...
describe("pickWalkTarget — prefer a deep page over a bare homepage", () => {
  ...
  it("skips malformed punctuation and walks the valid same-domain search route", () => {
    const req = "https://www.mycareersfuture.gov.sg/";
    const ranked = [
      { url: "https://www.mycareersfuture.gov.sg/%EF%BC%89%7C", score: 1 },
      { url: "https://www.mycareersfuture.gov.sg/search?sortBy=new_posting_date", score: 0.8 },
    ];
    expect(pickWalkTarget(req, ranked)?.url).toBe(
      "https://www.mycareersfuture.gov.sg/search?sortBy=new_posting_date",
    );
  });
});
