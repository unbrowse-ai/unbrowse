...
describe("capture pipeline settings", () => {
  it("defaults to local indexing with checkpoint publish disabled", () => {
  ...
  it("requires confirmation for explicit publish on guarded domains", () => {
    ...
    expect(decideExplicitPublish("linkedin.com", false)).toEqual({
      allowed: false,
      mode: "prompt",
      reason: "Explicit publish requires confirmation for linkedin.com.",
    });

    expect(decideExplicitPublish("github.com", false)).toEqual({
      allowed: false,
      mode: "prompt",
      reason: "Explicit publish requires confirmation for github.com.",
    });

    expect(decideExplicitPublish("example.com", false)).toEqual({
      allowed: false,
      mode: "prompt",
      reason: "Explicit publish requires confirmation for example.com.",
    });
