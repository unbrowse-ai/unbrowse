...
function makeDeps(overrides: {
  ...
  publish?: () => Promise<{ status: "published" | "queued" | "skipped" | "failed"; reason?: string }>;
}): { deps: RunPlannerDeps; calls: DepCalls } {
  const calls: DepCalls = { resolve: 0, execute: 0, capture: 0, browse: 0, publish: 0 };
  ...
  const deps: RunPlannerDeps = {
    ...
    publish: async () => {
      calls.publish++;
      return overrides.publish ? overrides.publish() : { status: "published" };
    },
  };
  return { deps, calls };
}
...
describe("run-planner: golden path", () => {
  it("resolve.hit + safe GET → execute → status ok with full run_plan", async () => {
    ...
    expect(out.run_plan).toEqual([
      ...
      expect.objectContaining({ step: "publish", status: "published" }),
    ]);
  });
});
...
describe("run-planner: canonical action DAG", () => {
  it("executes an evidenced read-only POST, then publishes by default", async () => {
    const { deps, calls } = makeDeps({
      resolve: async () => ({
        status: "hit",
        skill: { skill_id: "skill-read-post" },
        available_endpoints: [{
          endpoint_id: "ep-search",
          method: "POST",
          idempotency: "safe",
          trigger_url: "https://example.com/jobs",
          response_schema: { type: "object" },
          semantic: { action_kind: "search" },
    ...
    expect(out.run_plan.map((s) => s.step)).toEqual(["resolve", "execute", "publish"]);
  });

  it("explicit publish opt-out executes but makes zero marketplace calls", async () => {
    const { deps, calls } = makeDeps({
      resolve: async () => ({ status: "hit", skill: { skill_id: "private" }, available_endpoints: [{ endpoint_id: "ep", method: "GET" }] }),
    });
    const out = await runPlanner({ ...baseInput, policy: { publish_to_marketplace: false } }, deps);
    expect(out.status).toBe("ok");
    expect(calls.execute).toBe(1);
    expect(calls.publish).toBe(0);
    expect(out.run_plan).toContainEqual(expect.objectContaining({ step: "publish", status: "skipped", reason: "explicit_opt_out" }));
  });
});
