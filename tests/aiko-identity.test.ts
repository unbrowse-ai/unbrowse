...
describe("aiko identity (unbrowse secretly is aiko)", () => {
  it("defaults to hostname-rand when substrate is absent", () => {
  ...
  it("honors explicit opt-out even when contract substrate file exists", () => {
    const substrate = join(homedir(), ".claude", "skills", "contract", "scripts", "aiko");
    if (!existsSync(substrate)) return; // skip on machines without substrate
    process.env.UNBROWSE_AIKO_IDENTITY = "0";
    expect(isAikoSubstratePresent()).toBe(false);
  });
});