...
describe("shouldScrollStimulate — generalized off the cardinality signal", () => {
  ...
  it("fires for feed/timeline-shaped intents without a list keyword (X profile case)", () => {
    // "top posts with text and metrics" has no list|search|feed keyword, but
    // posts/tweets are a collection shape — deriveIntentHints maps them to
    // tweet/timeline/status. This is what makes capture explore + scroll an X
    // profile so its internal GraphQL XHRs are observable.
    expect(shouldScrollStimulate("https://x.com/thedankoe", "top posts with text and metrics")).toBe(true);
    expect(shouldScrollStimulate("https://x.com/thedankoe", "latest tweets with metrics")).toBe(true);
    expect(shouldScrollStimulate("https://x.com/", "the feed of posts")).toBe(true);
  });
  ...
  it("fires for quotes.toscrape.com/scroll under a list intent (cardinality, not /scroll allowlist)", () => {
    // Path is bare /scroll — no search|explore token. List intent is the signal.
    expect(shouldScrollStimulate("http://quotes.toscrape.com/scroll", "list quotes")).toBe(true);
    expect(shouldScrollStimulate("http://quotes.toscrape.com/scroll", "open this page")).toBe(false);
  });
});
