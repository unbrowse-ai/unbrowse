/**
 * Day-6 Dominion worker 1 (2026-05-28) — v7 stateless act-input tests
 ...
 *            boundary; the canary substring appears nowhere in the
 ...
 *
 * No mocks of the audit platform logic — only the network endpoint is
...
describe("Day-6 W1: stateless act-act per-handler (T1..T6)", () => {
  ...
  for (const tc of PER_HANDLER_CASES) {
    it(`${tc.name}: POSTs to /v1/audit/act-act, wire body passes validateAuditBody`, async () => {
...
describe("Day-6 W1: binding-missing surfaces, never throws (T8)", () => {
  ...
  it("503 + _binding_missing yields bindingMissing:true, warns stderr, never throws", async () => {
    ...
    expect(stderrCaptured.includes("act-act")).toBe(true);
...
// ───────────────────────────────────────────────────────────────────────────
// Schema-gate parity — backend rejects bogus act-act bodies
// ───────────────────────────────────────────────────────────────────────────

describe("Day-6 W1: backend validateAuditBody arms for act-act variant", () => {
  ...
  it("accepts a well-formed act-act body", () => {
    const err = validateAuditBody(freshBreathActBody());
    expect(err).toBeNull();
  });

  it("accepts act-act with no selectorHash (focus-bound press)", () => {
  ...
  it("rejects act-act with unknown actType", () => {
    const body = freshBreathActBody();
  ...
  it("rejects act-act missing actType", () => {
    const body = freshBreathActBody();
    delete body.actType;
    const err = validateAuditBody(body);
    expect(err).not.toBeNull();
