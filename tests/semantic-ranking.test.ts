...
describe("rankEndpoints semantic intent adjustment", () => {
  test("keeps Glassdoor jobs intent bound to jobs rather than reviews", () => {
    const ranked = rankEndpoints([
      {
        endpoint_id: "company-reviews",
        method: "GET",
        url_template: "https://www.glassdoor.com/Reviews/acme-reviews-SRCH_KE0,4.htm",
        description: "Company reviews and ratings",
        idempotency: "safe",
        verification_status: "verified",
        reliability_score: 0.99,
        semantic: { action_kind: "list", resource_kind: "review" },
      } as any,
      {
        endpoint_id: "job-search",
        method: "GET",
        url_template: "https://www.glassdoor.com/Job/jobs.htm?sc.keyword={q}",
        description: "Search open jobs",
        idempotency: "safe",
        verification_status: "verified",
        reliability_score: 0.7,
        semantic: { action_kind: "search", resource_kind: "job" },
      } as any,
    ], "search jobs", "glassdoor.com");

    expect(ranked[0]?.endpoint.endpoint_id).toBe("job-search");
    expect(ranked.some((r) => r.endpoint.endpoint_id === "company-reviews")).toBe(false);
  });

  test("keeps npm search intent bound to search rather than package detail", () => {
    const ranked = rankEndpoints([
      {
        endpoint_id: "package-detail",
        method: "GET",
        url_template: "https://registry.npmjs.org/react",
      ...
      {
        endpoint_id: "package-search",
        method: "GET",
        url_template: "https://registry.npmjs.org/-/v1/search?text={q}",
        description: "Search packages",
        idempotency: "safe",
        verification_status: "verified",
        reliability_score: 0.7,
        semantic: { action_kind: "search", resource_kind: "package" },
      } as any,
    ], "search npm packages", "npmjs.org");

    expect(ranked[0]?.endpoint.endpoint_id).toBe("package-search");
    expect(ranked.some((r) => r.endpoint.endpoint_id === "package-detail")).toBe(false);
  });

  test("does not substitute a crates RSS feed for crate search", () => {
    const ranked = rankEndpoints([
      {
        endpoint_id: "crates-rss",
        method: "GET",
        url_template: "https://crates.io/feeds/new-crates.rss",
        description: "RSS feed of new crates",
        idempotency: "safe",
        verification_status: "verified",
        reliability_score: 0.99,
        semantic: { action_kind: "list", resource_kind: "crate" },
      } as any,
      {
        endpoint_id: "crate-search",
        method: "GET",
        url_template: "https://crates.io/api/v1/crates?q={q}",
    ...
    ], "search crates", "crates.io");

    expect(ranked[0]?.endpoint.endpoint_id).toBe("crate-search");
    expect(ranked.some((r) => r.endpoint.endpoint_id === "crates-rss")).toBe(false);
  });
