...
describe("rankEndpoints semantic intent adjustment", () => {
  ...
  test("prefers timeline api over page artifact for post search intents", () => {
    ...
    // Thin-client boundary: the trained ranking head (which sharpened bottom-ordering on cold
  ...
  test("prefers structured replay over sibling dom artifact for public search pages", () => {
    ...
    // was largely the trained ranking head's cold-cell signal, now SERVER-ONLY. The local degraded
...
describe("rankEndpoints with params (harness signal)", () => {
  ...
  test("params with value in response_schema boosts score", () => {
    ...
    // carried by the trained ranking head, now SERVER-ONLY. The local degraded fallback still
