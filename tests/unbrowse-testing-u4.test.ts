...
describe("U-4 — act capture must not kill a running act serve daemon", () => {
  ...
  it("U-4: capture rejects an unobservable serve broker and NEVER terminates it by port", async () => {
    const seen: string[] = [];
    ...
    // serve's broker stays up, but capture does not mistake health for network
    // observability; startOn will choose an isolated owned broker port.
