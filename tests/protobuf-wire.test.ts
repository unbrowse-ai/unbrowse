...
import { decodeProtobufBody, encodeProtobufRequest, inferProtobufRequestContract, isProtobufLikeEndpoint } from "../src/protobuf/wire.js";
...
describe("protobuf wire decoding", () => {
  it("infers and rewrites only a uniquely evidenced top-level string field", () => {
    const captured = bytes(strField(1, "hello"), intField(2, 7));
    const contract = inferProtobufRequestContract(`base64:${Buffer.from(captured).toString("base64")}`, { q: "hello" });
    expect(contract?.bindings).toEqual([{ param: "q", field_number: 1, captured_value: "hello" }]);
    const rewritten = encodeProtobufRequest(contract!, { q: "world" });
    expect(decodeProtobufBody(rewritten)?.fields).toEqual({ field_1: "world", field_2: 7 });
  });

  it("fails closed when evidence matches multiple fields", () => {
    const captured = bytes(strField(1, "hello"), strField(2, "hello"));
    const contract = inferProtobufRequestContract(`base64:${Buffer.from(captured).toString("base64")}`, { q: "hello" });
    expect(contract?.unsupported_reason).toBe("ambiguous_field:q");
    expect(() => encodeProtobufRequest(contract!, { q: "world" })).toThrow("protobuf_parameterization_unsupported");
  });

  it("fails closed when no captured scalar proves a binding", () => {
    const captured = bytes(strField(1, "hello"));
    const contract = inferProtobufRequestContract(`base64:${Buffer.from(captured).toString("base64")}`, { q: "other" });
    expect(contract?.unsupported_reason).toBe("no_evidence_backed_bindings");
  });
  ...
  it("admits structurally decoded protobuf collections in the local reverse-engineer", () => {
    const first = bytes(strField(1, "Backend Engineer"), intField(2, 8000));
    const second = bytes(strField(1, "AI Engineer"), intField(2, 9000));
    const body = `base64:${Buffer.from(bytes(msgField(1, first), msgField(1, second))).toString("base64")}`;
    const endpoints = revengLocal([makeRequest(body)], {
      pageUrl: "https://www.carousell.sg/search/engineer",
    });
    expect(endpoints).toHaveLength(1);
