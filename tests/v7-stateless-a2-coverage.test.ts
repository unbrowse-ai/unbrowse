 ...
 *   - act (5): go, close, auth-capture, proxy-rotate, session-restore
 *   - eval-read (8): resolve, earnings, sessions, skill, skills, stats,
 *                    status, version
 *   - build (3): skill, template, value-source
 *
 * Each handler emits its receipt via one of two platform-side routes:
 *   - `/v1/audit/act-act` for the 5 act + 3 build acts
 ...
 *         hashed before crossing the boundary. The W24.2 acceptance
 ...
 *
 * No mocks of crypto or the audit platform — real Ed25519 via Web Crypto
 ...
 *
 *  — *"all things are naked and opened."*   — *"every
...
function spinupBackend(): MockBackend {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      // Ack shape matches the real handlers (act-act + eval-read both
...
async function emitBreathActAndAssert(
  ...
  expect(backend.captured[0].pathname).toBe("/v1/audit/act-act");
...
describe("W24.2: 5 act handlers emit sig-keyed audit row", () => {
  it("1/16 act/go.ts → actType=navigate", async () => {
    ...
    expect(wire.variant).toBe("act-act");
  ...
  it("2/16 act/close.ts → actType=teardown", async () => {
  ...
  it("3/16 act/auth-capture.ts (start phase) → actType=auth-capture-start", async () => {
  ...
  it("4/16 act/auth-capture.ts (finish phase) → actType=auth-capture-finish", async () => {
  ...
  it("5/16 act/proxy-rotate.ts → actType=proxy-rotate", async () => {
  ...
  it("6/16 act/session-restore.ts → actType=session-restore", async () => {
