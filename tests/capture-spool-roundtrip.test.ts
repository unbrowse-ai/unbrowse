...
test("getCaptureSpoolDir is the capture-pending sibling of queue/pending, under the DATA ROOT", () => {
  // The argument is the data root, not a home. It was a home that the function
  // appended `.unbrowse` to, which gave one function two path vocabularies and
  // made its DEFAULT bypass U-3 — an isolated run spooled captures into the
  // caller's real ~/.unbrowse/queue.
  expect(getCaptureSpoolDir("/home/x/.unbrowse")).toBe("/home/x/.unbrowse/queue/capture-pending");
});

test("its default follows UNBROWSE_HOME — the point of U-3", () => {
  const prev = process.env.UNBROWSE_HOME;
  process.env.UNBROWSE_HOME = "/tmp/isolated-root";
  try {
    expect(getCaptureSpoolDir()).toBe("/tmp/isolated-root/queue/capture-pending");
  } finally {
...
//  — the one lost sheep. Two concurrent drainers racing a single
