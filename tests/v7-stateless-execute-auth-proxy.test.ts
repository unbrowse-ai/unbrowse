 ...
 *      `replay_receipt.binding_missing` in stdout and fails closed (exit 1)
...
describe("Day-6 W2 — execute under stateless surfaces 503 binding_missing honestly", () => {
  ...
  it("trace POST returns 503 + _binding_missing → replay_receipt carries binding_missing and exits nonzero", async () => {
    const server = Bun.serve({
      port: 0,
      async fetch(req) {
        const u = new URL(req.url);
    ...
    try {
      ...
      // The upstream fetch succeeded, but an unsettled replay receipt makes the
      // operation terminally unsuccessful under the fail-closed contract.
