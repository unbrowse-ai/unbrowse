/**
 * W23 — v7.2.0-preview.0 CLI breath session-park / session-restore tests.
 *
 * Scope (load-bearing):
...
describe("v7.2.0-preview.0 kind-map registration", () => {
  it("registers breath session-park with actuate_session_park", () => {
  ...
  it("registers breath session-restore with actuate_session_restore", () => {
  ...
  it("keeps breath close registered with a deprecation marker in summary", () => {
...
describe.if(W5W6_READY)(
  ...
  () => {
    ...
    it("1. `breath go` then `breath session-park` removes the local session file + POSTs to backend", async () => {
      const goRes = await runCli(
        ["breath", "go", "https://example.com", "--json"],
      ...
      const parkRes = await runCli(
        ["breath", "session-park", sessionId, "--json"],
    ...
    it("3. `breath close` after `breath go` still works AND prints the deprecation stderr line", async () => {
