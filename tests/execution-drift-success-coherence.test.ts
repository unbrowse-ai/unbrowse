...
// your data" are mutually exclusive — the substrate already computed the
...
describe("execute drift → success coherence (truth-telling)", () => {
  test("re_capture_signal emitted ⟹ trace.success is false (not a silent success on shell data)", async () => {
    ...
    try {
      ...
      // The substrate detected drift and emitted the recapture signal.
  ...
  test("no drift but a meta-only content result remains a terminal thin-extraction failure", async () => {
    ...
    try {
      ...
      // The independent thin-extraction gate already rejected title/url-only
      // content. Terminal truth must preserve that rejection, not overwrite it.
      expect(out.trace.success).toBe(false);
      expect(out.trace.error).toBe("extraction_too_thin");
      expect(out.intent_verdict).toBe("fail");
    } finally {
      globalThis.fetch = originalFetch;
    }
  });
