...
import { applyActionCorrelations, tagRequestProvenance } from "../src/capture/index.js";
...
describe("applyActionCorrelations", () => {
  it("tags only exact request deltas and leaves later background polling unclaimed", () => {
    ...
    const correlations = new Map([
      ['POST https://api.example.com/search\n{"page":1}', { step: 1, action: "click", ref: "e9" }],
    ]);
    applyActionCorrelations([changed, poll], correlations);
    expect(changed.triggered_by_step).toBe(1);
    expect(changed.triggered_by_ref).toBe("e9");
    expect(poll.triggered_by_step).toBeUndefined();
  });
});
