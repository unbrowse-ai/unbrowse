...
describe("resolveWalletConfig — pay adapter", () => {
  ...
  test("default is pay (pay.sh is the only wallet adapter)", () => {
    // Product: pay is default-ON; only UNBROWSE_WALLET_ADAPTER=none opts out.
    const cfg = resolveWalletConfig({ HOME: "/nonexistent-home-xyz" } as NodeJS.ProcessEnv);
    expect(cfg.adapter).toBe("pay");
  });

  test("UNBROWSE_WALLET_ADAPTER=none disables payment", () => {
    const cfg = resolveWalletConfig({ UNBROWSE_WALLET_ADAPTER: "none" } as NodeJS.ProcessEnv);
