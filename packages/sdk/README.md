...
TypeScript SDK for **Unbrowse** — the **route / action layer** for agents. Prefer the CLI hole for day-to-day work; use this package when you want programmatic resolve/execute, capture, or payments from Node.

```bash
# Agents / most scripts — one call, no SDK required:
npm i -g unbrowse@latest && unbrowse setup
unbrowse "top stories" --url https://news.ycombinator.com
```

> **Also available on the main package:** `import { createHole } from "unbrowse/sdk"` ships with the `unbrowse` npm CLI package (one hole fill). This `@unbrowse/sdk` package is the fuller client (`Unbrowse.local()`, payments, publish, search).

> **Sibling:** [`@unbrowse/client`](../sdk-v2/) is a thin HTTP-first SDK against `beta-api.unbrowse.ai` (browsers / edge, no local binary). Use it for pure HTTP; use this package for local capture and in-process runtime attachment.

License: MIT for this package. Runtime: install `unbrowse` via npm. See [`OPEN-SOURCE-NOTICE.md`](../../docs/OPEN-SOURCE-NOTICE.md).
...
```bash
npm install @unbrowse/sdk unbrowse@latest
```

The spawn path is lazy — `Unbrowse.connect()` callers never pull `child_process`, so tree-shaking stays friendly in serverless/edge bundles.
...
`Unbrowse.local()` attaches to (or starts) a local runtime for SDK use.  
**Day-to-day agents should still use the flat CLI** (`unbrowse "task" --url …`) — in-process, no long-lived daemon.

If you already started an explicit HTTP facade (`unbrowse serve`), point the SDK at it with `Unbrowse.connect`.

## Three factories, three lifecycles

```ts
// 1. Common case — attach to (or start) a local runtime for SDK use.
const u = await Unbrowse.local();

// 2. Explicit HTTP facade already running (`unbrowse serve` in another terminal).
const u = await Unbrowse.connect(process.env.UNBROWSE_URL ?? "http://127.0.0.1:6969");

// 3. Always spawn a fresh owned process — useful for tests and isolated workers.
...
- Default `baseUrl` for HTTP transport is loopback only when you use `connect` / `spawn` (or set `UNBROWSE_URL`). Day-to-day agent work prefers the in-process CLI, not a fixed port.
