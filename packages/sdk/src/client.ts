...
export function telemetryEnvDisabled(value: string | undefined): boolean {
...
const SAFE_SDK_USAGE_ACTIONS = new Set([
  "account", "agents", "annotate", "dashboard", "execute", "feedback", "health", "login", "publish", "resolve",
  "search", "skill", "skills", "stats", "unlock",
]);

/** Collapse an SDK route to a fixed category without retaining path parameters. */
export function sdkUsageOperationForPath(path: string, method = "GET"): string {
  const pathname = path.split("?", 1)[0] ?? "";
  const segments = pathname.split("/").filter(Boolean).map((part) => part.toLowerCase());
  if (segments[0] === "v1") segments.shift();
  if (segments[0] === "auth" && segments[1] === "steal") return "sdk:steal_auth";
  if (segments[0] === "transactions" || segments[0] === "attribution") return "sdk:earnings";
  if (segments[0] === "account") return "sdk:account";
  if (segments[0] === "skills" && segments.length === 1 && method.toUpperCase() === "POST") return "sdk:publish";
  if (segments[0] === "skills" && segments.length >= 3 && segments.at(-1) === "execute") return "sdk:execute";
  if (segments[0] === "skills" && segments.length >= 2) return "sdk:skill";
  const action = [...segments].reverse().find((segment) => SAFE_SDK_USAGE_ACTIONS.has(segment)) ?? "request";
  return `sdk:${action}`;
}
...
export class Unbrowse {
  ...
  constructor(options: UnbrowseClientOptions = {}) {
    ...
    this.telemetryEnabled = options.telemetry !== false && !telemetryEnvDisabled(readEnv("UNBROWSE_TELEMETRY"));
    this.telemetryEndpoint = options.telemetryEndpoint ?? "https://beta-api.unbrowse.ai/v1/telemetry/usage";
  }
  ...
  private reportUsage(method: string, path: string): void {
    if (!this.telemetryEnabled || path === "/v1/telemetry/usage") return;
    const operation = sdkUsageOperationForPath(path, method);
    const verb = operation.split(":")[1] ?? "request";
    const executionScope = /^https:\/\//.test(this.baseUrl) ? "cloud" : "local";
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 1500);
    void fetch(this.telemetryEndpoint, {
      method: "POST", headers: { "content-type": "application/json" }, signal: controller.signal,
      body: JSON.stringify({ verb, operation, surface: "sdk", execution_scope: executionScope,
        telemetry_schema_version: 2, created_at: new Date().toISOString() }),
    }).catch(() => {}).finally(() => clearTimeout(timer));
  }

  async request<T>(method: string, path: string, body?: unknown, options: RequestOptions = {}): Promise<T> {
    const url = `${this.baseUrl}${path}`;
    this.reportUsage(method, path);
