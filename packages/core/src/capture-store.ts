...
export function harEntriesToCapturedExchanges(
  ...
  // HAR `time` is total duration, not a monotonic ordering key.
  // Prefer `startedDateTime` when available; otherwise preserve input order.
  const withIdx = (entries ?? []).map((e, idx) => {
    let startedMs: number | null = null;
    const sdt = (e as any)?.startedDateTime;
    if (typeof sdt === "string" && sdt.trim().length > 0) {
      const ms = Date.parse(sdt);
      if (Number.isFinite(ms)) startedMs = ms;
    }
    return { e, idx, startedMs };
  });
  const hasStarted = withIdx.some((x) => typeof x.startedMs === "number" && Number.isFinite(x.startedMs));
  const sorted = hasStarted
    ? withIdx
      .slice()
      .sort((a, b) => {
        const am = a.startedMs ?? Number.POSITIVE_INFINITY;
        const bm = b.startedMs ?? Number.POSITIVE_INFINITY;
