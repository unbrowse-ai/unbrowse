...
describe("correlation-engine: value links + chain planning", () => {
  ...
  it("links response array -> request url path segment (preserves .json suffix)", () => {
    ...
    const exchanges: CapturedExchange[] = [
      {
        ...
        response: {
          ...
          headers: { "content-type": "application/json" },
          cookies: {},
          body: [PLACEHOLDER, 2, 3],
          bodyRaw: JSON.stringify([PLACEHOLDER, 2, 3]),
          bodyFormat: "json",
          contentType: "application/json",
        },
      },
      {
        ...
        request: {
          method: "GET",
          url: `https://hacker-news.firebaseio.com/v0/item/${PLACEHOLDER}.json`,
          headers: {},
          cookies: {},
          queryParams: {},
        },
    ...
    const graph = inferCorrelationGraphV1(exchanges);
    const link = graph.links.find((l) => l.sourceRequestIndex === 0 && l.targetRequestIndex === 1 && l.targetLocation === "url");
    ...
    runtime.set(0, {
      status: 200,
      headers: {},
      bodyText: JSON.stringify([123456789, 2, 3]),
      contentType: "application/json",
      bodyJson: [123456789, 2, 3],
    });

    const prepared = prepareRequestForStep(exchanges, graph, 1, runtime, { sessionHeaders: {} });
    expect(prepared?.url).toContain("/v0/item/123456789.json");
  });

  it("allows short numeric IDs (4+ digits) to correlate across steps", () => {
    const exchanges: CapturedExchange[] = [
      {
        index: 0,
        timestamp: 1,
        ...
        response: {
          ...
          headers: { "content-type": "application/json" },
          cookies: {},
          body: { items: [{ user_id: 4383615 }] }, // 7 digits
          bodyRaw: JSON.stringify({ items: [{ user_id: 4383615 }] }),
          bodyFormat: "json",
          contentType: "application/json",
        },
      },
      {
        ...
        request: {
          method: "GET",
          url: "https://example.com/users/4383615",
          headers: {},
          cookies: {},
          queryParams: {},
        },
    ...
    const link = graph.links.find(
      (l) =>
        l.sourceRequestIndex === 0 &&
        l.targetRequestIndex === 1 &&
        l.sourceLocation === "body" &&
        l.targetLocation === "url",
    );
    expect(link).toBeDefined();

    expect(planChainForTarget(graph, 1)).toEqual([0, 1]);
  });
});
