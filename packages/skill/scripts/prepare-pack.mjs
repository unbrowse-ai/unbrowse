...
// Ship the python fetch helpers BESIDE the bundled runtime. The runtime resolves
// them module-relatively (resolveHelperScript in src/capture/curl-impersonate-fallback.ts);
// without this copy the file simply is not in the tarball, and the TLS-impersonation
// rung silently returns null on every install — a dead ladder rung that looks
// identical to "the site blocked us".
const runtimeDir = path.join(packageRoot, "runtime");
mkdirSync(runtimeDir, { recursive: true });
for (const helper of ["curl-impersonate-fetch.py", "camoufox-fetch.py"]) {
  const src = path.join(repoRoot, "scripts", helper);
  if (!existsSync(src)) {
    throw new Error(
      `[prepare-pack] missing python helper ${src}. The impersonation rung depends on it being packaged; ` +
        "fix the path or remove it from the ship list rather than publishing a runtime that cannot use it.",
    );
  }
  const dest = path.join(runtimeDir, helper);
  cpSync(src, dest);
