...
// obscura: same host convention, but keep chmod even when prune is skipped (never break capture).
try {
  const obscuraDir = join(root, "vendor", "obscura", `${platform}-${arch}`);
  for (const bin of ["obscura", "obscura-capture", "obscura-worker"]) {
    const exe = join(obscuraDir, platform === "win32" ? bin + ".exe" : bin);
