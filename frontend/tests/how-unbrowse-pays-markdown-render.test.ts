/**
 * Exercises `loadDocMarkdown` against `docs/HOW_UNBROWSE_PAYS.md`.
 *
 * Honest scope note: `frontend/src/app/how-unbrowse-pays/page.tsx` is
 * hand-written JSX and does NOT call `loadDocMarkdown` — no app page does
 * (grep `loadDocMarkdown` under frontend/src/: only the definition). So this
 * file is a unit test of the renderer plus a content check on the doc, not
 * proof that the shipped page is markdown-driven. It used to claim the
 * latter; the claim outlived the wiring.
 *
...
describe("how-unbrowse-pays markdown rendering", () => {
	...
	// Replaced a "rendered HTML contains no em dashes (style rule)" assertion.
	// That rule had no source anywhere in the repo, and the house style
	// contradicts it everywhere — frontend/design.md, the page.tsx copy for
	// THIS page, and docs/HOW_UNBROWSE_PAYS.md itself all use em dashes. Every
	// edit to the doc since re-broke it, which is what kept `bun test
	// frontend/tests/` red. What is worth asserting on the real doc is the
	// property the renderer actually owes: raw HTML in the markdown source is
	// escaped, on the LIVE content and not only on a synthetic temp file.
	test("real doc renders with raw HTML escaped (XSS guard on live content)", () => {
		const { html, source } = loadDocMarkdown("HOW_UNBROWSE_PAYS");
		expect(source.length).toBeGreaterThan(500);
		expect(html).not.toMatch(/<script|<iframe|<object|\son[a-z]+\s*=/i);
	});
