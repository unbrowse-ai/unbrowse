...
plan_text: "Test unbrowse npm-install + setup + MCP-wire + first-resolve adoption journey end-to-end in a podman sandbox container. Fresh Ubuntu 24.04 + node 22 + npm. Drive: (1) npm i -g unbrowse, (2) unbrowse setup --mcp, (3) unbrowse health, (4) unbrowse resolve --intent 'top stories' --url https://news.ycombinator.com, (5) unbrowse execute on top result, (6) unbrowse go https://example.com --session test, (7) unbrowse snap, (8) unbrowse close. Capture per-step exit code, wallclock, stderr tail, agent-visible output. Verify gate: container actually spawns, npm install completes, setup writes mcp.json without prompting, health returns ok, resolve returns >=1 available_endpoint OR explicit no_cached_match next_step, browse session opens OR returns actionable next_step (never one-word error). Ship surface: agent-experience findings report + scoped commits fixing any surfaced blocker (cli.ts die() message, setup.ts prompt, mcp.ts tool description). Apply find-skills-first principle: rivet-dev/skills@sandbox-agent (6.6K) + joelhooks/joelclaw@docker-sandbox + the already-installed cli-help-writer + mcp-builder skills inform the audit checklist."
project: /Users/lekt9/Projects/unbrowse-ecosystem/unbrowse/frontend
template: agent-system
scope: project
shipping_surface: "git commit + corpus-gate stamp"
ship_command: |
...
# Test unbrowse npm-install + setup + MCP-wire + first-resolve adoption journey end-to-end in a podman sandbox container. Fresh Ubuntu 24.04 + node 22 + npm. Drive: (1) npm i -g unbrowse, (2) unbrowse setup --mcp, (3) unbrowse health, (4) unbrowse resolve --intent 'top stories' --url https://news.ycombinator.com, (5) unbrowse execute on top result, (6) unbrowse go https://example.com --session test, (7) unbrowse snap, (8) unbrowse close. Capture per-step exit code, wallclock, stderr tail, agent-visible output. Verify gate: container actually spawns, npm install completes, setup writes mcp.json without prompting, health returns ok, resolve returns >=1 available_endpoint OR explicit no_cached_match next_step, browse session opens OR returns actionable next_step (never one-word error). Ship surface: agent-experience findings report + scoped commits fixing any surfaced blocker (cli.ts die() message, setup.ts prompt, mcp.ts tool description). Apply find-skills-first principle: rivet-dev/skills@sandbox-agent (6.6K) + joelhooks/joelclaw@docker-sandbox + the already-installed cli-help-writer + mcp-builder skills inform the audit checklist.
...
```
.claude/test-unbrowse-npm-install-setup-mcp-wire-first-r/
├── scripts/
...
```bash
bash .claude/test-unbrowse-npm-install-setup-mcp-wire-first-r/scripts/iterate.sh
```
