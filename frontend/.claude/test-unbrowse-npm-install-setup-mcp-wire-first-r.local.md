...
plan_text: "Test unbrowse npm-install + setup + first-resolve adoption journey end-to-end in a podman sandbox container. Fresh Ubuntu 24.04 + node 22 + npm. Drive: (1) npm i -g unbrowse, (2) unbrowse setup, (3) unbrowse health, (4) unbrowse resolve --intent 'top stories' --url https://news.ycombinator.com, (5) unbrowse execute on top result, (6) unbrowse go https://example.com --session test, (7) unbrowse snap, (8) unbrowse close. Capture per-step exit code, wallclock, stderr tail, agent-visible output. Verify gate: container actually spawns, npm install completes, setup installs the Agent Skill without writing MCP config, health returns ok, resolve returns >=1 available_endpoint OR explicit no_cached_match next_step, browse session opens OR returns actionable next_step (never one-word error). Ship surface: agent-experience findings report + scoped commits fixing any surfaced blocker (cli.ts die() message, setup.ts prompt, skill docs). Apply find-skills-first principle: rivet-dev/skills@sandbox-agent (6.6K) + joelhooks/joelclaw@docker-sandbox + the already-installed cli-help-writer skills inform the audit checklist."
...
# Test unbrowse npm-install + setup + first-resolve adoption journey end-to-end in a podman sandbox container. Fresh Ubuntu 24.04 + node 22 + npm. Drive: (1) npm i -g unbrowse, (2) unbrowse setup, (3) unbrowse health, (4) unbrowse resolve --intent 'top stories' --url https://news.ycombinator.com, (5) unbrowse execute on top result, (6) unbrowse go https://example.com --session test, (7) unbrowse snap, (8) unbrowse close. Capture per-step exit code, wallclock, stderr tail, agent-visible output. Verify gate: container actually spawns, npm install completes, setup installs the Agent Skill without writing MCP config, health returns ok, resolve returns >=1 available_endpoint OR explicit no_cached_match next_step, browse session opens OR returns actionable next_step (never one-word error). Ship surface: agent-experience findings report + scoped commits fixing any surfaced blocker (cli.ts die() message, setup.ts prompt, skill docs). Apply find-skills-first principle: rivet-dev/skills@sandbox-agent (6.6K) + joelhooks/joelclaw@docker-sandbox + the already-installed cli-help-writer skills inform the audit checklist.
