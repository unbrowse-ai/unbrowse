...
import { readLocalRuntimeOrigin } from "./local-runtime";
...
export interface LocalRuntimeSettings {
  mutation_policy: MutationPolicySettings;
  [key: string]: unknown;
}
...
async function localRuntimeRequest<T>(
  method: string,
  path: string,
  apiKey: string,
  body?: unknown,
): Promise<T> {
  const origin = readLocalRuntimeOrigin();
  if (!origin) {
    throw new AccountClientError(
      0,
      "Local Unbrowse runtime is not paired. Run `unbrowse dashboard --account` to open this panel against your CLI.",
    );
  }
  const headers: Record<string, string> = { Authorization: `Bearer ${apiKey}` };
  if (body !== undefined) headers["Content-Type"] = "application/json";
  ...
  try {
    ...
    throw new AccountClientError(
      0,
      `Could not reach local Unbrowse at ${origin}: ${(err as Error).message}. Is the CLI still running?`,
    );
  }
  if (!res.ok) {
    throw new AccountClientError(res.status, await readErrorBody(res));
...
export async function fetchLocalRuntimeSettings(apiKey: string): Promise<LocalRuntimeSettings> {
  return localRuntimeRequest<LocalRuntimeSettings>("GET", "/v1/settings", apiKey);
}

export async function patchLocalRuntimeSettings(
  apiKey: string,
  patch: { mutation_policy?: MutationPolicyMode; mutation_whitelist?: string[] },
): Promise<LocalRuntimeSettings> {
  return localRuntimeRequest<LocalRuntimeSettings>("POST", "/v1/settings", apiKey, patch);
}
