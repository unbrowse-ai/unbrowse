 ...
 * user's own client first. The worker holds the Codegraff key, so each LLM round
...
import { systemPrompt, TOOLS, LLM_URL, MODEL } from "@/lib/hero-tools";
...
export async function POST(req: Request): Promise<Response> {
  const key = process.env.CODEGRAFF_API_KEY?.trim() || process.env.NEBIUS_API_KEY?.trim();
