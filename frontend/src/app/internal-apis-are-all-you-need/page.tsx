...
const PAPER_SUBTITLE = "Shadow APIs, Shared Discovery, and Why Browser-First Agents Are Over";
const CANONICAL_PATH = "/internal-apis-are-all-you-need";
const PAPER_PDF_URL = "/whitepaper";
...
const abstract = `Autonomous web agents repeatedly pay a discovery tax: opening sites, inspecting DOMs, and reverse-engineering callable routes. Internal APIs Are All You Need introduces Unbrowse, a shared route graph that converts browser-based route discovery into a collectively maintained, usage-priced index of callable web interfaces. Every modern website already exposes first-party internal APIs — sometimes called shadow APIs — behind its UI. Routes are learned passively from real browsing traffic and reused as cached API calls, making agents faster, cheaper, and less brittle than browser automation.`;

const highlights = [
  "Introduces the discovery tax as the core bottleneck for autonomous web agents",
  "Frames internal APIs (shadow APIs) as the machine-native substrate behind human web interfaces",
...
const sections = [
  {
    ...
    body: "Most web-capable agents still waste time rediscovering the same website workflows over and over. This paper argues that a site's own internal APIs — the shadow APIs powering every modern frontend — already form the real machine-native interface layer. Unbrowse turns that repeated private reverse engineering into a shared route graph, so agents can call cached routes directly instead of browsing human interfaces by default.",
  },
  {
    title: "Core claim",
    body: "Internal APIs are all you need because modern websites already expose callable first-party interfaces behind their UIs. These shadow APIs are not hidden — they are simply undocumented. The bottleneck is not the absence of interfaces. It is the absence of shared memory, routing, and maintenance around them.",
  },
  {
    title: "What Unbrowse adds",
    body: "Unbrowse passively indexes callable internal APIs from real traffic, keeps execution local, and gives agents a voluntary outside option: use the shared graph only when its route fee is lower than the expected cost of rediscovery. That keeps adoption disciplined by real product economics rather than speculation.",
  },
  {
