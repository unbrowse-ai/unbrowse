...
import {
  ...
  patchLocalRuntimeSettings,
  type MutationPolicyMode,
  type MutationPolicySettings,
...
function MutationPolicySection({
  apiKey,
  onAuthError,
}: {
  apiKey: string;
  ...
  const [policy, setPolicy] = useState<MutationPolicySettings | null>(null);
  const [mode, setMode] = useState<MutationPolicyMode>("ask");
  ...
  useEffect(() => {
    let cancelled = false;
    fetchLocalRuntimeSettings(apiKey).then((result) => {
      if (cancelled) return;
      const next = result.mutation_policy ?? { mode: "ask" as const, whitelist: [] };
      setPolicy(next);
      setMode(next.mode);
      setRules(next.whitelist.join("\n"));
    }).catch((err: unknown) => {
      if (!cancelled) {
        onAuthError(err);
        setError(err instanceof Error ? err.message : String(err));
  ...
  async function save() {
    ...
    setError(null);
    try {
      const whitelist = rules.split("\n").map((rule) => rule.trim()).filter(Boolean);
      const next = await patchLocalRuntimeSettings(apiKey, { mutation_policy: mode, mutation_whitelist: whitelist });
      setPolicy(next.mutation_policy);
      setSaved(true);
    } catch (err: unknown) {
      onAuthError(err);
      setError(err instanceof Error ? err.message : String(err));
  ...
  if (error) {
    return (
      <SectionCard title="Mutation safety">
        <div className="space-y-2">
          <ErrorChip message={error} />
          <p className="text-xs leading-5 text-text-muted">
            Open the panel against your local CLI with <code className="font-mono">unbrowse dashboard --account</code>.
          </p>
        </div>
      </SectionCard>
    );
  }
  if (!policy) return <SectionCard title="Mutation safety"><div className="text-sm text-text-muted">Loading...</div></SectionCard>;

  return (
    <SectionCard title="Mutation safety">
      <div className="space-y-4">
        <div>
          <div className="text-sm text-text-primary">Control write requests before they leave your browser</div>
          <p className="mt-1 text-xs leading-5 text-text-muted">
            Read routes still use the direct-first DAG. Unwhitelisted POST, PUT, PATCH, and DELETE calls follow this policy.
            Settings are stored on your local Unbrowse runtime after <code className="font-mono">unbrowse dashboard --account</code> pairing.
          </p>
        </div>
        <label className="block space-y-1">
          <span className="text-xs font-mono uppercase tracking-[0.12em] text-text-muted">Default mutation action</span>
          <select
            value={mode}
            onChange={(event) => setMode(event.target.value as MutationPolicyMode)}
            className="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500"
          >
            <option value="ask">Ask me each time (recommended)</option>
            <option value="whitelist">Allow whitelist only</option>
            <option value="deny">Deny all mutations</option>
          </select>
        </label>
        <label className="block space-y-1">
          <span className="text-xs font-mono uppercase tracking-[0.12em] text-text-muted">Mutation whitelist</span>
          <textarea
            value={rules}
            onChange={(event) => setRules(event.target.value)}
            rows={4}
            spellCheck={false}
            placeholder={"POST https://api.example.com/items\nDELETE https://api.example.com/items/*"}
            className="w-full resize-y rounded-xl border border-border bg-surface px-3 py-2 font-mono text-xs leading-5 text-text-primary placeholder:text-text-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500"
          />
          <span className="block text-xs text-text-muted">One <code>METHOD URL</code> per line. Use <code>*</code> for a path wildcard.</span>
        </label>
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            onClick={() => void save()}
            disabled={busy}
            className="rounded-xl bg-text-primary px-4 py-2 text-sm font-medium text-surface transition-[opacity,transform] duration-200 hover:opacity-90 active:translate-y-px focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-500 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {busy ? "Saving..." : "Save mutation policy"}
          </button>
          {saved && <span role="status" className="text-xs text-text-muted">Saved locally.</span>}
        </div>
      </div>
    </SectionCard>
  );
...
export default function AccountPage() {
  ...
  return (
    <main className="mx-auto max-w-[70ch] px-6 py-16 space-y-6">
      ...
      <MutationPolicySection apiKey={apiKey} onAuthError={handleAuthError} />
