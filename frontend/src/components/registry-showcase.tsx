    ...
    <section id="registry" className="relative max-sm:h-auto max-sm:min-h-0 max-sm:pb-8 min-h-[100dvh] flex flex-col py-10 sm:py-14 pb-14">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 w-full flex flex-col flex-1 min-h-0">
        ...
        <div className="text-center mb-6 shrink-0">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-orange-50 border border-border text-text-secondary text-xs font-mono font-medium uppercase tracking-widest mb-4" style={{ textShadow: '0 0 6px rgba(255,176,96,0.4)' }}>
        ...
        <div className="flex flex-col flex-1 min-h-0 bg-surface-ink border border-border rounded-sm overflow-hidden" style={{ boxShadow: '0 0 40px rgba(255,82,0,0.08)' }}>
          ...
          <div className="flex items-center justify-between px-5 py-3.5 shrink-0 bg-[rgba(0,0,0,0.35)]" style={{ borderBottom: '1px solid rgba(255,122,32,0.18)' }}>
            <h3 className="text-sm font-mono flex items-center gap-2 text-orange-500">
              <Globe2 className="w-4 h-4 text-orange-500" />
              Recently Indexed Skills
            </h3>
            <Link
              href="/search"
              className="text-xs font-mono text-orange-500 hover:text-text-secondary transition-colors flex items-center gap-1 group"
            >
          ...
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            {displaySkills.length > 0 ? (
                ...
                {displaySkills.map((skill, i) => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex items-center gap-4 px-5 py-3.5 hover:bg-orange-50 transition-colors group"
                      style={i > 0 ? { borderTop: '1px solid rgba(255,122,32,0.1)' } : undefined}
                    >
                      {/* Domain initial */}
                      <div className="w-9 h-9 rounded-sm bg-[#0a0806] border border-border flex items-center justify-center text-sm font-bold font-mono text-orange-500 shrink-0 group-hover:border-border-strong transition-colors uppercase">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="font-mono text-sm text-orange-500 truncate">{skill.name}</span>
                          {skill.lifecycle === 'active' && (
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                          )}
                        </div>
                        <span className="text-xs text-text-muted font-mono">{skill.domain}</span>
                      </div>

                      {/* Stats */}
                      <div className="text-right shrink-0 hidden sm:block">
                        <div className="text-xs font-mono text-text-muted">{endpointCount} endpoints</div>
              ...
              <div className="flex items-center justify-center h-32">
                <p className="text-text-muted text-sm font-mono">Loading registry skills...</p>
              </div>
            )}
          </div>
        </div>
