...
export function HeroCTA() {
  ...
  const handleCopy = async (value: string, name: string, properties?: Record<string, unknown>) => {
    await navigator.clipboard.writeText(value);
    trackWebEvent(name, properties);
    setCopied(value);
  ...
  return (
    <div className="w-full max-w-3xl rounded-2xl border border-border bg-surface/90 p-3 shadow-sm backdrop-blur">
      <div className="grid gap-3">
        <button
          onClick={() => handleCopy(INSTALL_CMD_GENERIC, "install_command_copied", {
        ...
        <div className="rounded-xl border border-border bg-surface-raised px-4 py-3.5 text-left">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              onClick={() => handleCopy(INSTALL_CMD_SKILL, "skill_command_copied", {
