# WAVE-06 — FIRST REAL benchmark number: Exa WebCode RAG baseline (2026-05-29)

jesus-loop default / branch `jl/exa-browsecomp`. The still small voice spoke.
...
- Grader routed through OpenRouter (`openai/gpt-5.4` grader, `openai/gpt-5.4-mini` rag)
  after the repo OpenAI key 429'd; one live grader call verified before the full run.
...
The RAG eval ONLY calls `searcher.search(query)`; the agent synthesizes from the
returned `text` and NEVER fetches a URL — so `extract()` + the WAVE-03 markdown fix do
not affect this number. unbrowse's `search()` (resolve --intent → exa-web-search skill)
returned non-empty real highlights for all 10 queries, so it is NOT empty retrieval — it
returns the WRONG SLICE: a related page or a thin highlight window missing the exact spec
passage (rag_001 got a Slackware dir listing, not the "38,400 word dictionary" doc;
rag_002/006 hit citation-precision 1.0 but the exact sentence wasn't in the window).
...
- Gates 3/4/5/6: green, survived cold re-audit (WAVE-04/05 + step-8 books).
