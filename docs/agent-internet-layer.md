...
values stay local behind wallet-bound / pointer-only holes; the wire carries typed pointers,
...
handled by the aiko platform and is out of scope (see [Out of scope](#out-of-scope)).
...
1. **`unbrowse resolve --intent X --url Y`** returns a ranked shortlist of candidate
...
3. **`unbrowse execute --skill <id> --endpoint <id>`** commits the chosen route.

Resolve gathers options; the agent judges; execute commits. Both commands return
pointer-only receipts. This is no longer the preferred surface for new
integrations; it is the route-inspection view under the one-hole contract
(`unbrowse "task" --url …`).
...
Pointer-only is load-bearing. `unbrowse fill` dereferences a `value:ptr`
...
How ops are scored, ranked, and value-populated is handled by the aiko platform
and is not part of this public API. Unbrowse emits an opaque op and a
wallet-signed receipt pointer; the aiko platform decides which routes are worth
...
learned from. That work lives in the aiko platform and never crosses Unbrowse's
