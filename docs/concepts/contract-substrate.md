...
can resolve, sign, cache, and account for. This page describes the substrate
...
Contracts are never edited in place; the substrate appends events. Declaring a
contract, iterating it, satisfying it, retiring it — each is a new row. That
append-only trace is the substrate's memory: any value is reconstructible from
...
Re-resolving an unchanged contract should be free. The substrate keys its cache
...
not four. The substrate binds them into a single source of truth with a
...
## The economy rides the same substrate
...
peer-to-peer ledger (the substrate is server-canonical today), a validator
