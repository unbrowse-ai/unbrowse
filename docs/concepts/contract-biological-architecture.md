# The Contract Substrate as a Biological Architecture

A first-principles reading of why the [contract substrate](./contract-substrate.md)
is shaped the way a living information-processing body is shaped — and why that
shape falls out of two engines, an **energy-based model (EBM)** for selection and
...
their own registers — [Energy-Based Route Ranking](../the-unbrowse-papers.md) is
...
…all under **one identity** that persists between moments. The substrate is built
as exactly that loop. The two engines doing the work are the two halves the
energy-based-model literature keeps pointing at: an LLM that *proposes* and an EBM
that *judges*.

## The load-bearing split: generator vs selector

A generator proposes; an energy model scores. An LLM is a fluent proposer that
will happily dream a wrong continuation. An EBM is the opposite organ — it does
...
selection loop that says *that one, not that one.* The substrate refuses to
...
- **Selection / instinct — the train-on-fire EBM.** Each time a contract fires,
...
- **Coherence / settlement — the fixed-point witness.** Iterate the learned
  ...
  timer.
- **The generator — the local model.** The substrate's own local energy-based LLM
  ...
  leashed by the selector. That is the EBM↔LLM marriage made mechanical.
...
organism doesn't re-derive the world on every breath.
...
any useful sense. The substrate's self-deployment is the literal version of that:
...
the substrate this very document describes actually shipped.
...
substrate's defense is a benchmarking discipline with one unglamorous rule:
...
located and run.** Applied to the energy-based selector, that means a win is only
...
*Energy-Based Route Ranking* paper.

## The honest wall — where the body is still young

This is the part not to oversell. The EBM/predictor half is **selection and
...
invent toward a novel goal. The LLM proposes across that gap; the EBM only judges
...
loop because it survives a noisy world, and an autonomous agent substrate arrives
