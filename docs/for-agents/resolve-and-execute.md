# One Request and the Advanced Route View

The current Unbrowse interface is one typed-hole request. The caller supplies an intent and
...
`resolve` and `execute` are the advanced decomposition of the same request.
...
* `resolve` searches local/server routes and returns candidate endpoints with evidence.
...
* manual replay of a known route
* old MCP/tool hosts
