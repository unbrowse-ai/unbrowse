...
| x402 settlement | Shipped | The x402-compatible gate path ships today end-to-end. v6.16 settles via Faremeter Flex: the client signs an off-chain Ed25519 authorization with a registered session key, the self-hosted facilitator verifies + holds + settles + flushes on Solana, and atomic split distribution happens at finalize. |
| USDC on Solana settlement | Shipped | The shipped lane settles USDC on Solana mainnet. EVM is on Faremeter's roadmap; Unbrowse stays Solana-only for paid execute until then. |
