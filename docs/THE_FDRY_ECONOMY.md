...
mints/addresses the `/contract` platform pins as the live stake platform:
...
(`(vault address withheld)`) had its admin/manager key
...
Doctrine, the compromised vault is the branch that is *cast off*; the
...
**Bound in code.** This binding is not prose-only. The `/contract` platform pins
