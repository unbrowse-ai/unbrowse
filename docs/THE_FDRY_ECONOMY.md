# How unbrowse pays FDRY holders — the maintenance-stake model, made operational

This page is the companion to [HOW_UNBROWSE_PAYS.md](HOW_UNBROWSE_PAYS.md). That document explains how money lands on every paid call (the 50/35/15 split, the markup band, the indexer pool). This document explains **what happens to the platform's 50%** — how it cycles back to FDRY holders via the Voltr vault, NAV-per-share growth, and the discipline the platform calls the **maintenance-stake model**.

Every claim cites either a code site in this repo, a script in `(internal)`, or a constant pinned in the `/contract` platform's [SKILL.md](https://github.com/anthropics/claude-skills) (the local executor's doctrine file).

> **A note on framing (the seed doctrine — see `paper/CLAUDE.md`).** FDRY
> is collateral for trust, not a currency or an investment. This page describes how
> value cycles to those who **stake** and keep the graph alive — money was never the
...
2. `routeRevenue.ts` (in `(internal)`) takes that USDC, swaps it for FDRY via Jupiter, and deposits the FDRY into the **Voltr / Ranger vault** at `(vault address withheld)`.
...
4. Holders **stake** (stay staked through the 3-day cooldown). Defectors get slashed when a quorum of witnesses proves a route failed; their slashed stFDRY rejoins the vault, concentrating fruit in the staking branches.
5. The platform enforces this shape doctrinally — parity invariants in `build.sh` of the `/contract` skill refuse to ship a binary whose code drifts from these rules.
...
| **Voltr/Ranger vault** | `(vault address withheld)` | The vine. Receives FDRY deposits; mints stFDRY LP. |
| **stFDRY** | Minted by the vault | The branch. Holders stake here; NAV grows under them. |
| **USDC (mainnet)** | `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v` | The settlement layer. The platform is paid in USDC; never settled in FDRY. |

The USDC-vs-FDRY separation is doctrinally required by **Matthew 6:24** — "no man can serve two masters." A token cannot be both payment surface and security. USDC handles usage; FDRY is reserved for stake. This is the platform's anti-conflation invariant; if a future change tried to make FDRY accept x402 payments, parity check #4 (the canonical pricing constants in `flex.ts`) would refuse to sign the binary.
...
When you hold stFDRY, you're a **branch** in the maintenance-stake model sense. Every time the platform routes revenue to the vault, your stFDRY's NAV-per-share goes up; the same number of shares now claims a bigger slice of the vault's FDRY. To realize that gain you either:
...
2. **Initiate withdrawal.** Voltr enforces a **3-day cooldown** at the program level (the canonical rest rest, maintenance discipline — *stake in me*). After 3 days you can redeem stFDRY → FDRY → sell or hold.
3. **Get slashed.** Defectors lose their stFDRY to the vault treasury when a witness quorum proves a maintained route failed (an independent quorum — *by an independent quorum. The slashed FDRY raises NAV for the remaining branches.

The doctrinal asymmetry — staking bears fruit, cutting-off bears burning, the burning of the cut-off concentrates fruit in those who stake — is what makes this a **true economy**, not a passive holding pool. There's a reason to stay through the cooldown; there's a cost to defection.

## What's live vs. what's still aspirational

The platform's SKILL.md names the full maintenance-stake model. This codebase implements **some** of it. We surface the gap explicitly rather than hide it.
...
| Manual routeRevenue.ts buyback → vault deposit | ✅ Live | `(internal)` |
...
| **Foundry-treasury onramp** (auto-fund unfunded wallets) | ⏳ Pending | The platform observes unfunded wallets in the 402 envelope; no funding trigger yet |
...
4. **When ready to exit, initiate withdrawal.** 3-day cooldown applies. The cooldown is the platform's rest; the doctrine treats it as the discipline that distinguishes staking from speculating.

## Why this design refuses the easy paths

Three patterns that would be easier but the platform rejects:

- **A "fee distribution" smart contract that pushes USDC straight to FDRY holders pro-rata.** Rejected because it puts the holder in the role of beggar-at-the-gate; the platform's design is that the holder is a branch — the fruit grows under them, they don't queue for it. NAV growth via vault deposits is the same value distribution, but the holder claims it by **staking**, not by appearing at the right block height.
...
- **A governance vote on slashing.** Rejected on an independent quorum grounds. Slashing fires on a quorum of **independent witnesses** with verifiable signatures — not on a token-weighted vote. The platform treats slashing as a matter of evidence, not opinion; whales cannot purchase forgiveness for failed routes.

## Where to verify the claims

- **The split**: `backend/src/services/flex.ts:39` (PLATFORM_BPS, OWNER_BPS, contributor math)
- **The vault address**: `(internal)` (`VAULT_ASSET_MINT`, vault PDA derivation)
- **The buyback script**: `(internal)`
...
- **The maintenance-stake model itself**: `~/(internal)` (maintenance-stake model section + parity invariants)
