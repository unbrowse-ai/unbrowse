...
- **Execute, don't guess — at model scale.** The same small on-device model (Qwen2.5-1.5B). *Tools vs no tools*: retrieval+execution turn what it can't answer from weights into what it solves — knowledge-not-in-weights **0% → 95%** (retrieve the formula, run it), applying a retrieved skill vs reasoning from scratch **63% → 93%**. *Distillation on the served model* (in-distribution): correct code-trace distillation lifts code-correctness **68% → 100%** on the trained families (raw base → distilled); hard-reasoning families **50% → 92%** (baseline under audit).
...
|---|---|---|
| **Corpus** | A list of `intent \| url` probes. Each row is a real agent task: a goal a user might give a calling agent, plus a starting URL where the data lives. | versioned corpora in the benchmark harness (default, per-axis, and full 323-probe sets) |
| **Executor** | Drives `unbrowse resolve` against each probe and dumps raw per-probe artifacts. Emits NO heuristic verdict. | the benchmark executor |
...
The deletion of the early bench shell scripts on 2026-05-26 (commit
message: "benches should never be scripts") was the formal closing of
this loop. The executor that replaced them is deliberately incapable
of writing a verdict column. It only collects evidence.
...
The executor and corpora live in the project's benchmark harness and
are run as part of post-release verification. Each run writes

- `results.jsonl` — one evidence row per probe
...
Probes are rows in a versioned corpus file (lines starting with `#`
are comments). Two-field format `intent|url` is the canonical shape.
Use the agent-experience harness conventions: the intent is a real
user goal (`search rust crates`, `get top stories`), not a synthetic
test assertion (`assert search returns 10 items`).

For dimensional axis coverage (INDEX / AUTH / CSRF / SEARCH / RETR /
EXEC / META), use the per-axis corpus, which prefixes each row with
...
/ 5 PRODUCT_FAIL / 1 AUTH_GATED excluded), on the 19-probe default
corpus set as of commit `1c59517fd`. Full
...
The corpus is re-harvested each run by the internal benchmark harness
(dogfooding unbrowse on r/webscraping); the five axes run against the
fresh corpus and a dated report is emitted. Per-(site,axis) evidence
is appended to an append-only history file.
