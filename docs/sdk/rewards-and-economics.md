...
- **`commitment_only` proofs (live)**: every published skill carries a SHA-256 commitment over the captured response. This is **not** cryptographic origin proof (see [***REMOVED***.md](../***REMOVED***.md)) — it's tamper-evident metadata for after-the-fact-edit detection. The four-state proof model and the boundary are documented in `docs/***REMOVED***.md`.
