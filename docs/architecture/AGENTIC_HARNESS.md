...
cleanup. The local agent membrane is `src/harness/front-door.ts`: it gates each invocation
before transport, owns deadline/cancellation state, emits an immutable lifecycle trace, and
selects/compresses/persists the one result crossing back to the agent. The resolver remains
injected into that membrane so there is one orchestrator, not a competing agent framework.

`src/agent/loop.ts` and the transport-bound browser dispatchers are optional embedding
primitives, not the shipped one-shot execution path and not a security claim about legacy
CLI browser gestures. They fail closed when embedded, but only a production caller crossing
that dispatcher receives its protection.
...
| Per-call safety | Classify each operation invocation, not an endpoint forever. Reads may auto-run; mutations, payments, terms, CAPTCHA, and sensitive publication are ask/deny gates. | `src/runtime/permission-gate.ts`, API execute admission, publish admission |
| Context budget | The agent sees result, compact plan, and one recovery step—not HAR, DOM dumps, route catalogs, or internal ranking traces. Optional model-driven work keeps a complete audit trace and a separately bounded model projection. | `src/agent-path.ts`, `src/agent/loop.ts`, MCP agent surface |
