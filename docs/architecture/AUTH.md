...
> **contract (aiko) parent platform**, which signs the on-chain parent root;
...
- **Parent/child platform model.** The wallet at `~/.unbrowse/wallet.json`
  is unbrowse's identity. The contract platform (aiko) is the **parent** —
...
- **Bridge:** `src/bridges/contract-mcp-bridge.ts` + the HTTP thin client at
  ...
  dereferences through the platform, and acts on the resolved truth.
...
account-bound continuity — under a contract (aiko) parent platform where the
