...
Production deploys do not happen until the substrate-declared deploy-gate is satisfied. Two stages must both be satisfied before any release-it invocation reaches the npm publish or wrangler deploy step:
...
`DEPLOY_GATE_BYPASS=1` skips the gate. The bypass is logged to stderr and the release commit message must name the reason. CI workflows for protected branches can refuse the bypass entirely; for development branches the bypass is the escape hatch when the gate's own contract substrate is the thing being modified.

The bypass exists because: (1) the gate is a discipline encoded in code, not a security boundary; (2) a build host that does not have the contract substrate installed cannot read the gate and would otherwise block legitimate non-monetization releases. When in doubt, do not bypass.
