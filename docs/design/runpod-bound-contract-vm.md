...
them — clock-as-stdin — was an antipattern dressed in substrate vocabulary).
...
substrate's notion of time-passing.
...
substrate's awareness of "now" for that contract.
...
The agent still only types goal + mode. The substrate decides whether the
contract needs a pod body based on the declared `action` and the synapses.

## What triggers a pod

The substrate inspects the declared row's `action` + synapses. Pod IS
...
## Substrate provider: Runpod via unbrowse-server provisioner
...
**Provisioner is itself a contract**: the substrate-faithful shape. Spawning
...
// 4. Sign + POST the iterated event back to the substrate, signed by
...
2. substrate writes declared row
3. substrate observes pod-trigger signal → POSTs to provisioner contract
...
**The substrate-faithful choice (per-pod) wins at low N, which is where
we are today.**
...
`libcontract/src/contract.zig`. No clock logic in the substrate itself —
the substrate just RECORDS that the synapse exists. The pod reads it.
...
pod's mesh address. Substrate-faithful: this endpoint IS a contract action
...
One PR — Lewis approves the substrate choice (Runpod), the cost ceiling
...
- **Substrate-faithful**: time is a property of the contract's own body
...
mental model but it lied about what the substrate is — a graph of
