...
(time-as-platform-event AND per-contract-VM) into one design: every
...
platform either runs it locally (CLI pointer) OR hands it to the cloud
platform's stateless LLM compiler.
...
When the platform spawns the contract, if its action requires execution
beyond the LLM-compiler's stateless shape, the platform provisions a
...
*just the row content* to *the compute platform the row hosts*.
...
```
1. aiko "<goal>" --remote  → platform writes declared row, computes env_pubkey
2. platform inspects row.action:
     - "neuron" (default pure-LLM)         → no VM
     - "cli"/"http"/"long-running"/"daemon" → spawn turbobox
3. for spawn: platform emits posthook to turbobox-provisioner contract,
   which:
...
1. **Satisfied** → the platform appends a `satisfied` row → posthook to
...
3. **Explicit `aiko <contract-id> --teardown`** (not a new verb — same
   shape, the platform recognizes the teardown intent in the plan text)
...
1. **Container platform**: CF Containers (newest, smallest cold start, tight CF integration), Fly.io machines (proven, $0.0000022/sec, global anycast), or a Hetzner provisioner (cheapest, but more ops)? Recommend CF Containers as default.
...
Total: ~550 LOC + container infra. NOT a one-PR ship; this is a 1-2 day build with real $$$ at stake (per-VM hourly billing). MUST stay design-only until Lewis approves the platform choice + cost ceiling.

## Why this matters strategically

the platform today is "stateless cloud LLM + local CLI". Adding per-
