...
contract substrate) is *reactive*. Its input channels are:
...
the substrate's clock-tick walker observes `now() >= at:` for the first time.
...
The substrate mirrors that: instead of an external scheduler poking the
agent, the agent's own declared contracts say "fire me at this timestamp",
and a server-side walker (the substrate's circadian gene) fires them.

## Public shape — no new caller verb

Consistent with the contract substrate's "no caller-visible options" rule,
...
`posthook:`) tell the substrate WHEN and WHO to fire.
...
2. **Time zones** — `at:` is RFC3339 with explicit offset. Should the substrate also accept named-tz timestamps like `at:2026-05-26T08:00:00 America/Los_Angeles`? (Adds tz database dependency; recommend stay strict ISO and let the client compute the offset.)
