...
The contract substrate becomes a **meta-tool-registry**. Any OpenAI-compatible
client (OpenAI Assistants API, Anthropic Tool Use, Mistral function calling,
Gemini function calling, MCP host) consumes the substrate's tool array the
...
|---|---|---|
| **Substrate primitives** | Always-on built-ins | `aiko_declare`, `aiko_status`, `aiko_iterate`, `aiko_pick_channel`, `aiko_search` |
...
see public + marketplace + substrate primitives. Owners see their lineage's tools.
...
- A substrate primitive (calls handleDeclare/handleStatus/etc directly)
...
a contract iterate; the substrate writes an `iterated` row signed by the
caller's wallet.

### `POST /v1/contract/register-tool` — bring a client's tool into the substrate (future PR)
...
that asks the substrate for its tool registry.
...
- **Substrate primitives** = constitutive receptors expressed on every cell
...
| Component | Status |
|---|---|
| `backend/src/services/openai-tools.ts` (schema generator + 5 substrate primitives) | ✅ scaffold |
...
calls run on the substrate — needs Lewis approval on:
...
3. **Sandbox for client-registered tools**: when Cursor registers `read_file`,
   the file read happens on Cursor's side, not the substrate's. The substrate
...
The substrate today is "/contract neurons that fire each other." Adding the
...
LLM calls becomes an iterated contract on the substrate's ledger. The substrate
absorbs the function-calling ecosystem the way it absorbed scheduling (clock-as-
substrate) and execution (per-contract VMs).
